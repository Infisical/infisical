...
import { encryptSymmetric128BitHexKeyUTF8 } from "../../src/utils/crypto";
  ...
  return new Promise((resolve, reject) => {
    ...
    client.init(
      ...
      async () => {
        ...
        const loginOneRes = await axiosInstance.post(
          "http://localhost:4000/api/v1/auth/login1",
          reqBody
        );
        ...
        const response2 = await axiosInstance.post(
          "http://localhost:4000/api/v1/auth/login2",
          reqBody2
        );
...
export const getServiceTokenFromTestUser = async () => {
  ...
  const newServiceToken = await axiosInstance.post(
    "http://localhost:4000/api/v2/service-token/",
    {
