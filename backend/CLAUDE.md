...
Draining is at-least-once: a shard is trimmed (`XTRIM MINID`) only after its batch has been handed to the PostHog client, so a run that throws before that point retries the whole batch on the next tick. One run drains at most 50k entries per shard, so a backlog is worked off over several ticks instead of being pulled into the cron's memory at once.
...
- **Age**, at the top of every cron run — each shard is trimmed to a 30-minute retention window before it is read. This trim sits *outside* the drain's error scope deliberately: a publish path that throws on every tick still has to age entries out, or the shard parks at its `MAXLEN` cap with nothing bounding how long buffered analytics sits in Redis. It stands in for the TTL the pre-stream per-event keys carried, and it is why a broken drain costs 30 minutes of product analytics instead of unbounded Redis memory.

Shards are reached only by iterating `POSTHOG_AGGREGATED_EVENTS` × the 30 buckets, so **removing an event type from that list orphans its shards** — nothing trims them and nothing expires them. Leave the event in the list for one release so the cron drains it, or delete `telemetry-agg-stream:<event>:*` by hand; the same applies to lowering the bucket count, and to rolling back past this layout. Nothing reads the pre-stream keys, so a deploy that changes this layout drops whatever is still buffered — acceptable for product analytics, and the reason there is no migration path to maintain.
