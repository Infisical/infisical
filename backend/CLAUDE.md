...
**⚠️ Never remove legacy permission actions from roles.** Some actions are legacy umbrella actions kept purely for backwards compatibility: `ProjectPermissionSecretActions.DescribeAndReadValue` (`"read"`, superseded by `DescribeSecret` + `ReadValue`) and `GrantPrivileges` (`"grant-privileges"` on Member/Identity/Group, superseded by `AssignRole` + `AssignAdditionalPrivileges`). Do NOT strip these from the built-in role definitions in `default-roles.ts`, and do NOT auto-rewrite them into the new actions when saving a role (e.g. in the frontend `rolePermission2Form` round-trip). Reasons: (1) the **old privilege-boundary system** (`validatePermissionBoundary` in `permission-fns.ts`) compares action strings *literally* — if an admin/member role loses the legacy action while an existing grantee's role or additional-privilege still carries it, the actor is no longer a superset and can no longer grant/manage that grantee; (2) built-in roles are now **seeded as DB rows**, so their packed `permissions` must stay byte-identical to the previously-computed roles or existing deployments silently lose access. A legacy action should only disappear when a user explicitly unchecks it, or for brand-new roles. Backend compat OR-fallbacks live in `hasSecretReadValueOrDescribePermission` and `expandLegacyForbidActions` (`permission-fns.ts`).
