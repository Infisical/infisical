...
- **The matcher is copied too, in the frontend.** `frontend/src/helpers/policyMatch.ts` is a TS port of the CLI's `packages/agentproxy/policy_match.go` (pattern parsing, wildcard-one-label, scheme/port/path/method clauses, the exact-host → named-port → longest-path specificity order). It backs the Test Request sheet, which re-evaluates on every keystroke, so it deliberately runs in the browser with no endpoint behind it. **The proxy is still the only thing that decides a real request**; the copy must never grow semantics the Go side lacks, and `policy_match_test.go` is the reference it has to agree with. It differs from Go in one visible way: it collects *every* failing clause instead of returning on the first, because the UI names why a rule turned a request away.
