...
- **Chunked deletes are FK-order-sensitive.** You may only manually chunk-delete a table whose inbound FKs are all `CASCADE`/`SET NULL`. A `DEFERRABLE … NO ACTION` inbound FK (e.g. `secret_rotation_v2_secret_mappings.secretId → secrets_v2`) fails the deferred check at COMMIT if you delete the parent outside the full cascade tree — leave those tables to the final `deleteById` cascade (PG resolves the whole tree in one transaction). `secret_versions_v2` is chunk-deleted by `folderId` only because it has no such inbound FK (and the cascade would otherwise orphan it — it has no `folderId`/`secretId` FK). Use `SET LOCAL statement_timeout` per batch so it can't leak to pooled connections.
- **Index every FK referencing column** that participates in a cascade — Postgres does **not** auto-index FK columns, so an unindexed FK forces a seq-scan per parent delete. For a mostly-NULL nullable FK, a partial index `WHERE col IS NOT NULL` serves the cascade lookup at a fraction of the size/write cost (e.g. `idx_secret_versions_v2_envid`).
