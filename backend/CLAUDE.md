...
- **Session tokens never expire.** Revocation is structural instead: `resolveSession` re-checks the agent flag, project membership and policies on every call, so losing any of them ends the session on the next refresh. Do not add a cache in front of it. Humans revoke through `POST /agent-sessions/:sessionId/revoke` (idempotent) and read the list through `GET /agent-sessions`; both are authorized on `ProjectPermissionSub.AgentPolicies` (Read to list, Delete to revoke) rather than a subject of their own, because a session is agent-policy state at runtime. The proxy only notices on its next poll, so the UI says so rather than implying an instant kill.
- **The `AGENT_PROXY_REQUEST` audit event is a UI contract, not just a log line.** The Activity tab (`AgentProxyPage`) is a filtered audit-log query, so its metadata shape (decision, host, port, path, statusCode, policyName, reason, agentName, identityId, userId) is what the feed renders and what "replay this request" rebuilds a URL from. A new event type also has to be added to the frontend `EventType` enum **and** `eventToNameMap` (`hooks/api/auditLogs/`): the map is exhaustive over the frontend enum, so a missing entry silently makes the event unfilterable and unlabelled in the audit log UI.
...
- **Placeholder values are preserved across policy edits.** Rotating one breaks every agent already running with it in its environment. They are also returned on policy reads (`placeholderValue` on each credential) so the UI can show the decoy the agent holds beside the credential the proxy substitutes: a placeholder is fabricated, and every agent on the policy is handed it at session start anyway.
...
- **The matcher is copied too, in the frontend.** `frontend/src/helpers/policyMatch.ts` is a TS port of the CLI's `packages/agentproxy/policy_match.go` (pattern parsing, wildcard-one-label, scheme/port/path/method clauses, the exact-host → named-port → longest-path specificity order). It backs the Test Request dialog (`PolicySimulationModal.tsx`, a vertical two-gate stack that re-evaluates on every keystroke), so it deliberately runs in the browser with no endpoint behind it. **The proxy is still the only thing that decides a real request**; the copy must never grow semantics the Go side lacks, and `policy_match_test.go` is the reference it has to agree with. It differs from Go in one visible way: it collects *every* failing clause instead of returning on the first, because the UI names why a rule turned a request away.
