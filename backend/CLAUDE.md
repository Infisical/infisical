...
**Never log an outbound URL verbatim — a URL is often itself a credential.** Incoming-webhook providers put the bearer secret in the path (`https://hooks.slack.com/services/T…/B…/<secret>`, Discord, Teams, Telegram) and many APIs accept a token as a query param, so a raw URL in a log line ships a working credential to the log sink. Pass it through `sanitizeUrlForLog` from `@app/lib/logger` first (`src/lib/logger/sanitize-url.ts`): it strips userinfo and the fragment, redacts every query value, drops the path for known credential-in-URL hosts, and redacts token-shaped path segments on hosts we can't enumerate (users can point a webhook channel at any host). The global axios response interceptor (`src/lib/config/request.ts`) and `safeRequest`'s dispatch log already do this — when adding a new provider whose path is a secret, add its host to `CREDENTIAL_PATH_HOSTS`.
