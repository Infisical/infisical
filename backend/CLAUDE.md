...
Draining is at-least-once: a shard is trimmed (`XTRIM MINID`) only after its batch has been handed to the PostHog client, so a run that throws before that point retries the whole batch on the next tick. Growth is bounded by the `MAXLEN` cap on write (100k entries per shard), not by a TTL, and one run drains at most 50k entries per shard, so a backlog is worked off over several ticks instead of being pulled into the cron's memory at once. Nothing reads the pre-stream keys, so a deploy that changes this layout drops whatever is still buffered — acceptable for product analytics, and the reason there is no migration path to maintain.
