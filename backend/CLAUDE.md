...
**Never let an unbounded attribute reach the SDK's `record()`, and never acquire a meter outside `src/lib/telemetry/`.** An attributes processor (the allowlist View) bounds memory because the SDK filters attributes *before* hashing them; a `DROP` aggregation does not, because it is consulted *after*. Under cumulative temporality — which `OTEL_EXPORT_TYPE=prometheus` mandates — `TemporalMetricProcessor` retains one map entry per distinct attribute set with no cardinality cap, for the lifetime of the process. So a dropped meter that is still recorded to leaks heap, which is why `OTEL_DROP_HIGH_CARDINALITY_METERS` gates at the call site (`highCardinalityMeter` / `shouldRecordHighCardinalityMetrics` in `metrics.ts`) *and* strips attributes in the View. A `no-restricted-syntax` ESLint rule blocks `getMeter` outside `src/lib/telemetry/` so a new meter can't bypass the gate; use `highCardinalityMeter` for per-actor instruments and `resolveCoreMeter` for `InfisicalCore` observable gauges.
