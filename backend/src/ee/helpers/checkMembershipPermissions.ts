...
export const userHasWorkspaceAccess = async (
...
export const userHasWriteOnlyAbility = async (
  userId: Types.ObjectId,
  workspaceId: Types.ObjectId,
  environment: string
) => {
  ...
  const deniedMembershipPermissions = membershipForWorkspace.deniedPermissions;
  const isWriteDisallowed = _.some(deniedMembershipPermissions, {
    environmentSlug: environment,
    ability: PERMISSION_WRITE_SECRETS
  });
  const isReadDisallowed = _.some(deniedMembershipPermissions, {
    environmentSlug: environment,
...
export const userHasNoAbility = async (
  userId: Types.ObjectId,
  workspaceId: Types.ObjectId,
  environment: string
) => {
  ...
  const isReadBlocked = _.some(deniedMembershipPermissions, {
    environmentSlug: environment,
    ability: PERMISSION_READ_SECRETS
  });
