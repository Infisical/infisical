...
import { IUser, Membership, Workspace } from "@app/models";
import { EventType } from "@app/ee/models";
import { IMembershipPermission } from "@app/models/membership";
import { BadRequestError, UnauthorizedRequestError } from "@app/utils/errors";
import { ADMIN, MEMBER } from "@app/variables/organization";
...
export const denyMembershipPermissions = async (req: Request, res: Response) => {
  ...
  const sanitizedMembershipPermissions: IMembershipPermission[] = permissions.map(
    (permission: IMembershipPermission) => {
      if (
        !permission.ability ||
        !permission.environmentSlug ||
        ![PERMISSION_READ_SECRETS, PERMISSION_WRITE_SECRETS].includes(permission.ability)
      ) {
        throw BadRequestError({
          message: "One or more required fields are missing from the request or have incorrect type"
        });
      }
  ...
  const sanitizedMembershipPermissionsUnique = _.uniqWith(
    sanitizedMembershipPermissions,
    _.isEqual
  );
