...
export const digestForOcsp = (hashAlgorithmOid: string, data: Buffer): Buffer | null => {
...
export const buildOcspRequestFingerprint = (entries: { hashAlgorithmOid: string; serialNumber: string }[]): string =>
  crypto.nativeCrypto
    .createHash("sha256")
    .update(
      entries
        .map((entry) => `${entry.hashAlgorithmOid}:${entry.serialNumber}`)
        .sort()
        .join("|")
    )
    .digest("hex");
...
const buildCertStatus = (status: TOcspCertStatus): asn1js.BaseBlock => {
  ...
  if (status.kind === OcspCertStatus.Revoked) {
    const revokedInfo: asn1js.BaseBlock[] = [new asn1js.GeneralizedTime({ valueDate: status.revokedAt })];
