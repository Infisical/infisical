...
describe("buildSignedOcspResponse", () => {
  ...
  it("should truncate a revocation time carrying milliseconds, which DER forbids", async () => {
    const revokedAt = new Date("2026-09-20T10:00:00.360Z");
    const basic = readBasic(await build({ kind: OcspCertStatus.Revoked, revokedAt, reason: 1 }));

    const certStatus = basic.tbsResponseData.responses[0].certStatus as asn1js.Constructed;
    const encoded = Buffer.from((certStatus.valueBlock.value[0] as asn1js.GeneralizedTime).toBER(false));

    // a fractional part here makes Go's encoding/asn1 reject the whole response
    expect(encoded.subarray(2).toString("ascii")).toBe("20260920100000Z");
  });
