...
- **Web Resource accounts** store a user-configured `http`/`https` target URL in connection details. `parseWebResourceUrl` normalizes it into `{ scheme, host, port, basePath }`, rejects credentials, fragments, query strings, and malformed URLs, and proxy requests treat query strings as request-time data. Browser access uses a signed iframe token plus the durable `pam_sessions` row; each proxied request creates a short-lived gateway relay from the session/account state and closes it after the upstream response, so no process-local launch state is required. The iframe must remain sandboxed without `allow-same-origin`; proxied responses also set a CSP `sandbox` directive.
