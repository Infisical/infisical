...
- **Provider contract** (`TPamDiscoveryFactory`): returns `{ validateConnection, scan }`. `scan()` returns `TDiscoveredAccount[]` (`accountType`, `name`, `fingerprint`, `details`); the **service** owns encryption, staging, and fingerprint dedupe. Providers never touch the DB. `executeWithGateway` (`pam-discovery-fns.ts`) is the shared helper that opens a gateway TCP proxy to `targetHost:targetPort` and runs an operation against the local proxy port; both factories use it.
- **Credential accounts are a list.** The factory receives `credentialAccounts: TDiscoveryCredentialAccount[]`. Single-account types (AD) read `credentialAccounts[0]`. Unix scans with several SSH accounts, whose ids live in `configuration.credentialAccountIds`; `getCredentialAccountIds` in the service unions them with the source's primary `credentialAccountId` column, and create/update validate every id. `credentialAccountId` (the column) stays the primary for FK integrity and gateway resolution.
...
`unix/unix-discovery-factory.ts` scans Unix hosts over SSH (via `ssh2`) and stages `ssh` accounts. Config (`unix/unix-discovery-schemas.ts`): `cidrRanges` (IPs, IPv4 CIDR ranges, or hostnames — CIDRs expand, IPs/hostnames pass through and the gateway resolves hostnames in the target network), `credentialAccountIds` (the SSH accounts to scan with), `includeSystemAccounts`. Per target host it orders the credential accounts (host match first, then each account) and tries them until one authenticates, reading `/etc/passwd` via `getent passwd` in one exec that also returns the `login.defs` uid bounds. Unless `includeSystemAccounts` is on it keeps only login-capable accounts (uid within `UID_MIN`/`UID_MAX`, no `nologin`/`false` shell, not a `_`-prefixed or well-known system name; `root` always kept). Each account stages with `fingerprint = host:username` and `credentials: { authMethod: "password", username }` so imports arrive without a secret. Unreachable/auth-failed hosts go to `machineErrors`, never aborting the scan (dead IPs in a range are dropped silently; only hosts that answered SSH, or hosts explicitly named as a credential account's host, are reported).

Auth methods (all run through the gateway, so `ssh2` is never used on the backend for discovery): the gateway has a dedicated **SSH-exec RPC** handler (CLI repo `packages/gateway-v2/ssh_handler.go`, ALPN `infisical-ssh-exec`) that performs the SSH login itself and runs one command. The backend `sshExecWithGateway` (`pam-discovery-fns.ts`) opens `getPlatformConnectionDetailsByGatewayId` + `withGatewayV2Proxy(SshExec)` and POSTs `{ command, credentials }` to `/v1/exec` via `callSshExec` (`lib/gateway-v2/ssh-rpc.ts`); the target host/port come from the signed gateway cert (SSRF-safe), credentials from the body. This handles password, private-key, **and certificate** auth uniformly — the gateway's Go SSH client can present a cert, which `ssh2` cannot. For certificate accounts the backend signs an ephemeral user cert in `runScan` (`buildGatewaySshCredentials`, reusing `createSshCert`) and sends `privateKey`+`certificate` in the RPC body. This path touches nothing in the PAM session machinery, so scans produce no session rows, recordings, or `PAM_SESSION_START` audit. It mirrors the WinRM RPC handler pattern (Infisical/cli #309, Infisical/infisical #7258).

Speed: before enumerating, `scan()` does one **port sweep** (gateway ALPN `infisical-port-sweep`, `handlePortSweep`) that TCP-probes the whole expanded target set in-network and returns the reachable `host:port` set (`sweepReachableTargets`). Only reachable hosts (plus explicitly-named credential-account hosts) get the per-host SSH-exec, at `SCAN_CONCURRENCY` parallelism — so a sparse CIDR costs one sweep op instead of a gateway op per dead IP.
