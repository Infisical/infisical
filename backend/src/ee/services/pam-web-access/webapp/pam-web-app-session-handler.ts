...
const WEBAPP_TCP_CONNECT_TIMEOUT_MS = 30_000;
...
export const handleWebAppSession = async (ctx: TSessionContext): Promise<TSessionHandlerResult> => {
  ...
  const teardown = () => {
    ...
    try {
      ...
      logger.debug(err, "webapp session: error destroying tcp socket");
    }
  };

  return new Promise<TSessionHandlerResult>((resolve, reject) => {
    const connectTimeout = setTimeout(() => {
      logger.error({ sessionId }, "webapp session: relay connect timed out");
      if (!cleanedUp) onCleanup();
      teardown();
      reject(new Error("WebApp relay connection timed out"));
    }, WEBAPP_TCP_CONNECT_TIMEOUT_MS);
    ...
    tcpSocket.once("connect", () => {
      ...
      tcpSocket.on("data", (chunk: Buffer) => {
        ...
        socket.send(chunk, { binary: true }, (err) => {
          if (err) {
            logger.error(err, "webapp session: ws send failed");
            if (!cleanedUp) onCleanup();
            teardown();
            return;
          }
      ...
      tcpSocket.on("close", () => {
        logger.info({ sessionId }, "webapp session: tcp socket closed");
        if (!cleanedUp) onCleanup();
        teardown();
        try {
          socket.close();
        } catch (err) {
          logger.debug(err, "webapp session: error closing ws after tcp close");
        }
      });

      tcpSocket.on("error", (err) => {
        logger.error(err, "webapp session: tcp socket error");
        if (!cleanedUp) onCleanup();
        teardown();
      });
    ...
    tcpSocket.once("error", (err) => {
      clearTimeout(connectTimeout);
      logger.error(err, "webapp session: failed to connect to relay");
      if (!cleanedUp) onCleanup();
      teardown();
      reject(err);
    });
