...
import { getLicenseKey, getLicenseServerKey, getLicenseServerUrl } from "@app/config";
import {
  licenseKeyRequest,
  ...
  public async getPlan(
    ...
    try {
      if (this.instanceType === "cloud") {
        const cachedPlan = this.localFeatureSet.get<FeatureSet>(
          `${organizationId.toString()}-${workspaceId?.toString() ?? ""}`
        );
        if (cachedPlan) {
          return cachedPlan;
        }
        ...
        let url = `${await getLicenseServerUrl()}/api/license-server/v1/customers/${
        ...
        const {
          data: { currentPlan }
        } = await licenseServerKeyRequest.get(url);

        // cache fetched plan for organization
        this.localFeatureSet.set(
          `${organizationId.toString()}-${workspaceId?.toString() ?? ""}`,
          currentPlan
        );
    ...
    try {
      ...
      if (licenseKey) {
        ...
        if (token) {
          const {
            data: { currentPlan }
          } = await licenseKeyRequest.get(`${await getLicenseServerUrl()}/api/license/v1/plan`);
