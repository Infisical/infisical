...
const INDEX_NAME = "idx_secret_approval_requests_secrets_v2_secret_version";
...
export async function up(knex: Knex): Promise<void> {
  ...
  try {
    ...
    if (
      ...
      (await knex.schema.hasColumn(TableName.SecretApprovalRequestSecretV2, "secretVersion"))
    ) {
      await knex.raw(`
        CREATE INDEX CONCURRENTLY IF NOT EXISTS "${INDEX_NAME}"
        ON ${TableName.SecretApprovalRequestSecretV2} ("secretVersion")
        WHERE "secretVersion" IS NOT NULL
      `);
    }
  } finally {
