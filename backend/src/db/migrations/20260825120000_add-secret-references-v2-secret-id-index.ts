...
const INDEX_NAME = "idx_secret_references_v2_secret_id";
...
export async function up(knex: Knex): Promise<void> {
  ...
  try {
    ...
    if (
      (await knex.schema.hasTable(TableName.SecretReferenceV2)) &&
      (await knex.schema.hasColumn(TableName.SecretReferenceV2, COLUMN))
    ) {
      await knex.raw(`
        CREATE INDEX CONCURRENTLY IF NOT EXISTS "${INDEX_NAME}"
        ON ${TableName.SecretReferenceV2} ("${COLUMN}")
      `);
    }
  } finally {
