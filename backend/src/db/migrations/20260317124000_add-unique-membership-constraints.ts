...
export async function up(knex: Knex): Promise<void> {
  // Wrap entire migration in a transaction to prevent partial failure leaving
  // the database in a corrupt intermediate state
  await knex.transaction(async (trx) => {
    ...
    // The NOT EXISTS guard compares role, customRoleId, and temporal fields to avoid
    // silently dropping temporary role assignments that differ from permanent ones.
    await trx.raw(`
      WITH duplicates AS (
      ...
      WHERE ${TableName.MembershipRole}."membershipId" = td.dup_id
        AND NOT EXISTS (
          ...
          WHERE mr2."membershipId" = td.keeper_id
            ...
            AND mr2."isTemporary" = ${TableName.MembershipRole}."isTemporary"
            AND COALESCE(mr2."temporaryRange", '') = COALESCE(${TableName.MembershipRole}."temporaryRange", '')
        )
    `);

    // Delete orphaned membership roles that couldn't be moved (exact duplicates including temporal fields)
    await trx.raw(`
      WITH duplicates AS (
        SELECT id, "actorUserId", "scopeProjectId",
          ROW_NUMBER() OVER (
    ...
    await trx.raw(`
      CREATE UNIQUE INDEX IF NOT EXISTS membership_user_project_unique
      ON ${TableName.Membership} ("actorUserId", "scopeProjectId")
      WHERE scope = 'project' AND "actorUserId" IS NOT NULL;
    `);

    await trx.raw(`
      CREATE UNIQUE INDEX IF NOT EXISTS membership_user_org_unique
    ...
    await trx.raw(`
      CREATE UNIQUE INDEX IF NOT EXISTS membership_identity_project_unique
      ON ${TableName.Membership} ("actorIdentityId", "scopeProjectId")
      WHERE scope = 'project' AND "actorIdentityId" IS NOT NULL;
    `);

    await trx.raw(`
      CREATE UNIQUE INDEX IF NOT EXISTS membership_identity_org_unique
