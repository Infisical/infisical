...
const INDEX_NAME = "idx_secret_rotation_output_v2_secret_id";
const COLUMN = "secretId";
...
export async function up(knex: Knex): Promise<void> {
  ...
  try {
    ...
    if (
      (await knex.schema.hasTable(TableName.DeprecatedSecretRotationOutputV2)) &&
      (await knex.schema.hasColumn(TableName.DeprecatedSecretRotationOutputV2, COLUMN))
    ) {
      await knex.raw(`
        CREATE INDEX CONCURRENTLY IF NOT EXISTS "${INDEX_NAME}"
        ON ${TableName.DeprecatedSecretRotationOutputV2} ("${COLUMN}")
      `);
    }
  } finally {
