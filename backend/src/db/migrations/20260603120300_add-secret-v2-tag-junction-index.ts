...
const INDEX_NAME = "idx_secret_v2_tag_junction_secret_id";
const COLUMN = "secrets_v2Id";
...
export async function up(knex: Knex): Promise<void> {
  ...
  try {
    ...
    if (
      (await knex.schema.hasTable(TableName.SecretV2JnTag)) &&
      (await knex.schema.hasColumn(TableName.SecretV2JnTag, COLUMN))
    ) {
      await knex.raw(`
        CREATE INDEX CONCURRENTLY IF NOT EXISTS "${INDEX_NAME}"
        ON ${TableName.SecretV2JnTag} ("${COLUMN}")
      `);
    }
  } finally {
