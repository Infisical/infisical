...
const INDEX_NAME = "idx_identity_group_membership_group_id_identity_id";
...
export async function up(knex: Knex): Promise<void> {
  ...
  try {
    ...
    if (
      (await knex.schema.hasTable(TableName.IdentityGroupMembership)) &&
      (await knex.schema.hasColumn(TableName.IdentityGroupMembership, "groupId"))
    ) {
      await knex.raw(`
        CREATE INDEX CONCURRENTLY IF NOT EXISTS "${INDEX_NAME}"
        ON ${TableName.IdentityGroupMembership} ("groupId", "identityId")
      `);
    }
  } finally {
