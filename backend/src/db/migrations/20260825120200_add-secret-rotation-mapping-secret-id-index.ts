...
const INDEX_NAME = "idx_secret_rotation_v2_secret_mappings_secret_id";
...
export async function up(knex: Knex): Promise<void> {
  ...
  try {
    ...
    if (
      (await knex.schema.hasTable(TableName.SecretRotationV2SecretMapping)) &&
      (await knex.schema.hasColumn(TableName.SecretRotationV2SecretMapping, COLUMN))
    ) {
      await knex.raw(`
        CREATE INDEX CONCURRENTLY IF NOT EXISTS "${INDEX_NAME}"
        ON ${TableName.SecretRotationV2SecretMapping} ("${COLUMN}")
      `);
    }
  } finally {
