...
export async function down(): Promise<void> {
  // The reverse transformation (unique-secret-value -> prevent-value-reuse /
  // prevent-duplicated-values) would be lossy when both secretVersions and
  // otherSecrets are enabled on one constraint. The read-time compatibility
  // layer in parseSecretValidationRuleInputs handles old data gracefully,
  // so rolling back the code without reversing the data is safe.
}
