...
const INDEX_NAME = "idx_secret_version_v2_tag_junction_version_id";
const COLUMN = "secret_versions_v2Id";
...
export async function up(knex: Knex): Promise<void> {
  ...
  try {
    ...
    if (
      (await knex.schema.hasTable(TableName.SecretVersionV2Tag)) &&
      (await knex.schema.hasColumn(TableName.SecretVersionV2Tag, COLUMN))
    ) {
      await knex.raw(`
        CREATE INDEX CONCURRENTLY IF NOT EXISTS "${INDEX_NAME}"
        ON ${TableName.SecretVersionV2Tag} ("${COLUMN}")
      `);
    }
  } finally {
