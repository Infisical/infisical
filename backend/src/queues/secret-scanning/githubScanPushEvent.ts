...
export const githubPushEventSecretScan = new Queue(
  "github-push-event-secret-scanning",
  "redis://redis:6379"
);
...
githubPushEventSecretScan.process(async (job: Job, done: Queue.DoneCallback) => {
    ...
    for (const filepath of [...commit.added, ...commit.modified]) {
      try {
        ...
        for (const finding of findings) {
          ...
          new Error(
            `gitHubHistoricalScanning.process: unable to fetch content for [filepath=${filepath}] because [error=${error}]`
          ),
          null
        );
      }
  ...
  const usersToNotify = pusher?.email
    ? [pusher.email, ...adminOrOwnerEmails]
    : [...adminOrOwnerEmails];
