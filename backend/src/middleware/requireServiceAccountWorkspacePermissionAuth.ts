...
import { ServiceAccountNotFoundError } from "../utils/errors";
...
const requireServiceAccountWorkspacePermissionAuth = ({
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const serviceAccountWorkspacePermission = await ServiceAccountWorkspacePermission.findById(
    ...
    if (!serviceAccountWorkspacePermission) {
      return next(
        ServiceAccountNotFoundError({
          message: "Failed to locate Service Account workspace permission"
        })
      );
    }
