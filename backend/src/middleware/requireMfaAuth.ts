 ...
 */
const requireMfaAuth = async (req: Request, res: Response, next: NextFunction) => {
  ...
  const decodedToken = <jwt.UserIDJwtPayload>jwt.verify(AUTH_TOKEN_VALUE, await getJwtMfaSecret());
  ...
  if (!user)
    return next(
      UnauthorizedRequestError({
        message: "Unable to authenticate for User account completion. Try logging in again"
      })
    );
