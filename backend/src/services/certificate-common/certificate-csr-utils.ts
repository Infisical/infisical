 ...
 */
const mapPublicKeyToKeyAlgorithm = (publicKey: { algorithm: { name: string } }): CertKeyAlgorithm => {
  if (publicKey.algorithm.name === "RSASSA-PKCS1-v1_5") {
    ...
    switch (keySize) {
      ...
      case 3072:
        return CertKeyAlgorithm.RSA_3072;
      case 4096:
        return CertKeyAlgorithm.RSA_4096;
      default:
        throw new BadRequestError({
          message: `Unsupported RSA key size: ${keySize}. Supported: 2048, 3072, 4096`
        });
    }
    ...
    switch (namedCurve) {
      ...
      case "P-521":
        return CertKeyAlgorithm.ECDSA_P521;
      default:
        throw new BadRequestError({
          message: `Unsupported ECDSA curve: ${namedCurve}. Supported: P-256, P-384, P-521`
        });
    }
  } else if (publicKey.algorithm.name.startsWith("ML-DSA")) {
    switch (publicKey.algorithm.name) {
      case "ML-DSA-44":
        return CertKeyAlgorithm.ML_DSA_44;
      case "ML-DSA-65":
        return CertKeyAlgorithm.ML_DSA_65;
      case "ML-DSA-87":
        return CertKeyAlgorithm.ML_DSA_87;
      default:
        throw new BadRequestError({
          message: `Unsupported ML-DSA variant: ${publicKey.algorithm.name}. Supported: ML-DSA-44, ML-DSA-65, ML-DSA-87`
        });
    }
  } else if (publicKey.algorithm.name.startsWith("SLH-DSA")) {
    switch (publicKey.algorithm.name) {
      case "SLH-DSA-SHA2-128f":
        return CertKeyAlgorithm.SLH_DSA_SHA2_128F;
      case "SLH-DSA-SHA2-128s":
        return CertKeyAlgorithm.SLH_DSA_SHA2_128S;
      case "SLH-DSA-SHA2-192f":
        return CertKeyAlgorithm.SLH_DSA_SHA2_192F;
      case "SLH-DSA-SHA2-192s":
        return CertKeyAlgorithm.SLH_DSA_SHA2_192S;
      case "SLH-DSA-SHA2-256f":
        return CertKeyAlgorithm.SLH_DSA_SHA2_256F;
      case "SLH-DSA-SHA2-256s":
        return CertKeyAlgorithm.SLH_DSA_SHA2_256S;
      case "SLH-DSA-SHAKE-128f":
        return CertKeyAlgorithm.SLH_DSA_SHAKE_128F;
      case "SLH-DSA-SHAKE-128s":
        return CertKeyAlgorithm.SLH_DSA_SHAKE_128S;
      case "SLH-DSA-SHAKE-192f":
        return CertKeyAlgorithm.SLH_DSA_SHAKE_192F;
      case "SLH-DSA-SHAKE-192s":
        return CertKeyAlgorithm.SLH_DSA_SHAKE_192S;
      case "SLH-DSA-SHAKE-256f":
        return CertKeyAlgorithm.SLH_DSA_SHAKE_256F;
      case "SLH-DSA-SHAKE-256s":
        return CertKeyAlgorithm.SLH_DSA_SHAKE_256S;
      default:
        throw new BadRequestError({
          message: `Unsupported SLH-DSA variant: ${publicKey.algorithm.name}`
        });
    }
  }
  throw new BadRequestError({
    message: `Unsupported key algorithm: ${publicKey.algorithm.name}. Supported: RSASSA-PKCS1-v1_5, ECDSA, ML-DSA, SLH-DSA`
  });
};

export const extractAlgorithmsFromCertificate = (certDer: Buffer) => {
  try {
    const certObj = new x509.X509Certificate(certDer);
    const keyAlg = mapPublicKeyToKeyAlgorithm(certObj.publicKey);
    return { keyAlgorithm: keyAlg };
  } catch {
    return null;
  }
};
