...
import { azureDnsDeleteTxtRecord, azureDnsInsertTxtRecord, getAzureDnsRelativeRecordName } from "./azure-dns";

vi.mock("@app/lib/config/request", () => ({ request: { put: vi.fn(), delete: vi.fn() } }));
vi.mock("@app/services/app-connection/azure-dns/azure-dns-connection-fns", () => ({
...
describe("getAzureDnsRelativeRecordName", () => {
  ...
  it("rejects names outside the hosted zone before changing DNS", () => {
    expect(() => getAzureDnsRelativeRecordName(zoneId, "_acme-challenge.host.example.net")).toThrow(
      "outside the hosted zone"
    );
  });
});
...
describe("Azure DNS challenge writes", () => {
  const zoneId = "/subscriptions/sub/resourceGroups/rg/providers/Microsoft.Network/dnsZones/acme.example.com";
  const connection = {
    credentials: { tenantId: "tenant", clientId: "client", clientSecret: "secret" }
  } as Parameters<typeof azureDnsInsertTxtRecord>[0];
  ...
  it("rejects out-of-zone inserts and deletes before token or DNS requests", async () => {
    await expect(azureDnsInsertTxtRecord(connection, zoneId, "_acme-challenge.outside.net", '"txt"')).rejects.toThrow(
      "outside the hosted zone"
    );
    await expect(azureDnsDeleteTxtRecord(connection, zoneId, "_acme-challenge.outside.net", '"txt"')).rejects.toThrow(
      "outside the hosted zone"
    );
  ...
  it("writes and deletes an apex CNAME target at Azure's @ record", async () => {
    await azureDnsInsertTxtRecord(connection, zoneId, "ACME.EXAMPLE.COM.", '"txt"');
    await azureDnsDeleteTxtRecord(connection, zoneId, "acme.example.com", '"txt"');
    expect(request.put).toHaveBeenCalledWith(expect.stringContaining("/TXT/%40?api-version="), expect.anything(), expect.anything());
    expect(request.delete).toHaveBeenCalledWith(expect.stringContaining("/TXT/%40?api-version="), expect.anything());
  });
});
