...
import { TConsumerSecretsKmsKeyDALFactory } from './kms-key-dal';
...
type TConsumerSecretsKmsServiceFactoryDep = {
  kmsDAL: TConsumerSecretsKmsKeyDALFactory;
  projectDAL: Pick<
    TProjectDALFactory,
    'findById' | 'updateById' | 'transaction'
  >;
  orgDAL: Pick<TOrgDALFactory, 'findById' | 'updateById' | 'transaction'>;
  kmsRootConfigDAL: Pick<TKmsRootConfigDALFactory, 'findById' | 'create'>;
  keyStore: Pick<
    TKeyStoreFactory,
    'acquireLock' | 'waitTillReady' | 'setItemWithExpiry'
  >;
  internalKmsDAL: Pick<TInternalKmsDALFactory, 'create'>;
};

export type TConsumerSecretsKmsServiceFactory = ReturnType<
  typeof kmsServiceFactory
>;
...
export const kmsServiceFactory = ({
  ...
  orgDAL,
  projectDAL,
}: TConsumerSecretsKmsServiceFactoryDep) => {
  ...
  const generateKmsKey = async ({
    ...
    const kmsKeyMaterial = randomSecureBytes(
      getByteLengthForAlgorithm(encryptionAlgorithm),
    );

    const encryptedKeyMaterial = cipher.encrypt(
      kmsKeyMaterial,
      ROOT_ENCRYPTION_KEY,
    );
    const sanitizedName = name
      ? slugify(name)
      : slugify(alphaNumericNanoId(8).toLowerCase());
  ...
  const deleteInternalKms = async (kmsId: string, orgId: string, tx?: Knex) => {
    ...
    if (kms.orgId !== orgId)
      throw new ForbiddenRequestError({
        message: "KMS doesn't belong to organization",
      });
   ...
   */
  const encryptWithInputKey = async ({
    key,
  }: Omit<TEncryptionWithKeyDTO, 'plainText'>) => {
   ...
   * This can be even later exposed directly as api for encryption as function
   */
  const decryptWithInputKey = async ({
    key,
  }: Omit<TDecryptWithKeyDTO, 'cipherTextBlob'>) => {
    const cipher = symmetricCipherService(SymmetricEncryption.AES_GCM_256);

    return ({
      cipherTextBlob: versionedCipherTextBlob,
    }: Pick<TDecryptWithKeyDTO, 'cipherTextBlob'>) => {
      const cipherTextBlob = versionedCipherTextBlob.subarray(
        0,
        -KMS_VERSION_BLOB_LENGTH,
      );
  ...
  const getOrgKmsKeyId = async (orgId: string, trx?: Knex) => {
    ...
    if (!org.kmsDefaultKeyId) {
      const lock = await keyStore
        .acquireLock([KeyStorePrefixes.KmsOrgKeyCreation, orgId], 3000, {
          retryCount: 3,
        })
        .catch(() => null);
      ...
      try {
        if (!lock) {
          await keyStore.waitTillReady({
            ...
            waitingCb: () =>
              logger.info('KMS. Waiting for org key to be created'),
          });
  ...
  const encryptWithRootKey = () => {
    ...
    return (plainTextBuffer: Buffer) => {
      const encryptedBuffer = cipher.encrypt(
        plainTextBuffer,
        ROOT_ENCRYPTION_KEY,
      );
  ...
  const decryptWithKmsKey = async ({
    ...
    depth = 0,
    tx,
  }: Omit<TDecryptWithKmsDTO, 'cipherTextBlob'> & {
    depth?: number;
    tx?: Knex;
  }) => {
    if (depth > 2)
      throw new BadRequestError({ message: 'KMS depth max limit' });
    ...
    if (kmsDoc.externalKms) {
      ...
      return async ({
        cipherTextBlob,
      }: Pick<TDecryptWithKmsDTO, 'cipherTextBlob'>) => {
    ...
    const dataCipher = symmetricCipherService(
      kmsDoc.internalKms?.encryptionAlgorithm as SymmetricEncryption,
    );
  ...
  const encryptWithKmsKey = async (
    { kmsId }: Omit<TEncryptWithKmsDTO, 'plainText'>,
    tx?: Knex,
  ) => {
    const kmsDoc = await kmsDAL.findByIdWithAssociatedKms(kmsId, tx);
    if (!kmsDoc) {
  ...
  const $getOrgKmsDataKey = async (orgId: string, trx?: Knex) => {
    ...
    if (!org.kmsEncryptedDataKey) {
      const lock = await keyStore
        .acquireLock([KeyStorePrefixes.KmsOrgDataKeyCreation, orgId], 500, {
          retryCount: 0,
        })
        .catch(() => null);
      ...
      try {
        if (!lock) {
          await keyStore.waitTillReady({
            ...
            keyCheckCb: (val) => val === 'true',
            waitingCb: () =>
              logger.info('KMS. Waiting for org data key to be created'),
          });
  ...
  const getProjectSecretManagerKmsKeyId = async (
    projectId: string,
    trx?: Knex,
  ) => {
    ...
    if (!project.kmsSecretManagerKeyId) {
      const lock = await keyStore
        .acquireLock(
          [KeyStorePrefixes.KmsProjectKeyCreation, projectId],
          3000,
          { retryCount: 0 },
        )
        .catch(() => null);
      ...
      try {
        if (!lock) {
          await keyStore.waitTillReady({
            ...
            keyCheckCb: (val) => val === 'true',
            waitingCb: () =>
              logger.debug('KMS. Waiting for project key to be created'),
            delay: 500,
          });
  ...
  const $getProjectSecretManagerKmsDataKey = async (
    projectId: string,
    trx?: Knex,
  ) => {
    ...
    if (!project.kmsSecretManagerEncryptedDataKey) {
      const lock = await keyStore
        .acquireLock(
          [KeyStorePrefixes.KmsProjectDataKeyCreation, projectId],
          3000,
          { retryCount: 0 },
        )
        .catch(() => null);
      ...
      try {
        if (!lock) {
          await keyStore.waitTillReady({
            ...
            waitingCb: () =>
              logger.debug(
                'KMS. Waiting for secret manager data key to be created',
              ),
            delay: 500,
          });
          ...
          const projectDataKey = await (trx || projectDAL).transaction(
            async (tx) => {
              project = await projectDAL.findById(projectId, tx);
              if (project.kmsSecretManagerEncryptedDataKey) {
                return;
  ...
  const createCipherPairWithDataKey = async (
    encryptionContext: TEncryptWithKmsDataKeyDTO,
    trx?: Knex,
  ) => {
  ...
  const updateProjectSecretManagerKmsKey = async ({
    projectId,
    kms,
  }: TUpdateProjectSecretManagerKmsKeyDTO) => {
    ...
    if (kms.type === KmsType.External) {
      ...
      if (!kmsDoc) {
        throw new NotFoundError({
          message: `KMS with ID '${kmsId}' not found`,
        });
      }
    ...
    return kmsDAL.transaction(async (tx) => {
      ...
      return KmsSanitizedSchema.parseAsync({
        isExternal: !currentKms.isReserved,
        ...newKms,
      });
    });
  };
  ...
  const getProjectKeyBackup = async (projectId: string) => {
    ...
    const secretManagerDataKey =
      await $getProjectSecretManagerKmsDataKey(projectId);
    ...
    const { cipherTextBlob: encryptedSecretManagerDataKeyWithOrgKms } =
      await kmsEncryptor({
  ...
  const loadProjectKeyBackup = async (projectId: string, backup: string) => {
    ...
    const computedHash = generateHash(
      backup.substring(0, backup.lastIndexOf('.')),
    );
  ...
  const startService = async () => {
    ...
    if (!encryptionKey)
      throw new Error('Root encryption key not found for KMS service.');
    ...
    const lock = await keyStore
      .acquireLock([`KMS_ROOT_CFG_LOCK`], 3000, { retryCount: 3 })
      .catch(() => null);
    if (!lock) {
      await keyStore.waitTillReady({
        ...
        waitingCb: () =>
          logger.info(
            'KMS. Waiting for leader to finish creation of KMS Root Key',
          ),
      });
    }

    // check if Consumer Secrets KMS root key was already generated and saved in DB
    const kmsRootConfig = await kmsRootConfigDAL.findById(KMS_ROOT_CONFIG_UUID);
    const cipher = symmetricCipherService(SymmetricEncryption.AES_GCM_256);
    if (kmsRootConfig) {
      if (lock) await lock.release();
      logger.info(
        'Consumer Secrets KMS: Encrypted ROOT Key found from DB. Decrypting.',
      );
      const decryptedRootKey = cipher.decrypt(
        kmsRootConfig.encryptedRootKey,
        encryptionKeyBuffer,
      );
      // set the flag so that other instancen nodes can start
      await keyStore.setItemWithExpiry(
        KMS_ROOT_CREATION_WAIT_KEY,
        KMS_ROOT_CREATION_WAIT_TIME,
        'true',
      );
      logger.info('Consumer Secrets KMS: Loading ROOT Key into Memory.');
      ROOT_ENCRYPTION_KEY = decryptedRootKey;
      return;
    }

    logger.info('Consumer Secrets KMS: Generating ROOT Key');
    ...
    logger.info('Consumer Secrets KMS: Saved and loaded ROOT Key into memory');
    if (lock) await lock.release();
