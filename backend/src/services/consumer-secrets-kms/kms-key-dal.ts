...
export type TConsumerSecretsKmsKeyDALFactory = ReturnType<
  typeof kmskeyDALFactory
>;
    ...
    try {
      const result = await (tx || db.replicaNode())(TableName.KmsKey)
        ...
        .join(
          ...
          `${TableName.KmsKey}.orgId`,
          `${TableName.Organization}.id`,
        )
        .leftJoin(
          TableName.InternalKms,
          `${TableName.KmsKey}.id`,
          `${TableName.InternalKms}.kmsKeyId`,
        )
        .leftJoin(
          TableName.ExternalKms,
          `${TableName.KmsKey}.id`,
          `${TableName.ExternalKms}.kmsKeyId`,
        )
        .first()
        ...
        .select(
          ...
          db
            .ref('encryptedKey')
            .withSchema(TableName.InternalKms)
            .as('internalKmsEncryptedKey'),
          db
            .ref('encryptionAlgorithm')
            .withSchema(TableName.InternalKms)
            .as('internalKmsEncryptionAlgorithm'),
          db
            .ref('version')
            .withSchema(TableName.InternalKms)
            .as('internalKmsVersion'),
          db.ref('id').withSchema(TableName.InternalKms).as('internalKmsId'),
        )
        .select(
          ...
          db
            .ref('provider')
            .withSchema(TableName.ExternalKms)
            .as('externalKmsProvider'),
          db
            .ref('encryptedProviderInputs')
            .withSchema(TableName.ExternalKms)
            .as('externalKmsEncryptedProviderInput'),
          db
            .ref('status')
            .withSchema(TableName.ExternalKms)
            .as('externalKmsStatus'),
          db
            .ref('statusDetails')
            .withSchema(TableName.ExternalKms)
        ...
        .select(
          db
            .ref('kmsDefaultKeyId')
            .withSchema(TableName.Organization)
            .as('orgKmsDefaultKeyId'),
          db
            .ref('kmsEncryptedDataKey')
            .withSchema(TableName.Organization)
            .as('orgKmsEncryptedDataKey'),
        );
