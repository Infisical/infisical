...
interface NamespaceInvitationTemplateProps extends Omit<BaseEmailWrapperProps, "preview" | "title"> {
  callback_url: string;
  namespaceName: string;
}

export const NamespaceInvitationTemplate = ({
  callback_url,
  namespaceName,
  siteUrl
}: NamespaceInvitationTemplateProps) => {
  return (
    <BaseEmailWrapper
    ...
    >
      ...
      <Section className="px-[24px] mb-[28px] mt-[36px] pt-[12px] pb-[8px] border text-center border-solid border-gray-200 rounded-md bg-gray-50">
        <Text className="text-black text-[14px] leading-[24px]">
          You've been invited to join the namespace <strong>{namespaceName}</strong>.
        </Text>
      </Section>
      <Section className="text-center">
        <BaseButton href={callback_url}>Join Namespace</BaseButton>
      </Section>
...
export default NamespaceInvitationTemplate;

NamespaceInvitationTemplate.PreviewProps = {
  namespaceName: "Example Namespace",
  siteUrl: "https://infisical.com",
  callback_url: "https://app.infisical.com"
} as NamespaceInvitationTemplateProps;
