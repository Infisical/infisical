...
export const AuditReportTemplate = ({ siteUrl, projectName, reports }: AuditReportTemplateProps) => {
  return (
    <BaseEmailWrapper title="Audit Report" preview="Your requested audit report is ready." siteUrl={siteUrl}>
      <Heading className="text-black text-[18px] leading-[28px] text-center font-normal p-0 mx-0">
        Your <strong>audit report</strong> is ready
      </Heading>
      <Section className="px-[24px] mt-[36px] pt-[8px] pb-[8px] text-[14px] border border-solid border-gray-200 rounded-md bg-gray-50">
        <Text className="text-[14px]">
          The audit report you requested for the project <strong>{projectName}</strong> has been generated and is
          attached to this email as a CSV file. It includes the following {reports.length === 1 ? "report" : "reports"}:
        </Text>
        {reports.map((report) => (
