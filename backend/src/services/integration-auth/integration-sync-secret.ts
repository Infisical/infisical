...
const syncSecretsAWSParameterStore = async ({
  ...
  if (parameterList) {
    awsParameterStoreSecretsObj = parameterList.reduce(
      (obj: any, secret: any) => ({
        ...obj,
        [secret.Name.substring((integration.path as string).length)]: secret
      }),
      {}
    );
  }
...
const syncSecretsVercel = async ({
  ...
  const vercelSecrets: VercelSecret[] = (
    await request.get(`${IntegrationUrls.VERCEL_API_URL}/v9/projects/${integration.app}/env`, {
      params,
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Accept-Encoding": "application/json"
  ...
  for await (const vercelSecret of vercelSecrets) {
    if (vercelSecret.type === "encrypted") {
      ...
      const decryptedSecret = (
        await request.get(
          `${IntegrationUrls.VERCEL_API_URL}/v9/projects/${integration.app}/env/${vercelSecret.id}`,
          {
            params,
            headers: {
              Authorization: `Bearer ${accessToken}`,
  ...
  for await (const secret of updateSecrets) {
    if (secret.type !== "sensitive") {
      ...
      await request.patch(
        `${IntegrationUrls.VERCEL_API_URL}/v9/projects/${integration.app}/env/${id}`,
        updatedSecret,
        {
          params,
          headers: {
  ...
  for await (const secret of deleteSecrets) {
    await request.delete(
      `${IntegrationUrls.VERCEL_API_URL}/v9/projects/${integration.app}/env/${secret.id}`,
      {
        params,
        headers: {
          Authorization: `Bearer ${accessToken}`,
  ...
  Object.keys(secrets).forEach((key) => {
    if (!(key in res)) {
      ...
      if ((integration.targetEnvironment as string) in contexts) {
        ...
        if (secrets[key] !== contexts[integration.targetEnvironment as string].value) {
...
const syncSecretsGitLab = async ({
  ...
  const gitLabApiUrl = integrationAuth.url
    ...
    : IntegrationUrls.GITLAB_API_URL;
  ...
  const getAllEnvVariables = async (integrationAppId: string, accToken: string) => {
    ...
    let url: string | null =
      `${gitLabApiUrl}/v4/projects/${integrationAppId}/variables?per_page=100`;
...
const syncSecretsCheckly = async ({
  ...
  if (integration.targetServiceId) {
    ...
    let getSecretsRes = (
      await request.get(`${IntegrationUrls.CHECKLY_API_URL}/v1/variables`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          "Accept-Encoding": "application/json",
          "X-Checkly-Account": integration.appId
    ...
    for await (const key of Object.keys(secrets)) {
      if (!(key in getSecretsRes)) {
        ...
        // -> update/set secret
      } else if (secrets[key] !== getSecretsRes[key]) {
        await request.put(
...
const syncSecretsCloudflareWorkers = async ({
  ...
  const data: ConvertedSecret[] = Object.entries(secretsObj as SecretsObj).map(
    ([name, secret]) => ({
      name,
      text: secret.value,
      type: "secret_text"
...
const syncSecretsWindmill = async ({
  ...
  const res = (
    await request.get(`${IntegrationUrls.WINDMILL_API_URL}/w/${integration.appId}/variables/list`, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Accept-Encoding": "application/json"
      }
