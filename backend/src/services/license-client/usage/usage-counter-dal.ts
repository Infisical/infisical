...
export const usageCounterDALFactory = (db: TDbClient) => {
  ...
  // Distinct users + machine identities that belong to a Secret Management project, either through a
  ...
  const countSecretManagementIdentities = async (orgId?: string): Promise<number> => {
    try {
      // The live Secret Management project ids, resolved once per source. Scoping happens here on
      // projects.orgId (indexed via projects_orgid_type_idx) rather than on memberships.scopeOrgId:
      // leading with the org's handful of SM projects lets each source drive off the small project set
      ...
      // scope = project + a non-null actor column + whereIn against the SM project ids matches the
      ...
      const directUsers = db
        ...
        .whereIn(`${TableName.Membership}.scopeProjectId`, secretProjectIds())
      ...
      const row = (await db
        .replicaNode()
        .count("* as count")
        .from(distinctEntities.as("secret_identities"))
  ...
  return { countInternalCas, countActiveCerts, countPamResources, countSecretManagementIdentities };
};
