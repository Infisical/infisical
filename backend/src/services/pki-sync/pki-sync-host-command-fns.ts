...
// Windows sends the command as a UTF-16 + base64 `-EncodedCommand`, expanding it ~2.7x against an
// 8191-character command line, and substituted values share that budget.
export const HOST_COMMAND_MAX_LENGTH = 2048;
...
export const buildHostCommandContext = (args: {
  ...
  const { kind, command, destinationDirectory, certificates, pkcs12Password } = args;
  ...
  return {
    ...
    certificateFiles: certificates.flatMap((certificate) => certificate.paths),
    pkcs12Password
  };
};
...
export const withHostCommandDeadline = <T>(
  execute: () => Promise<T>,
  timeoutMs: number,
  graceMs: number
): Promise<T> => {
...
export const buildHostCommandFailureMessage = (kind: HostCommandKind, result: THostCommandResult): string => {
  ...
  if (result.exitCode === undefined) {
    ...
    return reason
      ? `${sentenceCase(kind)} failed: ${truncate(reason, MAX_FAILURE_DETAIL_CHARS)}`
      : `${sentenceCase(kind)} failed`;
  }

  const prefix = `${sentenceCase(kind)} failed (exit ${result.exitCode})`;
