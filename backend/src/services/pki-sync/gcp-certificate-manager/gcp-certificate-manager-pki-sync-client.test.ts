...
describe("mapGcpError", () => {
  test("names the missing permission and does not retry", () => {
    const mapped = mapGcpError(googleError(403, "PERMISSION_DENIED", "caller lacks permission"), {
      ...context,
      permission: "certificatemanager.certificates.create"
    });

    expect(mapped.shouldRetry).toBe(false);
    expect(mapped.message).toContain("certificatemanager.certificates.create");
...
describe("mapped messages fit the lastSyncMessage column", () => {
  ...
  ])("%s stays within 255 characters", (_label, error) => {
    const mapped = mapGcpError(error, {
      ...
      permission: "certificatemanager.certificatemapentries.create"
    });

    expect(mapped.message.length).toBeLessThanOrEqual(255);
  });
});
