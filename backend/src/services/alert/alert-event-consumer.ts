...
// The contract every event-triggered alert shares: an emitter names the targets by id and the provider
// rehydrates them at delivery time via findTargetsByIds. Keeping the payload to identifiers is what
// lets one consumer serve every provider without knowing their shapes.
...
export const alertEventConsumerFactory = ({
  ...
  // subscribesTo only sees the event type, so an emitter that pairs a valid event key with the wrong
  // resourceType gets past the gate. Catching it here, terminally and with a message naming both
  // halves, is what stops such a row from being silently marked delivered as "no matching alert".
  ...
  const handle = async (rows: TEventOutboxRow[]): Promise<TOutboxRowResult[]> => {
    // A batch shares one resource, so rows differ at most by event type and scope. Resolving the
    // matching alerts once per distinct combination keeps a burst of events from repeating the same
    ...
    for (const row of rows) {
      try {
        ...
        // Contained to this row: throwing out of handle() would send the whole batch back for retry,
        // re-notifying on every row that had already delivered ahead of this one.
        results.push({
          id: String(row.id),
