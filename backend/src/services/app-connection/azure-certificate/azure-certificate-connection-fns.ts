...
import {
  ...
  TAzureCertificateConnectionClientSecretCredentials,
  TAzureCertificateConnectionConfig,
  TAzureCertificateConnectionCredentials
} from "./azure-certificate-connection-types";

export const getAzureCertificateConnectionListItem = () => {
  const { INF_APP_CONNECTION_AZURE_CERTIFICATE_CLIENT_ID } = getConfig();

  return {
    name: "Azure Certificate" as const,
    app: AppConnection.AzureCertificate as const,
    methods: Object.values(AzureCertificateConnectionMethod) as [
      AzureCertificateConnectionMethod.OAuth,
      AzureCertificateConnectionMethod.ClientSecret
    ],
    oauthClientId: INF_APP_CONNECTION_AZURE_CERTIFICATE_CLIENT_ID
  };
};
...
export const getAzureConnectionAccessToken = async (
  ...
  if (appConnection.app !== AppConnection.AzureCertificate) {
    throw new BadRequestError({
      message: `Connection with ID '${connectionId}' is not an Azure Certificate connection`
    });
  }
  ...
  })) as TAzureCertificateConnectionCredentials;
  ...
  switch (appConnection.method) {
    case AzureCertificateConnectionMethod.OAuth:
      if (
        !appCfg.INF_APP_CONNECTION_AZURE_CERTIFICATE_CLIENT_ID ||
        !appCfg.INF_APP_CONNECTION_AZURE_CERTIFICATE_CLIENT_SECRET
      ) {
        throw new BadRequestError({
      ...
      const { data } = await request.post<ExchangeCodeAzureResponse>(
        ...
        new URLSearchParams({
          ...
          client_id: appCfg.INF_APP_CONNECTION_AZURE_CERTIFICATE_CLIENT_ID,
          client_secret: appCfg.INF_APP_CONNECTION_AZURE_CERTIFICATE_CLIENT_SECRET,
    ...
    case AzureCertificateConnectionMethod.ClientSecret:
      const accessTokenCredentials = (await decryptAppConnectionCredentials({
        ...
        kmsService,
        encryptedCredentials: appConnection.encryptedCredentials
      })) as TAzureCertificateConnectionClientSecretCredentials;
    ...
    default:
      throw new InternalServerError({
        message: `Unhandled Azure connection method: ${appConnection.method as AzureCertificateConnectionMethod}`
      });
  }
};

export const validateAzureCertificateConnectionCredentials = async (config: TAzureCertificateConnectionConfig) => {
  ...
  switch (method) {
    case AzureCertificateConnectionMethod.OAuth:
      ...
      if (!INF_APP_CONNECTION_AZURE_CERTIFICATE_CLIENT_ID || !INF_APP_CONNECTION_AZURE_CERTIFICATE_CLIENT_SECRET) {
        throw new InternalServerError({
      ...
      try {
        tokenResp = await request.post<ExchangeCodeAzureResponse>(
          ...
          new URLSearchParams({
            ...
            scope: `openid offline_access https://graph.microsoft.com/.default`,
            client_id: INF_APP_CONNECTION_AZURE_CERTIFICATE_CLIENT_ID,
            client_secret: INF_APP_CONNECTION_AZURE_CERTIFICATE_CLIENT_SECRET,
    ...
    default:
      throw new InternalServerError({
        message: `Unhandled Azure connection method: ${method as AzureCertificateConnectionMethod}`
      });
  }
};
