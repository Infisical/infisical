...
import { APIKeyData, ITokenVersion, IUser, ServiceTokenData, TokenVersion, User } from "../models";
import {
...
import { ServiceTokenAuthData, UserAuthData } from "../interfaces/middleware";
...
export const validateAuthMode = ({
  ...
  if (typeof authHeader === "string") {
    ...
    if (tokenType.toLowerCase() !== "bearer")
      throw BadRequestError({
        message: `The provided authentication type '${tokenType}' is not supported.`
      });
    if (tokenValue === null)
  ...
  if (!authMode || !authTokenValue)
    throw BadRequestError({
      message: "Missing valid Authorization or X-API-KEY in request header."
    });

  if (!acceptedAuthModes.includes(authMode))
    throw BadRequestError({ message: "The provided authentication type is not supported." });
...
export const getAuthUserPayload = async ({
  ...
  const decodedToken = <jwt.UserIDJwtPayload>jwt.verify(authTokenValue, await getJwtAuthSecret());
  ...
  if (!user?.publicKey)
    throw UnauthorizedRequestError({
      message: "Failed to authenticate user with partially set up account"
    });
...
export const getAuthSTDPayload = async ({
  ...
  const [_, TOKEN_IDENTIFIER, TOKEN_SECRET] = <[string, string, string]>(
    authTokenValue.split(".", 3)
  );

  const serviceTokenData = await ServiceTokenData.findById(
    TOKEN_IDENTIFIER,
    "+secretHash +expiresAt"
  );
  ...
  const serviceTokenDataToReturn = await ServiceTokenData.findOneAndUpdate(
    {
...
export const getAuthAPIKeyPayload = async ({
  ...
  let apiKeyData = await APIKeyData.findById(TOKEN_IDENTIFIER, "+secretHash +expiresAt").populate<{
    user: IUser;
  }>("user", "+publicKey");
