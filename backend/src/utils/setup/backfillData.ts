...
export const backfillRemoveDeniedPermissions = async () => {
  const lockKey = "backfill_remove_denied_permissions";
  const timeout = 900000; // 15 min lock timeout in milliseconds
  const lock = await redisClient?.set(lockKey, 1, "PX", timeout, "NX");

  if (lock) {
    try {
      console.info("Lock acquired for script [backfillRemoveDeniedPermissions]");
      ...
      console.error(
        "An error occurred when running script [backfillRemoveDeniedPermissions]:",
        error
      );
    }
  } else {
    console.info("Could not acquire lock for script [backfillRemoveDeniedPermissions], skipping");
  }
};
