...
import { parseSecretPathSearch } from "@app/lib/helpers/parseSecretPathSearch";
...
export const registerDashboardRouter = async (server: FastifyZodProvider) => {
  ...
  server.route({
    ...
    handler: async (req) => {
      ...
      const limit = 500; // arbitrary limit to prevent excessive results
