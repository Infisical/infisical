...
import { registerSecretNotesRouter } from '@app/server/routes/v1/secret-notes-router';
...
export const registerV1Routes = async (server: FastifyZodProvider) => {
  ...
  await server.register(registerOrgAdminRouter, {
  ...
  await server.register(registerSecretImportRouter, {
    prefix: '/secret-imports',
  });
  await server.register(registerSecretFolderRouter, { prefix: '/folders' });

  await server.register(
    async (workflowIntegrationRouter) => {
      await workflowIntegrationRouter.register(
        registerWorkflowIntegrationRouter,
      );
      await workflowIntegrationRouter.register(registerSlackRouter, {
        prefix: '/slack',
      });
    },
  ...
  await server.register(
    async (pkiRouter) => {
      ...
      await pkiRouter.register(registerCertificateTemplateRouter, {
        prefix: '/certificate-templates',
      });
      await pkiRouter.register(registerPkiAlertRouter, { prefix: '/alerts' });
      await pkiRouter.register(registerPkiCollectionRouter, {
        prefix: '/collections',
      });
    },
    { prefix: '/pki' },
  ...
  await server.register(registerIntegrationAuthRouter, {
    prefix: '/integration-auth',
  });
  await server.register(registerWebhookRouter, { prefix: '/webhooks' });
  await server.register(registerIdentityRouter, { prefix: '/identities' });
  await server.register(registerSecretSharingRouter, {
    prefix: '/secret-sharing',
  });
  await server.register(registerUserEngagementRouter, {
    prefix: '/user-engagement',
  });
  await server.register(registerDashboardRouter, { prefix: '/dashboard' });
  await server.register(registerCmekRouter, { prefix: '/kms' });
  await server.register(registerSecretNotesRouter, { prefix: '/csms' });
  await server.register(registerExternalGroupOrgRoleMappingRouter, {
    prefix: '/external-group-mappings',
  });
};
