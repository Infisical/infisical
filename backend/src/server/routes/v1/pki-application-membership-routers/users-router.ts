...
export const registerPkiApplicationUserMembershipRouter = async (server: FastifyZodProvider) => {
  ...
  server.route({
    ...
    schema: {
      ...
      description:
        "Add user members to a Cert Manager application by userId, email, or username. Bootstraps org and project membership for users not already present, then attaches them to the application.",
      tags: [ApiDocsTags.PkiApplications],
      params: ApplicationIdParamsSchema,
