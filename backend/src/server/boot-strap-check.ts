...
const bootstrapCb = async () => {
  ...
  // Use SITE_URL directly ONLY if it's not the default value
  const baseUrl = appCfg.SITE_URL && appCfg.SITE_URL !== "http://localhost:8080" 
    ? appCfg.SITE_URL 
    : `http://localhost:${appCfg.displayPort}`;
