...
import { CreateServiceAccountDto } from "@app/interfaces/serviceAccounts/dto";
import { BadRequestError, ServiceAccountNotFoundError } from "@app/utils/errors";
...
export const createServiceAccount = async (req: Request, res: Response) => {
  const { name, organizationId, publicKey, expiresIn }: CreateServiceAccountDto = req.body;
...
export const addServiceAccountKey = async (req: Request, res: Response) => {
  const { workspaceId, encryptedKey, nonce } = req.body;
...
export const addServiceAccountWorkspacePermission = async (req: Request, res: Response) => {
  const { serviceAccountId } = req.params;
  const { environment, workspaceId, read = false, write = false, encryptedKey, nonce } = req.body;

  if (
    !req.membership.workspace.environments.some(
      (e: { name: string; slug: string }) => e.slug === environment
    )
  ) {
  ...
  if (existingPermission)
    throw BadRequestError({
      message: "Failed to add workspace permission to service account due to already-existing "
    });
...
export const deleteServiceAccountWorkspacePermission = async (req: Request, res: Response) => {
  ...
  const serviceAccountWorkspacePermission =
    await ServiceAccountWorkspacePermission.findByIdAndDelete(serviceAccountWorkspacePermissionId);
