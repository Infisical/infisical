	...
	try {
		...
		const getSecretManagerSecrets = async ({
			...
			do {
				...
				const currPageSecretsRes: GCPSecretManager.IListSecretsResponse =
					(await standardRequest.get(url.href, { headers: DEFAULT_HEADERS }))
						?.data;
		...
		const getLatestSecretVersion = async ({
			...
			const latestSecretVersion: GCPSecretManager.IAccessSecretVersionResponse =
				(await standardRequest.get(url, { headers: DEFAULT_HEADERS }))?.data;
		...
		const createSecret = async ({
			...
			const createdSecret: GCPSecretManager.Secret = (
				await standardRequest.post(url.href, body, { headers: DEFAULT_HEADERS })
			)?.data;
		...
		const deleteSecret = async ({
			...
			await standardRequest.delete(url.href, { headers: DEFAULT_HEADERS });
		};
		...
		const addSecretVersion = async ({
			...
			await standardRequest.post(url, body, { headers: DEFAULT_HEADERS });
		};
