...
- [x] Docs tree under `docs/documentation/platform/agent-vault/` and `docs/cli/commands/av.mdx`, `docs.json` entries, the five must-document items from §4.2 including the plaintext-token note, `make lint-docs-branch` clean. **Open: `docs.json` already carries an "Agent Vault" group under Agent proxy linking to the external docs.agent-vault.dev (commit fd5d202c87, an unrelated product of the same name); the new top-level item sits beside it until the product owner says which stays**
