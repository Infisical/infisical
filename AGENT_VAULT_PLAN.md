...
| 4 — backend tail, `av run`, docs, visibility commit | **done** | `agent_vault_identities` is metered beside `pam_identities` at every membership write path (36 sibling emits plus the bootstrap paths), `AuditLogStreamProduct.AgentVault` exists with its picker entry, the invite route takes `grantAgentVaultAccess` and routes it through a PAM-shaped `agentVaultMembership.addProductUserMembers` (settled with the product owner over the generic call), and `DailyResourceCleanup` runs `sweepRetiredSessions`: it emits `session-expire` for sessions whose expiry fell since the last sweep (watermark in the keystore, one-day look-back on first run) and hard-deletes rows 30 days past `expiresAt` / `revokedAt`. Harness note: the backend container sets `REDIS_READ_REPLICAS` to the dev Redis, so a keystore write to the throwaway Redis reads back as null unless `REDIS_READ_REPLICAS=` (empty) is also passed to `docker exec`; the container mounts only `backend/src`, so a changed spec has to be `docker cp`'d in before it runs. The sweep test builds the service from the real DALs because the services decorator is scoped inside the routes plugin and not reachable from a test. `av run` landed in the CLI repo (`2ffb2c7`) and was verified live with `--token`; the docs tree (overview, quickstart, access bundles, sessions, proxies, `cli/commands/av`) is written and Vale-clean, and the three `CLAUDE.md` files carry the cross-cutting notes. The `--access-bundle` path was then verified live, the docs nav collision was settled by removing the external link, `make reviewable-api` and `make reviewable-ui` pass, and the visibility commit closes the branch. Both PRs remain to be opened from the drafted descriptions; nothing is pushed |
...
`DB_CONNECTION_URI`, `REDIS_URL` and an empty `REDIS_READ_REPLICAS` explicitly to `docker exec`**: the container sets both in its own
...
- [x] `av run`: `--access-bundle` mint path and `--token` path, CA fetched from `/_agent-vault/ca`, optional `--ca-fingerprint` pin, trust variables, `NO_PROXY` merge, revoke-on-exit, `ensureCATrusted` on macOS. Verified live with `--token` against the enrolled `local-test` proxy (credential arrived at postman-echo, child env clean, wrong pin aborted); the `--access-bundle` path verified live after a CLI login for the dev domain: a 1h session minted as the user, the credential arrived upstream, the row was revoked on exit, and an unknown bundle name is refused naming the reachable ones
- [x] Docs tree under `docs/documentation/platform/agent-vault/` and `docs/cli/commands/av.mdx`, `docs.json` entries, the five must-document items from §4.2 including the plaintext-token note, `make lint-docs-branch` clean. `docs.json` carried an "Agent Vault" group under Agent proxy linking to the external docs.agent-vault.dev (commit fd5d202c87, a different product with the same name and pitch); settled with the product owner: the external link is removed and the new top-level item beside PAM is the only Agent Vault entry
- [x] End-to-end on one box (§Verification, Phase 4): `av proxy` on 17324 (17323 is held by an older process on this machine) and `av run` with both `--token` and `--access-bundle`, the agent holding no credential. The old `secrets agent-proxy start` was not started alongside, since it needs machine-identity credentials the dev stack does not have; the port split is enforced by the differing defaults rather than observed live
- [x] Root, backend and frontend `CLAUDE.md` updated for the cross-cutting pieces
- [x] The visibility commit: `PRODUCT_TYPES` ×2 and `SIGNUP_PRODUCTS`, last on the branch. It also carries what the arrays alone did not cover: the org-overview tile needs its own stats case, membership gate and join-on-behalf entry (mirroring PAM's), the signup steps need an Agent Vault arm where PAM skips project creation and picks a launch destination, and the signup-onboarding product unions on both sides had to widen before the two signup pages could pass `grantAgentVaultAccess`
- [x] `make reviewable-api` and `make reviewable-ui` once (both pass; the api run produced import-order fixes, committed as a chore); both PR descriptions drafted from the template and cross-linked (not pushed; they live in the session scratchpad as `PR_INFISICAL.md` and `PR_CLI.md` until the PRs are opened); Progress table finalised
