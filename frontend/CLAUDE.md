...
That global handler only fires for failures that go through `useMutation`. Raw exported API functions bypass `MutationCache` entirely. `selectOrganization` (`src/hooks/api/auth/queries.tsx`) is exported both raw and as `useSelectOrganization`, and its raw callers (router `beforeLoad` and components alike) get no toast unless they call `onRequestError(error)` themselves, as `src/pages/auth/SelectOrgPage/route.tsx` does. Check which form a call site uses before adding or removing error UI.
