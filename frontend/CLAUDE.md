...
- Unsaved changes: use `useDiscardChangesGuard` with `DiscardChangesAlertDialog`. Overlay editors (Sheet/Dialog) wire dismiss/close to `requestDiscard`. In-page editors also pass `blockNavigation: true` so TanStack Router navigations reuse that dialog and refresh/tab close get the native `beforeunload` prompt (browsers cannot customize it). Do not use `window.confirm`. Do not use `useNavigationBlocker` — that hook is secrets batch-mode only.
