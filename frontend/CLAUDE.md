...
PAM and Agent Vault live at `/organizations/$orgId/<slug>` over a single implicit project, so their URLs carry no `$projectId`. `useProject()` and `useProjectPermission()` resolve that project through `useImplicitProjectId()`, which takes whichever of two sources still names the product: the `implicitProjectId` a product layout's `beforeLoad` publishes into route context, or the product in the pathname mapped to the org's `<product>ProjectId`. Both are needed because the router moves them at different times — location changes first, matches swap after — so components gating on the pathname mount on the leading edge while the product layout unmounts on the trailing one, and reading either source alone throws `ErrorPage` at whichever end it has already left. A new org-scoped product therefore needs four things: a `<product>ProjectId` on the `Organization` type, an entry in `ORG_SCOPED_PRODUCT_TYPES` and `getOrgScopedProductFromPath` (`helpers/project.ts`), a layout `beforeLoad` that bootstraps the id and patches it into the cached org, and that same `beforeLoad` returning it as `implicitProjectId` (see `pages/agent-vault/layout.tsx`). Use `isOrgScopedProduct(type)` rather than adding another pathname check; the sidebar, navbar, project switcher and nav links all key off it.
