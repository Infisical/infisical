...
export const getProjectDescription = (type: ProjectType) => {
  const descriptions: Partial<Record<ProjectType, string>> = {
    [ProjectType.SecretManager]:
      "Store, sync, and rotate secrets across environments with dynamic credentials and lifecycle policies.",
    [ProjectType.CertificateManager]:
    ...
    [ProjectType.PAM]:
      "Grant just-in-time access, broker sessions, and vault credentials for privileged users and machines."
  };
