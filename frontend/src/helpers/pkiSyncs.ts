...
export const SINGLE_CERTIFICATE_HOST_COMMAND_VARIABLES = [
...
/**
 * The reason a sync operation failed, as stored. Some destinations report a JSON body, so it is
 * pretty-printed when it parses. Returns null when the operation did not fail or reported nothing:
 * inventing a message would tell the reader less than showing none.
 */
