...
const serviceAccountKeys = {
  getServiceAccountById: (serviceAccountId: string) =>
    [{ serviceAccountId }, "service-account"] as const,
  getServiceAccounts: (organizationID: string) => [{ organizationID }, "service-accounts"] as const,
  getServiceAccountProjectLevelPermissions: (serviceAccountId: string) =>
    [{ serviceAccountId }, "service-account-project-level-permissions"] as const
};
  ...
  return useMutation<CreateServiceAccountRes, {}, CreateServiceAccountDTO>({
    ...
    onSuccess: ({ serviceAccount }) => {
      queryClient.invalidateQueries(
        serviceAccountKeys.getServiceAccounts(serviceAccount.organization)
      );
    }
  });
};
...
export const useRenameServiceAccount = () => {
  ...
  return useMutation<ServiceAccount, {}, RenameServiceAccountDTO>({
    mutationFn: async ({ serviceAccountId, name }) => {
      const {
        data: { serviceAccount }
      } = await apiRequest.patch(`/api/v2/service-accounts/${serviceAccountId}/name`, { name });
...
export const useDeleteServiceAccount = () => {
  ...
  return useMutation<ServiceAccount, {}, string>({
    mutationFn: async (serviceAccountId) => {
      const {
        data: { serviceAccount }
      } = await apiRequest.delete(`/api/v2/service-accounts/${serviceAccountId}`);
      return serviceAccount;
    },
...
const fetchServiceAccountProjectLevelPermissions = async (serviceAccountId: string) => {
  const {
    data: { serviceAccountWorkspacePermissions }
  } = await apiRequest.get<{
    serviceAccountWorkspacePermissions: ServiceAccountWorkspacePermission[];
...
export const useCreateServiceAccountProjectLevelPermission = () => {
  ...
  >({
    mutationFn: async (body) => {
      const {
        data: { serviceAccountWorkspacePermission }
      } = await apiRequest.post(
        `/api/v2/service-accounts/${body.serviceAccountId}/permissions/workspace`,
        body
      );
    ...
    onSuccess: ({ serviceAccount }) => {
      queryClient.invalidateQueries(
        serviceAccountKeys.getServiceAccountProjectLevelPermissions(serviceAccount)
      );
    }
  });
};
...
export const useDeleteServiceAccountProjectLevelPermission = () => {
  ...
  >({
    ...
    onSuccess: (serviceAccountWorkspacePermission) => {
      queryClient.invalidateQueries(
        serviceAccountKeys.getServiceAccountProjectLevelPermissions(
          serviceAccountWorkspacePermission.serviceAccount
        )
      );
    }
  });
