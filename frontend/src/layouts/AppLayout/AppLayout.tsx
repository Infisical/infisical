					...
					<aside className="dark w-full border-r border-mineshaft-600 bg-gradient-to-tr from-mineshaft-700 via-mineshaft-800 to-mineshaft-900 md:w-60">
						<nav className="items-between flex h-full flex-col justify-between overflow-y-auto dark:[color-scheme:dark]">
                          ...
                          >
								...
								<div className={`px-1 ${!router.asPath.includes("personal") ? "block" : "hidden"}`}>
									{(router.asPath.includes("project") || router.asPath.includes("integrations")) &&
										...
										<Menu className="mt-4">
											...
											<Link href={`/org/${currentOrg?.id}/user-credentials`} passHref>
												<a>
													<MenuItem
														isSelected={router.asPath === `/org/${currentOrg?.id}/user-credentials`}
														icon="system-outline-189-domain-verification"
													>
														User Credentials
													</MenuItem>
							...
							<div
								className={`relative mt-10 ${subscription && subscription.slug === "starter" && !subscription.has_used_trial
										? "mb-2"
										: "mb-4"
