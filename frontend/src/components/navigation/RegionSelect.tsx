...
export const RegionSelect = ({ compact }: RegionSelectProps) => {
  ...
  return (
    <div className={compact ? "shrink-0" : "mb-8 flex flex-col items-center"}>
      <Select
        className={compact ? "w-20 pl-2 pr-2 [&>*:last-child]:!ml-1" : "w-full max-w-md"}
        dropdownContainerClassName="min-w-[var(--radix-select-trigger-width)] max-w-md"
        ...
        renderValue={(value) => {
          ...
          return region ? (
            <div className="flex items-center gap-2">
              <div className="h-4 w-5 shrink-0">{region.flag}</div>
              ...
              <ul className="ml-8 flex flex-col gap-1 text-sm text-mineshaft-400">
                {bullets.map((bullet) => (
                  <li key={bullet} className="flex items-start gap-2">
                    <FontAwesomeIcon size="xs" className="mt-1 shrink-0 text-primary" icon={faCheck} />
                    <span>{bullet}</span>
                  </li>
                ))}
              </ul>
