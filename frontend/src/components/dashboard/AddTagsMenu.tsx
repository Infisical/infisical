            ...
            return (
              <Menu.Item key={tag._id}>
                <button
                  ...
                  } w-full text-left bg-mineshaft-800 px-2 py-0.5 text-bunker-200 rounded-sm flex items-center`}
                  onClick={() => {
                ...
                >
                  {currentTags?.map((currentTag) => currentTag.name).includes(tag.name) ? (
                    <FontAwesomeIcon icon={faCheckSquare} className="text-xs mr-2 text-primary" />
                  ) : (
                    <FontAwesomeIcon icon={faSquare} className="text-xs mr-2" />
                  )}{" "}
                  {tag.name}
                </button>
              </Menu.Item>
