...
const KeyPair = ({
  ...
  return (
    <div
      className={`group flex flex-col items-center border-b border-mineshaft-500 hover:bg-white/[0.03] duration-100 ${
    ...
    >
      <div className="relative flex flex-row justify-between w-full mr-auto max-h-14 items-center">
        <div className="w-[23%] border-r border-mineshaft-600 flex flex-row items-center">
          <div className="text-bunker-400 text-xs flex items-center justify-center w-14 h-10 cursor-default">
            {keyPair.pos + 1}
          </div>
            ...
            {keyPair.tags?.map(
              (tag, index) =>
                index < 2 && (
                  <div
                    key={keyPair.pos}
                    className={`ml-2 px-1.5 ${tagData.filter((tagDp) => tagDp._id === tag._id)[0]
                      ?.color} rounded-sm text-sm ${tagData.filter(
                      (tagDp) => tagDp._id === tag._id
                    )[0]?.colorText} flex items-center`}
                  >
                    ...
                    <FontAwesomeIcon
                      ...
                      onClick={() =>
                        modifyTags(
                          keyPair.tags.filter((ttag) => ttag._id !== tag._id),
                          keyPair.id
                        )
                      }
                    />
        ...
        <div
          ...
          className={`cursor-pointer w-[1.5rem] h-[2.35rem] ml-auto group-hover:bg-mineshaft-700 z-50 rounded-md invisible group-hover:visible flex flex-row justify-center items-center ${
            isSnapshot ?? "invisible"
          }`}
        >
          <FontAwesomeIcon
            className="text-bunker-300 hover:text-primary text-lg"
            icon={faEllipsis}
          />
        </div>
