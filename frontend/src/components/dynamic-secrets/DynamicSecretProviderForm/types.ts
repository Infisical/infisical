...
export type TDynamicSecretProviderFieldGroup<TValues extends FieldValues> =
  | (TDynamicSecretProviderFieldGroupBase<TValues> & {
      presentation?: "panel";
      /** Optional. Panels can rely on field labels alone. */
      title?: string;
      /**
       * Opt-in bordered surface. Groups are unstyled by default — reserve for
       * clusters that render a real header (`title`).
       */
      surface?: boolean;
    })
  | (TDynamicSecretProviderFieldGroupBase<TValues> & {
      presentation: "collapse";
      /** Required — rendered as a sheet-section header peer to Configuration. */
      title: string;
    });
...
): item is TDynamicSecretProviderFieldGroup<TValues> =>
...
> = {
  ...
  create: {
    ...
    getDefaultValues: (context: TCreateDynamicSecretProviderFormContext) => TCreateValues;
    toPayload: (
  ...
  edit: {
    ...
    getDefaultValues: (context: TEditDynamicSecretProviderFormContext) => TEditValues;
    toPayload: (
      values: TEditValues,
