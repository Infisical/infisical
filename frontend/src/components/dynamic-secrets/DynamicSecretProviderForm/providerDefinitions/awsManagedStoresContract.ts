...
const commonCreateDefaults = (context: TCreateDynamicSecretProviderFormContext) => ({
  name: "",
  defaultTTL: "1h",
  maxTTL: "24h",
...
export const getAwsMemoryDbCreateDefaultValues = (
  ...
  inputs: {
    clusterName: "",
    region: "",
    auth: { type: AwsMemoryDbAuthType.IAM, accessKeyId: "", secretAccessKey: "" },
    creationStatement: MEMORYDB_CREATION_STATEMENT,
    revocationStatement: MEMORYDB_REVOCATION_STATEMENT
  }
});

const commonEditDefaults = (context: TEditDynamicSecretProviderFormContext) => ({
...
export const getAwsElastiCacheCreatePayload = (
  ...
  ...createBase(values, context),
  provider: {
    type: DynamicSecretProviders.AwsElastiCache as const,
...
export const getAwsMemoryDbCreatePayload = (
  ...
  ...createBase(values, context),
  provider: {
    type: DynamicSecretProviders.AwsMemoryDb as const,
...
export const getAwsElastiCacheEditPayload = (
  ...
  context: TEditDynamicSecretProviderFormContext
) => editPayload(values, context, awsElastiCacheEditInputsSchema.parse(values.inputs));
export const getAwsMemoryDbEditPayload = (
  values: TAwsMemoryDbEditValues,
  context: TEditDynamicSecretProviderFormContext
) => editPayload(values, context, awsMemoryDbEditInputsSchema.parse(values.inputs));
