...
export const clickHousePasswordRequirementsSchema = z
  .object({
    length: z.coerce.number().min(1).max(250),
    required: z
      .object({
        lowercase: z.coerce.number().min(0),
        uppercase: z.coerce.number().min(0),
        digits: z.coerce.number().min(0),
        symbols: z.coerce.number().min(0)
      })
      .refine(
...
export type TClickHouseFormValues = TDynamicSecretProviderFormValues<
  z.input<typeof clickHouseCreateInputsSchema>
