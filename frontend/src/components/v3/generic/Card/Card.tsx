...
function UnstableCard({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex h-fit flex-col gap-6 rounded-md border border-border bg-container px-4 py-6 text-foreground shadow-sm",
        className
      )}
      {...props}
    />
...
function UnstableCardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center gap-1.5 text-lg leading-none font-semibold [&>svg]:inline-block [&>svg]:size-[18px]",
        className
      )}
      {...props}
    />
