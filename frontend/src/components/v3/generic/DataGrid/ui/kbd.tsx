...
export function Kbd({ className, ...props }: React.HTMLAttributes<HTMLElement>) {
  return (
    <kbd
      className={cn(
        "pointer-events-none inline-flex h-5 items-center gap-1 rounded border border-mineshaft-600 bg-mineshaft-800 px-1.5 font-mono text-[10px] font-medium text-mineshaft-300 select-none",
        className
      )}
      {...props}
    />
