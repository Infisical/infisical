...
export const TableContainer = forwardRef<HTMLDivElement, TableContainerProps>(
  ({ children, className, isRounded = true, ...props }, ref): JSX.Element => (
    <div
      ref={ref}
      className={twMerge(
        "relative w-full overflow-x-auto border border-solid border-border-subtle bg-surface-raised font-inter",
...
export const Table = ({ children, className }: TableProps): JSX.Element => (
  <table
    className={twMerge("w-full bg-surface-raised p-2 text-left text-sm text-label-cool", className)}
  >
    {children}
  </table>
);
...
export const THead = ({ children, className, style }: THeadProps): JSX.Element => (
  <thead
    className={twMerge("bg-surface-raised text-xs text-label-secondary uppercase", className)}
    style={style}
  >
    {children}
  </thead>
...
export const TFoot = ({ children, className }: TFootProps): JSX.Element => (
  <tfoot className={twMerge("bg-surface-raised text-xs text-label-secondary uppercase", className)}>
    {children}
  </tfoot>
);
...
export const Tr = ({
  ...
  <tr
    className={twMerge(
      "cursor-default border-b border-solid border-border-control last:border-b-0",
      isHoverable && "hover:bg-surface-active",
...
export const Th = ({ children, className, style }: ThProps): JSX.Element => (
  <th
    className={twMerge(
      "border-b-2 border-border-control bg-surface-raised px-5 pt-4 pb-3.5 font-medium",
      className
    )}
    style={style}
  >
