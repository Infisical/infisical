...
export const NoticeBanner = ({ icon = faWarning, title, children, className }: Props) => (
  <div
    className={twMerge(
      "flex w-full flex-row items-center rounded-md border border-primary-600/70 bg-primary/[.07] p-4 text-base text-foreground",
      className
    )}
  >
    <FontAwesomeIcon icon={icon} className="pr-6 text-4xl text-foreground/80" />
