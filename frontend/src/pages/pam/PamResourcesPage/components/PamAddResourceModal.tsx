      ...
      >
        <SheetContent className="flex h-full max-h-full flex-col gap-y-0 sm:max-w-lg">
          <SheetHeader className="border-b">
            <SheetTitle>
              Create{" "}
              {selectedResourceType === PamResourceType.ActiveDirectory ? "Domain" : "Resource"}
            </SheetTitle>
            <SheetDescription>
              {selectedResourceType
                ? `Input ${selectedResourceType === PamResourceType.ActiveDirectory ? "domain" : "resource"} connection details`
