...
const PERMISSION_ACTIONS = [
  { action: NamespacePermissionNamespaceActions.Edit, label: "Edit" },
  { action: NamespacePermissionNamespaceActions.Delete, label: "Delete" }
] as const;
  ...
  return (
    <>
      ...
      >
        ...
        <Td className="w-full select-none">Namespace</Td>
        <Td>
          <Select
          ...
          <Td colSpan={3} className="border-mineshaft-500 bg-mineshaft-900 p-8">
            <div className="flex flex-grow flex-wrap justify-start gap-x-8 gap-y-4">
              {PERMISSION_ACTIONS.map(({ action, label }) => {
                return (
                  <Controller
                    name={`permissions.namespace.${action}`}
                    key={`permissions.namespace.${action}`}
                    control={control}
                    render={({ field }) => (
                      <Checkbox
