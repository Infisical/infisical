...
type Props = {
  identityNamespaceMembership: TNamespaceIdentityMembership;
};
...
export const IdentityRoleModify = ({ identityNamespaceMembership }: Props) => {
  ...
  const roleForm = useForm<TRoleForm>({
    ...
    values: {
      roles: identityNamespaceMembership?.roles?.map(({ customRoleSlug, role, ...dto }) => ({
        slug: customRoleSlug || role,
        temporaryAccess: dto.isTemporary
          ? {
              isTemporary: true,
              temporaryRange: dto.temporaryRange as string,
              temporaryAccessEndTime: dto.temporaryAccessEndTime as string,
              temporaryAccessStartTime: dto.temporaryAccessStartTime as string
            }
          : {
  ...
  const updateIdentityNamespaceRole = useUpdateNamespaceIdentityMembership();

  const handleRoleUpdate = async (data: TRoleForm) => {
    if (updateIdentityNamespaceRole.isPending) return;
    ...
    const sanitizedRoles = data.roles.map((el) => {
      ...
      return {
        ...
        temporaryMode: NamespaceIdentityMembershipTemporaryMode.Relative,
            ...
            <div key={id} className="flex items-center space-x-2">
              <Controller
                ...
                render={({ field: { onChange, ...field } }) => (
                  ...
                  >
                    {roles?.map(({ name, slug, id: namespaceRoleId }) => (
