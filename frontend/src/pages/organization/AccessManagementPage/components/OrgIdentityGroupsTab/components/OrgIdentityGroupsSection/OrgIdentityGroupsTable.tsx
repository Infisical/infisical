...
export const OrgIdentityGroupsTable = ({ handlePopUpOpen }: Props) => {
  const navigate = useNavigate();
  const { currentOrg } = useOrganization();
  const orgId = currentOrg?.id || "";
  const { isPending, data: identityGroups = [] } = useGetOrganizationIdentityGroups(orgId);
  const { mutateAsync: updateMutateAsync } = useUpdateIdentityGroup();
  ...
  const {
    ...
    initPerPage: getUserTablePreference("orgIdentityGroupsTable", PreferenceKey.PerPage, 20)
  });

  const handlePerPageChange = (newPerPage: number) => {
    setPerPage(newPerPage);
    setUserTablePreference("orgIdentityGroupsTable", PreferenceKey.PerPage, newPerPage);
  };
  ...
  }, [search, identityGroups, orderBy, orderDirection]);
                          ...
                          <DropdownMenuContent sideOffset={2} align="end">
                            <DropdownMenuItem
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                createNotification({
                                  text: "Copied identity group ID to clipboard",
                                  type: "info"
                                });
          ...
          <EmptyState
            title={
              identityGroups.length
                ? "No organization identity groups match search..."
                : "No organization identity groups found"
            }
            icon={identityGroups.length ? faSearch : faUsers}
          />
        )}
      </TableContainer>
    </div>
