...
type Props = {
  popUp: UsePopUpState<["identityGroup"]>;
  handlePopUpClose: (popUpName: keyof UsePopUpState<["identityGroup"]>) => void;
  handlePopUpToggle: (popUpName: keyof UsePopUpState<["identityGroup"]>, state?: boolean) => void;
};
...
export const OrgIdentityGroupModal = ({ popUp, handlePopUpClose, handlePopUpToggle }: Props) => {
  ...
  useEffect(() => {
    const group = popUp?.identityGroup?.data as {
  ...
  }, [popUp?.identityGroup?.data, roles]);
  ...
  const onGroupModalSubmit = async ({ name, slug, role }: TGroupFormData) => {
    try {
      ...
      handlePopUpToggle("identityGroup", false);
      reset();

      createNotification({
        text: `Successfully ${popUp?.identityGroup?.data ? "updated" : "created"} identity group`,
        type: "success"
      });
    } catch {
      createNotification({
        text: `Failed to ${popUp?.identityGroup?.data ? "updated" : "created"} identity group`,
        type: "error"
      });
    }
  };
  ...
  return (
    <Modal
      isOpen={popUp?.identityGroup?.isOpen}
      onOpenChange={(isOpen) => {
        handlePopUpToggle("identityGroup", isOpen);
        reset();
      }}
    >
      <ModalContent
        bodyClassName="overflow-visible"
        title={`${popUp?.identityGroup?.data ? "Update" : "Create"} Identity Group`}
      >
        <form onSubmit={handleSubmit(onGroupModalSubmit)}>
          ...
          <Controller
            ...
            render={({ field: { onChange, value }, fieldState: { error } }) => (
              <FormControl
                label={`${popUp?.identityGroup?.data ? "Update" : ""} Role`}
          ...
          <div className="mt-8 flex items-center">
            ...
            >
              {!popUp?.identityGroup?.data ? "Create" : "Update"}
            </Button>
            <Button
              colorSchema="secondary"
              variant="plain"
              onClick={() => handlePopUpClose("identityGroup")}
            >
              Cancel
            </Button>
          </div>
