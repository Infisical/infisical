...
import { useGetProjectRoles, useUpdateIdentityGroupWorkspaceRole } from "@app/hooks/api";
import { TIdentityGroupMembership } from "@app/hooks/api/identity-groups/types";
import { TProjectRole } from "@app/hooks/api/roles/types";
import { IdentityGroupUserMembershipTemporaryMode } from "@app/hooks/api/workspace/types";
...
export type TMemberRolesProp = {
  ...
  className?: string;
  roles: TIdentityGroupMembership["roles"];
...
const IdentityGroupRolesForm = ({ projectRoles, roles, identityGroupId, onClose }: FormProps) => {
  ...
  const updateGroupWorkspaceRole = useUpdateIdentityGroupWorkspaceRole();
  ...
  const handleRoleUpdate = async (data: TForm) => {
    const selectedRoles = Object.keys(data)
      ...
      .map((el) => {
        ...
        return {
          ...
          temporaryMode: IdentityGroupUserMembershipTemporaryMode.Relative,
    ...
    try {
      ...
      createNotification({ text: "Successfully updated identity group role", type: "success" });
      onClose();
      setSearchRoles("");
    } catch {
      createNotification({ text: "Failed to update identity group role", type: "error" });
    }
  };
...
export const IdentityGroupRoles = ({
  roles = [],
  disableEdit = false,
  identityGroupId,
  className,
