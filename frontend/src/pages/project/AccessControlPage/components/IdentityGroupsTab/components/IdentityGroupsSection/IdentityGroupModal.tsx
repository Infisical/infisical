...
const Content = ({ popUp, handlePopUpToggle }: Props) => {
  ...
  const { data: groups } = useGetOrganizationIdentityGroups(orgId);
  const { data: groupMemberships } = useListWorkspaceIdentityGroups(currentWorkspace?.id || "");

  const { data: roles } = useGetProjectRoles(currentWorkspace?.id || "");

  const { mutateAsync: addGroupToWorkspaceMutateAsync } = useAddIdentityGroupToWorkspace();
  ...
  const onFormSubmit = async ({ group, role }: FormData) => {
    try {
      ...
      createNotification({
        text: "Successfully added identity group to project",
        type: "success"
      });
    } catch {
      createNotification({
        text: "Failed to add identity group to project",
        type: "error"
      });
    }
  };
  ...
  return filteredGroupMembershipOrgs.length ? (
    <form onSubmit={handleSubmit(onFormSubmit)}>
      ...
      <div className="mt-6 flex items-center">
        ...
        >
          {popUp?.group?.data ? "Update" : "Add Identity Group"}
        </Button>
        <Button
    ...
    <div className="flex flex-col space-y-4">
      <div className="text-sm">
        All identity groups in your organization have already been added to this project.
      </div>
      <Link to="/organization/access-management">
        <Button variant="outline_bg">Create a new identity group</Button>
      </Link>
    </div>
  );
};

export const IdentityGroupModal = ({ popUp, handlePopUpToggle }: Props) => {
  return (
    <Modal
      isOpen={popUp?.group?.isOpen}
      onOpenChange={(isOpen) => handlePopUpToggle("group", isOpen)}
    >
      <ModalContent bodyClassName="overflow-visible" title="Add Identity Group to Project">
