      ...
      <Collapsible open={open} onOpenChange={setOpen}>
        <div className="group flex items-center rounded-sm px-2 py-1">
          ...
          <div className="mr-2">
            {level > 0 ? (
              open ? (
                <FolderOpenIcon className="h-4 w-4 text-yellow" />
              ) : (
                <FolderIcon className="h-4 w-4 text-yellow" />
              )
            ) : (
              <FolderTreeIcon className="h-4 w-4 text-muted" />
            )}
          </div>
          {!isDisabled && (
            <Checkbox
          ...
          <label
            htmlFor={`folder-${path}`}
            className={`ml-2 flex-1 cursor-pointer truncate ${basePath ? "text-muted italic" : ""}`}
            title={displayName}
          >
...
export const SecretTreeView: React.FC<TreeViewProps> = ({
  ...
  return (
    <div className="flex w-full items-start gap-3 rounded-lg border border-border bg-container">
