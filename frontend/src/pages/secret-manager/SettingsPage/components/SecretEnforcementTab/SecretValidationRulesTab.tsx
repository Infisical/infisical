      ...
      <div className="flex h-full w-full flex-1 flex-col rounded-lg border border-border bg-card py-4">
        ...
        <FormProvider {...form}>
          ...
          >
            ...
            {/* Footer */}
            <div className="flex items-center justify-end gap-2 border-t border-border px-4 pt-4">
...
type ViewState = { mode: "list" } | { mode: "create" } | { mode: "edit"; ruleId: string };
...
export const SecretValidationRulesTab = () => {
  ...
  const handleSubmit = async (data: TRuleForm, isActive?: boolean) => {
    try {
      ...
      createNotification({
        text: `Failed to ${view.mode === "edit" ? "update" : "create"} rule`,
        type: "error"
      });
    }
  };
                        ...
                        <UnstableTableRow key={rule.id}>
                          ...
                          <UnstableTableCell>
                            <UnstableDropdownMenu>
                              ...
                              <UnstableDropdownMenuContent align="end">
                                <UnstableDropdownMenuItem onClick={() => handleEdit(rule.id)}>
  ...
  const editingRule = view.mode === "edit" ? rules.find((r) => r.id === view.ruleId) : undefined;
