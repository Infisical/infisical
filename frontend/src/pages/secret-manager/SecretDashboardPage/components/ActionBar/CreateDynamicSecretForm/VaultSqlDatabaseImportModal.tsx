...
const Content = ({ onClose, onImport }: ContentProps) => {
  ...
  // Filter to only show database mounts
  ...
  const handleImport = () => {
    if (!selectedRole) {
      createNotification({
        type: "error",
        text: "Please select a Vault database role to load"
      });
      return;
    }
  ...
  return (
    <>
      ...
      <FormControl
        label="Namespace"
        className="mb-4"
        tooltipText="Select the Vault namespace containing the database secrets engine."
      >
        <>
          <FilterableSelect
          ...
          <p className="mt-1 text-xs text-mineshaft-400">
            Select the Vault namespace to fetch available database secrets engines
          </p>
        </>
      </FormControl>
      ...
      <FormControl
        ...
        className="mb-4"
        tooltipText="Select the database secrets engine mount to fetch available roles."
      >
        <>
          <FilterableSelect
      ...
      <FormControl label="Database Role" className="mb-6">
        <>
          <FilterableSelect
            value={selectedRole}
            onChange={(value) => {
...
export const VaultSqlDatabaseImportModal = ({ isOpen, onOpenChange, onImport }: Props) => {
  return (
