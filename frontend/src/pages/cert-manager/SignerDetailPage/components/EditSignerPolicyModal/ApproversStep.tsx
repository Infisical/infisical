...
export const ApproversStep = ({
  ...
  <>
    ...
    <div className="mb-4 flex items-start justify-between gap-4">
      <div>
        <p className="text-sm font-semibold text-foreground">Approves</p>
    ...
    {steps.length === 0 ? (
      <div className="flex flex-col items-center gap-2 rounded-md border border-dashed border-border-subtle p-8 text-center">
        <ShieldIcon className="h-5 w-5 text-surface-selected" />
        <p className="text-sm text-label">No approval steps. Members can sign directly.</p>
        <Button variant="outline" size="sm" onClick={onAddStep}>
          <PlusIcon className="h-3.5 w-3.5" />
          ...
          <div key={s.key}>
            {idx > 0 && (
              <div className="flex justify-center py-1">
                <ArrowDownIcon className="h-4 w-4 text-surface-selected" />
              </div>
            )}
            <div className="rounded-md border border-border-subtle bg-surface-base/40 p-4">
              <div className="flex items-start gap-3">
                <div className="mt-1 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-border bg-surface-raised text-[11px] font-medium text-muted">
                  {idx + 1}
                </div>
                <Input
