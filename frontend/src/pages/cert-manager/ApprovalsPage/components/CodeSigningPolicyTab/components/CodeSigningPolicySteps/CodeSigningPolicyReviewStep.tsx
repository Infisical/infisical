...
const ReviewField = ({ label, value }: { label: string; value: string | number }) => (
  <div className="flex items-baseline gap-x-4">
    <span className="w-[180px] shrink-0 text-sm text-muted">{label}</span>
    <span className="text-sm font-medium text-foreground">{value}</span>
  </div>
);

export const CodeSigningPolicyReviewStep = () => {
  const { watch } = useFormContext<TCodeSigningPolicyForm>();
  ...
  const { name, constraints, maxRequestTtl, bypassForMachineIdentities, steps } = watch();
    ...
    <div className="space-y-6">
      <div>
        <div className="mb-3 border-b border-border pb-2">
          <h3 className="text-sm font-medium text-foreground">Signing Policy Configuration</h3>
        </div>
