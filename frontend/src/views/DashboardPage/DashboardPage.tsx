...
export const DashboardPage = () => {
  ...
  const { data: secrets, isLoading: isSecretsLoading, refetch: refetchSecrets } = useGetProjectSecrets({
    workspaceId,
    env: selectedEnvSlug,
    decryptFileKey: latestFileKey!,
  ...
  const { data: wsTags, refetch: refetchWsTags } = useGetWsTags(workspaceId);
  ...
  /** Reset checked secrets and tags when route is changed */
  useEffect(() => {
  ...
  const onCreateWsTag = useCallback(
    async (tagName: string, selectedSecrets: { _id: string, isChecked: string | boolean }[], tagColor: string) => {
      ...
      try {
        ...
        createNotification({
          text: `Successfully created a tag for ${selectedSecrets.length > 1 ? "secrets" : "secret"}`,
          type: "success"
        });
        refetchSecrets();
        refetchWsTags()
  ...
  const onMoveSecrets = async ($folderId: string, selectedSecrets: { _id: string, isChecked: string | boolean }[]) => {
    ...
    const mappedCheckedSecrets = selectedSecrets.map(checkedSecret => {
      return {
  ...
  const deleteSecrets = async () => {
    ...
    try {
      ...
      handlePopUpClose("confirmAction")
    } catch (error) {
      createNotification({
  ...
  const handleSecretsBulkDelete = async () => {
    handlePopUpToggle("confirmAction", true)
  }
  ...
  return (
    <div className="container mx-auto h-full px-6 text-mineshaft-50 dark:[color-scheme:dark]">
      ...
      <ConfirmActionModal
        isOpen={popUp.confirmAction.isOpen}
        title={`Are you sure want to delete this ${checkedSecrets.length > 1 ? "secrets" : "secret"}?`}
        onConfrimApproved={() => deleteSecrets()}
        onClose={() => handlePopUpClose("confirmAction")}
      />
