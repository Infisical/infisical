...
export const SecretListView = ({
  ...
  const { popUp, handlePopUpToggle, handlePopUpOpen, handlePopUpClose } =
    usePopUp([
      "deleteSecret",
      "secretDetail",
      "createTag",
  ...
  const handleSaveSecret = useCallback(
    async (
      ...
      try {
        ...
        queryClient.invalidateQueries(
          secretApprovalRequestKeys.count({ workspaceId }),
        );
  ...
  const handleSecretDelete = useCallback(async () => {
    const { key, id: secretId } = popUp.deleteSecret
      ?.data as SecretV3RawSanitized;
    try {
      await handleSecretOperation("delete", SecretType.Shared, key, {
        secretId,
      });
  ...
  return (
    <>
      ...
      <SecretDetailSidebar
        ...
        onDeleteSecret={() =>
          handlePopUpOpen("deleteSecret", popUp.secretDetail.data)
        }
        onClose={() => handlePopUpClose("secretDetail")}
        onSaveSecret={handleSaveSecret}
        tags={wsTags}
        onCreateTag={() => handlePopUpOpen("createTag")}
        handleSecretShare={(value: string) =>
