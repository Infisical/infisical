...
import { PopUpNames, usePopUpAction } from "../../ConsumerSecretMainPage.store";
...
export const SecretDropzone = ({
  ...
  const { popUp, handlePopUpToggle, handlePopUpOpen, handlePopUpClose } =
    usePopUp(["importSecEnv", "overlapKeyWarning"] as const);
  const queryClient = useQueryClient();
  const { openPopUp } = usePopUpAction();

  const { mutateAsync: updateSecretBatch, isLoading: isUpdatingSecrets } =
    useUpdateSecretBatch({
      options: { onSuccess: undefined },
    });
  const { mutateAsync: createSecretBatch, isLoading: isCreatingSecrets } =
  ...
  const handleParsedEnv = (env: TParsedEnv) => {
    ...
    if (
      !Object.keys(overlappedSecrets).length &&
      !Object.keys(nonOverlappedSecrets).length
    ) {
      createNotification({
        type: "error",
  ...
  const handleSaveSecrets = async () => {
    const { update, create } = popUp?.overlapKeyWarning
      ?.data as TSecOverwriteOpt;
    try {
        ...
        <ModalContent
          ...
          footerContent={[
            <Button
              ...
              colorSchema={
                isUploadedDuplicateSecretsEmpty ? "primary" : "danger"
              }
              key="overwrite-btn"
          ...
          {isUploadedDuplicateSecretsEmpty ? (
            ...
            <div className="flex flex-col space-y-2 text-gray-300">
              ...
              <div className="text-sm text-gray-400">
                {Object.keys(
                  (popUp?.overlapKeyWarning?.data as TSecOverwriteOpt)
                    ?.update || {},
                )
                  ?.map((key) => key)
                  .join(", ")}
              ...
              <div>
                Are you sure you want to overwrite these secrets and create
                other ones?
              </div>
            </div>
          )}
