...
import { TAlertChannel, useDeleteAlertChannel } from "@app/hooks/api/alertChannels";
...
export const DeleteChannelModal = ({ isOpen, onOpenChange, channel }: Props) => {
  ...
  const usage = channel.usageCount
    ? `This channel is used by ${channel.usageCount} alert${channel.usageCount === 1 ? "" : "s"}; it will be detached from ${channel.usageCount === 1 ? "it" : "them"}.`
