...
export const ConsumerSecretOverviewPage = () => {
  ...
  const { isLoading: isOverviewLoading, data: overview } =
  ...
  const { folderNames, getFolderByNameAndEnv, isFolderPresentInEnv } =
    useFolderOverview(folders);

  const { dynamicSecretNames, isDynamicSecretPresentInEnv } =
    useDynamicSecretOverview(dynamicSecrets);

  const { secKeys, getSecretByKey, getEnvSecretKeyCount } =
    useSecretOverview(secrets);
  ...
  const handleFolderCreate = async (folderName: string) => {
    ...
    const isFoldersAdded = results.some(
  ...
  const handleEnvSelect = (envId: string) => {
    if (visibleEnvs.map((env) => env.id).includes(envId)) {
      ...
      setVisibleEnvs(
        visibleEnvs.concat(userAvailableEnvs.filter((env) => env.id === envId)),
      );
    }
  };
  ...
  const allRowsSelectedOnPage = useMemo(() => {
    if (!secrets?.length && !folders?.length)
      return { isChecked: false, isIndeterminate: false };

    if (
      (!secrets?.length ||
        secrets?.every(
          (secret) => selectedEntries[EntryType.SECRET][secret.key],
        )) &&
      (!folders?.length ||
        folders?.every(
          (folder) => selectedEntries[EntryType.FOLDER][folder.name],
        ))
    )
    ...
    userAvailableEnvs.forEach((env) => {
      secrets?.forEach((secret) => {
        if (allRowsSelectedOnPage.isChecked) {
          ...
          if (resource)
            newChecks[EntryType.SECRET][secret.key][env.slug] = resource;
        }
      });
      ...
      folders?.forEach((folder) => {
        if (allRowsSelectedOnPage.isChecked) {
          ...
          if (resource)
            newChecks[EntryType.FOLDER][folder.name][env.slug] = resource;
        }
      });
    });
      ...
      <div className="container mx-auto px-6 text-mineshaft-50 dark:[color-scheme:dark]">
        ...
        <div className="space-y-8">
          <div className="flex w-full items-baseline justify-between">
            <div className="mt-6">
              <p className="text-3xl font-semibold text-bunker-100">
                Consumer Secrets Overview
              </p>
              ...
              {userAvailableEnvs.length > 0 && (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    ...
                    >
                      <Tooltip
                        content="Choose visible environments"
                        className="mb-2"
                      >
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    {userAvailableEnvs.map((availableEnv) => {
                      ...
                      const isEnvSelected = visibleEnvs
                        .map((env) => env.id)
                        .includes(envId);
                      return (
                        <DropdownMenuItem
              ...
              <THead>
                <Tr className="sticky top-0 z-20 border-0">
                  <Th className="sticky left-0 z-20 min-w-[20rem] border-b-0 p-0">
                    <div className="flex items-center border-b border-r border-mineshaft-600 pr-5 pl-3 pt-3.5 pb-3">
                      ...
                      >
                        <div className="mr-4 ml-2">
                          <Checkbox
                            ...
                            isIndeterminate={
                              allRowsSelectedOnPage.isIndeterminate
                            }
                      ...
                      >
                        <FontAwesomeIcon
                          icon={
                            orderDirection === "asc" ? faArrowDown : faArrowUp
                          }
                        />
                      </IconButton>
                    </div>
                  ...
                  {visibleEnvs?.map(({ name, slug }, index) => {
                    ...
                    const importedSecKeyCount =
                      getEnvImportedSecretKeyCount(slug);
                ...
                {userAvailableEnvs.length > 0 && visibleEnvs.length === 0 && (
                  <Tr>
                    <Td colSpan={visibleEnvs.length + 1}>
                      <EmptyState
                        title="You have no visible environments"
                        iconSize="3x"
                      />
                    </Td>
                  </Tr>
                      ...
                      <Td colSpan={visibleEnvs.length + 1}>
                        ...
                        >
                          <Button
                            ...
                            size="md"
                            onClick={() =>
                              handlePopUpOpen("addSecretsInAllEnvs")
                            }
                          >
                            Add Secrets
                          </Button>
                ...
                <Tr className="sticky bottom-0 z-10 border-0 bg-mineshaft-800">
                  ...
                  {visibleEnvs?.map(({ name, slug }) => (
                    <Td
                      ...
                      className="border-0 border-mineshaft-600 p-0"
                    >
      ...
      >
        <ModalContent title="Edit Folder Name">
          <FolderForm
            isEdit
            defaultFolderName={
              (popUp.updateFolder?.data as Pick<TSecretFolder, "name">)?.name
            }
