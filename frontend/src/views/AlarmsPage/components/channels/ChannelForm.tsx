...
import {
  ...
  TAlarmChannel,
  TAlarmChannelForm,
  TAlarmChannelRecipientForm,
  useCreateAlarmChannel,
  useUpdateAlarmChannel
} from "@app/hooks/api/alarmChannels";
...
const CHANNEL_TYPES: { type: AlarmChannelType; icon: typeof MailIcon }[] = [
  { type: AlarmChannelType.Email, icon: MailIcon },
  { type: AlarmChannelType.Slack, icon: HashIcon },
  { type: AlarmChannelType.Webhook, icon: LinkIcon },
  { type: AlarmChannelType.PagerDuty, icon: AlertTriangleIcon }
];

const DIRECTED_TYPES = new Set<AlarmChannelType>([AlarmChannelType.Email]);
...
const buildDefaults = (channel?: TAlarmChannel): Partial<TAlarmChannelForm> => {
  if (!channel) {
    return { name: "", channelType: AlarmChannelType.Email, enabled: true, recipients: [] };
  }
...
export const ChannelForm = ({ projectId, channel, onComplete, onCancel }: Props) => {
  ...
  const createChannel = useCreateAlarmChannel();
  const updateChannel = useUpdateAlarmChannel();
  const [clearSigningSecret, setClearSigningSecret] = useState(false);

  const formMethods = useForm<TAlarmChannelForm>({
    resolver: zodResolver(buildChannelFormSchema(isEditing)),
    defaultValues: buildDefaults(channel) as TAlarmChannelForm
  });
  const {
    control,
    register,
  ...
  const buildConfig = (data: TAlarmChannelForm): Record<string, unknown> => {
  ...
  const onSubmit = async (data: TAlarmChannelForm) => {
    ...
    <FormProvider {...formMethods}>
      ...
      >
        <div className="flex flex-col gap-6 p-4">
          <Field>
            ...
            <FieldContent>
              <Input
                ...
                {...register("name")}
              />
              <FieldDescription>Shown when picking a channel on an alarm.</FieldDescription>
                      ...
                      {CHANNEL_TYPES.map(({ type, icon: Icon }) => (
                        <SelectItem key={type} value={type}>
                          <span className="flex items-center gap-2">
                            ...
                            {ALARM_CHANNEL_TYPE_LABELS[type]}
                          </span>
                        </SelectItem>
                      ))}
                    </SelectContent>
          ...
          {channelType === AlarmChannelType.Email && <ChannelRecipientsField />}
