...
// FindByKey finds a single secret by folder ID, key, and type.
// For personal secrets, userID must be provided.
// Returns nil if not found.
func (d *DAL) FindByKey(
	ctx context.Context,
	folderID uuid.UUID,
	key string,
	secretType string,
	...
	conditions := []postgres.BoolExpression{
		secretsV2.FolderId.EQ(postgres.UUID(folderID)),
		secretsV2.Key.EQ(postgres.String(key)),
		secretsV2.Type.EQ(postgres.String(secretType)),
	}

	if secretType == "personal" && userID != nil {
		conditions = append(conditions, secretsV2.UserId.EQ(postgres.UUID(*userID)))
	} else {
		conditions = append(conditions, secretsV2.UserId.IS_NULL())
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("FindByKey query: %w", err)
	}
