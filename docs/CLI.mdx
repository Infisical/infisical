...
- [workspaceId]: the workspace identifier found on the dashboard.
...
The command connects the local project to a workspace with id [workspaceId] on the remote server to be set as the reference workspace for future potential push/pull operations. It validates the user's membership for the given workspace, creates a new .env.infisical file in the project, and writes the id to it. It is commonly used for two scenarios:

- To establish a fresh connection between project and a workspace.
- To change an existing connection to a workspace to a new workspace.

```jsx
npx infisical connect [workspaceId]
```
...
The command parses the local .env file into key-value pairs (i.e. environment variables), generates a random symmetric key, encrypts the environment variables with that symmetric key, gets the public keys of workspace teammates, creates multiple encrypted symmetric keys under each teammate's public key, and sends the encrypted environment variables and symmetric keys off to the remote server.

If there is an existing (encrypted) symmetric key being shared for the workspace, then it fetches and uses that symmetric key to encrypt the environment variables.
