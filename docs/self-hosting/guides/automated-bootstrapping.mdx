  ...
  <Tab title="Helm Chart">
    ...
    autoBootstrap:
      ...
      secretDestination:
        ...
        namespace: "" # leave empty to use the release namespace
    ...
    <Warning>
      Chart versions before 1.10.1 shipped `secretDestination.namespace` as the literal string
      `default`, so the fallback to the release namespace never applied. Those releases wrote the
      instance admin token to the `default` namespace and still reported a successful install.

      This default applies to future bootstrap runs only. Upgrading an existing release does not move
      the secret that is already there, so follow the migration steps below.
    </Warning>
    ...
    ### Migrating from chart versions before 1.10.1

    Upgrading moves the bootstrap Role and RoleBinding to the release namespace, but it leaves the
    secret itself in `default`. The bootstrap job is a post-install hook, and an instance can be
    bootstrapped only once, so the job neither re-runs on upgrade nor can reissue the token. Until
    you migrate, the instance admin token stays readable in `default` while your automation looks for
    it in the release namespace.

    Copy the secret into the release namespace, keeping whatever keys your `secretTemplate` produced:

    ```bash
    release_namespace=<your-release-namespace>

    kubectl get secret infisical-bootstrap-secret -n default -o json \
      ...
      | kubectl apply -n "$release_namespace" -f -
    ```

    Confirm the copy, then remove the original:

    ```bash
    kubectl get secret infisical-bootstrap-secret -n "$release_namespace"
    kubectl delete secret infisical-bootstrap-secret -n default
    ```

    The token was readable by anything with secret access in `default` for as long as it sat there,
    so treat it as exposed and rotate the root identity credentials after you migrate.
...
- Security controls prevent privilege escalation: instance admin identities can't be managed by non-instance admin users and identities
