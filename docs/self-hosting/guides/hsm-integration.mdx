...
- **Key Recovery**: If the HSM device is lost or destroyed, you'll no longer be able to decrypt your data stored within Infisical. Most HSM providers offer recovery options, which you should consider when setting up an HSM device.
...
- `HSM_LIB_PATH`: The path to the PKCS#11 library provided by the HSM provider. This usually comes in the form of a `.so` for Linux and macOS, or a `.dll` file for Windows. For Docker, you need to mount the library path as a volume. Further instructions can be found below. If you're using Docker, make sure to set the HSM_LIB_PATH environment variable to the path where the library is mounted in the container.
...
<Accordion title="Bring your own key minimum requirements (optional)">
  ...
  * `CKA_TOKEN`: `true` — The key material will persist in your HSM so it can be reused.
  ...
  <Warning>
    Note that for security reasons it's highly recommended to create an AES key with the full set of key attributes seen below if you're going to bring your own key.
  </Warning>
</Accordion>
...
* `CKA_DECRYPT`: `true` — Decryption capabilities enabled.
* `CKA_TOKEN`: `true` — The key material will persist in your HSM so it can be reused.
* `CKA_EXTRACTABLE`: `false` — The key material isn't extractable from the HSM.
...
<Accordion title="Bring your own key minimum requirements (optional)">
  ...
  * `CKA_TOKEN`: `true` — The key material will persist in your HSM so it can be reused.
  ...
  <Warning>
    Note that for security reasons it's highly recommended to create an HMAC key with the full set of key attributes seen below if you're going to bring your own key.
  </Warning>
</Accordion>
...
<Steps>
    ...
    <Step title="Open the Server Console">
        Select **Server Console** in the top navigation bar. It's only shown to server administrators.

        ![Open the Server Console](/images/platform/kms/hsm/server-admin-console.png)
    </Step>
    <Step title="Update the KMS encryption strategy to HSM">
        Go to **Encryption** and set **Encryption strategy** to **Hardware Security Module (HSM)**.

        ![Set the encryption strategy](/images/platform/kms/hsm/encryption-strategy.png)

        Select **Save**, and your instance switches to the HSM encryption strategy right away, re-encrypting your KMS key with keys from the HSM device.
    </Step>
...
<Tabs>
  ...
  <Tab title="Kubernetes">
    When you're deploying Infisical with the [Kubernetes self-hosting option](/self-hosting/deployment-options/kubernetes-helm), you can still use HSM encryption, but you need to ensure that the HSM client files are present in the container.
...
To disable HSM encryption, go to **Encryption** in the [Server Console](/documentation/platform/admin-panel/server-admin) and set **Encryption strategy** back to **Software-based Encryption**.

<Note>
  In order to disable HSM encryption, the Infisical instance must be able to access the HSM device. If the HSM device is no longer accessible, you won't be able to disable HSM encryption.
</Note>