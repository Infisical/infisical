---
title: "Migrate Mongo to PostgreSQL"
...
The newly released PostgreSQL version of Infisical is the only version of Infisical that will receive feature updates and patches going forward. 
...
<Steps>
  <Step title="Backup Production MongoDB Data">
     While the migration script won't mutate any MongoDB production data, we recommend you to take a backup of your MongoDB instance if possible.
  </Step>
  ...
  <Step title="Start local instances of Mongo and PostgreSQL databases">
    Start local instances of MongoDB and PostgreSQL. This will be used in later steps to process and transform the data locally.
...
<Steps>
  ...
  <Step title="Execute Migration Script">
    ...
    Next, you will be asked to enter the PostgreSQL connection string for the database where the transformed data should be stored. 
    Input the connection string of the local PostgreSQL instance that was set up earlier in the guide.
...
At this stage, the data from the Mongo instance of Infisical should have been successfully converted into its PostgreSQL equivalent. 
The remaining step involves transferring the local PostgreSQL database, which now contains all the migrated data, to your chosen production PostgreSQL environment. 
Rather than transferring the data row-by-row from your local machine to the production PostgreSQL database, we will first create a dump file from the local PostgreSQL and then upload this file to your production PostgreSQL instance.
...
<Steps>
  ...
  <Step title="Upload to production PostgreSQL">
   ...
   <Tip>
    Remember to replace `<host>`, `<db-user-name>`, `<database-name>` with the corresponding details of your production PostgreSQL database.
   </Tip>
  </Step>
  <Step title="Verify Data Upload">
    Use a tool like Beekeeper Studio to confirm that the data has been successfully transferred to your production PostgreSQL DB.
  </Step>
</Steps>
...
Remember to transfer the necessary [environment variables](/self-hosting/configuration/envars) from the MongoDB version of Infisical to the new PostgreSQL based Infisical; rest assured, they are fully compatible.

<Warning>
The first deployment of PostgreSQL based Infisical must be deployed with Docker image tag `v0.46.11-postgres`. 
