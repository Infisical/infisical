...
These recommendations are specific to deployments using the [Infisical Linux package](/self-hosting/deployment-options/native/linux-package/installation), which is managed by the `infisical-ctl` CLI tool. This is distinct from the standalone Linux binary deployment above.
...
ReadWritePaths=/run/infisical-core
