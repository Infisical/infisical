...
**Force direct egress to preserve the IP pin.** If your instance doesn't rely on
an outbound forward proxy, enable `SAFE_REQUEST_FORCE_DIRECT_EGRESS` so requests
can't be routed through an ambient `HTTP_PROXY` / `HTTPS_PROXY` that would
re-resolve the target and bypass the validated-IP pin. It's `false` by default
(Infisical Cloud enables it through its own deployment configuration); set it
explicitly for hardened self-hosted deployments that don't use an egress proxy:
...
**Note:** Kubernetes secrets are only base64-encoded by default and aren't encrypted at rest unless you explicitly enable etcd encryption. For production environments, you should:
...
- **Privilege dropping**: The package uses `chpst` to automatically drop the running application process from `root` to the dedicated `infisical` user. You don't need to configure this manually.
...
<Warning>
**Don't** set `User=` or `Group=` in the systemd override. The Infisical omnibus package uses `chpst` internally to drop privileges to the `infisical` user. Setting these directives in systemd will conflict with `chpst` and break the service.
</Warning>

<Warning>
**Don't** enable `ProtectKernelTunables=true`. The `runsvdir-start` script adjusts `/proc/sys/fs/file-max` at startup. Enabling this setting makes `/proc/sys` read-only, which will cause errors. While the service may still start, it will log warnings and may not perform optimally.
</Warning>
...
For complete HSM setup instructions, see the [HSM Integration Guide](/self-hosting/guides/hsm-integration).
