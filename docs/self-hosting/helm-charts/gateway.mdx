...
| `service.port` | `80` | Port the Service exposes. It forwards to the port in `gateway.listenAddress`, so the two don't have to match. |
...
The chart takes the container port from `gateway.listenAddress` and points the Service at it, so they can't drift apart. `service.port` is the port the Service itself exposes, which is why the example sets it to 8443 as well: the address you register has to name the port callers reach the Service on. A `host:port` value is required, and the chart fails the render if the port is missing.
