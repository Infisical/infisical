...
| `gateway.enrollment.gcp.serviceAccountKeyFilePath` | `""` | Only for `type: iam` on a pod with no workload identity. Path to a service account key file you mount yourself. |
...
gateway:
  ...
  enrollment:
    ...
    gcp:
      ...
      # service account key file, which you must mount yourself.
