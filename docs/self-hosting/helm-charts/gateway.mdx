...
<Warning>
  `gateway.listenAddress` needs an `image.tag` whose CLI supports `--listen-address`. An older image rejects the flag and the pod crash-loops with `unknown flag: --listen-address`, so check the tag before setting it.
</Warning>
...
The chart takes the container port from `gateway.listenAddress` and points the Service at it, and the Service exposes that same port, so none of the three can drift apart. A `host:port` value is required, and the chart fails the render if the port is missing. Set `service.port` only if you want the Service to answer on a different port than the gateway binds, in which case the address you register has to name the Service's port.

The Service is created only in direct listen mode. In relay mode the gateway accepts no connections, so the chart creates none.
...
Direct listen is available on self-hosted Infisical. Infisical Cloud runs outside your cluster and has no route into it, so there's no address it could dial. On Cloud, leave `gateway.listenAddress` empty and use a relay.
...
gateway:
  ...
  # Requires an image.tag whose CLI supports --listen-address; older images reject the flag and the
