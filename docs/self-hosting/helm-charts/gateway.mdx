...
<Warning>
  `gateway.listenAddress` needs an `image.tag` whose CLI supports `--listen-address`. Check the tag before setting it: an older image rejects the flag and the pod crash-loops with `unknown flag: --listen-address`.
</Warning>
...
The chart takes the container port from `gateway.listenAddress`, points the Service at it, and exposes that same port, which keeps all three in step. A `host:port` value is required, and the chart fails the render if the port is missing. Set `service.port` only if you want the Service to answer on a different port than the gateway binds, in which case the address you register has to name the Service's port.
...
Direct listen is available on self-hosted Infisical. Infisical Cloud runs outside your cluster and has no route into it, leaving no address it could dial. On Cloud, leave `gateway.listenAddress` empty and use a relay.
