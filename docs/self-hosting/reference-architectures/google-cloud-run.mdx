...
It's intended to provide a scalable, secure, and production-ready baseline for organizations choosing Google Cloud Platform (GCP) as their infrastructure provider.

## Core components

- **Cloud Run:** Infisical service is containerized and deployed as fully managed Cloud Run services.

- **Cloud SQL:** Infisical uses PostgreSQL as its persistence layer. As such, Cloud SQL for PostgreSQL is used.
...
<Steps>
  ...
  **Memorystore (Redis):**
  ...
  [View all available configurations](/self-hosting/configuration/envars).
    ...
    You will want to setup PostgreSQL and Redis within Google Cloud Platform to connect to Infisical.
