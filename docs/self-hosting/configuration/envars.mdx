...
>
  ...
  <Note>
    Enable this only if your instance **doesn't** rely on an outbound
...
<Tip>
  ...
  near `max_connections`, and note that a pooler in transaction mode **doesn't** reclaim connections held
...
<Accordion title="Stripe OAuth Connection">
    ...
    <ParamField query="INF_APP_CONNECTION_STRIPE_OAUTH_AUTHORIZE_URL" type="string" default="none" optional>
        The OAuth install link for your Stripe App, copied from the Stripe dashboard. Stripe issues a different link per mode, so make sure it matches the mode of the secret key below.
    </ParamField>
