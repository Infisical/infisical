...
<Note>
  ...
  cloning, and the scanner runs under a memory ceiling it previously did not have. If you scan
  repositories larger than the defaults, raise both values or set them to `0` for the previous
  unbounded behaviour.
</Note>
...
<ParamField
  ...
  optional
>
  Soft memory ceiling for the scanner process. It collects garbage more
  aggressively as it approaches this limit rather than growing. Set to `0` to
  disable.
</ParamField>
...
<ParamField
  ...
  optional
>
  Repositories larger than this are rejected instead of scanned. Set to `0` to
  disable.
</ParamField>
  ...
  <ParamField query="SECRET_SCANNING_STUCK_SCAN_TIMEOUT_MS" type="number" default="3600000" optional>
    ...
    marked failed. Must be larger than the clone and scan timeouts combined.
  </ParamField>
</Accordion>
