...
{/* vale Infisical.Contractions = NO */}
  <Note>
...
{/* vale Infisical.Contractions = YES */}
</ParamField>
...
<Accordion title="Stripe OAuth Connection">
    ...
    <ParamField query="INF_APP_CONNECTION_STRIPE_OAUTH_AUTHORIZE_URL" type="string" default="none" optional>
        Overrides the install link, which is otherwise derived from the Client ID above. Set it to the test-mode or sandbox link from the Stripe dashboard when the secret key below is not a live-mode key.
    </ParamField>
