...
>
  ...
  <Note>
    Enable this only if your instance has **no** outbound
...
<Tip>
  ...
  near `max_connections`, and note that a pooler in transaction mode **never** reclaims connections held
...
<ParamField query="DISABLE_AUDIT_LOG_GENERATION" type="string" default="false">
  Disable audit log generation entirely. When set to `true`, no audit log events are produced, so neither PostgreSQL, ClickHouse, nor audit log streams will receive events.
</ParamField>
...
<Accordion title="Stripe OAuth Connection">
    <ParamField query="INF_APP_CONNECTION_STRIPE_OAUTH_CLIENT_ID" type="string" default="none" optional>
        The Client ID of your Stripe App.
    </ParamField>

    <ParamField query="INF_APP_CONNECTION_STRIPE_OAUTH_AUTHORIZE_URL" type="string" default="none" optional>
        Leave this unset for a published Stripe App. Infisical builds the install link from the Client ID above, and takes the redirect URI from the address your instance is served on. Set it only when the secret key below is a test-mode or sandbox key, using that mode's install link from the Stripe dashboard.
    </ParamField>

    <ParamField query="INF_APP_CONNECTION_STRIPE_SECRET_KEY" type="string" default="none" optional>
        The secret key of the Stripe account that owns your Stripe App.
    </ParamField>
</Accordion>
...
<Accordion title="Scan limits">
  These bound the resources a single scan can consume. The defaults only trip on outliers, such as
  a repository large enough to exhaust a worker, so most deployments never need to change them.
...
<Note>
  ...
  cloning when the provider reports a size, otherwise once the clone is measured on disk. The
  scanner also runs under a memory ceiling it previously did not have. If you scan repositories
  larger than the defaults, raise both values or set them to `0` for the previous unbounded
  behaviour.
</Note>

{" "}

<ParamField query="SECRET_SCANNING_SCAN_TIMEOUT" type="string" default="10m" optional>
  How long a single scan may run before it is cancelled and the scan is marked
  failed. Takes a duration string such as `90s`, `10m`, or `2h`. Defaults to
  `10m`.
</ParamField>

{" "}

<ParamField query="SECRET_SCANNING_CLONE_TIMEOUT" type="string" default="10m" optional>
  How long cloning a repository may take before it is cancelled and the scan is
  marked failed. Takes a duration string such as `90s`, `10m`, or `2h`. Defaults
  to `10m`.
</ParamField>
...
>
  ...
  <ParamField query="SECRET_SCANNING_STUCK_SCAN_TIMEOUT" type="string" default="1h" optional>
    How long a scan may stay in progress before it is assumed dead, meaning its worker was killed,
    and marked failed. Takes a duration string such as `90s`, `10m`, or `2h`. Defaults to `1h`. Must
    exceed the clone and scan timeouts plus the time a scan spends measuring the repository and
    writing its results; the server refuses to start otherwise.
  </ParamField>
</Accordion>
...
<ParamField query="OTEL_SEMCONV_STABILITY_OPT_IN" type="string" default="http" optional>
  Which OpenTelemetry semantic conventions the HTTP metrics follow. Leave it unset and Infisical applies `http`, the stable conventions, and logs that it did so at startup.

  Set it to `http/dup` to emit the old metric names alongside the stable ones while you move dashboards across. Both name sets are exported for as long as it's set, so the volume of those metrics roughly doubles. See [Monitoring & Telemetry](/self-hosting/guides/monitoring-telemetry#system-metrics) for the old and new names.

  Tokens for other namespaces, such as `database`, are left alone.
</ParamField>
