...
Infisical is stateless and scales horizontally by running across multiple containers/instances. Each instance typically doesn't need more than **2–4 CPU cores** and **4–8 GB** of memory. 
...
PostgreSQL is the only database supported by Infisical. Infisical has been extensively tested with PostgreSQL version 16. We recommend using versions 14 and up for optimal compatibility.
The compute required for PostgreSQL is largely dependent on the number of secret operations (reads and writes) you expect. The more frequently you read and write secrets, the more compute you'll need.
...
<Info>
**Active-passive** setup is recommended for Redis. **Active-active** setup for Redis hasn't been tested.
</Info>
