...
<Steps>
  ...
  <Step title="Download the Docker Compose file">
    Download the Infisical production Docker Compose file:
  ...
  <Step title="Configure environment variables">
    Download the example environment file and configure it for your deployment:
    ...
    Open the `.env` file in your preferred editor and configure the following required values:

    | Variable | Description | How to Generate |
    |----------|-------------|-----------------|
    | `ENCRYPTION_KEY` | Key used to encrypt secrets in the database | `openssl rand -hex 16` |
    | `AUTH_SECRET` | Secret used for JWT signing | `openssl rand -base64 32` |
    | `POSTGRES_PASSWORD` | Password for the PostgreSQL database | Use a strong, unique password |
    | `SITE_URL` | URL where Infisical will be accessed | e.g., `https://secrets.example.com` |

    Other fields (e.g., `POSTGRES_USER`, `POSTGRES_DB`, and `REDIS_URL`) can remain as default for basic usage.
  ...
  <Step title="Start Infisical">
    ...
    - **infisical**: The main Infisical application (port 80)
...
<Steps>
  ...
  <Step title="Configure environment variables">
    ...
    Edit the `.env` file and configure `ENCRYPTION_KEY`, `AUTH_SECRET`, `POSTGRES_PASSWORD`, and `SITE_URL` as described in the Docker Compose tab.
  </Step>
...
```bash
docker compose -f docker-compose.prod.yml restart infisical
```
...
|--------|---------|-------------|
| `postgres-data` | PostgreSQL data | All encrypted secrets, users, projects, and configuration |
| `redis-data` | Redis data | Cache and job queue data (can be regenerated) |

<Warning>
  **Critical**: The `postgres-data` volume contains all your encrypted secrets. Never delete this volume unless you intend to lose all data. Always back up before any maintenance operations.
</Warning>
...
```bash
docker compose -f docker-compose.prod.yml logs -f infisical
...
```bash
docker compose -f docker-compose.prod.yml logs --tail=100 infisical
```
...
<AccordionGroup>
  ...
  <Accordion title="Enable Prometheus Monitoring">
    ...
    **2. Expose port `9464` in `docker-compose.prod.yml`** by adding to the infisical service:
    ```yaml
    ports:
      - "80:8080"
      - "9464:9464"
...
<AccordionGroup>
  ...
  <Accordion title="Database connection errors">
    ...
    **Check Infisical logs for connection errors:**
    ```bash
    docker compose -f docker-compose.prod.yml logs infisical | grep -i "database\|postgres\|connection"
    ```
  ...
  <Accordion title="Email/SMTP not working">
    **Test SMTP connectivity:**
    ```bash
    docker compose -f docker-compose.prod.yml exec infisical nc -zv smtp.example.com 587
    ```

    **Check Infisical logs for email errors:**
    ```bash
    docker compose -f docker-compose.prod.yml logs infisical | grep -i "smtp\|email\|mail"
    ```
