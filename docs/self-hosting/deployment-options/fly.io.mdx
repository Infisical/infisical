...
<Steps>
  ...
  <Step title="Edit the fly.toml configuration file">
    ...
    ```
    [build]                                                                         
    image = "infisical/infisical:v0.43.4-postgres"
    ```
  ...
  <Step title="Set secrets for your Fly.io app">
    ...
    At minimum, Infisical requires that you set the variables `ENCRYPTION_KEY`, `AUTH_SECRET`, `DB_CONNECTION_URI`, and `REDIS_URL`
    ...
    <Tabs>
      <Tab title="CLI">
        ...
        ```
        flyctl secrets set ENCRYPTION_KEY=VALUE AUTH_SECRET=VALUE DB_CONNECTION_URI=VALUE REDIS_URL=VALUE...
        ```
      </Tab>
      <Tab title="Dashboard">
