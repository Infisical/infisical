...
<Steps>
    <Step title="Deploy the Infisical template with Railway">
        ...
        ![Railway deploy template](/images/self-hosting/y deployment-options/railway/railway-deploy-template.png)
        ...
        1.2. At minimum, Infisical requires that you set the variables `ENCRYPTION_KEY`, `AUTH_SECRET`, `DB_CONNECTION_URI`, and `REDIS_URL`
        which you can read more about [here](/self-hosting/configuration/envars).
        
        By default, the Infisical template on Railway pre-configures environment variables on each service in the deployment but requires you to supply two for the Redis and Postgres services.
