...
<Step title="Securely store Infisical secrets and configuration">
  ...
  **Configure IAM for Secret Access (if using Secrets Manager with Workload Identity):**
...
<AccordionGroup>
  <Accordion title="Terraform Example">
    ...
    <Note>
      This is a simplified example to get you started. For a complete deployment, you'll need to add Secrets Manager resources, IAM bindings, and Kubernetes resources. Adapt this example to your infrastructure standards.
    </Note>
  </Accordion>
</AccordionGroup>
