...
sidebarTitle: "GKE with Cloud SQL"
...
<Warning>
  Don't use the `latest` tag in production. Always pin to a specific version to avoid unexpected changes during upgrades.
</Warning>
...
<Step title="Securely store Infisical secrets and configuration">
  ...
  <Tabs>
    <Tab title="Google Secret Manager (Recommended)">
      ...
      **Verify:** Confirm secrets are stored:
      ...
      <Note>
        The Helm chart reads its configuration from a Kubernetes Secret (`infisical-secrets` in this guide), not from Secret Manager directly. Sync the Secret Manager entries into that Kubernetes Secret with the [Secret Manager add-on for GKE](https://cloud.google.com/secret-manager/docs/secret-manager-managed-csi-component) or [External Secrets Operator](https://external-secrets.io/latest/provider/google-secrets-manager/), mapping them to the keys `ENCRYPTION_KEY`, `AUTH_SECRET`, `DB_CONNECTION_URI`, and `REDIS_URL`, and add a `SITE_URL` key set to your public URL (for example `https://infisical.example.com`). The Workload Identity binding in the next section grants the Infisical service account read access to the entries.
      </Note>
    </Tab>
  ...
  **Configure IAM for Secret Access (if using Secret Manager with Workload Identity):**
  ...
  # Bind to the Kubernetes service account created by the Helm chart (<release-name>-infisical)
  gcloud iam service-accounts add-iam-policy-binding \
    infisical-gsa@<YOUR_PROJECT_ID>.iam.gserviceaccount.com \
    --role roles/iam.workloadIdentityUser \
    --member "serviceAccount:<YOUR_PROJECT_ID>.svc.id.goog[infisical/infisical-infisical]"
  ```
</Step>

<Step title="Deploy Infisical using Helm">
  Deploy Infisical to your GKE cluster using the official `infisical-standalone` Helm chart. For all available configuration options, see the [full values.yaml reference](https://raw.githubusercontent.com/Infisical/infisical/main/helm-charts/infisical-standalone-postgres/values.yaml).
  ...
  **Create a Helm Values File:**
  ...
  infisical:
    # Names the Deployment and Service "infisical" instead of "<release-name>-infisical-standalone-infisical"
    fullnameOverride: infisical
    replicaCount: 2

    image:
      repository: infisical/infisical
      tag: "v0.161.11"  # Use a specific version
      pullPolicy: IfNotPresent

    # Kubernetes Secret holding ENCRYPTION_KEY, AUTH_SECRET, DB_CONNECTION_URI, REDIS_URL, and SITE_URL
  ...
  ingress:
    ...
    nginx:
      enabled: false  # Use the GKE ingress controller instead of the bundled ingress-nginx
    annotations:
      # GKE Ingress ignores spec.ingressClassName and only claims Ingresses carrying this annotation
      kubernetes.io/ingress.class: "gce"
      kubernetes.io/ingress.global-static-ip-name: "infisical-ip"
      networking.gke.io/managed-certificates: "infisical-cert"

  # Use Cloud SQL and Memorystore instead of the bundled databases
  postgresql:
    enabled: false
  ...
  The chart reads `DB_CONNECTION_URI` and `REDIS_URL` from the `infisical-secrets` Secret created in the previous step, so the bundled PostgreSQL and Redis subcharts stay disabled.
  ...
  **Verify:** Confirm deployment is successful:
  ...
  # Check pod logs
  kubectl logs deployment/infisical -n infisical --tail=50
  ```
...
<AccordionGroup>
  <Accordion title="SMTP/Email Configuration">
    ...
    **Using Gmail:**
    ...
    Update your Helm values to reference the SMTP secret:
    ```yaml
    infisical:
      extraEnv:
        - name: SMTP_HOST
    ...
    Alternatively, add the `SMTP_*` keys directly to the `infisical-secrets` Secret, which the chart already mounts with `envFrom`.
    ...
    **Verify:** Check logs for SMTP configuration:
    ```bash
    kubectl logs deployment/infisical -n infisical | grep -i smtp
    ```
  </Accordion>
  ...
  <Accordion title="Database Migrations">
    ...
    **Check migration status:**
    ```bash
    kubectl logs deployment/infisical -n infisical | grep -i migration
    ```
  ...
  <Accordion title="Upgrade Instructions">
    ...
    **Upgrade process:**
    ...
    # Edit infisical-values.yaml: infisical.image.tag: "v0.X.X"
  ...
  <Accordion title="Auto-Scaling Configuration">
    **Horizontal Pod Autoscaler:**
    ...
    The `infisical-standalone` chart does not create an HPA, so apply one that targets the `infisical` Deployment. When an HPA manages the Deployment, it overrides `infisical.replicaCount` from the values file.
    ```yaml
    # infisical-hpa.yaml
    ...
    ```bash
    kubectl apply -f infisical-hpa.yaml
    ```

    **Verify:** Check the HPA status:
    ```bash
    ...
    **Adjust scaling parameters:**
    ...
    Edit `minReplicas`, `maxReplicas`, or the `metrics` list in `infisical-hpa.yaml` and apply it again. For example, to also scale on memory:
    ```yaml
    metrics:
      - type: Resource
        resource:
...
<AccordionGroup>
  ...
  <Accordion title="Ingress not working">
    **Symptoms:** Can't access Infisical via the external URL.
