...
To fix this, set the following environment variable before starting the backend:
...
If you see `Unable to validate connection to ssh: Handshake failed: signature verification failed.`, FIPS mode is likely enabled. Set `FIPS_ENABLED=false` in your environment before starting the backend. See [Disable FIPS Mode for SSH](#disable-fips-mode-for-ssh) above.
</Accordion>
</AccordionGroup>
