...
### Disable FIPS Mode for SSH

FIPS mode is enabled by default in the local development environment. However, SSH PAM relies on cryptographic operations that are not fully supported in FIPS mode, which will cause SSH handshake failures with errors like:

```
Unable to validate connection to ssh: Handshake failed: signature verification failed.
```

To fix this, add the following line to your `.env` file in the project root (the file loaded by `docker-compose` via `env_file`):

```bash
echo 'FIPS_ENABLED=false' >> .env
```

Then restart the backend service for the change to take effect.

<Note>
A host shell `export` will **not** work here — `FIPS_ENABLED` is set to `true` inside the Docker image and is not listed in the compose `environment:` block, so only the `.env` file can override it.

This is only required for local development involving SSH PAM. Other PAM resource types (databases, etc.) are not affected.
</Note>
...
<Accordion title="SSH handshake fails with signature verification error">
If you see `Unable to validate connection to ssh: Handshake failed: signature verification failed.`, FIPS mode is likely enabled. Add `FIPS_ENABLED=false` to your `.env` file and restart the backend. See [Disable FIPS Mode for SSH](#disable-fips-mode-for-ssh) above for details.
</Accordion>
</AccordionGroup>
