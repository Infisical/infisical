...
| Gateway | `go run main.go gateway start --token=<token> --domain=http://localhost:8080 --relay=local-relay --name=local-gateway --pam-session-recording-path=$(pwd)/session` |
...
- Check `--relay` matches relay's `--name`
- Ensure port 2222 isn't blocked
</Accordion>
