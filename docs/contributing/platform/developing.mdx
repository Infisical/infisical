...
The suffix belongs to the proxy, not to an individual route, and it is stamped
onto each route when that route is registered. A proxy already running on a
different suffix therefore has to be restarted before a route can pick the new
one up. `stack-init` checks the route it just created and prints this when that
has happened:

```bash
$ portless proxy stop && portless proxy start --tld test
$ make stack-init
```
