...
`stack-proxy` starts the proxy on the `.test` suffix. It binds port 443, so it
may ask for your password, and it is safe to re-run. `stack-init` calls it for
you before registering a hostname.

The suffix belongs to the proxy rather than to an individual route, and a
running proxy keeps whichever suffixes it started with. `stack-init` handles
that: it asks `portless doctor` what the proxy is serving and restarts it only
if `.test` is missing. The restart serves `.localhost` too, so routes under the
default suffix keep working.

<Note>
  This needs portless 0.15 or newer, for `portless doctor` and for serving more
  than one suffix at a time. Upgrade with `npm install -g portless@latest`.
</Note>

If a hostname does not resolve, `portless doctor` diagnoses the proxy, routes,
DNS and certificate trust in one go, and `portless hosts sync` fixes resolution
when the `/etc/hosts` entries have drifted.
...
from your main checkout when there is one, because that file holds the key your
existing database volumes were encrypted with. Failing that it falls back to
`.env.example`, and says which it used.
...
| `make stack-proxy` | Start the portless proxy on the `.test` suffix |
