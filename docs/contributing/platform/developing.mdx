...
<Accordion title=".env file content">
  ```env 
    ...
    # Postgres
    # Backend will connect to the Postgres instance at connection string DB_CONNECTION_URI which can either be a ref
    # to the postgres container instance
    # Required
    DB_CONNECTION_URI=postgres://<user-name>:<passwowrd>@<host>:<port>/infisical
