...
- It seeds the database from `STACK_SEED_VOLUME`, defaulting to your main
  checkout's own stack volume, and starts empty if that volume is missing or
  the setting is blank. The default follows the main checkout's directory
  rather than a branch, so a worktree cut from another branch still seeds from
  the main checkout and not from that branch's database.
