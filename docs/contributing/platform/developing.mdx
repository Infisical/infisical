...
running proxy keeps the suffix it started with. `stack-init` handles that for
you: if it registers a route and sees the wrong suffix, it restarts the proxy
and registers again. Routes are re-served afterwards, so the only cost is a
brief interruption to anything else running through the proxy.
