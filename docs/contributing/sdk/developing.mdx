...
<Warning>
	If you change any of the structs defined in the base SDK, you will need to run
	this script to re-generate the types.
</Warning>
...
<Warning>
	After running the commands above, it's very important that you rename the
	generated .so file to `infisical_py.so`. After renaming it you also need to
	move it into the root of the `crates/infisical-py` folder.
</Warning>
...
<Warning>
	After generating the C bindings, the generated .so or .dll has been created in
	the `/target` directory at the root of the project. You have to manually move
	the generated file into the `languages/java/src/main/resources` directory.
</Warning>
...
export class InfisicalClient {
	...
	constructor(settings: ClientSettings) {
		const settingsJson =
			settings == null ? null : Convert.clientSettingsToJson(settings);
	...
	): Promise<CreateSecretResponse["secret"]> {
