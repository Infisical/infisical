...
AI tools can connect to multiple MCP servers at once. Connected servers don't consume context until the AI invokes a search tool, and the AI decides when to search based on query relevance — it doesn't search every connected server for every question.
...
<AccordionGroup>
  ...
  <Accordion title="Do I need an Infisical account to use the MCP server?">
    No. The Infisical Docs MCP server is publicly available and doesn't require authentication. It exposes only the public documentation.
  </Accordion>
