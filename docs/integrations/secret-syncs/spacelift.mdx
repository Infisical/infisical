    ...
    <Steps>
      ...
      <Step title="Configure destination">
        ...
        - **Config Type**: Choose how secrets are stored in the Spacelift context:
            ...
            - **File Mount**: Secrets are mounted as files under `/mnt/workspace/`.
        - **File Format** *(File Mount only)*: Choose the file format for mounted secrets:
            - **.env File**: All secrets are bundled into a single `.env` file at the specified path (e.g., `/mnt/workspace/secrets.env`).
            - **One Secret Per File**: Each secret is stored as a separate file. The filename is the secret key and the content is the secret value (e.g., `/mnt/workspace/secrets/MY_SECRET`).
        - **File Path** *(.env File only)*: The path for the mounted `.env` file relative to `/mnt/workspace/` (e.g., `secrets.env`). Required.
        - **Directory Path** *(One Secret Per File only)*: The directory under `/mnt/workspace/` where secret files are created (e.g., `secrets/`). Optional -- leave empty to mount files directly in `/mnt/workspace/`.
