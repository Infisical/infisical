...
<Note>
  Azure Key Vault secret names can contain hyphens (`-`). They can't contain underscores (`_`). Infisical keeps hyphens in a secret name when it imports that secret. Infisical converts each underscore in a secret name to a hyphen when syncing to Azure Key Vault. If two or more secret names become the same vault name, Infisical stops the sync and reports those names.
</Note>


<Note>
    If your Azure Key Vault uses a private endpoint, it can only be reached from allowlisted IPs inside your private network. Configure this app connection to use an Infisical Gateway running from an allowed network location.
</Note>
...
<Tabs>
    <Tab title="Infisical UI">
        ...
        ![Configure Options](/images/secret-syncs/azure-key-vault/vault-options.png)
            ...
            - **Disable Certificate Import**: If enabled, Infisical will not import certificate objects from Azure Key Vault when syncing secrets. Enable this option if you want your secrets project to contain only secrets and not certificate objects that Azure Key Vault exposes through the secrets API.
