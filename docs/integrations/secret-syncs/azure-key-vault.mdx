...
<Note>
  Azure Key Vault secret names can contain hyphens (`-`). They can't contain underscores (`_`). Infisical keeps hyphens in a secret name when it imports that secret. Infisical converts each underscore in a secret name to a hyphen when syncing to Azure Key Vault.
</Note>
