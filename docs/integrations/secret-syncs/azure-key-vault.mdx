...
<Note>
  Secrets in Infisical that contain an underscore (`_`) will be converted to a hyphen (`-`) when synced to Azure Key Vault. Secret names imported from Azure Key Vault into Infisical are preserved as-is.
</Note>
