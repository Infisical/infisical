...
<Visibility for="agents">
    Before creating the sync, make sure a [1Password app connection](/integrations/app-connections/1password) exists in the target project (or at the org level). Save its `id` — you'll pass it as `connectionId` below.

    Send a `POST` request to `/api/v1/secret-syncs/1password`. The full request and response schema lives in the [Create 1Password Sync](/api-reference/endpoints/secret-syncs/1password/create) reference.
    ...
    - `name` (string, required) — slug-friendly name for the sync.
    - `projectId` (string, required) — the project the sync belongs to.
    - `connectionId` (string, required) — the 1Password app connection's `id`.
    - `description` (string, optional) — free-form note.
    - `environment` (string, required) — slug of the source environment.
    - `secretPath` (string, required) — folder path to sync secrets from, e.g. `/`.
    - `isAutoSyncEnabled` (boolean, optional, default `true`) — sync automatically as secrets change.
    - `destinationConfig` (object, required):
        - `vaultId` (string, required) — the target 1Password vault's ID. Retrieve it from 1Password, or list vaults reachable through the connection with `GET /api/v1/app-connections/1password/{connectionId}/vaults`.
        - `valueLabel` (string, optional) — label of the item field that will hold each secret value. Defaults to `value` when omitted.
    - `syncOptions` (object, required) — shared sync options. Fields include `initialSyncBehavior` (one of `overwrite-destination`, `import-prioritize-source`, `import-prioritize-destination`), `keySchema`, and `disableSecretDeletion`. See the endpoint reference for the exact shape.

    ```bash
    curl -X POST "$INFISICAL_URL/api/v1/secret-syncs/1password" \
      -H "Authorization: Bearer $INFISICAL_TOKEN" \
      -H "Content-Type: application/json" \
      -d '{
        "name": "prod-to-1password",
    ...
    If `isAutoSyncEnabled` is `true` the sync begins immediately. To trigger a sync manually, `POST /api/v1/secret-syncs/1password/{syncId}/sync-secrets`.
</Visibility>

<Visibility for="agents">
    The walkthrough below shows how a person would create this sync through the Infisical UI. Use it to answer a user's questions about the UI. If you're completing the task yourself, use the API instructions above rather than the UI-based instructions.
</Visibility>
...
<Steps>
    ...
    <Step title="Configure the app connection" titleSize="h3">
        ...
        <Tabs>
            <Tab title="Create a new app connection">
                ...
                <Note>
                    This creates a connection scoped to the current project. To share the connection across other projects, you can [create it at the organization level](/integrations/app-connections/1password#step-2-create-the-app-connection) first, then select it here.
                </Note>
            </Tab>
    ...
    <Step title="Review and create" titleSize="h3">
        ...
        <Check>
            The sync is created. If [auto-sync](#configure-the-sync-options) is enabled, it begins syncing your secrets to the selected context right away.
        </Check>
    </Step>
</Steps>
