...
<Tabs>
    <Tab title="Infisical UI">
        ...
        ![Configure Options](/images/secret-syncs/vercel/vercel-options.png)
            ...
            <Warning>
                If this sync's target environments cover all of an existing shared variable's environments, there is nothing left to rescope. The existing record is deleted and recreated with this sync's target projects only, so any other projects it was linked to lose it. To avoid this, match the sync's target environments and target projects to the existing variable exactly, or use a [Key Schema](/integrations/secret-syncs/overview#key-schemas) so Infisical-managed keys never collide with existing shared variables.
            </Warning>
