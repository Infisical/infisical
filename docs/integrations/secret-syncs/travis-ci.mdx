...
Infisical's Travis CI Sync keeps your Travis CI repository environment variables in sync with an Infisical project. Account-level environment variables (`account_env_vars`) aren't managed by this sync, only repository-scoped environment variables are.

<Note>
    `account_env_vars` aren't exposed by the Travis CI API for listing or creation, so Infisical can't sync them. 
</Note>
