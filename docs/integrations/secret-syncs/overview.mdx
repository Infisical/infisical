...
1. <strong>Create App Connection:</strong> If you haven't already done so, create an [App Connection](/integrations/app-connections/overview)
...
2. <strong>Create Secret Sync:</strong> Configure a Secret Sync in the desired project by specifying the following parameters via the UI or API:
    ...
    - <strong>Options:</strong> Customize how secrets should be synced, such as whether secrets should be imported from the destination on the initial sync, or whether the sync should [include all subfolders](#include-all-subfolders) beneath the source path.
...
<Info>
    Some third-party services don't support importing secrets.
</Info>
...
<Note>
    ...
    you can contact us at support@infisical.com to make a request.
</Note>

## Include all subfolders

A Secret Sync covers only the secrets in its source folder by default. Turn on **Include secrets from all subfolders** on the source step and the sync covers every folder beneath that path as well, however deep. Adding or changing a secret in any of those folders syncs it to the destination, the same as a change in the source folder itself. These syncs carry an **Includes All Subfolders** badge next to their source path in the Secret Syncs list.

Destinations store secrets in one flat list, so the folder structure isn't carried over. A secret named `DB_URL` in `/backend` and a secret named `DB_URL` in `/backend/api` both map to the same destination key, and the sync fails with an error naming both folders rather than picking one. Infisical checks for these collisions while you configure the sync, so you can rename or move one of the two before you save.

<div align="center">
    ```mermaid
    graph LR
      A[Infisical: **/backend**] --> C[Destination]
      B[Infisical: **/backend/api**] --> C
      style A fill:#E6F4FF,stroke:#0096D6,stroke-width:2px,color:black,rx:15px
      style B fill:#E6F4FF,stroke:#0096D6,stroke-width:2px,color:black,rx:15px
      style C fill:#F4FFE6,stroke:#96D600,stroke-width:2px,color:black,rx:15px
    ```
</div>

<Note>
    A sync that includes subfolders can't import secrets from the destination into Infisical, because there's no single folder to import them into. Its initial sync behavior is always overwrite destination.
</Note>
...
Any destination secrets which don't match the schema won't get deleted or updated by Infisical.
