...
<Steps>
  ...
  <Step title="Configure Connection Details">
    ...
    - **CA Host**: The network address the Gateway uses to reach your AD CS server. This can be a fully qualified domain name (for example, `ca01.corp.example.com`), a short hostname, or an IP address, as long as the Gateway can resolve and connect to it. Because the Gateway runs inside your network, this is almost always a private, internal address (a private DNS name or private IP) rather than a public one. Enter the address only: don't include a scheme (`https://`), port, or path, since AD CS is reached over DCOM (MS-WCCE).
