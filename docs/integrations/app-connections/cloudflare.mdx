      ...
      <Tab title="Secret Rotation">
        <AccordionGroup>
          ...
          <Accordion title="Cloudflare R2 Access Key">
            ...
            **API Tokens** - **Edit** is what allows Infisical to create the replacement key and revoke the previous one on each rotation cycle. **Workers R2 Storage** - **Read** is used only to populate the bucket picker when you configure the rotation; without it the picker comes back empty and you cannot select buckets. If you already know which bucket you plan to use and it is going to this using the API, you don't need it.
