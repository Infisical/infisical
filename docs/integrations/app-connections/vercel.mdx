...
<Steps>
    <Step title="Move to API Tokens on Vercel">
        Navigate to the Vercel **Account Settings** page by selecting your profile icon in the top-right corner.
    ...
    <Step title="Create the API Token">
        Select the **Create** button and provide a name for your token (e.g., "Infisical Integration"). 
        Choose appropriate scope permissions based on your requirements.
        <Note>
            If you configure an expiry date for your API token, you'll need to manually rotate to a new token prior to expiration to avoid integration downtime. Consider setting a calendar reminder for this task.
        </Note>
