...
<Steps>
  <Step title="Navigate to Organization Settings">
    Select your profile name and select **Organization Settings** at the bottom left side of the Spacelift dashboard.

    ![Navigate to Organization Settings](/images/app-connections/spacelift/spacelift-navigate-to-org-settings.png)
  </Step>
  <Step title="Open API Keys">
    Select **API Keys** under the identity management section.
  ...
  <Step title="Set a Name">
    Set a name for the API key, choose the type **Secret** and select **Continue**.
  ...
  <Step title="Configure Roles, Space, and Create">
    Select the **Space admin** role (or a custom role with at least **Space reader**, **Space writer**, and **Context update** permissions), choose the Space that will be managed by this credential, then select **Add** and **Create API Key**. A file will be downloaded containing your credentials.
...
<Steps>
  ...
  <Step title="Input Credentials">
    ...
    select **Connect to Spacelift** to establish the connection.
