...
<Steps>
  <Step title="Open your Hasura Cloud account settings">
    Log in to [Hasura Cloud](https://cloud.hasura.io), then select **My Account** in the bottom-left corner of the Projects page.
  ...
  <Step title="Generate the token">
    Enter a descriptive name for the token, then select **Generate**.

    ![Generate Hasura Cloud Token](/images/app-connections/hasura-cloud/step-4.png)
  </Step>
  <Step title="Save the token">
    Select **Copy** and store the token securely for the next steps. The token is revealed only once and can't be retrieved again.
