---
title: "Azure Certificate Connection"
description: "Learn how to configure an Azure Certificate Connection for Infisical."
---

Infisical currently only supports two methods for connecting to Azure, which are OAuth and Client Secrets.

<Accordion title="Self-Hosted Instance">
  Using the Azure Certificate connection on a self-hosted instance of Infisical requires configuring an application in Azure
  ...
  **Prerequisites:**
  ...
  <Steps>
    ...
    <Step title="Assign API permissions to the application">
      ...
      For the Azure Connection to work with Certificate, you need to assign the following permissions to the application.

      #### Azure Certificate permissions
    ...
    <Step title="Add your application credentials to Infisical">
      Obtain the **Application (Client) ID** and **Directory (Tenant) ID** (this will be used later in the Infisical connection) in Overview and generate a **Client Secret** in Client Secrets for your Azure application.
      ...
      - `INF_APP_CONNECTION_AZURE_CERTIFICATE_CLIENT_ID`: The **Application (Client) ID** of your Azure application.
      - `INF_APP_CONNECTION_AZURE_CERTIFICATE_CLIENT_SECRET`: The **Client Secret** of your Azure application.

      Once added, restart your Infisical instance and use the Azure Certificate connection.
    </Step>
    </Steps>    
...
<Accordion title="Client Secret Authentication">
  Ensure your Azure application has the required permissions that Infisical needs for the Azure Certificate connection to work.
  ...
  **Prerequisites:**
  ...
  <Steps>
    <Step title="Assign API permissions to the application">
      For the Azure Certificate connection to work, assign the following permissions to your Azure application:
...
<Steps>
	...
	<Step title="Add Connection">
		Select the **Azure Connection** option from the connection options modal. ![Select Azure Connection](/images/app-connections/azure/certificates/select-connection.png)
	</Step>
  <Step title="Create Connection">
    <Tabs>
      ...
      <Step title="Authorize Connection">
        ...
        ![Connect via Azure OAuth](/images/app-connections/azure/certificates/create-oauth-method.png)
      </Step>
      ...
      <Step title="Create Connection">
        ...
        ![Connect via Azure OAuth](/images/app-connections/azure/certificates/create-client-secrets-method.png)
      </Step>
      </Tab>
    </Tabs>
  </Step>
	<Step title="Connection Created">
		Your **Azure Certificate Connection** is now available for use. ![Azure Certificate](/images/app-connections/azure/certificates/connection-created.png)
	</Step>
</Steps>
