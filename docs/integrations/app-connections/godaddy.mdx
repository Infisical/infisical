...
<Note>
  ...
  can't issue certificates without a provisioned product, so Infisical always targets
  `api.godaddy.com`.
</Note>
...
<Steps>
  ...
  <Step title="Create a production key">
    Select **Create New API Key**, give it a name (e.g. `infisical`), and select **Production** under **Environment**. Select **Next**.
