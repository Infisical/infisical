...
- **API Token** (recommended): a Datadog API token that is sent as a bearer token (`Authorization: Bearer <token>`).
...
<Note>
    If you use the **Application Keys** method, we recommend issuing the Application Key on a dedicated Service Account so the credentials Infisical uses can be rotated and revoked independently of any individual user. The steps below walk through creating those credentials; the **API Token** method only requires a Datadog API token and the site URL.
</Note>

## Create Datadog Application Keys Credentials
    ...
    <Tab title="Infisical UI">
        <Steps>
            ...
            <Step title="Fill out the Datadog Connection Modal">
                ...
                - The **Method** you want to use to authenticate (**API Token**, recommended, or **Application Keys**)
                - The Datadog **URL** for your region (e.g. `https://api.datadoghq.com`, `https://api.us5.datadoghq.com`, or `https://api.ddog-gov.com`)
                - If using **API Token**: the Datadog **API Token**
