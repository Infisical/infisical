...
<Steps>
  ...
  <Step title="Configure OIDC authentication">
    ...
    - **Subject**: The expected principal that is the subject of the JWT. The format is:
      ...
      <Note>
        Repositories created after July 15, 2026 (and older repositories that opt in to GitHub's **Use immutable subject claim** setting) use an immutable subject format where the owner and repository names are suffixed with their permanent numeric IDs using `@` as the delimiter:

        ```
        repo:<owner>@<owner-id>/<repo>@<repo-id>:<context>
        ```

        For example: `repo:octocat@583231/example-repo@1296269:ref:refs/heads/main`. If the subject you configure doesn't include these IDs, authentication fails with a `403` error. You can find the IDs with the GitHub API: `GET /repos/{owner}/{repo}` returns the repository ID (`id`) and the owner ID (`owner.id`). Alternatively, a glob pattern such as `repo:octocat@*/example-repo@*:ref:refs/heads/main` matches the immutable format without hardcoding the IDs.
      </Note>
