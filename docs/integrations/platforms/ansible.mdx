...
Tested with the Ansible Core >= 2.12.0 versions, and the current development version of Ansible. Ansible Core versions prior to 2.12.0 haven't been tested.
...
The python module dependencies aren't installed by ansible-galaxy. They can be manually installed using pip:
...
<Accordion title="I'm getting a error related to objc[72832]: +[__NSCFConstantString initialize]">
  ...
  ```
  objc[72832]: +[__NSCFConstantString initialize] may have been in progress in another thread when fork() was called. We can't safely call it or ignore it in the fork() child process. Crashing instead. Set a breakpoint on objc_initializeAfterForkError to debug.
  Fatal Python error: Aborted
  ```

  You'll need to add this to your shell environment or ansible wrapper script:
