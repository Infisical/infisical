...
In order to use the Infisical CSI provider, you'll first have to install the [Secrets Store CSI driver](https://secrets-store-csi-driver.sigs.k8s.io/getting-started/installation) to your cluster.
...
<Info>
  If you don't wish to use the auto-syncing feature of the secrets store CSI
  ...
  Infisical, they won't propagate to the pods unless auto-syncing is enabled
  for the CSI driver.
</Info>
...
<Warning>
  **Environments without custom audience support**: Don't set a custom audience
...
In order for the Infisical CSI provider to pull secrets from your Infisical project, you'll have to configure
...
<Accordion title="useDefaultAudience">
  ...
  reject tokens with non-default audiences. When using this option, don't set a
...
<AccordionGroup>
  <Accordion title="Do I have to list out every Infisical single secret that I want to sync?">
    Yes, you'll need to explicitly list each secret you want to sync in the
