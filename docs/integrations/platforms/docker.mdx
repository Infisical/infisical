...
<Note>
  If your self-hosted Infisical instance is running locally on the Docker host, don't set `INFISICAL_DOMAIN` to a `localhost` domain, since this will point to the container itself. Instead, set it to `host.docker.internal`, which points to the container's host domain.
</Note>
