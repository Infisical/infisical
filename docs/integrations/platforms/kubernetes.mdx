...
spec:
  ...
  managedSecretReference:
    ...
    secretNamespace: default
```
<Accordion title="hostAPIReference">
  The `hostAPI` feature allows users to sync secrets across resources within the same Kubernetes cluster by using internal DNS in Infisical's backend. 
  This feature is useful for those who want to sync secrets easily and securely within the cluster without exposing them externally.
  
  To access the Infisical backend directly you can use a hostAPI configuration as below.
  ``` bash
