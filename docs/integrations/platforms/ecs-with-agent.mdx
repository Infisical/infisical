...
Once the secrets/access tokens are retrieved, they're then stored in a shared [Amazon Elastic File System](https://aws.amazon.com/efs/) (EFS) volume. This volume is then made accessible to your application and all of its replicas.
...
<Warning>
  Volumes that contain sensitive secrets shouldn't be publicly accessible. The
...
Take note of the Machine Identity ID as you'll be needing this in the preceding steps.
...
Inside the `aws-ecs-with-agent` directory, you'll find a sample `agent-config.yaml` file. This agent config file will connect with Infisical Cloud using AWS Auth and deposit access tokens at path `/infisical-agent/access-token` and render secrets to file `/infisical-agent/secrets`.
...
In the above container definition, you'll notice that the Infisical agent has a `mountPoints` defined.
...
Because we'll be deploying the example file browser application to AWS via Terraform, you'll need to obtain a set of `AWS Access Key` and `Secret Key`.
...
![file browser dashboard](/images/guides/agent-with-ecs/filebrowser_afterlogin.png)
