...
<Tip>
  For a complete Helm chart reference including all configurable values, see the [Agent Injector Helm chart documentation](/self-hosting/helm-charts/agent-injector).
</Tip>
...
To install the Infisical Agent Injector, you'll need to install our helm charts using [Helm](https://helm.sh/).
...
However, if you're trying to run the agent injector itself on a Windows-based pod, you'll need to configure your helm values.yaml file to point to a Windows-based image.
...
You'll need to set the `nodeSelector.kubernetes.io/os` label to `windows` and set the image tag to a Windows-based image. Below are two examples for Windows Server 2019 and Windows Server 2022.
...
<Tip>
  Note that Windows support is only supported in version `v0.1.4` and above. If you're using an older version, you'll need to upgrade to `v0.1.4` or above to use Windows support.
</Tip>
...
When you're configuring a pod to use the injector, you must create a config map in the same namespace as the pod you want to inject secrets into.
...
<Accordion title="infisical.revoke-credentials-on-shutdown">
  ...
  **Note:** In disaster events such as cluster power outages, a `SIGTERM` signal won't be sent to the agents container, and the credentials won't be revoked.
...
To use the config map in your pod, you'll need to add the `org.infisical.com/agent-config-map` annotation to your pod's deployment. The value of the annotation is the name of the config map you created above. The config map must be in the same namespace as the pod you're injecting into.
```yaml
apiVersion: v1
kind: Pod
metadata:
...
To inject secrets into your pod, you'll need to add the `org.infisical.com/inject: "true"` annotation to your pod's deployment.
...
We're creating a nginx deployment with a PVC to store the database data.
