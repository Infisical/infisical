...
It can be used to request a certificate from Infisical using the [API enrollment method](/documentation/platform/pki/applications/enrollment-methods/api) configured on a [certificate profile](/documentation/platform/pki/settings/profiles), persist it to a specified path on the filesystem, and automatically monitor and renew it before expiration.
...
This follows a [client-driven approach](/documentation/platform/pki/applications/certificates#renewal) to certificate renewal.
