...
It can also run in distribution mode, where it fetches a certificate that already exists in Infisical by ID instead of requesting a new one. This lets many machines share a single centrally managed certificate, such as a wildcard certificate, without each of them triggering an issuance against the certificate authority.
...
- Distributing a shared certificate: The agent can deliver an existing certificate to many machines and pick up server-side renewals of it automatically.
- Leveraging workload identity: The agent can authenticate with Infisical as a [machine identity](/documentation/platform/identities/machine-identities) using an infrastructure-native authentication method such as [AWS Auth](/documentation/platform/identities/aws-auth), [Azure Auth](/documentation/platform/identities/azure-auth), [GCP Auth](/documentation/platform/identities/gcp-auth), [Kubernetes Auth](/documentation/platform/identities/kubernetes-auth), etc.
...
| ------------------------------------ | ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `certificate-id`                     | Conditional                                            | The ID of an existing certificate in Infisical to deliver instead of requesting a new one. See [Distributing an existing certificate](/integrations/platforms/certificate-agent#distributing-an-existing-certificate). Cannot be combined with `profile-name`, `project-slug`, `application-name`, `attributes`, or a CSR.                                                                               |
| `profile-name`                       | Conditional                                            | The name of the [certificate profile](/documentation/platform/pki/certificates/profiles) to request a certificate against (e.g., `web-server-12345`). Not used when `certificate-id` is set.                                                                               |
| `project-slug`                       | Conditional                                            | The slug of the project to request a certificate against (e.g., `my-project-slug`). Not used when `certificate-id` is set.                                                                                                                                                 |
...
| `lifecycle.renew-before-expiry`      | Optional (auto-renewal is disabled if not set)         | Duration before certificate expiration when renewal checks should begin, specified as a duration string (e.g. `72h`, `90d`, `1y`, etc.). Ignored when `certificate-id` is set, because the agent delivers a certificate rather than renewing one.                                                                                            |
...
| `lifecycle.use-latest`               | Optional (defaults to `false`)                         | Whether the agent should deliver the most recent renewal of the certificate named by `certificate-id` instead of that certificate itself. Only valid together with `certificate-id`.                                                                                               |
...
### Distributing an existing certificate

By default, the agent requests a new certificate from a certificate profile each time it starts on a new machine. That suits machines that each need their own identity. It does not suit machines meant to serve the same certificate: ten web servers behind a load balancer terminating TLS for `*.example.com` should share one wildcard certificate, not hold ten.

Set `certificate-id` to the ID of a certificate that already exists in Infisical and the agent delivers that certificate instead of issuing one. Nothing reaches the certificate authority, and your certificate inventory does not grow with your fleet.

Copy the ID from the certificate's page in Infisical, or take it from the response of the [issue certificate](/api-reference/endpoints/certificates/create-certificate) endpoint.

```yaml distribution-agent-config.yaml
version: v2
...
certificates:
  - certificate-id: "8f2a1c40-5f3b-4e7a-9c21-b3d4e5f60718"
    lifecycle:
      status-check-interval: "6h"
    file-output:
      certificate:
        ...
        permission: "0644"
      chain:
        path: "/etc/ssl/certs/wildcard-chain.crt"
        permission: "0644"
      private-key:
...
`certificate-id` replaces the fields that describe a new certificate. Combining it with `profile-name`, `project-slug`, `application-name`, `attributes`, `csr`, or `csr-path` is rejected when the agent starts, rather than silently ignored.

<Warning>
  `lifecycle.renew-before-expiry` has no effect in distribution mode. The agent delivers a certificate, it does not renew one, so a renewal threshold has nothing to act on. The agent logs a warning and carries on. To have it deliver renewals made in Infisical, use [`lifecycle.use-latest`](/integrations/platforms/certificate-agent#keeping-up-with-renewals) instead.
</Warning>
...
[Renewing a certificate](/documentation/platform/pki/applications/certificate-renewal) produces a new certificate with its own ID, and the certificate it replaced stays valid until its own expiry. An agent pinned to the original ID therefore keeps serving the original until it expires.

Set `lifecycle.use-latest` to `true` and the agent delivers the most recent renewal instead:
...
Renewing the certificate once in Infisical then reaches every machine running the agent, with no configuration change and no manual step on the machines. Leave `certificate-id` pointing at the certificate you started from. You never need to update it, and Infisical resolves the most recent renewal for you however many times the certificate has been renewed.

The agent rewrites the files only when the certificate it resolves to changes, and runs the `on-renewal` hook when that happens. Use that hook to reload whatever service consumes the certificate. When nothing has changed, the agent writes nothing and runs no hooks, including across a restart: it compares what it resolves against the certificate already on disk, not just against what it delivered earlier in the same run.

<Note>
  Leaving `use-latest` unset keeps the agent on exactly the certificate you named. Choose that when you want a deliberate step before a new certificate reaches production.
</Note>
...
- A certificate that Infisical does not hold a private key for, such as one issued from a certificate signing request or through ACME, is written along with its chain. The agent logs a warning and skips the private key file. If such a certificate would *replace* one already delivered, the agent refuses the write instead, because overwriting the certificate while leaving the previous key in place would put a non-matching pair on disk and the `on-renewal` hook would reload a service onto it.
- `status-check-interval` sets how often the agent checks, and so how quickly a renewal reaches the machine.
- Keep the certificate named in `certificate-id`, even after it has been renewed. The agent starts from it, so deleting it leaves a restarted agent unable to resolve. Certificates that replaced it can be deleted safely, including the one directly after it.
- `use-latest` requires an Infisical server that supports it. Against an older self-hosted instance the agent cannot tell that a renewal exists, so it logs a warning and stays on the certificate it already delivered. Upgrade the server, or leave `use-latest` unset and point `certificate-id` at the current certificate.
...
### Distributing a shared wildcard certificate

The configuration below instructs the agent to fetch an existing wildcard certificate by ID rather than issue a new one, and to keep it current as the certificate is renewed in Infisical. The same file can be deployed unchanged to every machine that terminates TLS for the domain.
...
certificates:
  - certificate-id: "8f2a1c40-5f3b-4e7a-9c21-b3d4e5f60718"
    lifecycle:
      ...
      use-latest: true # Deliver the most recent renewal of the certificate above
    file-output:
      private-key:
      ...
      certificate:
        ...
        permission: "0644"
      chain:
        path: "/etc/ssl/certs/wildcard.example.com.chain.crt"
        permission: "0644"
    post-hooks:
      on-renewal:
        command: "systemctl reload nginx"
        timeout: 30
      on-failure:
        command: "logger 'Shared certificate could not be refreshed'"
        timeout: 10
```
