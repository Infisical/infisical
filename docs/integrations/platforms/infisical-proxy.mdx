...
<ParamField query="--secret-refresh-interval" type="duration" default="1h">
  How often to refresh cached secrets from Infisical. Defaults to `1h`. Supports duration formats like `60s|5m|1h|1d|1w|1y`.
</ParamField>

<ParamField query="--cache-size" type="number" default="1000">
  The maximum number of cached secrets to store. Defaults to `1000`.
</ParamField>
...
# Call the proxy:
curl http://localhost:8081/api/v4/secrets \
...
If a token is compromised, only the cache entries associated with that specific token are exposed. Other token's cached data remains isolated and protected.
