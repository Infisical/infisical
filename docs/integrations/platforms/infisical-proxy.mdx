...
<Warning>
  For production deployments, it's strongly recommended to enable TLS by providing `--tls-cert-file` and `--tls-key-file`, and setting `--tls-enabled` to `true`.
</Warning>

<Accordion title="Using event subscriptions">
  <Note>
  ...
  You can create a machine identity in the [Infisical Dashboard](https://app.infisical.com/settings/machine-identities).
...
<ParamField query="--enable-event-subscriptions" type="boolean" default={false}>
  Enables real-time cache refresh via [Event Subscriptions](/documentation/platform/event-subscriptions). When enabled, the proxy subscribes to secret mutation events from Infisical and automatically re-fetches affected cache entries when changes occur. Requires `--client-id` and `--client-secret` to be provided.
</ParamField>
...
<AccordionGroup>
  <Accordion title="Stale data after secret updates">
    If you're updating secrets directly in Infisical (not through the proxy), the proxy won't automatically invalidate the cache. You'll need to manually purge the cache by restarting the proxy, or by waiting for the next `static-secrets-refresh-interval` cycle.
