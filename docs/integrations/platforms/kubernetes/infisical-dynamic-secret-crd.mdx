...
<Warning>
  The `InfisicalDynamicSecret` CRD currently uses the **v1alpha1** API version. A v1beta1 version using the new
  [InfisicalConnection](/integrations/platforms/kubernetes/infisical-connection-crd) and [InfisicalAuth](/integrations/platforms/kubernetes/infisical-auth-crd)
  pattern is coming soon. The v1alpha1 version will be deprecated once the v1beta1 replacement is available.
</Warning>
