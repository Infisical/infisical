---
sidebarTitle: "InfisicalDynamicSecret"
...
After getting the secret, you should see that the secret has data that contains the lease credentials.
...
<Accordion title="hostAPI">
  If you're fetching secrets from a self-hosted instance of Infisical set the value of `hostAPI` to
  `https://your-self-hosted-instace.com/api`

When `hostAPI` isn't defined the operator fetches secrets from Infisical Cloud.
...
- `None`: The operator won't override existing secrets in Infisical. If a secret with the same key already exists, the operator will skip pushing that secret, and the secret won't be managed by the operator.
