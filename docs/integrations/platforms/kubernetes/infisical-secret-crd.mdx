...
spec:
  ...
  managedKubeSecretReferences:
    ...
    # Example: Create the same secret in multiple namespaces
    - secretName: shared-config
      secretNamespace: ns1,ns2,ns3
      creationPolicy: "Orphan"
```
...
<Accordion title="managedKubeSecretReferences[].secretNamespace">
  The namespace(s) of the managed Kubernetes secret to be created.
...
**Multiple Namespaces:**
You can specify multiple namespaces as a comma-separated list. The operator will create the same secret in all specified namespaces:
```yaml
secretNamespace: ns1,ns2,ns3
```

<Warning>
When using multiple namespaces with `creationPolicy: "Owner"`, all target namespaces must be the same as the InfisicalSecret CRD namespace. For cross-namespace secrets, use `creationPolicy: "Orphan"`.
</Warning>
...
<Accordion title="managedKubeConfigMapReferences[].configMapNamespace">
  The namespace(s) of the managed Kubernetes config map that your Infisical data will be stored in.
...
**Multiple Namespaces:**
You can specify multiple namespaces as a comma-separated list. The operator will create the same config map in all specified namespaces:
```yaml
configMapNamespace: ns1,ns2,ns3
```

<Warning>
When using multiple namespaces with `creationPolicy: "Owner"`, all target namespaces must be the same as the InfisicalSecret CRD namespace. For cross-namespace config maps, use `creationPolicy: "Orphan"`.
</Warning>
