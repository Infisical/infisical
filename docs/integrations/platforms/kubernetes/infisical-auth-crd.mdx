...
```yaml infisical-auth-kubernetes.yaml
...
In addition, you must provide the configuration object that matches your chosen `method` (e.g., `kubernetes`, `universal`, `awsIam`, etc.).

## Authentication methods

Only define **one** authentication method per resource. The method name in `spec.method` must match the configuration object you provide.
...
Once created, reference the `InfisicalAuth` from your secret resources:
...
This keeps authentication centralized. If you rotate credentials or change identity, you only need to update the `InfisicalAuth` resource — the operator will detect the change, invalidate the cached credentials, and re-authenticate automatically.
