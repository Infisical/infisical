...
<Info>
  ...
  The v1alpha1 `InfisicalSecret` CRD is deprecated in favor of `InfisicalStaticSecret` (v1beta1) and will be removed in a future release — see the [migration guide](#migrating-from-v1alpha1-to-v1beta1) below.
  The v1alpha1 `InfisicalPushSecret` and `InfisicalDynamicSecret` CRDs are **not deprecated** and remain the current way to use those features until v1beta1 replacements are available.
</Info>
