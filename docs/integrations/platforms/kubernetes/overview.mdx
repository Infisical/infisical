...
The operator is installed via [Helm](https://helm.sh). The quickest way to get started:
...
This installs the operator cluster-wide. For namespace-scoped installations, custom service accounts, and all configurable values, see the [Secrets Operator Helm chart documentation](/self-hosting/helm-charts/secrets-operator).
