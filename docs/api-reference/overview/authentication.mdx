...
To interact with the Infisical API, you'll need to obtain an access token. Follow the step by [step guide](/documentation/platform/identities/universal-auth) to get an access token via Universal Auth.
...
<Accordion title="Why is the Infisical API rejecting my identity credentials?">
  ...
  - You're attempting to access a `/raw` secrets endpoint that requires your project to disable E2EE.
