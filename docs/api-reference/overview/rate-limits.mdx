...
Infisical Cloud enforces per-minute rate limits on API requests. A global cap of 600 requests per minute per client IP applies to every endpoint. On top of that, category-specific limits are tracked separately per client IP address.
...
## Limits on Infisical Cloud
 ...
 A global cap of 600 requests per minute per client IP applies to every endpoint.  On top of that, category-specific limits are tracked separately per client IP address.
