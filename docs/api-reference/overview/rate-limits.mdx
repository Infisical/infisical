...
Infisical Cloud enforces per-minute rate limits on API requests. A global cap of 600 requests per minute per client IP applies to every endpoint (the [status endpoint](/api-reference/endpoints/instance/status) is exempt). On top of that, category-specific limits are tracked separately per client IP address.
...
## Limits on Infisical Cloud
 ...
 A global cap of 600 requests per minute per client IP applies to every endpoint (except the [status endpoint](/api-reference/endpoints/instance/status)).  On top of that, category-specific limits are tracked separately per client IP address.
