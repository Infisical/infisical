...
<Tabs>
  ...
  <Tab title="Dedicated proxy (start + connect)">
    ```bash
    # On the proxy host
...
- `connect` launches an agent behind a proxy **already running elsewhere**, setting up proxy routing, CA trust, and placeholder credentials. It needs a proxy address, which is why it goes with `start`. Both authenticate with a [machine identity](/documentation/platform/identities/machine-identities) via [Universal Auth](/documentation/platform/identities/universal-auth), and the proxy's is always separate from your agents'; see [Dedicated Agent Proxy](/documentation/platform/agent-proxy/dedicated-agent-proxy).
...
### Flags
  ...
  <Accordion title="--log-file">
    Write the proxy's [per-request activity](/documentation/platform/agent-proxy/activity-logs) to this path. The agent owns the terminal, so activity is never written to it: by default it goes to a file in the per-run temporary directory, which is deleted on exit. Pass this to keep it somewhere you can `tail -f`; that path is appended to on every run and never rotated for you, so use `logrotate` (`copytruncate`) if you keep one. Errors and warnings still reach stderr either way. `--log-level` filters which decisions are recorded, the same as on `start`.
...
### Dedicated proxy, on a host of its own
