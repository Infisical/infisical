...
### Environment variables
  ...
  <Accordion title="INFISICAL_AGENT_PROXY_ADDRESS">
    Address of the agent proxy as `host:port`. Alternative to the `--proxy` flag or the `agentProxyAddress` field in `.infisical.json`.
...
### Flags
  ...
  <Accordion title="--proxy">
    Address of the agent proxy as `host:port`. Required, unless provided via `INFISICAL_AGENT_PROXY_ADDRESS` or the `agentProxyAddress` field in `.infisical.json`.
