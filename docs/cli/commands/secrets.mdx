...
### Flags
  ...
  <Accordion title="--expand">
    Parse shell parameter expansions in your secrets, and process your referenced secrets
  ...
  <Accordion title="--token">
    Fetch secrets using a service token or [machine identity](/documentation/platform/identities/machine-identities) access token.
  ...
  <Accordion title="--include-imports">
    Used to include imported (linked) secrets in the output.
  ...
  <Accordion title="--tags, -t">
    Filter secrets by tag slugs. Accepts a comma-separated list of tag slugs.

    ```bash
    # Example
    infisical secrets --tags=backend,production
    ```

  </Accordion>
  <Accordion title="--secret-overriding">
    Prioritizes personal secrets, if any, with the same name over shared secrets.

    Default value: `true`

  </Accordion>
  <Accordion title="--output, -o">
    The output to format the secrets in. Supported formats are `yaml`, `json`, `dotenv`.
...
### Flags
  ...
  <Accordion title="--projectId">
    ...
    ```bash
    # Example
    infisical secrets get DOMAIN --projectId=<project-id>
    ```

  </Accordion>

  <Accordion title="--path">
    The `--path` flag indicates which project folder the secrets will be fetched from.

    ```bash
    # Example
    infisical secrets get DOMAIN --path="/" --env=dev
    ```
  ...
  <Accordion title="--output, -o">
    ...
    ```bash
    # Example
    infisical secrets get DOMAIN --output=json
    ```
...
### Flags
  ...
  <Accordion title="--token">
    Set secrets using a service token or [machine identity](/documentation/platform/identities/machine-identities) access token.

  </Accordion>
  <Accordion title="--projectId">
    The project ID to set secrets in. This is required when using a machine identity to authenticate.

    ```bash
    # Example
    infisical secrets set DOMAIN=example.com --projectId=<project-id>
    ```
  ...
  <Accordion title="--output, -o">
    ...
    ```bash
    # Example
    infisical secrets set DOMAIN=example.com --output=json
    ```
...
### Flags
  ...
  <Accordion title="--type">
    Used to select the type of secret to delete. This could be either personal or shared.

    ```bash
    # Example
    infisical secrets delete DOMAIN --type=personal
    ```
  ...
  <Accordion title="--token">
    Delete secrets using a service token or [machine identity](/documentation/platform/identities/machine-identities) access token.

  </Accordion>
  <Accordion title="--projectId">
    The project ID to delete secrets from. This is required when using a machine identity to authenticate.

    ```bash
    # Example
    infisical secrets delete DOMAIN --projectId=<project-id>
    ```

  </Accordion>
  <Accordion title="--path">
    The `--path` flag indicates the project folder to delete the secrets from.
  ...
  <Accordion title="--output, -o">
    ...
    ```bash
    # Example
    infisical secrets delete DOMAIN --output=json
    ```
...
### sub commands
  ...
  <Accordion title="get">
    ...
    <Accordion title="--projectId">
      The project ID to fetch folders from. This is required when using a machine identity to authenticate.

    </Accordion>

    <Accordion title="--output, -o">
      The output to format the folders in. Supported formats are `yaml`, `json`, `dotenv`.
  ...
  <Accordion title="create">
    ...
    <Accordion title="--name, -n">
      Name of the folder to be created in selected `--path`

    </Accordion>

    <Accordion title="--token">
      Create folders using a [machine identity](/documentation/platform/identities/machine-identities) access token.

    </Accordion>

    <Accordion title="--projectId">
      The project ID to create folders in. This is required when using a machine identity to authenticate.
  ...
  <Accordion title="delete">
    ...
    <Accordion title="--token">
      Delete folders using a [machine identity](/documentation/platform/identities/machine-identities) access token.

    </Accordion>

    <Accordion title="--projectId">
      The project ID to delete folders from. This is required when using a machine identity to authenticate.
...
### Flags
  ...
  <Accordion title="--projectId">
    ...
    ```bash
    # Example
    infisical secrets generate-example-env --projectId=<project-id>
    ```

  </Accordion>
  <Accordion title="--path">
    Fetch secrets from within a folder path.

    ```bash
    # Example
    infisical secrets generate-example-env --path="/some/path"
    ```
