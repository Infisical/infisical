...
```
  ...
  <Accordion title="--path">
    Used to select the project folder in which the secrets will be set. Useful when creating new secrets under a particular path.

    ```bash
    # Example
    infisical secrets set DOMAIN=example.com --path backend
    ```
  </Accordion>
</Accordion>
