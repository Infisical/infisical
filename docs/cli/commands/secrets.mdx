...
If the secret key doesn't exist, a new secret will be created using both the key and value provided.
...
### Flags
  ...
  <Accordion title="--tag">
    Used to attach tags to the secret. If a tag doesn't exist on the server, it will be created automatically. **This flag overrides any existing tags on the secret**. If not provided, existing tags are kept intact. You can specify multiple `--tag` flags to add multiple tags.
