...
Each login is stored as a profile: an account on the specified Infisical instance, with the organization that it uses by default. A login without flags becomes the default profile for this machine. To store a login under a name you choose, and leave the default alone, pass `--save-as`. See [profile](./profile) for how to select from existing profiles.
...
<ParamField query="Flags">
  <Expandable title="properties">
    ...
    <ParamField query="save-as" type="string" optional>
      ...
      an existing one's session. The new profile is not automatically selected.
    </ParamField>
