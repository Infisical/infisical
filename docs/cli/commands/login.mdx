...
A user login is stored as a profile: an account on the specified Infisical instance, with the organization that it uses by default. A user login without flags becomes the default profile for this machine. To store one under a name you choose, and leave the default alone, pass `--save-as`. See [profile](./profile) for how to select from existing profiles.
...
<ParamField query="Flags">
  <Expandable title="properties">
    ...
    <ParamField query="save-as" type="string" optional>
      Store this login as the named profile, creating a new profile or replacing
      an existing one's session. The new profile isn't automatically selected.
    </ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="--save-as">
    ...
    #### Description
    Stores this login as the named profile. The CLI creates the profile, or replaces the session of an existing profile that already has that name.

    A login with `--save-as` does not change the default profile, so other terminals keep the profile that they are already using.
