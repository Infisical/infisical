...
A user login is stored as a profile: an account on the specified Infisical instance, and the organization you chose for it.

Unless you pass `--save-as` or `--profile`, the CLI names the profile after your account and organization, and makes it the default profile that commands use when you don't specify one. Pass `--save-as <name>` to choose the name yourself. See [profile](./profile) for more.

Machine identity logins don't create a profile. They print an access token, which you supply through `INFISICAL_TOKEN` or `--token`.
...
<ParamField query="Flags">
  <Expandable title="properties">
    ...
    <ParamField query="save-as" type="string" optional>
      Store this login as a profile under the specified name. The CLI creates
      that profile, or replaces the stored session of an existing profile if the
      name is already taken.
    </ParamField>
    <ParamField query="profile" type="string" optional>
      Sign back in to a profile that already exists, restoring its account,
...
<AccordionGroup>
  ...
  <Accordion title="--organization-slug">
    ```bash
    infisical login --method=<auth-method> --organization-slug="team2" 
    ```
   #### Description
  ...
  <Accordion title="--save-as">
    ```bash
    infisical login --save-as <name>
    ```

    #### Description
    Stores this login under the specified name. The CLI creates that profile, or replaces the profile's stored session if the name is already taken.

    If you're already logged in, a login with `--save-as` doesn't change which profile is selected, so other terminals keep the profile that they're already using. Select the new one with `--profile <name>`, or make it the default with `infisical profile use <name>`.

    ```bash
    # Sign in to a second account and keep the first.
  ...
  <Accordion title="--profile">
    ...
    #### Description
    Signs back in to a profile that already exists. The profile keeps its account, its instance, and its organization, so you do not have to pass `--organization-id`.

    ```bash
    infisical login --profile globex
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Direct Login (Non-Interactive)">
    Direct login is ideal for CI/CD pipelines and automation scripts where browser-based authentication isn't possible.
...
<Accordion title="infisical login status" defaultOpen="true">
  Reports whether the CLI is currently authenticated to Infisical and, when available, the organization the active session is scoped to. The command checks the user session stored in your vault, machine identity tokens exported as environment variables, and a token passed via the `--token` flag.

```bash
infisical login status
```

In addition to local checks (token presence, local expiration), `status` calls the Infisical backend to verify that each credential is still valid — so revoked or rejected tokens are surfaced even when they look fine locally.

#### Sources checked

- **User session**: the active login stored in your system keyring (or the configured [vault](./vault)). The report names the profile that the session belongs to, and how the CLI selected it.
- **Machine identity environment variable**: `INFISICAL_TOKEN`.
- **`--token` flag**: a specific machine identity access token passed explicitly.

#### Exit codes

- `0`: at least one session was reported and none are expired or rejected.
- `1`: no active session was found, or a reported session is expired or rejected by the backend.

#### Flags

  <Accordion title="--json">
    Output the login status as JSON instead of the default human-readable format. Useful for scripting and CI/CD pipelines.
  ...
  <Accordion title="--token">
    Inspect the given machine identity access token instead of the active session or environment variables. When this flag is set, `--domain` (or the `INFISICAL_API_URL` environment variable) must also be provided so the status reflects the correct Infisical instance.

    ```bash
    infisical login status --token=<machine-identity-access-token> --domain=<domain-url>
    ```
...
#### Examples
  ...
  <AccordionGroup>
    <Accordion title="Check the active user session">
      ```bash
      infisical login status
      ```
      ...
      ✓ Authenticated as user@example.com
        ...
        - Profile: user@example.com--acme (via default profile)
        - Auth method: email
        - Token source: infisical login (keyring)
        - Token expiration: 6h 12m
        - User ID: 4377366c-e6b4-437f-865c-bbf90ace32f9
        - Organization: Acme (9c0f4df6-57fb-4b5f-927b-799dc0121520)
      ```

    </Accordion>

    <Accordion title="Check a machine identity token from the environment">
      ```bash
      export INFISICAL_TOKEN=$(infisical login --method=universal-auth --client-id=<client-id> --client-secret=<client-secret> --plain --silent)
      infisical login status
      ```

    </Accordion>

    <Accordion title="Inspect a specific token without changing the active session">
      ```bash
      infisical login status --token=<machine-identity-access-token> --domain=https://app.infisical.com
      ```

    </Accordion>

    <Accordion title="JSON output for scripts">
      ```bash
      infisical login status --json
      ```
