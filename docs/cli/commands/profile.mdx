...
<Accordion title="infisical profile create">
...
```bash
infisical profile create [name]
```

If you omit `--org`, the CLI will ask you to pick an organization.

```bash
infisical profile create globex --org globex-inc
```
...
#### Flags
  ...
  <Accordion title="--use">
    ...
    ```bash
    infisical profile create globex --org globex-inc --use
    ```
  ...
  <Accordion title="--pin">
    ...
    ```bash
    eval "$(infisical profile create globex --org globex-inc --pin)"
    ```
...
<Tip>
  ...
  second organization available as its own profile, run `infisical profile create`.
</Tip>
