  ...
  <Accordion title="Token Auth">
    ...
    <Tabs>
      ...
      <Tab title="Linux (systemd)">
        ...
        `infisical relay systemd install` does not accept `--enroll-method`, and its `--token` flag is a machine identity access token — not the one-time enrollment token used by `relay start`. It always installs an org-type relay (the default) unless `--type=instance` is passed. See [`infisical relay systemd`](#infisical-relay-systemd) below for its full flag set.
      </Tab>
    </Tabs>
