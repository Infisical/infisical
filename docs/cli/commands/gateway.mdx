...
### Flags
  ...
  <Accordion title="--bind">
    ...
    Use it to bind a single interface, or to listen on a different local port than the one advertised, such as behind a load balancer that terminates on 8443 and forwards to 9000.
...
<AccordionGroup>
  ...
  <Accordion title="Do PAM sessions work in direct listen mode?">
      Browser-based sessions work in either mode, because the platform brokers the connection. `infisical pam access` connects from your own machine instead, so it needs to reach the gateway's registered address.

      See [Gateway deployment](/documentation/platform/gateways/gateway-deployment#frequently-asked-questions) for what to give users outside the network.
  </Accordion>
