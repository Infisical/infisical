...
The Infisical gateway runs inside your network and lets Infisical reach systems that aren't exposed to the internet, such as a database in a private subnet or an on-premises appliance.
...
### Flags
  ...
  <Accordion title="--listen-address">
    ...
    Pass it alongside `--relay` to run both modes, so Infisical uses the direct address first and falls back to the relay. This is how you switch an existing gateway between modes without an outage, and [Running both modes](/documentation/platform/gateways/network-architecture#running-both-modes) covers when to keep both:
