...
The command lists the projects of the organization that your profile uses. To link a project in another organization, change the profile default with `infisical profile set-org`. You can also create a profile for that organization with `infisical profile new`. See [profile](./profile).
