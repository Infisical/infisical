...
infisical pam
├── db
│   └── access       (start local database proxy: PostgreSQL, MySQL, MS SQL, MongoDB, Oracle)
├── ssh
...
| MongoDB | `mongodb://<username>@localhost:<port>/<database>` |
| Oracle | `<username>/password@localhost:<port>/<service-name>` |
