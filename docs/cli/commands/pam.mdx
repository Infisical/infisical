...
The CLI connects to the gateway from your own machine, so you need a network path to it. A gateway in [relay mode](/documentation/platform/gateways/network-architecture) works from anywhere, because the relay is reachable from outside your network. A gateway in direct listen mode only works from a machine that can reach its registered address, so from outside that network you'll need a VPN into it, or a relay on the gateway.
