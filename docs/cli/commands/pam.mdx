...
<Note>
  This is the only command that can satisfy an MFA requirement. [`pam agentic access`](/cli/commands/pam-agentic) does not support MFA-gated accounts yet, so reach those here.
</Note>
