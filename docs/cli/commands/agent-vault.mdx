...
The two usually run on different machines. You change a proxy's settings on the **Proxies** page, not with flags. If the proxy is running, it applies the change within one [poll interval](/documentation/platform/agent-vault/proxies#poll-interval).
...
Create a session with this access bundle, by name. Pass it once. Creates it as the machine identity from `--client-id` and `--client-secret`, or as your login. The machine identity has to be an Agent Vault member with a grant on the access bundle. Can't be combined with `--session-token`.
