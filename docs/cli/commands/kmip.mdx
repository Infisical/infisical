...
The server's certificate configuration (hostnames/IPs and key algorithm) is defined on the server entity in the UI. The CLI fetches it on start, so you do not pass it on the command line. The running server renews its certificate automatically before it expires, with no restart required. To change the certificate configuration, edit it in the UI; the change applies at the next automatic renewal, or restart the server to apply it immediately.
