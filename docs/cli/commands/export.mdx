...
### Environment variables
  ...
  <Accordion title="INFISICAL_DISABLE_SECRETS_BACKUP">
    Used to disable the encrypted offline secrets cache. When you are logged in as a user, the CLI saves an encrypted copy of fetched secrets to disk so they can be served if you are offline. Set this variable to skip writing that cache and to never serve secrets from it.

    To use, simply export this variable in the terminal before running this command.

    ```bash
    # Example
    export INFISICAL_DISABLE_SECRETS_BACKUP=true
    ```

    Previously cached secrets are not removed by this variable. You can clear them with `infisical reset`, but note that this also removes your login session and all other Infisical-generated configuration on the machine.
...
### flags
  ...
  <Accordion title="--format">
    Format of the output file. Accepted values: `dotenv`, `dotenv-export`, `dotenv-eval`, `csv`, `json` and `yaml`
