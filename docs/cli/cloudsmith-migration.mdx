...
<Warning>
  The Cloudsmith package repository stopped serving on **September 16, 2026**. If a machine is still pointed at the Cloudsmith URL, `apt-get update` and similar commands will fail, and the Infisical CLI will stop installing or updating. Follow the migration steps below to point it at the new host.
</Warning>
