...
1. The `--domain` flag
2. The `INFISICAL_DOMAIN` environment variable (the legacy `INFISICAL_API_URL` is still honored)
3. The `domain` field in `.infisical.json`
4. The default (US Cloud, `https://app.infisical.com`)

The value must start with `http://` or `https://`, otherwise it is ignored.
