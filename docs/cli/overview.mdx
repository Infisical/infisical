...
<Warning>
  **Action required for Linux users.** The Infisical CLI Linux package repository moved from Cloudsmith to `artifacts-cli.infisical.com`. The Cloudsmith repository stopped serving on **September 16, 2026**. Machines still pointed at the old URL will fail to install or update. See the [migration guide](/cli/cloudsmith-migration) for step-by-step instructions.
</Warning>
