...
If you prefer not to have secrets cached on disk, set the `INFISICAL_DISABLE_SECRETS_BACKUP` environment variable. The CLI will then never save fetched secrets locally or serve them while offline. Run `infisical reset` to remove any previously cached secrets.
