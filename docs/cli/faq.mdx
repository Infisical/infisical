...
If you're still experiencing trouble, please seek support. 
...
If you prefer not to have secrets cached on disk, set the `INFISICAL_DISABLE_SECRETS_BACKUP` environment variable. The CLI will then never save fetched secrets locally or serve them while offline. You can remove any previously cached secrets with `infisical reset`, but note that this also clears your login session and all other Infisical-generated configuration on the machine.
...
<Accordion title="Where can I find my Project ID?">
  ...
  Visit the Infisical website and navigate to a project of your choice. Once on the project page, access the **Project Settings** from the sidebar. Within the Project name section, select the "Copy Project ID" button for copying the current Project ID to clipboard, or simply obtain it from the URL of the current page.
