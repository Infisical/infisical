...
sidebarTitle: "Ruby"
description: "Fetch and manage secrets in a Ruby application with the official Infisical Ruby SDK."
...
<Warning>
  We don't recommend hardcoding your [Machine Identity
...
If you already have a valid access token, set it on the client directly instead of logging in. This doesn't make a network request.
...
<ParamField query="access_token" type="string" required>
  The access token to authenticate with. This shouldn't include the `Bearer` prefix.
</ParamField>
...
Retrieve a single secret by name. Raises `Infisical::NotFoundError` if the secret doesn't exist.
...
Delete a secret by name. Raises `Infisical::NotFoundError` if the secret doesn't exist. Returns the deleted secret.
