...
sidebarTitle: "C++"
description: "Fetch and manage secrets in a C++ application with the official Infisical C++ SDK."
...
`getIsRotatedSecret(): bool`: Returns whether or not the secret is a rotated secret. If `true`, then `getRotationId()` returns the ID of the rotation.
