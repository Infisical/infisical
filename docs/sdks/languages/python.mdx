...
- `description` (str, optional): A new description for the folder. Omitting it leaves the existing description unchanged, so it cannot be used to clear one.
...
**Returns:**
- `DeleteFolderResponseItem`: The response containing the deleted folder. Unlike the other folder responses, it does not include a `path` field.
