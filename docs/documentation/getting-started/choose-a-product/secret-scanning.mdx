...
<Card title="Secrets Scanning docs" icon="radar" href="/documentation/platform/secret-scanning/overview">
  Data sources, scan modes, findings and lifecycle, ignore rules, and CLI usage.
</Card>

## Use cases

<CardGroup cols={2}>
  <Card title="Monitor connected repositories" icon="git-branch" href="/documentation/platform/secret-scanning/github">
  ...
  <Card title="Manage findings across scans" icon="list-checks" href="/documentation/platform/secret-scanning/overview">
...
The detection engine uses pattern matching and entropy analysis to identify credentials. Findings can be resolved, ignored, or marked as false positives, and each scan is recorded in an audit log. To reduce the number of false positives, users can skip known secrets with `infisical-scan:ignore` comments or a `.infisicalignore` file.
