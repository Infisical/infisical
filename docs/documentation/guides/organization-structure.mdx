...
- **Definition**: [Sub-organizations](/documentation/platform/organization#sub-organizations) are optional child organizations within a root organization, allowing large enterprises to segment by business unit or team.
- **Hierarchy**: Sub-organizations sit between organizations and projects. Each sub-org can have its own projects, groups, and machine identities.
- **Resource Sharing**: Users, groups, and machine identities from the root organization can be added to sub-organizations. Resources flow downward only and must be explicitly added.
- **Limitations**: Authentication (SSO, SCIM, LDAP) and billing are configured at the root organization level only.
