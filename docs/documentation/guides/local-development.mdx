...
<Note>
  `infisical run` injects secrets as environment variables, so it works with any language or framework. To read secrets from inside your application code instead, use a [framework integration](/integrations/all?type=Framework).
</Note>
