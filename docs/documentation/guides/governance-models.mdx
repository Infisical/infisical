...
<Note>
Infisical spans several products: [Secrets Management](/documentation/platform/secrets-mgmt/overview), [Certificate Manager](/documentation/platform/pki/overview), [KMS](/documentation/platform/kms/overview), [Secret Scanning](/documentation/platform/secret-scanning/overview), and [Privileged Access Management](/documentation/platform/pam/overview). This guide focuses on Secrets Management, but the same governance foundations apply across all of them: organization structure, authentication and identity, access control, approval workflows, and audit.
</Note>
...
<Info>
Sub-organizations are an Enterprise feature. On Infisical Cloud they're available on the **Enterprise Tier**; for self-hosted deployments, contact sales@infisical.com for an enterprise license.
</Info>

[Sub-organizations](/documentation/platform/sub-organizations) let you create child organizations within a root organization. Each sub-organization behaves like a regular organization—with its own projects, members, groups, and settings—while sharing authentication and billing with the root. Only one level of nesting is supported, and authentication (SSO, SCIM, LDAP) is always configured at the root level.

Sub-organizations are a key lever for governance because they create hard boundaries between teams or business units without requiring separate accounts:

- **Segment by business unit**: give departments (e.g., Engineering, Sales, Marketing) independent project spaces that are isolated from one another
- **Delegate administration**: let team leads act as admins of their own sub-organization without granting root-level access to the whole company
- **Maintain a single identity source**: users, groups, and machine identities defined at the root can be explicitly shared downward into sub-organizations, while projects and other resources stay isolated to the organization where they're created

| Approach | Centralized | Self-Service |
|----------|-------------|--------------|
| **Sub-org creation** | Root admins provision sub-organizations per business unit | Rarely needed; a single organization with project-level boundaries usually suffices |
| **Administration** | Root team retains oversight; delegates scoped admin to sub-org leads | Sub-org admins operate autonomously within their unit |
| **Resource sharing** | Root team curates which users, groups, and identities flow into each sub-org | Sub-orgs create and manage their own resources as needed |

<Note>
Sub-organizations provide stronger isolation than project-level boundaries alone, since resources in one sub-organization are never visible to the root or to sibling sub-organizations. If you only need to separate teams within a shared user directory, [groups](/documentation/platform/groups) and [project-level roles](/documentation/platform/access-controls/role-based-access-controls#project-level-access-controls) may be sufficient without introducing sub-organizations.
</Note>
...
- [SAML SSO](/documentation/platform/sso/overview) with providers like Okta, Entra ID, Google Workspace, and JumpCloud
- [OIDC SSO](/documentation/platform/sso/general-oidc/overview) for standards-based authentication
...
- [OIDC Auth](/documentation/platform/identities/oidc-auth/general) — Authentication using OIDC identity tokens
...
### Secret referencing and imports

[Secret referencing](/documentation/platform/secret-reference) and [imports](/documentation/platform/secret-reference) allow secrets to be shared across environments and folders. This helps reduce duplication when the same secret is needed in multiple places.
...
By default, references and imports operate within a single project. [Cross-project secret sharing](/documentation/platform/secret-reference#cross-project-secret-sharing) extends this to let one project reference or import secrets from another project in the same organization—useful for shared infrastructure secrets like database hosts or common API keys.

<Note>
Cross-project secret sharing is currently in private preview and available by invitation only. It's governed centrally: an organization admin must enable an org-level toggle, and access between projects is granted explicitly through project grants, so users in the target project don't need permissions in the source project.
</Note>
...
- [Infisical Kubernetes Operator](/integrations/platforms/kubernetes/overview) — Syncs secrets to Kubernetes Secrets resources
...
The [Infisical Agent](/integrations/platforms/infisical-agent) and [CLI](/cli/overview) provide flexible secret consumption patterns:
...
|---------|-----------------|------------------|
| [Sub-organizations](/documentation/platform/sub-organizations) | Segment business units with delegated admins | Rarely needed; use project boundaries |
