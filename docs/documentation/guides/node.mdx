...
description: "Manage secrets for your Node.js application from local development to production."
---
...
2. Add a secret to the development environment of this project so we can pull it back for local development. Select the **Development** environment from the environment selector, then select **Add Secret** and add a secret with the key `NAME` and value `YOUR_NAME`.
...
<AccordionGroup>
    ...
    <Accordion title="What's the point if I still have to manage a token for the SDK?">
        ...
        than if you managed dozens of secrets yourself without it. Here are some benefits:
