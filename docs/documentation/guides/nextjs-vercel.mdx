...
sidebarTitle: "Next.js + Vercel"
description: "Manage secrets for your Next.js and Vercel stack from local development to production."
---
...
2. Add a secret to the development environment of this project so we can pull it back for local development. Select the **Development** environment from the environment selector, then select **Add Secret** and add a secret with the key `NEXT_PUBLIC_NAME` and value `YOUR_NAME`.
...
<Warning>
  The following environment variable names are reserved by Vercel and can't be
...
<AccordionGroup>
    <Accordion title="Why should I use Infisical if I can centralize all my Next.js + Vercel environment variables across all environments directly in Vercel?">
        Vercel doesn't specialize in secret management which means it lacks many useful features for effectively managing environment variables.
