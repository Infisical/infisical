...
<Note>
  All YAML manifests used in this guide are provided inline. You can copy and paste each block directly, or save them to local files before applying.
</Note>
...
<Tabs>
  <Tab title="Apply all at once">
    Save the following YAML to a file called `rbac.yaml` and apply it:
...
Save the above to `service-account.yaml` and apply:
...
Save the above to `reviewer-token.yaml` and apply:

```bash
kubectl apply -f reviewer-token.yaml
```
...
Save the above to `cluster-role-binding.yaml` and apply:
...
- **YOUR_MACHINE_IDENTITY_ID**: The Identity ID you copied in Step 2.
- **YOUR_PROJECT_ID**: Found in your Infisical project under **Settings > General**.

<Note>
  If you are using a self-hosted Infisical instance, change the `infisical.address` field to your instance's URL.
</Note>

Save the ConfigMap YAML above to a file called `agent-config.yaml` and apply it:
        ...
        - name: demo-app
          ...
          args:
            - |
              ...
              set -a && . /infisical/secrets && set +a
              echo "API_KEY is: $API_KEY"
              echo "--- Starting nginx ---"
              nginx -g "daemon off;"
```

Save the above to `demo-deployment.yaml` and apply:
...
<Note>
  The demo uses `set -a && . /infisical/secrets && set +a` to source secrets as environment variables. The `set -a` flag automatically exports all variables defined in the file. This approach handles values with spaces more reliably than `export $(cat ... | xargs)`.
</Note>
...
<Warning>
  The `secret` function takes a **Project ID** (UUID format), while `dynamic_secret` takes a **Project Slug** (human-readable string). These are different values. You can find both in your Infisical project under **Settings > General**.
</Warning>
...
```
    ...
    Save the above to `agent-config-dynamic.yaml` and apply:

    ```bash
    kubectl apply -f agent-config-dynamic.yaml
    ```
  </Step>
