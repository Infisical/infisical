...
flowchart LR
    ...
    subgraph Your Pod
        ...
        V(Shared Volume<br/>/infisical/secrets)
...
Go back to the Machine Identity's **Kubernetes Auth** settings in Infisical:
- Paste the JWT output into the **Token Reviewer JWT** field.
- Paste the CA certificate output into the **CA Certificate** field (under the **Advanced** tab).
  ...
  config.yaml: |
    ...
    templates:
      - destination-path: "/infisical/api-key"
        template-content: |
          {{- with getSecretByName "<your-project-id>" "dev" "/" "API_KEY" }}
          {{ .Value }}
          {{- end }}
```
  ...
  config.yaml: |
    ...
    templates:
      ...
      - destination-path: "/infisical/db-creds"
        template-content: |
          {{- with dynamic_secret "<your-project-slug>" "dev" "/" "pg-dynamic-creds" "1h" }}
...
<AccordionGroup>
  <Accordion title="Pod stuck in Init state">
    ...
    ```bash
    kubectl logs <pod-name> -c infisical-agent-init
    ```
