...
3. **Use Each of the three CRDs**.
        - **InfisicalStaticSecret** [Sync secrets from Infisical to Kubernetes]
...
2. Select **Access Control** from the left navigational pane and select **Machine identities**
...
8. Once the Project is created, navigate into the Project to **Add Secrets**. You can add any key-value pair for this example, however if you want to use the InfisicalStaticSecret CRD example provided in this demo, use the following configurations:
    - **Key**: SMTP_HOST
    - **Value**: smtp@gmail.com
    - **Tags**: N/A (Not needed for this demonstration)
    - **Environments**: Production
9. Now lets navigate to the **Access Control** tab on the left hand navigation pane and select **Machine Identities**. 
...
```
  ...
  <Step title="Create the InfisicalStaticSecret CRD">

The [InfisicalStaticSecret](https://infisical.com/docs/integrations/platforms/kubernetes/infisical-static-secret-crd) CRD tells the operator to sync secrets from Infisical to Kubernetes. With the v1beta1 API, authentication and connection details are defined as separate reusable resources ([InfisicalConnection](https://infisical.com/docs/integrations/platforms/kubernetes/infisical-connection-crd) and [InfisicalAuth](https://infisical.com/docs/integrations/platforms/kubernetes/infisical-auth-crd)), which the `InfisicalStaticSecret` references.

Make sure to edit the provided CRD to match your specific configuration:
    ...
    - **Infisical Address**: Use `https://app.infisical.com` for Infisical Cloud, or your self-hosted instance URL

1. After editing the ```infisical-static-secret.yaml``` to contain your demo-specific values, apply it in your cluster:
```console
kubectl apply -f infisical-static-secret.yaml
```

  </Step>
  <Step title="Verify the InfisicalStaticSecret Status">

1. Check that the `InfisicalStaticSecret` was created successfully
```console
kubectl get infisicalstaticsecret -n default
```
