...
<Warning>
  **Network reachability matters in both directions.** Kubernetes Auth works by having the Infisical backend call your cluster's TokenReview API at the Kubernetes Host URL you configure later, and the operator calls your Infisical instance's API from inside the cluster.
  ...
  If you use Infisical Cloud, a local kind, minikube, or Docker Desktop cluster isn't reachable and won't work as-is. Use a cluster with a reachable API endpoint, or self-host Infisical somewhere that can reach the cluster. The same applies to the PostgreSQL database for the dynamic-secret step: it must be reachable from your Infisical instance, not just from your cluster, so use a database with a public or peered address.
</Warning>
...
<Steps titleSize="h3">
  ...
  <Step title="Create a project and add a secret">
    ...
    3. Record the **Project Slug** and confirm the environment slug; you need both for the CRDs.
        - In **Project Settings**, copy the **Slug** field (distinct from the project name). It fills the `projectSlug` field in every CRD.
        - In **Project Settings > Environments**, find the **Production** environment's slug. It's usually `prod`, but confirm it: each CRD's environment field must match exactly, or the sync targets the wrong environment.
...
This `v1alpha1` CRD doesn't use the `InfisicalConnection` or `InfisicalAuth` objects; it carries its own inline `authentication` block with the Identity ID pasted as a plain `identityId` string.
...
The [`InfisicalDynamicSecret`](/integrations/platforms/kubernetes/infisical-dynamic-secret-crd) CRD tells the operator to create and lease a [Dynamic Secret](/documentation/platform/dynamic-secrets/overview), then write the lease credentials into a Kubernetes Secret that any workload can consume. Like the push CRD, it's a `v1alpha1` resource with its own inline `authentication` block; the shared connection and auth objects aren't used here.
...
<Steps titleSize="h3">
  <Step title="Create the dynamic secret in Infisical">
    ...
    4. In the CA (SSL) section, upload your database's CA certificate if it enforces TLS with a custom CA. If it doesn't require a custom CA, leave this empty.
  ...
  <Step title="Verify the lease was created">
    Confirm the operator created the Kubernetes Secret holding the lease credentials. If it isn't there yet, wait a few seconds for the first reconcile and run the command again:
...
If a sync doesn't produce the expected result, inspect the resources' status conditions and the operator logs before changing anything. For the pull path, diagnose in dependency order: connection first, then auth, then the static secret. Each resource only becomes ready after the one it references.
...
Remove the resources this tutorial created so you don't leave an active database lease or orphaned objects behind. Delete consumers before the objects they reference, so nothing reconciles against a missing dependency.
