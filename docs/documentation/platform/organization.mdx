...
```mermaid
graph TD
    subgraph Root ["Acme Corp (Root Organization)"]
        Auth["SSO / SCIM / Billing"]
        Users["Users & Groups"]
        RootProj["shared-services (Project)"]
    end

    subgraph SubEng ["Engineering (Sub-Organization)"]
        EngProj1["backend-api (Project)"]
        EngProj2["frontend-app (Project)"]
    end

    subgraph SubSales ["Sales (Sub-Organization)"]
        SalesProj["crm-integration (Project)"]
    end

    Root --> SubEng
    Root --> SubSales
    Users -.->|Can be added| SubEng
    Users -.->|Can be added| SubSales
```
...
- The `shared-services` project remains at the root organization level

### Creating a Sub-Organization

![sub-organization list](/images/platform/organization/sub-org-list.png)

1. Navigate to **Organization Settings** → **Sub-Organizations**
2. Click **Create Sub-Organization**
