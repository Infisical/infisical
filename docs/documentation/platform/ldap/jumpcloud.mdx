...
<Info>
  LDAP is a paid feature. If you're using Infisical Cloud, then it's available
...
<Steps>
    ...
    <Step title="Prepare the LDAP configuration in Infisical">
            In Infisical, head to **SSO & Provisioning** and select the **SSO** tab. Select **Add Provider**, choose **LDAP** in the **Add Identity Provider** modal, then select **Continue** to open the **Manage LDAP Configuration** form.

            Input your JumpCloud LDAP server settings.
    ...
    <Step title="Define mappings from LDAP groups to groups in Infisical">
     ...
     Group mappings ensure that users who log into Infisical via LDAP are added to or removed from the Infisical group(s) that corresponds to the LDAP group(s) they're a member of.
