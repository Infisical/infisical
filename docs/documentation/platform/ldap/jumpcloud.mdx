...
<Steps>
    ...
    <Step title="Prepare the LDAP configuration in Infisical">
            In Infisical, head to **SSO & Provisioning** and select the **SSO** tab. Select **Add Provider**, then choose **LDAP** in the **Add Identity Provider** modal.

            Input your JumpCloud LDAP server settings in the **Manage LDAP Configuration** form.
