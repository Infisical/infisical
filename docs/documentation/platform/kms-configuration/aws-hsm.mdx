...
To set up an AWS KMS custom key store with AWS CloudHSM, you'll need the following:
...
<Steps>
  <Step title="Navigate to Key store creation page">
    In the AWS console, head over to `AWS KMS` > `AWS CloudHSM key stores` and select **Create key store**.
  </Step>
...
## Create AWS KMS key
Next, you'll need to create a AWS KMS key where you'll set the key store you created previously. 

<Steps>
  <Step title="Navigate to AWS KMS key creation page">
    In your AWS console, proceed to `AWS KMS` > `Customer managed keys` and select **Create**.
  </Step>
  ...
  <Step title="Select key material origin">
    In the advanced options, for the Key material origin field, select `AWS CloudHSM key store`. Then, select next. 
...
To secure project resources, you'll need to add this AWS KMS to your Infisical organization. To learn how, refer to the documentation [here](./aws-kms).