...
When you select a folder, the table will display the items within it across environments.
...
As a final step, you can select the specific secrets you wish to copy and then select **Replicate Secrets**.
...
<Info>
    If you don't select a **Source Root Path**, the replication will consider the contents of the *entire root* of the **Source Environment** (e.g., the Development environment). In this example that would mean copying the `/dev-folder` itself rather than just its contents.
</Info>
