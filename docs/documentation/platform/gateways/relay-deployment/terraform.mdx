...
module "infisical_relay_instance" {
  ...
  instance_type = "t3.micro" # 2 vCPU / 1 GB, the minimum for a production relay
  ...
  user_data = <<-EOT
    ...
    sudo infisical relay systemd install my-relay-example \
      --enroll-method=token \
      --token "your-enrollment-token" \
      --domain "https://app.infisical.com"

    # Start and enable the service to run on boot.
    # The systemd service is named after the relay.
    sudo systemctl start my-relay-example
  EOT
}
...
2.  **Customize values:** Update the placeholder values in `main.tf` to match your AWS environment and Infisical credentials. You'll need to replace:
    ...
    - The `--domain` in the `user_data` script if you're self-hosting Infisical.
