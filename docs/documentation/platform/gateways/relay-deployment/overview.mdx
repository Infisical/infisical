...
If you self-host Infisical and it can reach your gateway host at a stable address, you can skip relays entirely. Deploy the gateway in [direct listen mode](/documentation/platform/gateways/gateway-deployment) instead, and Infisical connects to it.

A relay is what you need when:

- The gateway sits behind NAT or a firewall you can't open for inbound connections
- You're on Infisical Cloud, which runs outside your network and has no route into it, so a gateway there can't be dialed and has to open the connection itself
- PAM CLI users connect from outside the gateway's network, since the CLI dials the gateway from their own machine

A gateway can also run both, which is mainly how you switch an existing gateway between modes without an outage. See [Running both modes](/documentation/platform/gateways/network-architecture#running-both-modes).
