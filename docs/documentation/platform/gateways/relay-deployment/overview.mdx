...
Before diving in, work out whether you need a relay at all, and if you do, whether you can use Infisical's managed infrastructure.

## Do you need a relay at all?

If you self-host Infisical and it can reach your gateway host at a stable address, you can skip relays entirely. Deploy the gateway in [direct listen mode](/documentation/platform/gateways/gateway-deployment#choose-a-connection-mode) instead, and Infisical connects to it.

A relay is what you need when:

- The gateway sits behind NAT or a firewall you can't open for inbound connections
- You're on Infisical Cloud, which runs outside your network and has no route into it, so a gateway there can't be dialed
- PAM CLI users connect from outside the gateway's network, since the CLI dials the gateway from their own machine

A gateway can also run both. See [Running both modes](/documentation/platform/gateways/gateway-deployment#running-both-modes).

## Do you need to deploy your own relay?

Infisical provides managed relay infrastructure in US/EU regions for Infisical Cloud users, which requires no setup or maintenance. You only need to deploy your own relay if you:
...
If you're using Infisical Cloud and don't have specific requirements, you can use the managed relays provided by Infisical and skip the rest of this guide.

## Host requirements

A relay forwards traffic between Infisical and your gateways, so it's sized by the traffic it carries rather than by what the gateways do with it. 2 vCPU and 1 GB of memory is the minimum for production.
...
<Steps>
   <Step title="Provision a Server">
    Provision a server or virtual machine where you plan to deploy the relay, sized as described in [Host requirements](#host-requirements). This server must have a static IP address or DNS name to be identifiable by the Infisical platform.
   </Step>

   <Step title="Create the Relay in the UI">
    1. Navigate to **Organization Settings > Networking > Relays**.
    2. Select **Create Relay**.
    3. Enter a name and host address (the static IP or DNS name of the server from Step 1).
    4. (Optional) Open the new relay's detail page and select the edit icon next to **Authentication** to switch the auth method. Two methods are supported:
    ...
    5. Select **Show deploy command** and copy the generated CLI command.
   </Step>
   ...
   <Step title="Verify Your Relay Deployment">
    ...
    2. **Verify registration** in the Infisical UI. Navigate to **Networking > Relays** and select your relay to confirm it shows a "Healthy" status.
...
No, relay servers can't decrypt any traffic passing through them due to end-to-end encryption:
...
**Platform can't connect to relay:**
...
- **Service interruption**: While the relay is down, the Infisical platform can't reach gateways through that relay. As a result, any secrets or resources accessed via those gateways will be temporarily unavailable until connectivity is restored.
