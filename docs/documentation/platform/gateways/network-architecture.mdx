...
| Connection | Source | Destination | Port | Protocol | Purpose |
|---|---|---|---|---|---|
| Gateway listener | Infisical Platform | Gateway's listen address | the port you set with `--listen-address` | TCP + mTLS | Platform connects to the gateway to serve every operation. **Ingress** on the Gateway host |
| Infisical API | Gateway | Infisical instance host | 443 | TCP (HTTPS) | API communication and certificate requests. **Egress** |
...
The listen address is the one Infisical dials, and it's written into the Gateway's server certificate, so it has to be stable. An internal DNS name or a load balancer in front of the Gateway both work. Changing it means updating `--listen-address` and restarting the Gateway, so it's issued a certificate for the new address.
...
<AccordionGroup>
  ...
  <Accordion title="Which mode should I use?">
    Use direct listen when Infisical can already reach the Gateway host at a stable address, which is the common case when you self-host Infisical in the same network as your resources.
