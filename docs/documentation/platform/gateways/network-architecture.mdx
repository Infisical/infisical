...
<AccordionGroup>
  ...
  <Accordion title="Can Gateway traffic egress through an HTTP forward proxy?">
    Not currently. The Gateway's connection to the relay is a raw SSH tunnel over TCP (port 2222), and its connection to the Infisical API is TLS over TCP (port 443). Neither is HTTP, so an HTTP forward proxy can't transparently carry this traffic.
