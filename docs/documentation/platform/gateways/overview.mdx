...
To monitor their operational status, gateways transmit heartbeats every 3 minutes. Each heartbeat includes a TTL (time-to-live) indicating how long the gateway should be considered healthy. If no successful heartbeat is received before the TTL expires, the gateway is marked as unhealthy. Relays continue to transmit hourly heartbeats and are considered unhealthy if a heartbeat is not received within one hour.
...
<Columns cols={2}>
  ...
  <Card title="Network Architecture" href="/documentation/platform/gateways/network-architecture">
    Understand how the gateway connects: ports, egress rules, and the relay tunnel.
  </Card>
</Columns>
<Columns cols={2}>
  <Card title="Relay Deployment" href="/documentation/platform/gateways/relay-deployment/overview">
