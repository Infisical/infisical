...
General use is the gateway acting as a network path to a private resource, such as a database reached through an app connection or a dynamic secret, a secret rotation target, or an ADCS or HSM server. PAM asks more of the gateway, because the session protocols and session recording both run there, and it scales on the number of sessions open at once. With recording enabled, the gateway also needs disk for recordings it hasn't uploaded yet.
