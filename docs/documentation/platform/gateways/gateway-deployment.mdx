  ...
  <Step title="Run the CLI Command">
    ...
    <AccordionGroup>
      ...
      <Accordion title="Kubernetes Auth">
        ...
        | Field | Description |
        | --- | --- |
        | **Kubernetes Host** | The cluster API server URL, reachable from Infisical (for example `https://my-cluster.example.com:6443`). |
        ...
        | **CA Certificate** | Optional, but required to verify the API server's TLS certificate. |
