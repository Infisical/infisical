    ...
    <AccordionGroup>
      ...
      <Accordion title="GCP Auth">
        ...
        **Allowed Service Account Emails** or **Allowed Projects** is required. Either one names something inside your organization that nobody else can create.
        ...
        <Warning>
          **Allowed Zones** is a narrowing on top of one of those, never a restriction on its own. Zone names are a global namespace, so any GCP customer can place an instance in `us-central1-a` and match a zone-only configuration. A config carrying only zones is refused.
        </Warning>
      ...
      <Accordion title="Kubernetes Auth">
        ...
        <Warning>
          ...
          This means the **first** gateway in a private cluster can't use Kubernetes auth. Enrol it with an enrollment token, AWS auth or GCP auth, then point later gateways at it. Deleting a gateway that other gateways use as their reviewer is refused until they're pointed elsewhere.
        </Warning>
