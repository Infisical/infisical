  ...
  <Step title="Run the CLI command">
    ...
    <AccordionGroup>
      ...
      <Accordion title="GCP Auth">
        ...
        | Field | Description |
        | --- | --- |
        | **Type** | **GCP ID Token Auth** reads a token from the instance metadata server, which covers Compute Engine VMs and GKE pods using workload identity. **GCP IAM Auth** signs one through the IAM Credentials API, for hosts outside Compute Engine. |
