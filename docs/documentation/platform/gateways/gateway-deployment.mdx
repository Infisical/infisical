...
You can also run both at once, which is how you move an existing Gateway from one mode to the other without an outage. See [Switching modes without an outage](#switching-modes-without-an-outage).
...
## Switching modes without an outage

A Gateway can run both modes at once. Pass `--target-relay-name` and `--listen-address` together, and Infisical uses the listen address first, falling back to the relay when that address stops answering. Health checks probe each mode separately. The Gateway stays healthy while either one works.

Running both is mainly how you switch an existing Gateway between modes without an outage. A Gateway declares its transports every time it registers, so the move is a flag change and a restart:
...
Stop at the middle step and check the Gateway's health. A wrong address costs you nothing there: operations keep completing over the relay, and the Gateway page reports the direct path as unreachable. Drop `--target-relay-name` once direct is healthy, or drop `--listen-address` to go back.

Running both permanently only helps clients that can reach the listen address. PAM CLI sessions dial the Gateway from the user's own machine. A user outside the network waits for that attempt to time out before the session falls back to the relay. If most of your CLI users are outside the network, relay mode alone serves them better.
...
Switching away from a mode drops it. To keep both, pass `--target-relay-name` and `--listen-address` together.
