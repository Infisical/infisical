...
Infisical Gateways run inside your network and let Infisical reach systems that aren't exposed to the internet, such as a database in a private subnet or an internal API.
...
| | Direct listen | Relay |
|---|---|---|
| Firewall change | Inbound port open to the Infisical host | None, outbound only |
| Relay to run | None | Managed on Cloud (US/EU), otherwise your own |
| Stable Gateway address | Required | Not needed |
| Infisical Cloud | Not available | Yes |
| PAM CLI from outside the network | No | Yes |

The difference underneath is which side opens the connection, and that's what drives every row above.

You can also run both at once. See [Running both modes](#running-both-modes).

For the ports, certificates, and traffic flow behind each mode, see [Network architecture](/documentation/platform/gateways/network-architecture).

## Host requirements

| Workload | Minimum |
| --- | --- |
| General use | 2 vCPU, 1 GB memory |
| PAM | 4 vCPU, 8 GB memory |

General use is the gateway acting as a network path to a private resource, such as a database reached through an app connection or a dynamic secret, a secret rotation target, or an ADCS or HSM server. For PAM, the gateway runs the session protocols and records the sessions, so its CPU, memory, and disk use grow with the number of sessions open at once.

## Deployment steps

<Steps>
  <Step title="Set up a relay server (relay mode only)">
    Skip this step if you're using direct listen.

    For relay mode, a relay server has to be running and reachable before you deploy any gateways. You have two options:
    ...
    - **Self-hosted relay:** For all other cases, deploy your own relay server. See the [Relay Deployment Guide](/documentation/platform/gateways/relay-deployment).
  </Step>
  <Step title="Create the gateway in the UI">
    To create the gateway as code instead, see [Terraform](/documentation/platform/gateways/terraform). The rest of this guide still applies, since Terraform manages the gateway record and the host is deployed the same way.
    ...
    3. Enter a name for the gateway, then select **Create Gateway**.
    ![Create Gateway form](/images/platform/gateways/gateway-create-form.png)
    4. (Optional) Open the new gateway's detail page and change the method under **Authentication** in the **Details** card. Four methods are supported:
        ...
        - **GCP**: the gateway authenticates with a token from the GCP metadata server, so no credential has to be distributed to the instance or cluster. Configure the allowed service accounts, and optionally the allowed projects and zones.
    ...
    5. In the **Deployment** card, select **Generate deploy command**. For the AWS, GCP and Kubernetes methods the command is shown without this step.
  ...
  <Step title="Configure network and firewall">
    What you open depends on the connection mode you chose.

    **Relay mode.** All connections are outbound, so no inbound ports need to be opened.
    ...
    If you're in a corporate environment with strict egress filtering, ensure outbound TCP 2222 to relay servers and outbound HTTPS 443 to Infisical API endpoints are allowed.

    **Direct listen mode.** The Gateway accepts inbound connections. On the Gateway host, allow:
  ...
  <Step title="Run the CLI command">
    ...
    The examples below are grouped by authentication method. The connection mode is set by two flags, which you can add to any of them:
    ...
    infisical gateway start my-gateway \
      --listen-address=gateway.internal:8443 \
      --enroll-method=token \
    ...
    helm install infisical-gateway infisical/infisical-gateway \
      ...
      --set gateway.listenAddress=infisical-gateway.default.svc.cluster.local:8443 \
    ...
    <AccordionGroup>
      <Accordion title="Token Auth">
        ...
        <Tabs>
          ...
          <Tab title="Kubernetes (Helm)">
            ...
            <Info>
              A PersistentVolumeClaim is created by default to store enrollment credentials across pod restarts. The enrollment token is single-use. Without persistent storage, the gateway can't restart after the initial enrollment.
            </Info>
      ...
      <Accordion title="GCP Auth">
        The gateway sends a token proving which GCP service account it runs as, and Infisical verifies that token against Google's signing keys. Nothing has to be copied onto the instance, so this is the method to use when gateway creation is automated on GCP. The gateway re-authenticates on every start.

        The token carries the gateway's ID as its audience, so a token minted for one gateway cannot authenticate as another.

        Before deploying, configure the GCP auth method on the gateway's detail page:

        | Field | Description |
        | --- | --- |
        | **Type** | **GCP ID Token Auth** reads a token from the instance metadata server, which covers Compute Engine VMs, GKE pods using workload identity, and other surfaces with a metadata server such as Cloud Run, Cloud Functions and App Engine. **GCP IAM Auth** signs one through the IAM Credentials API, for hosts outside Compute Engine. |
        | **Allowed Service Account Emails** | Service account emails that may authenticate as this gateway. |
        | **Allowed Projects** | Optional. Project IDs whose Compute Engine instances may authenticate as this gateway. |
        | **Allowed Zones** | Optional, and only a narrowing of the two above. Zones whose Compute Engine instances may authenticate as this gateway. |

        **Allowed Service Account Emails** or **Allowed Projects** is required. Either one names something inside your organization that nobody else can create.

        Prefer the service account where you can. Restricting by project alone trusts every instance in that project, including one created by anyone who holds `compute.instances.create` there.

        <Warning>
          **Allowed Zones** is a narrowing on top of one of those. Zone names are a global namespace, so any GCP customer can place an instance in `us-central1-a` and match a zone-only configuration. A config carrying only zones is refused.
        </Warning>

        <Warning>
          **Allowed Projects** and **Allowed Zones** are matched against Compute Engine instance details carried inside the token. Only a token from a Compute Engine VM carries them. A GKE workload identity token does not, nor does an IAM service account token, and a login presenting one is refused rather than skipping the check. On GKE, restrict by service account instead.
        </Warning>
        ...
        <Tabs>
          <Tab title="GKE (Helm)">
            ...
            Install the gateway with [Workload Identity Federation](https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity):
            ```bash
            helm install infisical-gateway infisical/infisical-gateway \
              --namespace infisical-gateway --create-namespace \
              --set gateway.name=<gateway-name> \
              --set gateway.enrollment.method=gcp \
              --set gateway.enrollment.gcp.gatewayId=<gateway-id> \
              --set gateway.domain=<your-infisical-domain> \
              --set serviceAccount.annotations."iam\.gke\.io/gcp-service-account"=<gsa-name>@<project-id>.iam.gserviceaccount.com
            ```

            <Info>
              The Google service account in that annotation is the one Infisical sees, so put its email in the gateway's **Allowed Service Account Emails**. Its IAM policy also needs a `roles/iam.workloadIdentityUser` binding for the Kubernetes service account the chart creates.
            </Info>
          </Tab>
          <Tab title="Compute Engine (Production)">
            ```bash
            sudo infisical gateway systemd install <gateway-name> \
              --enroll-method=gcp \
              --gateway-id=<gateway-id> \
          ...
          <Tab title="Foreground">
            ...
            Add `--gcp-auth-type=iam` to sign the token through the IAM Credentials API instead of reading it from the metadata server. That path needs the IAM Service Account Credentials API enabled and `roles/iam.serviceAccountTokenCreator` on the service account being signed as. On a Compute Engine instance it also needs the `cloud-platform` scope, because the default scopes omit `iamcredentials`. Point `--service-account-key-file-path` at a key file when the host has no credentials of its own.
          </Tab>
        </Tabs>
      </Accordion>
      <Accordion title="Kubernetes Auth">
        ...
        | **CA Certificate** | The CA that issued the API server's certificate. Needed unless that certificate is already trusted by the public trust store, which for a cluster CA it won't be. |
        ...
        A gateway pool can only be used with **Manual Token Reviewer JWT (API)**, where any healthy member performs the check and one gateway going offline doesn't stop the others from authenticating. **Gateway as Reviewer** requires a specific gateway, because in that mode the selected gateway decides the outcome and pool membership can change after the config is saved.
        ...
        <Warning>
          ...
          This means the **first** gateway in a private cluster can't use Kubernetes auth. Enroll it with an enrollment token, AWS auth or GCP auth, then point later gateways at it. Deleting a gateway that other gateways use as their reviewer is refused until they're pointed elsewhere.
        </Warning>
        ...
        <Warning>
          Infisical must be able to reach the cluster's API server to review the token. For a cluster with no publicly reachable API server, use Token, AWS or GCP auth instead. On GKE, GCP auth needs no inbound path at all.
        </Warning>
      </Accordion>
    </AccordionGroup>
...
- **Generate deploy command**: Generates a fresh enrollment token (token method) or re-displays the start command for the AWS, GCP and Kubernetes methods. Selecting this on a token-method gateway doesn't disconnect the running gateway; the next login with the new token rotates credentials atomically.
- **Edit auth method**: Switch between Token, AWS, GCP, and Kubernetes, or update the allowlists of the current method. Existing gateways keep their JWT until they restart and re-authenticate.
...
<Accordion title="Can I switch a gateway between relay and direct listen?">
Yes. Open the gateway's detail page, choose the mode you want under **Connection Mode**, and re-run the generated command on the host. The gateway re-registers with the new mode and is issued the certificate it needs for it.

The new mode replaces the old one. To switch without an outage, start the Gateway with both flags first. If the listen address is wrong, the relay keeps serving and the Gateway page shows the direct path as unreachable. Once direct is healthy, drop `--target-relay-name`.

</Accordion>

<Accordion title="Why does my direct listen gateway fail to register?">
The listen address has to be `host:port`, with a port in the range 1 to 65535. Use `gateway.internal:8443`, not `https://gateway.internal` or `gateway.internal`: an address is rejected if it has no port, or if it carries a scheme or a path.

Direct listen is also unavailable on Infisical Cloud, which runs outside your network and has no route into it. On Cloud, use relay mode.

</Accordion>

<Accordion title="Do PAM sessions work in direct listen mode?">
Only from a machine that can reach the gateway's listen address. The CLI connects to the gateway from the user's own machine rather than through Infisical, so a laptop outside the network can't reach a gateway listening on an internal address. Give those users a VPN into the network, or run a relay on the gateway alongside direct listen.

Browser-based PAM access works in either mode, because the platform brokers that connection.
...
<Accordion title="How do I troubleshoot direct listen connectivity issues?">
...
2. From the Infisical instance host, test the listen address:
  ```bash
  nc -zv gateway.internal 8443
  ```
3. Check the inbound firewall rule allows the Infisical instance host on that port
4. Confirm the listen address resolves from the Infisical host, not just from your own machine
...
For token method: ensure the enrollment token hasn't expired or already been used. Open the gateway's detail page and select **Generate deploy command** to generate a fresh token.
...
For **AWS-method**, **GCP-method** and **Kubernetes-method** gateways, no JWT is persisted to disk. The gateway re-authenticates on every start, by signing a fresh STS request with the host's AWS credentials, by presenting a GCP identity token, or by presenting the pod's service account token. Only the gateway id and domain are stored locally.
