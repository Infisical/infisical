...
A Gateway can run both modes at once. Pass `--relay` and `--listen-address` together, and Infisical uses the listen address first, falling back to the relay when that address stops answering. Health checks probe each mode separately. The Gateway stays healthy while either one works.
...
| Flags | Mode |
|---|---|
| `--relay=my-relay` | Relay only |
| `--relay=my-relay --listen-address=gateway.internal:8443` | Both |
| `--listen-address=gateway.internal:8443` | Direct only |

Stop at the middle step and check the Gateway's health. A wrong address costs you nothing there: operations keep completing over the relay, and the Gateway page reports the direct path as unreachable. Drop `--relay` once direct is healthy, or drop `--listen-address` to go back.
