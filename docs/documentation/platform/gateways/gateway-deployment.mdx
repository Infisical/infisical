  ...
  <Step title="Run the CLI Command">
    ...
    <AccordionGroup>
      ...
      <Accordion title="Kubernetes Auth">
        ...
        Prefer a gateway pool over a single gateway. Any healthy member performs the check, so one gateway going offline does not stop the others from authenticating.
