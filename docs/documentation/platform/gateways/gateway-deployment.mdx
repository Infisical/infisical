...
When using the enrollment token flow, the gateway's access token and domain are saved locally:

- **Running as root/sudo:** `/etc/infisical/gateway.conf`
- **Running as a regular user:** `~/.infisical/gateway.conf`

For systemd-based installations, the configuration is always at `/etc/infisical/gateway.conf`. The file is created with restricted permissions (0600) so only the owner can read it.
</Accordion>
