...
Direct listen isn't a paid feature you can turn on for Cloud. Cloud runs outside your network and has no route into it, leaving no address it could dial. Relay mode exists for exactly that case: the Gateway opens the connection from the inside.
...
<Steps>
  ...
  <Step title="Configure network and firewall">
    ...
    **Direct listen mode.** The Gateway accepts inbound connections. One port has to be open to Infisical.
