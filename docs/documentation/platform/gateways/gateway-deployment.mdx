...
Infisical Gateways give the platform access to resources in your private network, such as databases and internal APIs, without handing out their credentials.
...
Before you deploy, decide which side opens the connection between Infisical and the Gateway. This is the one choice that changes what else you have to set up.

| | Direct listen | Relay |
|---|---|---|
| Who connects | Infisical connects to the Gateway | The Gateway connects out to a relay |
| Firewall | Inbound port open to the Infisical host | Outbound only |
| Relay required | No | Yes |
| Needs a stable Gateway address | Yes | No |
| PAM CLI from outside the network | No | Yes |
| Infisical Cloud | Not available | Yes |

**Use direct listen** when Infisical can already reach the Gateway host at a stable address. This is the common case when you self-host Infisical in the same network as your resources, and it means there's no relay to deploy or operate.

**Use relay mode** when the Gateway sits behind NAT or a firewall you can't open, when you're on Infisical Cloud, or when PAM CLI users connect from outside the Gateway's network.

Direct listen isn't a paid feature you can turn on for Cloud. Cloud runs outside your network and has no route into it, so there's no address it could dial. Relay mode exists precisely so the Gateway can open that connection from the inside.

You can also run both at once, which gives you direct connections for platform operations and a relay for PAM CLI users outside the network. Pass `--relay` and `--listen-address` together, and Infisical uses the direct address first and falls back to the relay when it stops answering.
...
<Steps>
  ...
  <Step title="Create the gateway in the UI">
    ...
    5. Select **Show deploy command** in the **Deployment** card.
  ...
  <Step title="Configure network and firewall">
    ...
    **Direct listen mode.** The Gateway accepts inbound connections, so one port has to be open to Infisical.

    | Protocol | Direction | Source or destination | Port | Purpose |
    | -------- | --------- | --------------------- | ---- | ------- |
    | TCP      | Inbound   | Infisical instance host | the port in your listen address | Platform connects to the gateway |
    | TCP      | Outbound  | Infisical instance host | 443  | API communication and certificate requests |

    Scope the inbound rule to the Infisical instance host rather than opening the port broadly.
  ...
  <Step title="Run the CLI command">
    ...
    - `--relay=<relay-name>` to pin a relay, or omit it to let the gateway pick the healthiest one
    ...
    <AccordionGroup>
      ...
      <Accordion title="Kubernetes Auth">
        ...
        <Warning>
          ...
          This means the **first** gateway in a private cluster can't use Kubernetes auth. Enrol it with an enrollment token or AWS auth, then point later gateways at it. Deleting a gateway that other gateways use as their reviewer is refused until they're pointed elsewhere.
        </Warning>
    ...
    <Info>
      Token-method enrollment tokens are single-use and expire after 1 hour. If the token expires before deployment, select **Show deploy command** again on the detail page to generate a new one.
    </Info>
  ...
  <Step title="Verify your gateway deployment">
    ...
    1. **Check logs** for the "Gateway started successfully" message. In direct listen mode you'll also see "Direct gateway listener started" with the address it's advertising and the port it bound.
...
Yes. Open the gateway's detail page, select **Show deploy command**, choose the mode you want, and re-run the generated command on the host. The gateway re-registers with the new mode and is issued the certificate it needs for it.

Switching away from a mode drops it. To keep both, pass `--relay` and `--listen-address` together.

</Accordion>

<Accordion title="Why does my direct listen gateway fail to register?">
The listen address has to be `host:port`, with a port in the range 1 to 65535. An address with no port is rejected, and so is one that includes a scheme or a path, so use `gateway.internal:8443` rather than `https://gateway.internal` or `gateway.internal`.
...
<Accordion title="Do PAM CLI sessions work with direct listen?">
Only from a machine that can reach the gateway's registered address. The CLI connects to the gateway from the user's own machine rather than through Infisical, so a laptop outside the network can't reach a gateway listening on an internal address. Give those users a VPN into the network, or run a relay on the gateway alongside direct listen.

Browser-based PAM access works in either mode, because the platform brokers that connection.

Using direct listen also requires a current Infisical CLI. An older CLI can't connect to a gateway that only listens directly, and reports that the gateway needs a newer CLI.
...
2. From the Infisical instance host, test the registered address:
  ```bash
  nc -zv gateway.internal 8443
  ```
3. Check the inbound firewall rule allows the Infisical instance host on that port
4. Confirm the registered address resolves from the Infisical host, not just from your own machine
...
For token method: ensure the enrollment token hasn't expired or already been used. Open the gateway's detail page and select **Show deploy command** to generate a fresh token.
...
Enrollment tokens expire after 1 hour. If the token expires before deployment, open the gateway's detail page and select **Show deploy command** to generate a new one.
</Accordion>
...
- **Transport fallback**: A gateway running both modes falls back to its relay when its direct address stops answering, and returns to direct connections once it recovers
