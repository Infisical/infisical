...
- **Show deploy command**: Generates a fresh enrollment token (token method) or re-displays the start command for the AWS and Kubernetes methods. Selecting this on a token-method gateway does **not** disconnect the running gateway; the next login with the new token rotates credentials atomically.
...
For token method: ensure the enrollment token has not expired or already been used. Open the gateway's detail page and select **Show deploy command** to generate a fresh token.
