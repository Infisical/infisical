...
<Steps>
  ...
  <Step title="Generate Certificate Request on iDRAC">
    When iDRAC prompts you to generate a certificate request, you'll need to enter subject values. You can find the required values in Infisical by selecting **Generate Certificate** on your KMIP client and selecting the **CSR** request method - the modal will display the exact **Client ID** and **Project ID** to use.
  ...
  <Step title="Upload Certificate to iDRAC">
    ...
    3. Select **Test Network Connection** to verify connectivity
