...
If you are on-premise, your KMS root key will be created at random with the `ROOT_ENCRYPTION_KEY` environment variable. You can also use a Hardware Security Module (HSM), to create the root key. Read more about [HSM](/self-hosting/guides/hsm-integration). 
    ...
    <Tab title="Infisical UI">
        <Steps>
            ...
            <Step title="Verifying a MAC with the KMS key">
                Select verify MAC, paste your MAC and data into the text areas, and tap on the Verify MAC button.
...
Every KMS key tracks a version, starting at 1. Rotating a key generates new cryptographic key material for it and increments the version. The key keeps the same ID and name, so applications referencing the key do not need any changes.
...
- Rotation does not re-encrypt data that was already encrypted with the key. If you need ciphertexts to use the new material, decrypt and re-encrypt them.
...
<Warning>
  ...
  - **Exported or imported key material.** [Bulk export](#bulk-export) and the private-key export endpoints return only the current key material, and [bulk import](#bulk-import) creates a key at version 1. A key exported before rotation, or imported into another instance, will not carry its earlier versions, so ciphertexts encrypted under those earlier versions cannot be decrypted from the exported material alone. If you rely on exported material for escrow or disaster recovery, re-export after each rotation and keep every exported version.

  - **KMIP-managed keys.** A KMIP client retrieves raw key material and encrypts data itself. If you rotate a key that is used over KMIP, the client will retrieve different material on its next fetch and will no longer be able to decrypt data it encrypted with the previous material. Do not rotate a key that is in use by a KMIP client; provision a new key instead.
</Warning>
...
<Note>
  Keys created with **Allow Export** disabled are non-exportable. They cannot be selected for export,
