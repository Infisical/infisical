...
A running KMIP server renews its TLS certificate automatically once two-thirds of the certificate's validity period has elapsed. Renewal happens in the background: no restart is needed, existing client connections are not interrupted, and new connections use the renewed certificate right away. Each renewal is recorded in the organization audit log as a `register-kmip-server` event with `isRenewal: true`.

If a renewal attempt fails (for example, the Infisical platform is temporarily unreachable), the server keeps serving its current certificate and retries with increasing frequency as expiry approaches, logging warnings and errors so you can intervene. Once the platform is reachable again, the next retry succeeds and normal operation resumes.

If you revoke the server's access, renewal fails with an authentication error. AWS-enrolled servers recover automatically by re-authenticating via STS. Token-enrolled servers cannot, since enrollment tokens are single-use; the server logs an error asking you to re-enroll. Generate a new enrollment token via **Show deploy command** and restart the server with it to recover.
