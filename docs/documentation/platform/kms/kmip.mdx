...
<Steps>
  <Step title="Create the KMIP Server in the UI">
    ...
    2. Click **Create KMIP Server** and fill in:
        ...
        - **Common Name** (optional): subject common name for the server certificate. Defaults to the server name.
  ...
  <Step title="Run the CLI Command">
    ...
    <Note>
      The server's certificate configuration (hostnames/IPs, TTL, common name, key algorithm) is read from the server entity you configured in the UI, so the deploy command does not include it. To change the certificate, edit the **Certificate Configuration** card on the server's detail page and restart the server.
    </Note>
...
- **Edit certificate configuration**: Update the hostnames/IPs, common name, TTL, or key algorithm used for the server's TLS certificate. Restart the server after saving for the new certificate to take effect.
