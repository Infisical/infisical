...
<Steps>
  <Step title="Create the KMIP Server in the UI">
    ...
    2. Click **Create KMIP Server** and fill in:
        ...
        - **Certificate TTL** (optional): validity period of the server certificate (for example `1y`). Defaults to `1y`. The server renews its certificate automatically before it expires, so this value mainly controls how often that happens.
...
- **Edit certificate configuration**: Update the hostnames/IPs, TTL, or key algorithm used for the server's TLS certificate. Changes take effect at the next automatic certificate renewal; restart the server to apply them immediately.
