...
<Steps>
  <Step title="Navigate to Email Domain Settings">
    Go to **SSO & Provisioning** → **SSO** tab → **Email Domains** section.
  ...
  <Step title="Add an Email Domain">
    Select **Add Domain** and enter the domain you want to verify (e.g., `company.com`).

    ![Add Domain Modal](/images/platform/email-domain/add-domain-modal.png)

    Select **Add Domain** to create the verification request.

  </Step>

  <Step title="Copy DNS Verification Details">
    After adding the domain, select **Verify Domain** on the pending domain row.
  ...
  <Step title="Verify the Domain">
    Return to the Infisical Email Domains section and select **Verify Domain** on the pending domain row.

    In the modal that opens, select **Verify Domain**. Infisical will perform a DNS TXT lookup to confirm the record exists and matches.
...
<Steps>
  ...
  <Step title="Authenticate via IdP">
    You'll be redirected to your organization's identity provider (Okta, Entra ID, Google Workspace, etc.) to complete authentication.
  </Step>
</Steps>
...
- If Organization A verifies `company.com`, Organization B can't verify or claim the same domain.
...
- If the user's email domain isn't verified by the SCIM token's organization, the request is rejected.
...
<AccordionGroup>
  <Accordion title="What happens if DNS verification fails?">
    If the DNS TXT record isn't found or doesn't match, verification will fail with an error message.
...
<Accordion title="Does verifying a parent domain cover all subdomains?">
  ...
  doesn't automatically cover `app.company.com` or `dev.company.com`.
</Accordion>
</AccordionGroup>
