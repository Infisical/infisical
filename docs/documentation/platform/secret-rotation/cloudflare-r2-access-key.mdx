...
An R2 access key is a Cloudflare API token viewed through R2's S3-compatible API: the **Access Key ID** is the token's ID, and the **Secret Access Key** is the SHA-256 hash of the token's value. This rotation creates that token scoped to the buckets you select, then hands you all three values as secrets.
...
<Tabs>
    <Tab title="Infisical UI">
        ...
        ![Select Cloudflare R2 Access Key](/images/secret-rotations-v2/cloudflare-r2-access-key/select-cloudflare-r2-access-key.png)
        ...
        ![Rotation Configuration](/images/secret-rotations-v2/cloudflare-r2-access-key/configuration.png)
        ...
        ![Rotation Parameters](/images/secret-rotations-v2/cloudflare-r2-access-key/parameters.png)
        ...
        - **API Token** - The name of the secret in Infisical where the underlying Cloudflare API token value will be stored. Use this one when calling the Cloudflare REST API rather than R2's S3 API.

        ![Rotation Secrets Mapping](/images/secret-rotations-v2/cloudflare-r2-access-key/secrets-mapping.png)
        ...
        ![Rotation Details](/images/secret-rotations-v2/cloudflare-r2-access-key/details.png)

        7. Review your configuration, then click **Create Secret Rotation**.

        ![Rotation Review](/images/secret-rotations-v2/cloudflare-r2-access-key/review.png)

        8. Your **Cloudflare R2 Access Key** rotation is created. The credentials are available as secrets at the mapped path, and work with any S3 client pointed at `https://<account-id>.r2.cloudflarestorage.com`. Rotations will create a new key, switch the active secrets to it, then revoke the previous key for zero-downtime rotation.

        ![Rotation Created](/images/secret-rotations-v2/cloudflare-r2-access-key/created.png)
...
The generated credentials work with any S3-compatible client:

```bash
AWS_ACCESS_KEY_ID=$CLOUDFLARE_R2_ACCESS_KEY_ID \
AWS_SECRET_ACCESS_KEY=$CLOUDFLARE_R2_SECRET_ACCESS_KEY \
  aws s3 ls s3://my-bucket --endpoint-url https://<account-id>.r2.cloudflarestorage.com
```
