...
- Your Stripe account must have access to the [Managed API Keys API](https://docs.stripe.com/api-keys), which Stripe currently offers as a private preview.
...
<Tabs>
    <Tab title="Infisical UI">
        ...
        - **Permissions** - The permissions granted to the generated key. Stripe's Managed API Keys API has no wildcard for "all access", so a key with full access is one where every permission in the list is selected individually. If you already have an active Stripe API key with the permissions you want, you can select it from **Copy permissions from an existing key** instead of selecting them by hand.
