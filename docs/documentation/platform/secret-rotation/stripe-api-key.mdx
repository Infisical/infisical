...
- **The minted key's mode follows your Stripe account's own key.** If your Stripe account is in test mode, every key this rotation generates is a test-mode key. Your account decides the mode; the rotation doesn't, and there's no setting to override it.
...
- Your Stripe account must have access to the [Managed API Keys API](/integrations/app-connections/stripe), which Stripe currently offers as a private preview.
...
<Tabs>
    <Tab title="Infisical UI">
        ...
        3. Configure the rotation behavior, then select **Next**.
        ...
        4. Set the Stripe API key parameters, then select **Next**.

        - **Permissions** - The permissions granted to the generated key. Stripe's Managed API Keys API has no wildcard for "all access", so a key with full access is one where every permission in the list is selected individually. If you already have a Stripe API key with the permissions you want, you can copy its permissions instead of selecting them by hand.
        - **Connect permissions** (optional) - Permissions the generated key holds when acting on behalf of a connected account. This only matters if your Stripe account is a [Connect](https://docs.stripe.com/connect) platform; leave it empty otherwise.

        5. Specify the secret name that the rotated API key will be mapped to. Then select **Next**.

        - **API Key** - The name of the secret in Infisical where the generated restricted key's value will be stored.

        6. Give your rotation a name and description (optional). Then select **Next**.
        ...
        8. Your **Stripe API Key** rotation is created. The current restricted key is available as a secret at the mapped path. Rotations will create a new key, switch the active secret to it, then expire the previous key for zero-downtime rotation.
