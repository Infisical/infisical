...
Before you configure Secret Rotation for PostgreSQL credentials, you need to create your database users and configure a PostgreSQL connection in Infisical.

1. (Recommended) Create a dedicated manager user that Infisical will use to rotate credentials. This user requires the `CREATEROLE` privilege. For example:
   ...
   <Note>
     You can use a `superuser` such as `postgres` to perform the rotation.
     However, we recommend creating a dedicated manager user with `CREATEROLE`
     privileges to reduce the risk of privilege escalation.
   </Note>

2. Configure a [PostgreSQL connection](/integrations/app-connections/postgres) in Infisical using the `infisical_manager` credentials you just created.
3. Create rotation users that your application will use to connect to Infisical. `infisical_manager` must be able to modify them. For example:
...
Once you have created your rotation users and configured a PostgreSQL connection in Infisical, you can configure Secret Rotation for PostgreSQL credentials:
