...
<Tabs>
    <Tab title="Infisical UI">
        ...
        - **Rotation Method** - The method to use when rotating the target user's password.
            ...
            - **Login as Root** - Infisical will use the Windows Connection's administrator credentials to rotate the target user's password.
        ...
        If you suspect the credentials are out of sync (for example, after a manual password change on the server), you can regain access by using **Reconcile**. This will use the configured Windows Connection's administrator account to reset the target user's password and sync it with Infisical.
...
- Verify SMB is enabled on the target server: `Get-SmbServerConfiguration`
- Check that the firewall allows connections on port 445
- Ensure SMB3 protocol is supported (Windows Server 2012+)

**Authentication failed (NT_STATUS_LOGON_FAILURE)**
- Verify the username and password are correct
- Check if the account is locked or disabled
- For domain accounts, ensure the domain name is correct in the Windows Connection

**Access denied (NT_STATUS_ACCESS_DENIED)**
- Ensure the Windows Connection credentials have administrator privileges
- For "Login as Root" method, the connection user must have rights to change other users' passwords
- Verify the target user account exists and is not locked

**Password policy violations (NT_STATUS_PASSWORD_RESTRICTION)**
...
- Ensure generated passwords meet minimum length and complexity requirements

**Host unreachable**
- Verify network connectivity to the Windows server
- Check DNS resolution if using a hostname
- Ensure there are no network firewalls blocking port 445
- Consider using the Infisical Gateway for private network access
