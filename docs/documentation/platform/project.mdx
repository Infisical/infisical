...
Infisical supports project types, each representing a different security product with its own dashboard, workflows, and capabilities. The organization overview lists them as cards, and selecting one opens the projects of that type.
...
- [Secret Scanning](/documentation/platform/secret-scanning/overview): Detect hardcoded secrets in code, CI pipelines, and infrastructure—integrated with GitHub, GitLab, Bitbucket, and more.
- [Certificate Manager](/documentation/platform/pki/overview): Issue and manage X.509 certificates using protocols like EST, with support for internal and external CAs.
- [KMS](/documentation/platform/kms/overview): Encrypt and decrypt data using centrally managed keys with enforced access policies and full audit visibility.
- [Privileged Access Manager](/documentation/platform/pam/overview): Manage access to resources like databases, servers, and accounts with policy-based controls and approvals.
- [Agent Vault](/documentation/platform/agent-vault/overview): Broker outbound API calls from AI agents through a proxy that attaches stored credentials, so agents never hold them.
