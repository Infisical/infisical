...
<Info>
  Groups is a paid feature. If you're using Infisical Cloud, then it's
...
- If an identity was previously added to a project under a role and is later added to a group that has access to the same project under a different role, then the identity will now have access to the project under the composite permissions of the two roles. If the group is subsequently removed from the project, the identity retains the access granted by its direct project role.
- An identity can be part of multiple groups and inherits the combined permissions of those groups.
...
<Steps>
   ...
   <Step title="Adding identities to the group">
        Next, you'll want to assign identities (users and/or machine identities) to the group. To do this, select the group row to open the group details page, then select **Add Member**.
