...
<AccordionGroup>
    ...
    <Accordion title="Palo Alto Networks XSIAM">
        [XSIAM](https://www.paloaltonetworks.com/cortex/cortex-xsiam) accepts log data from external sources via it's **Custom HTTP Log Collector**, which exposes a unique HTTPS endpoint secured by an API key. 
