    ...
    <Accordion title="Sumo Logic">
        ...
        <Steps>
            <Step title="Create an HTTP Source in Sumo Logic">
                ...
                <Note>
                    The token authenticates the request, so treat it as a secret. You can regenerate it later from the source settings if needed.
                </Note>
            </Step>
            <Step title="Create Audit Log Stream on Infisical">
                ...
                <Note>
                    If you'd rather use a **Presigned URL** instead, select that option in Sumo Logic, set the full presigned URL (with the embedded token) as the **HTTP Source Address**, and leave the **Auth Token** empty. The **Auth Header** approach above is recommended.
                </Note>

                <Info>
                    Events are delivered as a newline-delimited JSON (NDJSON) batch, bounded to roughly 1 MB per request. See [Delivery Format](#delivery-format) for details.
                </Info>
