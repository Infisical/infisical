    ...
    <Accordion title="Splunk">
        ...
        <Note>
            **Splunk Cloud serves the HTTP Event Collector on port 443.** The default port `8088` applies to Splunk Enterprise and to Splunk Cloud free trials.
            ...
            Splunk Cloud also uses a dedicated collector hostname, which is not the address you use to reach the Splunk web interface:
...
1. **Automatic retries with backoff.** The event is retried automatically up to **5 attempts**. Retries use exponential backoff with jitter (starting at 30 seconds and capping at roughly 4 minutes between attempts) so a struggling or rate-limiting destination is not hammered.
...
<Warning>
    Backfilling depends on the events still being stored in Infisical. If you have set `DISABLE_POSTGRES_AUDIT_LOG_STORAGE=true` and are not using [ClickHouse](/documentation/platform/audit-logs-clickhouse-setup), the stream is your **only** sink and dropped events cannot be recovered. Keep at least one durable audit log store (PostgreSQL or ClickHouse) enabled if you rely on streams for compliance. Also note that backfilling is bounded by your plan's audit log **retention period**: events older than your retention window are no longer available to export.
</Warning>
