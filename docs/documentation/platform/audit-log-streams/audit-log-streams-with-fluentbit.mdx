...
This guide will demonstrate how you can send Infisical Audit log streams to storage solutions that don't support direct HTTP-based ingestion, such as AWS S3.
To achieve this, you'll learn how you can use a log collector like Fluent Bit to capture and forward logs from Infisical to non-HTTP storage options. 
...
Using this pattern, you're able to send Infisical Audit logs to various providers that don't support HTTP based log ingestion by default. 

<Info>
    Infisical delivers events to Custom endpoints (including Fluent Bit) as a [JSON array of events](/documentation/platform/audit-log-streams/audit-log-streams#delivery-format) per request. Fluent Bit's `http` input accepts JSON arrays and emits each element as a separate record, so no extra configuration is required.
</Info>
