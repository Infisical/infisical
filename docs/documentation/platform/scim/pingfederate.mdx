...
<Info>
    ...
    If you're using Infisical Cloud, then it's available under the **Enterprise Tier**. If you're self-hosting Infisical,
...
<Steps>
    ...
    <Step title="Configure the Provisioner Data Store">
        ...
        must be a JDBC database; LDAP data stores can't be used for this purpose.
    ...
    <Step title="Generate a SCIM token in Infisical">
        ...
        turn on the **Enable SCIM** toggle to allow PingFederate to provision and deprovision users for your organization.

        ![SCIM enable provisioning](/images/platform/scim/scim-enable-provisioning-new.png)

        Select **Configure** on the SCIM Provisioning card, then select **Create Token** in the **Manage SCIM Credentials** modal to generate a SCIM token for PingFederate.
        ...
        The token is shown once and can't be retrieved again.

        ![SCIM copy token](/images/platform/scim/scim-copy-token.png)
    </Step>

    <Step title="Create the SP Connection">
        Navigate to **Applications → Integration → SP Connections** and select **Create Connection**.
    ...
    <Step title="Configure the SCIM channel">
        On the **Manage Channels** tab, select **Create**. This opens the per-channel wizard with six tabs.
        ...
        **5. Attribute Mapping**
        ...
        For each row, select **Edit**, choose **Root Object Class** and **Attribute**, then select **Add Attribute** and **Done**.
    ...
    <Step title="Activate the SP Connection">
        ...
        On the parent SP Connection wizard's **Outbound Provisioning** tab, select **Next**.
