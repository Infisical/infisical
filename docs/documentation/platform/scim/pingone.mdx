...
<Steps>
    <Step title="Create a SCIM token in Infisical">
        ...
        turn on the **Enable SCIM** toggle to allow PingOne to provision and deprovision users for your organization.

        ![SCIM enable provisioning](/images/platform/scim/scim-enable-provisioning.png)

        Next, select **Configure** on the SCIM Provisioning card, then select **Create Token** in the **Manage SCIM Credentials** modal to generate a SCIM token for PingOne.
