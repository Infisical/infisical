...
<Steps>
    <Step title="Create a SCIM token in Infisical">
        ...
        turn on the **Enable SCIM** toggle to allow JumpCloud to provision and deprovision users and user groups for your organization.

        ![SCIM enable provisioning](/images/platform/scim/scim-enable-provisioning.png)

        Next, select **Configure** on the SCIM Provisioning card, then select **Create Token** in the **Manage SCIM Credentials** modal to generate a SCIM token for JumpCloud.
    ...
    <Step title="Configure SCIM in JumpCloud">
        ...
        **API Type** is set to **SCIM API** and **SCIM Version** is set to **SCIM 2.0**.
        ...
        Also, under HTTP Header > Authorization: Bearer, input the **New SCIM Token** from Step 1.
