...
<Steps>
  ...
  <Step title="Create a machine identity for decision callbacks">
    ...
    Next, give the identity permission to submit decisions. The permission is project scoped, so grant it in each project that uses external approvals:

    1. Inside the project, go to **Access Control** > **Project Roles** and create a role such as `servicenow-external-reviewer`.
...
| Callback Failed with HTTP 401 or 403        | The machine identity isn't a member of the project, is missing the **Review** permission on **External Approvals** there, or isn't the approver identity configured on the policy.        |
