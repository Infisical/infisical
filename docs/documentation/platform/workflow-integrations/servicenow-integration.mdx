...
2. The request matches an access policy routed to **ServiceNow**.
...
If Infisical can't reach your instance at step 3, it tries up to five times in total, waiting longer between each attempt. It doesn't retry a failure another attempt can't fix, such as credentials ServiceNow rejected or a missing role on the integration user. A request that still doesn't get through shows as **Not Submitted** in Infisical with a **Retry** button, and no record exists in ServiceNow.
...
<Steps>
  ...
  <Step title="Create a machine identity for decision callbacks">
    ...
    In Infisical, go to **Access Control** > **Machine Identities** and create a machine identity, for example `servicenow-approvals`.

    Configure it with **Universal Auth** and generate a Client ID and Client Secret. Save both, the secret is shown only once.

    Next, give the identity permission to submit decisions. The permission is organization scoped, so you grant it once and it covers every project:

    1. Go to **Access Control** > **Organization Roles** and create a role such as `servicenow-external-reviewer`.
    2. Grant it the **Review** permission on **External Approvals**.
    3. Assign that role to the machine identity.

    <Note>
      **Review** on **External Approvals** only allows submitting an approve or reject decision on
  ...
  <Step title="Build the approval flow">
    Approval routing is entirely yours to define in Flow Designer. Infisical doesn't dictate approvers, steps, or SLAs.
  ...
  <Step title="Enable ServiceNow approvals on an access policy">
    In your Secrets Management project, go to **Approvals > Policies** and create or edit an **Access Policy**. Under **Approval Routing**, select **External System**, then set:

    - **External Provider**: **ServiceNow**.
    - **App Connection**: the connection created in step 2.
    - **Approving Identity**: the machine identity created in step 3. Infisical includes this identity's ID on every request so ServiceNow can resolve the matching identity mapping.
    ...
    Requests matching this policy are now routed to ServiceNow. Policies left on **Infisical** routing keep using in-app approvers, so you can move one environment at a time.

    <Note>
      **Bypass Approvals** and **Self Approvals** have no effect on a policy routed to ServiceNow. Every request under it has to be decided in ServiceNow, break-glass included.
    </Note>
...
The record is a read-only mirror of the request. Editing the fields in ServiceNow doesn't change what Infisical grants; only the decision sent back matters.
...
| Callback Failed     | The decision couldn't be delivered. See the Callback Error field. |
...
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Requests never appear in ServiceNow         | The policy isn't routed to ServiceNow, or the environment and secret path on the policy don't match the request. Check the Infisical audit log for the delivery attempt. |
...
| Callback Failed: no active identity mapping | The request's identity ID has no active row under **Infisical > Identity Mappings**. Confirm the **Approving Identity** on the policy matches the identity ID in the mapping. |
| Callback Failed with HTTP 401 or 403        | The machine identity is missing the **Review** permission on **External Approvals** for the organization, or it isn't the **Approving Identity** configured on the policy. |
| Callback Failed with a connection error     | The ServiceNow instance can't reach your Infisical URL. Check the URL on the alias and any outbound firewall rules on the instance.                                                      |
| Request shows Not Submitted in Infisical    | Delivery failed and nothing reached ServiceNow. The status message names the cause. Fix it, then select **Retry** on the request. |
