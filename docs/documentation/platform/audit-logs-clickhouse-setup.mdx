...
<Warning>
Enabling ClickHouse is a hard cutover. All reads immediately route to ClickHouse — the application will no longer query PostgreSQL for audit logs. Any historical data that hasn't been migrated to ClickHouse first will be invisible in the UI until it's migrated. The data isn't deleted from PostgreSQL, but the application won't read from it while ClickHouse is enabled.
</Warning>
