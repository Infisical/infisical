...
A referenced secret may itself contain references. Infisical keeps expanding each resolved value until no references remain, up to a maximum depth of **10** levels. Circular references stop expanding instead of looping.

```
dev /common:
  DB_USER = "app"
  DB_HOST = "db.internal"
  DB_CREDS = "${dev.common.DB_USER}@${dev.common.DB_HOST}"

dev /:
  DB_URL = "postgres://${dev.common.DB_CREDS}/myapp"
```

`DB_URL` resolves to `postgres://app@db.internal/myapp`.

If a reference points to a secret that does not exist, the literal `${...}` text is left in place rather than being replaced with an empty string.

### References inside references (dynamic paths)

Building the *path* or *key* of a reference from the value of another secret is **not supported**. A reference is resolved in a single pass, so an expression like the one below will not produce the value of `DD_API_KEY`:

```
${dev.common.${dev.common.VENDOR}.DD_API_KEY}   # not supported
```

The inner `${dev.common.VENDOR}` is substituted first, but the outer expression it produces is not re-evaluated, so the value renders literally as `${dev.common.datadog.DD_API_KEY}`.

If you need indirection, use one of the following approaches instead:

<Tabs>
  <Tab title="Pointer secret">
    Store the *full reference* as the value of a pointer secret, then reference the pointer. Switching targets is then a one-line change to the pointer, and chained expansion resolves it in one hop.
    ...
    dev /common:
      ACTIVE_VENDOR_API_KEY = "${dev.common.datadog.DD_API_KEY}"   # swap this to point elsewhere

    dev /:
      MONITORING_KEY = "${dev.common.ACTIVE_VENDOR_API_KEY}"
    ```

    `MONITORING_KEY` resolves to `dd-api-key-value`.
  </Tab>
  <Tab title="Agent template">
    When rendering with [Infisical Agent](/integrations/platforms/infisical-agent) templates, compute the path in Go template syntax and fetch the secret with it. This supports fully dynamic paths derived from other secrets, node facts, or environment variables.

    ```bash
    {{- $vendor := (getSecretByName "<project-id>" "dev" "/common" "VENDOR").Value }}
    {{- with getSecretByName "<project-id>" "dev" (printf "/common/%s" $vendor) "DD_API_KEY" }}
    dd_api_key = "{{ .Value }}"
    {{- end }}
    ```
  </Tab>
