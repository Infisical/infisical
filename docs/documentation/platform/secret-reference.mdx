...
Enabling Cross-Project Secret Sharing on self-hosted Infisical (v0.162.0)

It's off by default and hidden behind two gates — both must be set, or the "Cross-Project Secret Sharing" card never shows up in project Settings.

Whitelist your org in the backend env (.env), then restart the backend:

in .env file: `CROSS_PROJECT_SECRET_SHARING_ORG_WHITELIST=<your-org-id>`
