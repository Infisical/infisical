...
![Secrets dashboard with values revealed, showing references highlighted inside them](../../images/platform/secret-references-imports/secret-reference.png)

Since secret referencing reconstructs values on the client side, any client (user, service token, or machine identity) fetching secrets must have proper permissions to access all base and dependent secrets. Without sufficient permissions, secret references won't resolve to their appropriate values.
...
To add a secret import, select the chevron next to **Add Secret**, then select **Add Secret Import**.

![Add Secret Import dialog](../../images/platform/secret-references-imports/secret-import-add.png)

Each import appears above the secrets on the dashboard, labeled with the environment and path it pulls from. In the example below, `/backend` and `/third-party` are both imported into the current folder.

![Secrets dashboard listing two imports above the secrets](../../images/platform/secret-references-imports/secret-import-added.png)

Hover over an import to reach its controls: select the trash icon on the right to delete it, or drag the handle on the left to reorder it.

![An import row hovered, showing the drag handle and the trash icon](../../images/platform/secret-references-imports/secret-import-delete.png)

Order matters. If two imports carry a secret with the same name, the value from the bottom-most import wins.
...
<Info>
  Cross-project secret sharing is a paid feature.
  
  If you're using Infisical Cloud, then it's available under the **Pro Tier**. If you're self-hosting Infisical,
...
1. Go to the **Secrets Management** product page.
![The Secrets Management product page](../../images/platform/secret-references-imports/01-secrets-management.png)
2. Select **Product Settings**.
![The Product Settings page](../../images/platform/secret-references-imports/02-product-settings.png)
3. Turn on **Cross-project secret sharing**.
![Product Settings with the cross-project secret sharing toggle on](../../images/platform/secret-references-imports/03-enable-secret-sharing.png)
...
1. Go to **Project Settings** in the source project.
![Project settings for the source project](../../images/platform/secret-references-imports/04-project-settings.png)
2. Select the **Policies** tab.
![The Policies tab, showing the Cross-Project Secret Sharing section](../../images/platform/secret-references-imports/05-project-policies.png)
3. Under **Cross-Project Secret Sharing**, select **Share Secrets**.
![The Cross-Project Secret Sharing section with no linked projects yet](../../images/platform/secret-references-imports/06-create-project-grant.png)
4. Under **Environments & folders**, choose the environment to share from, then type a folder path and select **Add**. Under **Share with projects**, choose the project that should get access, then select **Share**.
![The Share Secrets panel with an environment, a folder, and a target project chosen](../../images/platform/secret-references-imports/07-share-project-grant.png)
...
1. Select the chevron next to **Add Secret**, then select **Add Secret Import**.
![The Add Secret menu, showing Add Secret Import](../../images/platform/secret-references-imports/08-add-secret-import.png)
2. Select **Another Project**.
![The Add Secret Import dialog with Another Project selected](../../images/platform/secret-references-imports/09-other-project.png)
3. Choose the source **Project**, **Environment**, and **Folder path**, then select **Create Import**.
![The dialog filled in with a source project, environment, and folder](../../images/platform/secret-references-imports/10-create-import.png)
4. The import appears at the top of the secrets dashboard, labeled with the project it pulls from.
![The secrets dashboard showing the new cross-project import](../../images/platform/secret-references-imports/11-created-secret-import.png)
