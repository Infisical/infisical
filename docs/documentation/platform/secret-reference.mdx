...
A reference can be nested inside another reference, which lets you build the folder path or the key of a reference from the value of another secret:
...
dev /:
  MONITORING_KEY = "${dev.common.${dev.common.VENDOR}.API_KEY}"
```

`MONITORING_KEY` resolves to `dd-api-key-value`. Changing `VENDOR` to `newrelic` repoints it at `/common/newrelic` without editing `MONITORING_KEY`.

Nesting works in the key position too — `${dev.common.ENDPOINT_${dev.common.REGION}}` resolves `REGION` first, then reads `ENDPOINT_US_EAST_1`.

Each nesting level costs one expansion pass, and the same depth limit of 10 applies. If the reference produced by the inner substitution points at a secret that does not exist, the resolved-but-unmatched reference is left in the value as literal text (e.g. `${dev.common.newrelic.API_KEY}`), which makes the missing target easy to spot.

<Note>
  Only the inner reference is recorded as a dependency of the secret. Infisical cannot know which target a dynamic path will resolve to ahead of time, so features that act on the secret dependency graph (such as the reference tree view) show the reference that is written in the value, not the secret it currently resolves to.
</Note>
