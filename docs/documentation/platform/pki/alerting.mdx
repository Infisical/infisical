...
- **Webhook**: Send alert notifications to a webhook URL. Optionally configure a signing secret to verify the authenticity of webhook payloads.
...
```
x-infisical-signature: t=<timestamp>;<signature>
```
...
function verifyWebhookSignature(header, body, secret) {
  const [timestampPart, signature] = header.split(';');
