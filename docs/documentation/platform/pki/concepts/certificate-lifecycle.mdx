...
Renewal issues a new certificate with a new serial number instead of changing the existing one, because the certificate already installed on your systems has to keep working until the replacement is deployed. Infisical links the two so the relationship is visible from either side:

- The original reports a status of **Renewed** and links to its replacement under **Renewed by**.
- The replacement reports **Active** and links back under **Renewed from**.

A renewed certificate no longer counts as active in the inventory or on the dashboard and is left out of expiry alerts, and any Certificate Sync attached to it moves to the replacement, so the replacement is what gets pushed to your destinations. It stays cryptographically valid until it expires, which is what gives you a window to roll the replacement out. If the original needs to stop being trusted before then, revoke it.

<Note>
  Renewal never revokes the original certificate. Any client still holding it continues to trust it
