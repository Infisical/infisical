...
| [Application](/documentation/platform/pki/applications/overview) | A team's workspace for one service. Profiles are attached to it, and its members issue certificates through it. | Created by Product Admins, run by teams |
...
Product Admins build the first three layers once, then create each Application and attach its profile. Teams operate the fourth.
...
<Steps>
  <Step title="Create an Application per service and environment">
    ...
    Two things change across the rows. The attached profile decides what the team can produce. The membership decides who can produce it, so in production the wider team is read-only and issuance is limited to the on-call rotation and the deployment pipeline's identity.
...
**A new service needs no new infrastructure.** A Product Admin creates its three Applications and attaches the profiles that already exist. No new CA, no new policy, and no new profile, and the team configures its own enrollment from there.
