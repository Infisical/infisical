...
| `certificate-id` | Conditional | The ID of an existing certificate to fetch and track instead of issuing a new one. Can't be combined with the parameters above, `csr`/`csr-path`, or `attributes`. See [Tracking an existing certificate](#tracking-an-existing-certificate) and [Following renewals](#following-renewals) |
...
| `attributes.ttl` | Optional (uses profile default if not specified) | The time-to-live duration for the certificate (e.g. `72h`, `90d`). Can't be combined with `not-before`/`not-after` |
| `attributes.not-before` / `attributes.not-after` | Optional | Explicit validity bounds for the certificate, as ISO 8601 timestamps (e.g. `2026-09-01T00:00:00.000Z`). Can't be combined with `ttl` |
...
| `attributes.extended-key-usages` | Optional | The list of extended key usage values. One or more of: `server_auth`, `client_auth`, `code_signing`, `email_protection`, `timestamping`, `ocsp_signing`, `smart_card_logon` |
...
| `post-hooks.*.timeout` | Optional (defaults to `30`) | Maximum execution time in seconds for the hook command before it's terminated |
...
- The agent can't auto-renew the certificate, since it doesn't hold the private key required to generate a new CSR.
...
This is also how several machines share one certificate. Ten web servers terminating TLS for `*.example.com` can point at the same `certificate-id` and serve the same wildcard certificate, instead of each issuing its own. Nothing reaches the certificate authority, and your inventory doesn't grow with the fleet.
...
<Warning>
  `lifecycle.renew-before-expiry` has no effect alongside `certificate-id`. The agent delivers a certificate, it doesn't renew one. The agent logs a warning and carries on. Use `lifecycle.replace-on-renewals` instead.
</Warning>

**What to expect in less common cases:**

- A revoked or expired certificate is reported and runs the `on-failure` hook, but the agent doesn't overwrite what is already on disk. The machine keeps serving the last certificate known to be good.
- A certificate that Infisical doesn't hold a private key for, such as one issued from a certificate signing request or through ACME, is written along with its chain. The agent logs a warning and skips the private key file. If such a certificate would *replace* one already delivered, the agent refuses the write instead, because that would leave a non-matching certificate and key on disk.
...
- `replace-on-renewals` requires an Infisical server that supports it. Against an older self-hosted instance the agent can't tell that a renewal exists, so it logs a warning and stays on the certificate it already delivered.
