...
<Note>
  A certificate can be renewed when it was issued from a [certificate profile](/documentation/platform/pki/applications/enrollment-methods/api), is still active, and hasn't already been renewed. Certificates obtained through [ACME](/documentation/platform/pki/applications/enrollment-methods/acme), [EST](/documentation/platform/pki/applications/enrollment-methods/est), or [SCEP](/documentation/platform/pki/applications/enrollment-methods/scep) are renewed by the client that requested them, not from here.
</Note>
...
Defaults configured on the certificate profile are deliberately not applied. They describe what a **new** certificate should look like, so applying them during a renewal would rewrite fields you never touched. The profile's [certificate policy](/documentation/platform/pki/settings/policies) still validates the result, so a renewal can't produce a certificate the policy would have rejected at issuance.

## Choosing how the key pair is handled

The first step of the renewal asks how to handle the key pair. This is the one decision that can't be changed afterwards, because it determines the key the replacement certificate certifies.

| Option | What happens |
|--------|--------------|
| **Reuse the existing key pair** | The renewed certificate carries the same public key. Anything pinned to that key, such as a key-pinned client or a device that can't rotate its key, keeps working. |
...
Not every option applies to every certificate. The renewal wizard greys out the ones that don't, and explains why.
...
The wizard fills these in from the current certificate, including whether each one is critical. Each row has a format: **Text**, which is what most extensions use, or **DER** for a value that isn't text, such as OCSP must-staple. A DER value arrives as base64 and reaches the certificate byte for byte. Edit either one, switch a row's format, or remove the row if the replacement shouldn't carry it.

A few extensions are written by the issuing authority itself. A Certificate Transparency timestamp is signed over the exact certificate it sits on, so the same bytes on a replacement wouldn't verify, and the CA version and previous certificate hash ADCS adds describe that CA's own key and certificate history. Infisical shows them on the certificate and leaves them out of the renewal, where the issuing authority writes its own.

Some authorities, DigiCert among them, don't accept custom extensions at all. A certificate renewed there is issued without them. If the request asks for one anyway, the renewal fails rather than dropping it.
...
<Tabs>
  <Tab title="Web UI">
    <Steps>
      ...
      <Step title="Choose how the key pair is handled">
        Pick one of the three options above. If you chose **Use a CSR**, paste it on the next step and you're done.
      </Step>
  ...
  <Tab title="API">
    ...
    A custom extension carries its value in `value`, and `valueEncoding` says how to read it. The default, `text`, encodes the value as a DER UTF8String. Use `der` for a value that isn't text, such as OCSP must-staple: `{ "oid": "1.3.6.1.5.5.7.1.24", "value": "MAMCAQU=", "valueEncoding": "der" }`. Those bytes reach the certificate unchanged.
...
- The previous certificate stays valid until its own expiry, so you can roll over on your own schedule. It can't be renewed a second time.
- Because it's no longer the current certificate, it drops out of the active count on the dashboard and out of expiry alerts. Filter the inventory by the **Renewed** status, or open the **Renewed** view, to list every certificate that has been superseded.
...
Deploy the renewed certificate before the previous one expires. Renewing doesn't revoke the certificate it replaces, so if the old one was compromised, [revoke it](/documentation/platform/pki/applications/certificates) explicitly.
...
<AccordionGroup>
  ...
  <Accordion title="Why is the key algorithm locked when I reuse the key pair?">
    The algorithm is a property of the key itself, so it can't change while the key stays the same. To move from RSA to an elliptic curve key, or to a larger key size, renew with a new key pair.
  </Accordion>
