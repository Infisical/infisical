...
A renewal starts from the extensions on the current certificate. A value is text, or base64-encoded DER for anything that isn't text, such as OCSP must-staple, which reaches the certificate byte for byte.

Certificate Transparency timestamps and the ADCS CA version and previous certificate hash are left out, because the authority issuing the renewal writes its own. An authority that doesn't accept custom extensions at all, DigiCert for example, issues the renewal without them and refuses a request that asks for one.
...
<Tabs>
  ...
  <Tab title="API">
    ...
    `valueEncoding` says how to read a custom extension's `value`: `text` by default, or `der` for base64 that reaches the certificate byte for byte, as in `{ "oid": "1.3.6.1.5.5.7.1.24", "value": "MAMCAQU=", "valueEncoding": "der" }` for OCSP must-staple.
