...
When you deploy a `Certificate` CRD in your workload cluster, `cert-manager` uses the [ACME enrollment method](/documentation/platform/pki/applications/enrollment-methods/acme) configured on a [certificate profile](/documentation/platform/pki/settings/profiles) to authenticate using EAB credentials and request an intermediate CA certificate.
