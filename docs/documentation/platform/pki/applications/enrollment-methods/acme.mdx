...
<CardGroup cols={2}>
  ...
  <Card title="Load Balancers" icon="scale">
    Automate certificate provisioning for HAProxy, Traefik, and others.
  </Card>
  <Card title="Automated Renewal" icon="rotate-cw">
...
<Steps>
  ...
  <Step title="Configure enrollment on an attached profile">
    Go to the **Settings** tab and find the **Certificate Profiles** section. Select **Configure** on the profile you want to enable ACME enrollment for.
  ...
  <Step title="Add ACME enrollment">
    In the modal, select **Add enrollment method** and select **ACME**.
  </Step>
  ...
  <Step title="Get ACME credentials">
    After saving, select **Reveal ACME EAB** to get the credentials your ACME client needs:
...
<CardGroup cols={2}>
  <Card title="NGINX Guide" icon="server" href="/documentation/platform/pki/guides/applications/nginx-certbot">
    Set up HTTPS on NGINX with Certbot.
  </Card>
  <Card title="Kubernetes Guide" icon="settings" href="/documentation/platform/pki/guides/applications/k8s-cert-manager-acme">
