...
<Tabs>
  ...
  <Tab title="Microsoft Intune">
    ...
    - Requires a **Microsoft Intune** [App Connection](/integrations/app-connections/microsoft-intune) with permission to validate SCEP requests.
    - Infisical reports issuance success and failure back to Intune for each request.

    <Warning>
      **Do not attach an approval policy to a profile used with Microsoft Intune.** Intune SCEP issues synchronously, so a pending (approval-gated) response makes the device give up and re-enroll instead of installing the certificate.
    </Warning>
  </Tab>
