...
sidebarTitle: "SCEP"
description: "Issue certificates using the Simple Certificate Enrollment Protocol (SCEP) for network devices, mobile devices, and MDM systems."
---
...
<Info>
  SCEP enrollment is configured on profiles attached to your [Application](/documentation/platform/pki/applications/overview). Product Admins attach [profiles](/documentation/platform/pki/settings/profiles), and Application Admins configure enrollment methods on those profiles.
</Info>

## When to use SCEP enrollment

<CardGroup cols={2}>
  <Card title="MDM Systems" icon="smartphone">
    Jamf Pro, Ivanti, Microsoft Intune, Workspace ONE.
  </Card>
  <Card title="Network Devices" icon="network">
    Routers, switches, firewalls, and access points.
  </Card>
  <Card title="Printers & IoT" icon="printer">
    Network printers, scanners, and IoT devices.
  </Card>
  <Card title="Legacy Systems" icon="history">
...
Infisical supports three challenge authentication modes for SCEP enrollment:
...
<Tabs>
  ...
  <Tab title="Microsoft Intune">
    The challenge password is validated directly with Microsoft Intune instead of against a password stored in Infisical. Intune generates a unique challenge for each device it enrolls, and Infisical asks Intune to confirm that the challenge is valid before issuing the certificate.

    Use this mode when devices are managed by Microsoft Intune. It removes the need to distribute a shared password and lets Intune track certificate issuance per device.

    - Requires a **Microsoft Intune** [App Connection](/integrations/app-connections/microsoft-intune) with permission to validate SCEP requests. Microsoft Intune is an enterprise App Connection.
    - Infisical reports issuance success and failure back to Intune for each request.

    <Warning>
      **Do not attach an approval policy to a profile used with Microsoft Intune.** Intune SCEP issues synchronously and has no pending state. When issuance cannot complete within the request, Infisical reports the request to Intune as failed and returns a failure to the device rather than leaving either side waiting.
    </Warning>
  </Tab>
...
<Steps>
  ...
  <Step title="Add SCEP enrollment">
    In the modal, click **Add enrollment method** and select **SCEP**.
  </Step>
  <Step title="Configure SCEP settings">
    ...
    - **Sign RA certificate with the CA**: Choose whether the RA certificate is self-signed or signed by this profile's CA. Signing it with the CA makes it chain to your CA root, which strict clients such as Apple (macOS and iOS) and Microsoft Intune require. Currently only internal CAs can sign the RA, and requesting it for any other CA type is rejected. This cannot be changed once SCEP enrollment exists, because changing it regenerates the RA certificate and breaks devices that already trust the current one. To change it, disable SCEP enrollment and set it up again.
    ...
    - **Allow Certificate-Based Renewal**: When enabled, devices that already hold a valid certificate issued by the same CA can renew their certificate without providing the challenge password (default: enabled). This cannot be used with Microsoft Intune validation, because a renewal skips the challenge and would bypass Intune's per-request validation. Enabling it explicitly alongside Intune is rejected.
    ...
    <Tabs>
      ...
      <Tab title="Dynamic Challenge">
        ...
        **2. Use the challenge in your SCEP enrollment:**
        ...
        For MDM integrations (Jamf Pro, Ivanti, etc.), the MDM tool handles this automatically via webhook. See the [Jamf Pro integration guide](/documentation/platform/pki/guides/applications/jamf-pro-scep) for details.
      </Tab>
    </Tabs>
...
<AccordionGroup>
  <Accordion title="What is the RA certificate?">
    The RA (Registration Authority) certificate is Infisical's identity for the SCEP message exchange. It's generated automatically when SCEP enrollment is enabled on a profile, and Infisical holds its private key. It's either self-signed or signed by the profile's CA, controlled by the **Sign RA certificate with the CA** option (see above). Strict clients such as Microsoft Intune and Apple require a CA-signed RA, which currently only internal CAs support.
  </Accordion>
...
<CardGroup cols={2}>
  <Card title="Jamf Pro Guide" icon="smartphone" href="/documentation/platform/pki/guides/applications/jamf-pro-scep">
