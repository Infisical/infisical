...
<CardGroup cols={2}>
  <Card title="MDM Systems" icon="mobile">
  ...
  <Card title="Printers & IoT" icon="print">
    Network printers, scanners, and IoT devices.
  </Card>
  <Card title="Legacy Systems" icon="clock-rotate-left">
...
<AccordionGroup>
  <Accordion title="What is the RA certificate?">
    The RA (Registration Authority) certificate is Infisical's identity for the SCEP message exchange. It is generated automatically when SCEP enrollment is enabled on a profile, and Infisical holds its private key. It is either self-signed or signed by the profile's CA, controlled by the **Sign RA certificate with the CA** option (see above). Strict clients such as Microsoft Intune and Apple require a CA-signed RA, which currently only internal CAs support.
  </Accordion>
...
<CardGroup cols={2}>
  <Card title="Jamf Pro Guide" icon="mobile" href="/documentation/platform/pki/guides/applications/jamf-pro-scep">
