...
<Note>
  ...
  The Gateway must be running Infisical CLI **v0.43.120 or later**. Earlier Gateways either can't run the command at all or can't unlock an SSH Connection's passphrase-protected private key, so the command fails. Run `infisical --version` on the Gateway host to check, and see [Gateway deployment](/documentation/platform/gateways/gateway-deployment) for how to upgrade.
</Note>
...
A sync can only tell you the host wasn't ready once it has already tried. If the destination directory was renamed or the service that owns it is stopped, you find out from a failed sync, and on a renewal that is the day the certificate was going to expire anyway.
...
<Steps>
  ...
  <Step title="Write the check">
    Anything that exits non-zero when the host isn't ready works. The variables resolve to the files
...
Delivering a certificate file doesn't make the service use it. NGINX keeps serving the old certificate until something reloads it. A post-sync command is that last step, run for you right after the files land.
