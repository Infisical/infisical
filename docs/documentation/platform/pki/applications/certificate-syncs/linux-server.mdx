...
A sync can only tell you the host wasn't ready once it has already tried. If the destination directory was renamed or the service that owns it's stopped, you find out from a failed sync, and on a renewal that is the day the certificate was going to expire anyway.
