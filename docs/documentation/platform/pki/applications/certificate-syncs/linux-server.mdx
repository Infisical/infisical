...
- A Gateway is required to use a [post-sync command](#post-sync-commands), because the Gateway is what executes it. It must be running Infisical CLI v0.43.117 or later
...
<Note>
  ...
  The Gateway must be running Infisical CLI **v0.43.117 or later**, the release that shipped Gateway-side support for post-sync commands. On an older Gateway the command fails whenever the SSH Connection authenticates with a passphrase-protected private key, because that Gateway cannot unlock the key. Run `infisical --version` on the Gateway host to check, and see [Gateway deployment](/documentation/platform/gateways/gateway-deployment) for how to upgrade.
</Note>
