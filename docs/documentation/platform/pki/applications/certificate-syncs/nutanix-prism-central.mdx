...
Install certificates managed in Infisical directly on your Nutanix clusters. When a certificate is renewed, it is automatically updated on the cluster — no manual intervention required.
...
- A **Nutanix Prism Central** instance reachable from Infisical (hostname or IP address, port default `9440`).
...
<Tabs>
  ...
  <Tab title="API">
    To create a **Nutanix Prism Central Certificate Sync**, make an API request to the Create Nutanix Prism Central PKI Sync endpoint.
