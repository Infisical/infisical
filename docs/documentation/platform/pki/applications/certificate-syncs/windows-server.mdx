...
<Note>
  Windows delivery runs on the Gateway because it uses WinRM message encryption, which lets a certificate travel confidentially over the default WinRM listener without a server certificate. This is the same zero-configuration posture most Windows hosts already ship with.
</Note>
...
<Tabs>
    <Tab title="Infisical UI">
        ...
        3. Configure the **Destination**:
            ...
            - **Destination Directory**: The absolute Windows path where certificate files are written, either a drive path like `C:\certs` or a UNC share like `\\server\share\certs`. It is created if it does not exist. The path cannot contain `..` segments or consecutive separators.
...
IIS can serve a synced PKCS#12 certificate directly from a shared folder through the Centralized Certificate Store (CCS). To wire this up, set the sync to deliver into the CCS folder and make the two sides agree:
...
<Accordion title="Which transport does the sync use?">
  By default the connection uses the standard WinRM HTTP listener on port 5985 with NTLM message encryption. HTTPS on port 5986 is also supported for hosts that require it.
</Accordion>
