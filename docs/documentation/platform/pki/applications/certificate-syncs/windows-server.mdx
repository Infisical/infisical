...
A check is limited to 2048 characters, for the same reason a post-sync command is: Windows caps a command line at 8191 characters and the check is sent to PowerShell encoded, which expands it. For anything longer, put the logic in a script file on the server and have the check call that script.
...
<Note>
  The Gateway must be running Infisical CLI **v0.43.120 or later**. An earlier Gateway has no PowerShell run-command endpoint and rejects the request. Run `infisical --version` on the Gateway host to check, and see [Gateway deployment](/documentation/platform/gateways/gateway-deployment) for how to upgrade.
</Note>
