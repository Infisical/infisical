...
<AccordionGroup>
  ...
  <Accordion title="Where do I see the result of the daily check?">
    A failing daily check marks the sync failed and puts the reason in its last error, the same place a failed sync run reports one, so a broken host shows up wherever you already watch your syncs. A passing check clears a reason an earlier check reported, and changes nothing otherwise. Either way the run is recorded in the sync's logs with the command and the first part of its output.
  </Accordion>
