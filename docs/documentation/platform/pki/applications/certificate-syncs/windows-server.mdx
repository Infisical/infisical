...
<Accordion title="Which transport does the sync use?">
  By default (SSL disabled) the connection uses HTTP with NTLM message encryption on port 5985, which needs no server certificate. Enable SSL on the connection to use HTTPS on port 5986. For a self-signed HTTPS listener, paste its certificate as the SSL certificate so it can be authenticated.
</Accordion>
