...
import PreflightCheckVariables from "/snippets/documentation/platform/pki/applications/certificate-syncs/preflight-check-variables.mdx";
import PreflightCheckFlow from "/snippets/documentation/platform/pki/applications/certificate-syncs/preflight-check-flow.mdx";
...
<Tabs>
    <Tab title="Infisical UI">
        ...
        5. Configure the **Commands**, or leave them empty:
            - **Preflight check**: A PowerShell command run on the server before any certificate is written, to confirm the host is ready, for example `Get-Service W3SVC | Where-Object Status -ne 'Running' | ForEach-Object { throw "W3SVC is not running" }`. A failure stops the sync, so nothing is delivered. See [Preflight Checks](#preflight-checks).
    ...
    <Tab title="API">
        ...
        --data '{
            ...
            "syncOptions": {
                ...
                "preflightCommand": "if (-not (Test-Path {{certificateDirectory}})) { throw \"destination folder is missing\" }",
...
<PreflightCheckVariables />

### Length limit

A check is limited to 8192 characters, counted after the variables are substituted. For anything longer, put the logic in a script file on the server and have the check call that script.
...
<AccordionGroup>
  ...
  <Accordion title="Is there a time limit?">
    Yes, 10 seconds, and a check that runs out of time counts as a failure. The limit is deliberately much shorter than the post-sync command's, because the check gates delivery: a host that cannot answer a readiness question in ten seconds is itself the problem worth reporting. Keep the check to a question that is cheap to answer.

    On Windows the ten seconds bound how long Infisical waits, not the host. WinRM gives no way to cancel a command once it has started, so a check that hangs is reported as a timeout and the sync fails, but the command itself keeps running on the server until it finishes. This is the other reason to keep a check to a cheap question rather than real work.
  </Accordion>
