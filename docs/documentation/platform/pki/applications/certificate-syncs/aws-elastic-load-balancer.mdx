...
<Tabs>
    <Tab title="Infisical UI">
        In the sync details page, select **Set as Default** next to the certificate you want to use as the default for all configured listeners.
    </Tab>
    <Tab title="API">
...
<Note>
  AWS Elastic Load Balancer doesn't support importing certificates back into
  Infisical due to security limitations where private keys can't be extracted
