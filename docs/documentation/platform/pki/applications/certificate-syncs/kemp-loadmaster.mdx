...
<Tabs>
    <Tab title="Infisical UI">
        ...
        4. Configure the **Sync Options**:
            ...
            - **Preserve Certificate on Renewal**: Reuse the existing certificate identifier when a certificate renews, preserving the name and any Virtual Service binding.
            - **Sync Certificate Authorities**: When enabled, each certificate's chain (intermediate CA certificates) is uploaded into the LoadMaster's Intermediate Certs store so it can build the complete chain it presents to clients. When disabled, only the leaf certificates are managed.
            - **CA Certificate Name Schema**: Shown when Sync Certificate Authorities is enabled. Controls how CA certificates are named in the Intermediate Certs store using the `{{fingerprint}}` and `{{commonName}}` placeholders. Defaults to `Infisical-ca-{{fingerprint}}`.
...
When syncing certificates, Infisical performs the following steps on the LoadMaster via the RESTful API:

1. Lists the existing certificate identifiers on the LoadMaster.
2. When **Sync Certificate Authorities** is enabled, uploads each certificate's chain (intermediate CA certificates) into the LoadMaster's dedicated intermediate certificate store, which is how the LoadMaster builds the complete chain it presents to clients. The chain is also included in the leaf import as a fallback.
3. For each certificate, imports a PEM bundle (certificate, chain, and private key) under its identifier, replacing the entry if the identifier already exists.
4. If a Virtual Service is configured, reconciles its bound certificate list so the managed certificates are bound while any certificates the sync does not manage are left in place.
5. If removal is enabled, unbinds certificates that are no longer active in Infisical from the Virtual Service and then deletes them from the certificate store.
...
- **Preserve enabled (default)**: The existing certificate identifier is reused and its content replaced with the renewed certificate. The identifier and any Virtual Service binding are preserved. This is ideal for production environments.
- **Preserve disabled**: A new certificate identifier is created for the renewed certificate. The original certificate remains on the LoadMaster until it is revoked or removed.
