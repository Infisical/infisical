...
<Note>
  NetScaler caps `sslcertkey` names at **63 characters**. Full UUID placeholders (`{{certificateId}}`, `{{profileId}}`, `{{applicationId}}`) each resolve to 32 characters, so combining two of them exceeds the limit and the schema is rejected when you save the sync. For tight schemas, use `{{shortCertificateId}}` (22 characters) in place of `{{certificateId}}`, optionally with a short prefix or `{{commonName}}`.
</Note>
...
Infisical automatically keeps your NetScaler certificates in sync:

- Uploads certificates, with their private keys, to the NetScaler appliance.
- Optionally binds them to an SSL virtual server for HTTPS termination.
- Optionally removes certificates that are no longer active in Infisical.

Changes are saved to the running configuration so they persist across reboots.
