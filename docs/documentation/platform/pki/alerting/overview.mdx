...
- **Daily Alerts** *(Expiration alerts only)*: Off by default. When enabled, notifications are sent every day until the certificate expires or is renewed. When disabled, a single notification is sent when the threshold is first reached.
