...
- **Daily Alerts**: When enabled for expiration alerts, notifications are sent daily from the alert threshold until the certificate expires. By default, alerts send a single notification when the threshold is first reached.
