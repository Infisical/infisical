...
### Issuance, Renewal, and Revocation Alert Payload

Event driven alerts (issuance, renewal, revocation) share the same payload structure. The `type` and `subject` fields reflect the specific event, and `alertBefore` is omitted since these are real time notifications.

For revocation alerts, each certificate in the array will also include `revokedAt` and `revocationReason` fields when available.
