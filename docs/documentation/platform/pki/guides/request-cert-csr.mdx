...
Before requesting a certificate, you need to generate a CSR. Here's how to do it using OpenSSL.
  ...
  <Step title="Issue a certificate with your CSR">
    <Tabs>
      <Tab title="Infisical UI">
        ...
        Select your certificate profile and choose **CSR (Certificate Signing Request)** as the **Request Method**.
      ...
      <Tab title="API">
        ...
        - Ensure the CSR does not exceed 4096 characters
      </Tab>
    </Tabs>
  </Step>
</Steps>
...
Your CSR must comply with the certificate policy attached to the profile. The following attributes are validated:

| Attribute | Validation |
|-----------|------------|
| **Subject attributes** | CN, O, OU, C, ST, L must match allowed/required patterns |
| **Subject Alternative Names** | DNS, IP, Email, URI must match policy rules |
| **Key algorithm** | Must be in the policy's allowed key algorithms |
| **Signature algorithm** | Must be in the policy's allowed signature algorithms |
| **Key usages** | Must include required usages and not include denied usages |
| **Extended key usages** | Must include required usages and not include denied usages |
| **Basic constraints** | CA flag and path length must comply with policy |

If your CSR doesn't comply with the policy, you'll receive an error message indicating which constraint was violated.
...
<AccordionGroup>
  ...
  <Accordion title="Why am I getting a CSR format error in the UI?">
    ...
    **Solution**: Ensure your CSR:
    ...
    - Does not exceed 4096 characters
  ...
  <Accordion title="Why am I getting a CSR format error via API?">
    **Error**: "CSR cannot be empty" or "CSR cannot exceed 4096 characters"
