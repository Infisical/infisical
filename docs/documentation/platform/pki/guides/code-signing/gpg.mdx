...
<Warning>
  If multiple signers use the same certificate/key, they will share the same key grip. GPG will attempt to sign using the first matching slot. Ensure the signer in the first slot has an active signing grant, or configure your signers so each key is used by only one signer.
</Warning>
