---
title: "Sign with DigiCert"
sidebarTitle: "Sign with DigiCert"
...
<Tabs>
  <Tab title="Infisical UI">
    In the Create Signer wizard's **Certificate** step, open **Reuse an existing order** and pick one of the listed issued orders. The Common Name and Validity fields are hidden because they're inherited from the order. Leave the field empty to place a new order.
  </Tab>
  <Tab title="API">
    Set `externalConfiguration.reissueFromExternalOrderId` to the order id on [Create Signer](/api-reference/endpoints/code-signing/signers/create) or [Reissue](/api-reference/endpoints/code-signing/signers/reissue-certificate).
  </Tab>
</Tabs>
