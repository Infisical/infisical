...
sidebarTitle: "Sign container images"
...
<Warning>
  Pre-built cosign binaries (including Homebrew) don't include PKCS#11 support. You must build cosign from source with the `pkcs11key` build tag enabled.
</Warning>
...
cosign sign \
  ...
  --tlog-upload=false \
  myregistry.example.com/myapp@sha256:<digest>
```
...
<Note>
  ...
  - Use `--tlog-upload=false` if you aren't using a Sigstore transparency log.
...
<AccordionGroup>
  <Accordion title="'unimplemented' or 'not built with pkcs11-tool support'">
    Your cosign binary wasn't built with PKCS#11 support. Pre-built binaries (including Homebrew) don't include it. Rebuild from source with `CGO_ENABLED=1 go install -tags pkcs11key github.com/sigstore/cosign/v2/cmd/cosign@latest`.
  </Accordion>
...
<CardGroup cols={2}>
  <Card title="Sign JARs" icon="java" iconType="brands" href="/documentation/platform/pki/guides/code-signing/jarsigner">
