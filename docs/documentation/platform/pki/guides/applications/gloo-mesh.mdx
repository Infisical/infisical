---
title: "Gloo Mesh"
sidebarTitle: "Configure Gloo Mesh Certificates"
description: "Provision Istio intermediate CA certificates for Gloo Mesh using Infisical."
---

Provision and manage Istio intermediate CA certificates for your Gloo Mesh workload clusters using Infisical and [cert-manager](https://cert-manager.io/). For background on Istio certificates, see the [Istio CA overview](https://istio.io/latest/docs/concepts/security/#pki).

<Info>
  This guide uses [ACME enrollment](/documentation/platform/pki/applications/enrollment-methods/acme) configured on an Application.
</Info>
...
When you deploy a `Certificate` CRD in your workload cluster, `cert-manager` uses the [ACME enrollment method](/documentation/platform/pki/applications/enrollment-methods/acme) configured on an [Application](/documentation/platform/pki/applications/overview) to authenticate using EAB credentials and request an intermediate CA certificate.
