...
<Steps>
    ...
    <Step title="Create the SCEP certificate profile in Intune">
        ...
        The remaining settings fall into two categories. Some reach Infisical in the device's certificate signing request and shape the certificate it issues. Intune never transmits the others and does not verify them against the certificate it receives, so Infisical supplies those values:
        ...
        <Note>
          The available fields vary by platform. **Certificate type** appears on every platform and determines whether the subject and subject alternative name may carry user attributes or only device attributes. **Key storage provider** applies to Windows, and **Hash algorithm** applies to Windows and Android. **Deployment Channel** determines which keychain stores the certificate: the Device Channel uses the system keychain and the User Channel uses the user keychain. On macOS, a profile assigned to the user channel supports only the **User** certificate type. Unlike the other fields, the deployment channel cannot be changed once the profile is deployed, so a different channel requires a new profile.
        </Note>
    </Step>
