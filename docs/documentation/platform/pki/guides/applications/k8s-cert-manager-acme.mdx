...
<Steps>
  ...
  <Step title="Create the Certificate">
    ...
    <Info>
      ...
      For the same reason, `isCA: true` is not supported with an [AWS Private CA](/documentation/platform/pki/ca/aws-pca#issuing-ca-certificates) profile, which always requires a path length.
    </Info>
