...
<Steps>
  ...
  <Step title="Create the Certificate">
    ...
    <Info>
      ...
      For the same reason, `isCA: true` cannot be used with a profile backed by [AWS Private CA](/documentation/platform/pki/ca/aws-pca#issuing-ca-certificates), which requires an explicit path length on every CA certificate request.
    </Info>
