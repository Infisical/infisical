...
- **CSR-Based Issuance**: You generate and maintain your own private key externally, create a Certificate Signing Request (CSR), and submit it to Infisical. Infisical signs the CSR and returns only the certificate. This approach is ideal for workflows where remote teams need to maintain control of their private keys.

<Info>
  **Using CSR-Based Issuance**: If your workflow requires you to generate and maintain your own private keys (for example, when remote teams submit CSRs for certificate issuance), you can include a PEM-encoded CSR in your certificate request. See the [CSR-Based Issuance](#csr-based-issuance) section below for details.
</Info>
...
<Steps>
  ...
  <Step title="Issue a certificate">
    ...
    **Sample response**
    ...
    If you have an external private key and want to maintain control of it, you can submit a PEM-encoded Certificate Signing Request (CSR) to the same API endpoint. This workflow is ideal for remote teams that generate and maintain their own private keys and submit CSRs for certificate issuance.
    ...
    **Sample response**
    ...
    <Tip>
      **Generating a CSR**: You can generate a private key and CSR using OpenSSL:
      ```bash
      # Generate a private key
      openssl genrsa -out private.key 2048

      # Generate a CSR using the private key
      openssl req -new -key private.key -out request.csr -subj "/CN=service.acme.com"
      ```
      The CSR file contents can then be included in the `csr` field of your API request.
    </Tip>
