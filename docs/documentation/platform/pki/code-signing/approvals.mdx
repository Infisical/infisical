...
A request's scope is fixed once it is open. Nobody edits it during review, including the approvers: an approval means the parameters shown on the request, so if they are wrong the request is rejected and a new one is opened with the parameters you want.

For a pipeline that signs a different artifact on every run, do not fight this per request. Tell the signing client which parameters to leave out in the first place, with `approval.exclude_scope_fields` in the [PKCS#11 module](/documentation/platform/pki/code-signing/pkcs11-module#leaving-parameters-out-of-a-request) or the [Windows KSP](/documentation/platform/pki/code-signing/windows-ksp#leaving-parameters-out-of-a-request). Excluding `data_hash` is the common case: a timestamped signature changes the digest between runs even for the same file, so pinning it would mean a fresh approval for every build. Exclude one parameter at a time. Each one you drop widens what the approval covers, and because an approval belongs to the identity rather than to a machine, excluding all of them grants that identity any signature it asks for until the window or count runs out.
