...
Open the Signer's **Approvals** tab and select the pencil icon on the **Approval Policy** panel. The editor is a 2-step sheet.
...
| **IP Address** | The address sign calls have to arrive from, for example a build agent's egress address. Infisical reads the address off the sign call itself, the same way audit logs record it, so the caller can't present a different one. |
| **Data Hash** | The SHA-256 digest of the payload submitted for signing, which pins the approval to one artifact. Signing tools submit a digest of the file rather than the file itself, so this isn't the same value as `sha256sum yourfile`. The [PKCS#11 module](/documentation/platform/pki/code-signing/pkcs11-module) and the Windows KSP fill it in for you. |
...
<Note>
  A command scope is compared exactly. Only whitespace is forgiven: a run of spaces or a tab where the approval has a single space still matches. Everything else counts as a different command and needs its own approval, including reordering the options, invoking the tool by a different path, changing an argument, and writing `--flag value` as `--flag=value`. Infisical can't know which flags take values in an arbitrary tool, so it doesn't try to guess whether a rearranged command is equivalent.
</Note>

<Warning>
  On a self-hosted instance, an **IP Address** scope is only reliable once [`TRUSTED_PROXY_CIDRS`](/self-hosting/configuration/envars) names the reverse proxies in front of Infisical. Until it's set, forwarded-IP headers are honored from any source for backwards compatibility, so a caller can present an address that isn't theirs.
</Warning>
...
Pinning an approval to a **Data Hash** is the tightest option: the approver signs off on one file rather than on a window of time, and no other payload can be signed under it.
...
<Tip>
  For release pipelines, pair a data hash scope with a signature count of **1**. The approval then covers exactly one signature over exactly one file.
</Tip>

<Note>
  Filling in a digest by hand is fiddly, because a sign call carries a digest rather than the file, so the value to declare is the hash of the bytes the call submits and not `sha256sum yourfile`. If you're calling the API directly, see the [Sign API reference](/api-reference/endpoints/code-signing/signers/sign).
</Note>

### When the scope needs to change

A request's scope is fixed once it's open. Nobody edits it during review, including the approvers: an approval means the parameters shown on the request, so if they're wrong the request is rejected and a new one is opened with the parameters you want.

For a pipeline that signs a different artifact on every run, don't fight this per request. Tell the signing client which parameters to leave out in the first place, with `approval.exclude_scope_fields` in the [PKCS#11 module](/documentation/platform/pki/code-signing/pkcs11-module#leaving-parameters-out-of-a-request) or the [Windows KSP](/documentation/platform/pki/code-signing/windows-ksp#leaving-parameters-out-of-a-request). Excluding `data_hash` is the common case: a timestamped signature changes the digest between runs even for the same file, so pinning it would mean a fresh approval for every build. Exclude one parameter at a time. Each one you drop widens what the approval covers, and because an approval belongs to the identity rather than to a machine, dropping all of them grants that identity any signature it asks for until the window or count runs out.
  ...
  <Tab title="Pre-approve signing">
    ...
    <Steps>
      ...
      <Step title="Set the access terms">
        ...
        | **Scope this request** | Optional. Expand it to [scope](#scoping-an-approval) the access to a specific command, signing tool, checksum, machine, OS account, IP address, or data hash. Useful for pre-approving a known build agent: set its address and the access only works from there. |
  ...
  <Tab title="Request to sign">
    ...
    <Steps>
      ...
      <Step title="Specify what you need">
        ...
        | **Scope this request** | Optional. Expand it to [scope](#scoping-an-approval) your access to the command, signing tool, checksum, machine, OS account, IP address, or data hash you will sign with. Approvers see whatever you declare, and the access only covers matching sign calls. |
...
<Steps>
  <Step title="Open the request">
    Select the row on the Signer's **Approvals** tab. Full details: requester, recipient, justification, requested signings, requested duration, and which step is currently pending.
  </Step>
  <Step title="Review">
    Verify who's asking, for how many signings, for how long, and why. If the requester scoped the request, the detail page lists every parameter the access will be limited to, so you can see exactly which signing situation you're approving.
  </Step>
  <Step title="Decide">
...
<Note>
  You can never approve your own signing request, even if you're listed as an approver for the current step. This is always enforced and can't be disabled. (Pre-approving signing for yourself is allowed. It's an administrator action, not an approval of your own request.)
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="My sign call is denied even though I have active access.">
    If the error mentions your access being scoped to specific signing parameters, the request was scoped and your sign call doesn't match one of the declared values. Open the request to see the scope, and compare it against what you're signing with now: a different machine, tool, command, IP address, or payload is enough to be denied even when signatures remain. Either sign in the situation that was approved, or open a new request describing the one you need.
  </Accordion>
...
<CardGroup cols={2}>
  <Card title="Signers" icon="pen-tool" href="/documentation/platform/pki/code-signing/signers">
