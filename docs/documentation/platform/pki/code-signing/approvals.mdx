...
<Warning>
  **The signing window runs from when the request is opened, not from when it is approved**, because the requester picks the start and end times and approval activates that window. Time spent waiting for an approver comes out of it, so set the window wide enough to cover how long approvals usually take.
</Warning>
