...
A scoped request is not fixed once it is open. While it is still pending review, an approver deciding it can stop enforcing any parameter on it, from the **Request Scope** panel on the request. The requester cannot: widening is a reviewer's decision, and the requester is often the signing client itself. The parameters left in place still have to match at signing time.
