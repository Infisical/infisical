...
```bash
# 1. Hash the artifact you intend to sign
DIGEST=$(sha256sum installer-v2.4.0.msi | cut -d' ' -f1)

# 2. Ask for one signature over exactly that artifact
curl -s -X POST "$INFISICAL_URL/api/v1/cert-manager/signers/$SIGNER_ID/requests" \
  ...
  -d "{
    ...
    \"requestedSignings\": 1,
    \"scope\": { \"dataHash\": \"$DIGEST\" }
  }"

# 3. Sign the file itself, so the digest Infisical computes is the one you declared
curl -s -X POST "$INFISICAL_URL/api/v1/cert-manager/signers/$SIGNER_ID/sign" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "{
    \"data\": \"$(base64 -w0 installer-v2.4.0.msi)\",
...
<Warning>
  **Do not use `sha256sum yourfile` when your tool pre-hashes.** Tools like `signtool` and `jarsigner` submit a digest rather than the file, so Infisical hashes that digest and the scope value becomes a digest of a digest. Declaring `sha256sum yourfile` in that case never matches and every sign call is denied. Let the [PKCS#11 module](/documentation/platform/pki/code-signing/pkcs11-module) or the Windows KSP fill the digest in for you instead of computing it by hand.
</Warning>
