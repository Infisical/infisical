...
If the Administrator doesn't want to wait, they can [pre-approve signing](#pre-approve-signing) directly. Same outcome, no review workflow.
...
There are two ways someone gets active access on a Signer. Pick the tab that matches who you are.
  ...
  <Tab title="Pre-approve signing">
    ...
    <Steps>
      <Step title="Click Pre-approve signing">
        From the Signer's **Approvals** tab, click **Pre-approve signing**. (Hidden if the Signer has no approval policy. Without a policy, the concept doesn't apply.)
      </Step>
...
<Steps>
  ...
  <Step title="Review">
    Verify who's asking, for how many signings, for how long, why, and that you're the intended approver.
  </Step>
  <Step title="Decide">
...
| **Approved / Active** | All steps approved, access issued, still within window and signatures remaining. |
...
The **Expires** column shows a live countdown for Active rows ("Expires in 42 minutes") plus the remaining signature count ("7 of 10 signings left"). Pending rows show "Awaiting approval".
