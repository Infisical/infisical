...
Every parameter on a request has to match at signing time, so a request that names the artifact digest covers exactly one artifact. To have one approval cover a series of builds, name the parameters that vary in `approval.exclude_scope_fields` and the module leaves them off every request it opens:
