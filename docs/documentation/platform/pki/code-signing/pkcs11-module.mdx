...
1. **Create a Machine Identity** and enable **[Universal Auth](/documentation/platform/identities/universal-auth)** on it, then copy its **Client ID** and **Client Secret**. The module uses these to authenticate, and you'll set them in [Configuration](#configuration) below.
...
| `approval.ip_address` | No | None | Pin the requests the module opens to this address instead of the one Infisical sees them arrive from, which is what it uses when this is unset. It doesn't have to be this machine's, so you can name a build agent's egress address. |
...
The request is [scoped](/documentation/platform/pki/code-signing/approvals#scoping-an-approval) to what the module saw: the command, the tool, the machine, the OS account, and the artifact. Approvers review the real signing situation, and the approval only covers that situation.

<Note>
  The first sign call **still fails**, because an approver has to act on the request. Once approved, run the same command again and it succeeds.
...
For fully unattended CI, have an Administrator **pre-approve** signing access via [Pre-approve signing](/documentation/platform/pki/code-signing/approvals#approval-paths) before the pipeline runs.

## Troubleshooting

If the module fails before it can log anything, it prints the reason to stderr prefixed `infisical-pkcs11:`. That covers a config file it can't read, since the log path lives in the file that just failed, and PKCS#11 gives the calling tool nothing but a generic `CKR_GENERAL_ERROR`.
...
<AccordionGroup>
  <Accordion title="No slots listed">
    ...
    1. The machine identity isn't a member of any Signer in your organization. Have an Admin add it to a Signer (any role works).
  ...
  <Accordion title="CKR_GENERAL_ERROR on sign">
    ...
    - **No active access.** The Signer has an approval policy and you don't yet have access. See [Request to sign](/documentation/platform/pki/code-signing/approvals#approval-paths).
...
<CardGroup cols={3}>
  ...
  <Card title="Sign APKs" icon="android" iconType="brands" href="/documentation/platform/pki/guides/code-signing/apksigner">
