...
Code signing adds a tamper-proof seal to your software, so Windows and your users can trust that it really came from you and hasn't been changed.

The Infisical **Windows KSP** lets Microsoft's standard signing tool, `signtool`, add that seal to your `.exe`, `.dll`, and `.msi` files, using a signing key that stays locked inside Infisical. It's a small plug-in for Windows' built-in cryptography system, called [Cryptography API: Next Generation (CNG)](https://learn.microsoft.com/en-us/windows/win32/seccng/cng-portal). "KSP" is short for **Key Storage Provider**, which is just Windows' name for a plug-in that supplies signing keys.
...
1. **Create a Machine Identity** and enable **[Universal Auth](/documentation/platform/identities/universal-auth)** on it, then copy its **Client ID** and **Client Secret**. The provider uses these to authenticate, and you'll set them in [Configuration](#configuration) below.
2. **Add the identity to the Signer** with the Administrator or Operator role, from the Signer's **Members** tab. Auditors can't sign.
...
<Info>
  The [Signer](/documentation/platform/pki/code-signing/signers) itself is created by a Product Admin. If you don't have one yet, create it first.
</Info>

## Requirements

- A 64-bit Windows computer (Windows 10/11 or Windows Server 2016 and later) where you're an **Administrator**. Setting up the plug-in changes a system-wide Windows setting, which needs admin rights.
...
Download the plug-in file from the [releases page](https://github.com/Infisical/infisical-ksp/releases) into the folder you're working in. Most people use the 64-bit `signtool`, so get `infisical-ksp.dll`:
...
<Note>
  **Using the 32-bit `signtool`?** It loads plug-ins from `SysWOW64` instead of `System32`, so also place the 32-bit file there under the same name. The registry entries above are shared by both, so you don't repeat them: `Copy-Item .\infisical-ksp-x86.dll "$env:windir\SysWOW64\infisical-ksp.dll" -Force`.
</Note>
...
<Note>
  You can also put these credentials inside the config file (`auth.client_id` and `auth.client_secret`), but environment variables are safer, because the secret won't get saved into your code or log files. Whatever runs `signtool` must have these variables set: for a build pipeline that is the build job, and for hands-on use it's the same PowerShell window.
</Note>
...
| `approval.ip_address` | No | None | Pin the requests the provider opens to this address instead of the one Infisical sees them arrive from, which is what it uses when this is unset. It doesn't have to be this machine's, so you can name a build agent's egress address. |
...
First, put the Signer's certificate on the computer. A certificate is the public half of your signing identity, so it's safe to share. In Infisical, open the Signer and use **Export certificate** to download it, or paste it in:
...
<Note>
  If your certificate comes from your own private CA (instead of one Windows already trusts), `signtool verify` shows the warning `A certificate chain could not be built to a trusted root authority` (`0x800B010A`). The signature is still valid; Windows just doesn't recognize who issued the certificate. To clear the warning, add your CA to the computer's Trusted Root store.
</Note>
...
<Note>
  ...
  The approval covers that exact command and artifact, so a different artifact, a changed argument, or reordered flags needs a new request. A request's parameters are fixed once it's open, so when they're wrong the request is rejected and reopened rather than edited. To have one approval cover a batch, leave the parameters that vary out of the request from the start.
</Note>
...
<AccordionGroup>
  <Accordion title="signtool fails immediately and the log you configured is empty">
    The config file couldn't be read, so the log path named inside it is unknown too. The reason is written to the default log at `%ProgramData%\Infisical\ksp.log` instead, and it names the setting at fault: an `exclude_scope_fields` entry that isn't a signing parameter, an `ip_address` that isn't an address, or invalid JSON. A UTF-8 byte order mark counts as invalid JSON, so write the file with `[IO.File]::WriteAllText` rather than `Set-Content -Encoding utf8`.
  </Accordion>
  <Accordion title="signtool says 'Invalid provider specified' right after setup">
    Windows hasn't picked up the new plug-in yet. **Restart the computer once** and try again.
  </Accordion>
  <Accordion title="Access denied (HTTP 403 in the log)">
    Your identity isn't allowed to sign yet. Either it needs approved access to sign, or it must be a member of the Signer with the Administrator or Operator role (Auditors can't sign). Fix the access (see [Approvals](/documentation/platform/pki/code-signing/approvals)), then run the same command again.
  </Accordion>
  ...
  <Accordion title="'signtool' isn't recognized">
    `signtool` isn't installed or isn't on your `PATH`. Install the Windows SDK, or open a Developer Command Prompt, so Windows can find `signtool`.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={3}>
  ...
  <Card title="Manage Signers" icon="pen-tool" href="/documentation/platform/pki/code-signing/signers">
    Create and configure Signers.
  </Card>
  <Card title="Approvals" icon="check-check" href="/documentation/platform/pki/code-signing/approvals">
