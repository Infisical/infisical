...
<Steps>
  ...
  <Step title="Certificate">
    ...
    | **Key source** | Where the leaf key for this Signer is generated. **Infisical-managed** (default) lets Infisical generate and manage the key for you. **HSM-backed** generates the key inside your own HSM through an [HSM Connector](/documentation/platform/pki/settings/hsm-connectors). |
    ...
    | **Key algorithm** | The key for the signing certificate: `RSA-2048`, `RSA-3072`, `RSA-4096`, `ECDSA P-256`, `ECDSA P-384`, or `ECDSA P-521`. Shown only when **Key source = Infisical-managed**. |
    | **HSM Connector** | The Connector that decides which Gateway routes operations to your HSM. Shown only when **Key source = HSM-backed**. See [HSM Connectors](/documentation/platform/pki/settings/hsm-connectors). |
    | **HSM key algorithm** | Algorithm Infisical asks the HSM to generate: `RSA_2048`, `RSA_4096`, `ECC_P256`, or `ECC_P384`. Shown only when **Key source = HSM-backed**. |
...
When you pick **Key source = HSM-backed** on the Certificate step, two extra fields appear:

| Field | Description |
|-------|-------------|
| **HSM Connector** | The [HSM Connector](/documentation/platform/pki/settings/hsm-connectors) Infisical uses to reach your HSM. Set the Connector up first under **Cert Manager Settings > HSM Connectors**. |
| **HSM key algorithm** | Algorithm the HSM generates for this Signer: `RSA_2048`, `RSA_4096`, `ECC_P256`, or `ECC_P384`. |

The private key never leaves the HSM. Every sign operation routes through the linked Connector. Infisical only ever holds the public key and the issued certificate. Key source is fixed at create time. To switch a Signer between Infisical-managed and HSM-backed, create a new Signer with the desired key source.

See [HSM Connectors](/documentation/platform/pki/settings/hsm-connectors) for the Gateway + Connector setup an HSM-backed Signer depends on.
...
<AccordionGroup>
  ...
  <Accordion title="Can I migrate an existing Infisical-managed Signer to an HSM?">
    Not in place. Key source is fixed at create time so the Signer's certificate and its key never go out of sync. Create a new HSM-backed Signer alongside the old one, route new sign traffic to it, and disable or delete the old Signer once you're satisfied.
  </Accordion>
</AccordionGroup>
