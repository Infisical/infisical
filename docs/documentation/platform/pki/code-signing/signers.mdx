...
<CardGroup cols={2}>
  ...
  <Card title="Approval policy" icon="check-check">
...
| **Operator** | Sign artifacts and submit signing requests. Export the certificate. Can't manage members or the policy. |
| **Auditor** | View members, activity, and the audit log. Export the certificate. Can't sign or change anything. |
...
In **Certificate Manager → Code Signing → Signers**, select **Create Signer**. The wizard walks you through five steps in order: Basics, Certificate, Signing Key, Members, and Approval Policy.
...
<Steps>
  ...
  <Step title="Certificate">
    ...
    | Field | Description |
    |-------|-------------|
    | **Certificate Authority** | The CA that issues the certificate. Internal CAs issue immediately. External CAs (AWS Private CA, Azure AD CS, ADCS, DigiCert) issue asynchronously and the Signer enters **Pending** until the cert lands. |
    | **Certificate Template** | ADCS CAs only, required. The AD CS template this Signer requests. Pick a code-signing template published on your CA. |
...
| Status | Meaning |
|--------|---------|
| **Pending** | The Signer is created but the certificate hasn't been issued yet. Most common with external CAs that take time to respond. The PKCS#11 module sees this slot but signing is rejected until the certificate lands. |
...
<AccordionGroup>
  ...
  <Accordion title="Can I migrate an existing Signer between Infisical and an HSM?">
    ...
    The same operation is available via the [reissue endpoint](/api-reference/endpoints/code-signing/signers/reissue-certificate) by passing `certificate.keySource` and (when switching to HSM) `hsmConnectorId`.
...
<CardGroup cols={2}>
  ...
  <Card title="HSM Connectors" icon="shield-half" href="/documentation/platform/pki/settings/hsm-connectors">
  ...
  <Card title="Windows KSP" icon="windows" iconType="brands" href="/documentation/platform/pki/code-signing/windows-ksp">
