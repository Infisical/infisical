...
<Steps>
  ...
  <Step title="Certificate">
    ...
    | **Key algorithm** | The signing key algorithm. With **Key source = Infisical**: `RSA-2048`, `RSA-3072`, `RSA-4096`, `ECDSA P-256`, `ECDSA P-384`, or `ECDSA P-521`. With **Key source = HSM**: `RSA_2048`, `RSA_4096`, `ECC_P256`, or `ECC_P384`. |
...
When you pick **Key source = HSM** on the Certificate step, pick the [HSM Connector](/documentation/platform/pki/settings/hsm-connectors) Infisical should use and the HSM key algorithm. The private key never leaves the HSM and every sign operation routes through the linked Connector. Infisical only holds the public key and the issued certificate.

You can later switch an existing Signer between Infisical and HSM by reissuing the certificate with a different key source. Hit the [reissue endpoint](/api-reference/endpoints/code-signing/signers/reissue) with a `certificate.keySource` body, plus `hsmConnectorId` and `hsmKeyAlgorithm` when switching to HSM. The signer keeps its name, members, policy, and CA. Only the underlying key and the cert wrapping it change.

See [HSM Connectors](/documentation/platform/pki/settings/hsm-connectors) for the setup an HSM-backed Signer depends on.
...
<AccordionGroup>
  ...
  <Accordion title="Can I migrate an existing Signer between Infisical and an HSM?">
    Yes. Reissue the Signer's certificate with a different `certificate.keySource` via the [reissue endpoint](/api-reference/endpoints/code-signing/signers/reissue). When switching to HSM you also pass `hsmConnectorId` and `hsmKeyAlgorithm`. The signer keeps its name, members, approval policy, and CA. Only the key and the cert wrapping it change.

    The previous certificate stays in the inventory and still references its original HSM key (the HSM key itself is not deleted, only the Infisical-side reference). Delete or retire the old certificate before you try to delete the HSM Connector it referenced.
  </Accordion>
</AccordionGroup>
