...
- Templates that require manual approval or are held by CA policy aren't supported. Requests are issued immediately.
- Authentication uses NTLM. Kerberos isn't yet supported.

## Troubleshooting

<AccordionGroup>
  <Accordion title="The requested certificate template isn't supported">
  ...
  <Accordion title="Enroll permission denied">
    The connection account doesn't have Enroll permission on the requested template. Grant Enroll to the account on the template's security settings.
  </Accordion>
  <Accordion title="The certificate authority name could not be discovered">
    Automatic discovery reads the CA name from the host's registry over SMB (TCP 445), which requires the Remote Registry service to be running and reachable from the Gateway. If SMB isn't open to the Gateway, leave discovery aside and set the certificate authority name explicitly on the external CA. Run `certutil -getreg CA\CommonName` on the AD CS server to find it.
  </Accordion>
  <Accordion title="Could not reach certificate authority through the gateway">
    When you create or update the CA, Infisical validates the certificate authority name against the CA over the Gateway. This error means the name doesn't match a CA on the host, or the Gateway can't reach the AD CS server. Confirm the name with `certutil -getreg CA\CommonName` on the AD CS server and that the Gateway can connect to the host over DCOM (MS-WCCE).
  </Accordion>
  <Accordion title="The certificate was issued without a custom extension">
    AD CS copies a custom extension out of a request only when its OID is listed in the CA's `EnableRequestExtensionList`, which lists none by default. Add the OID and restart the service:
    ...
    Leaving an OID off that list is also how you stop requests from introducing it.
  </Accordion>
  <Accordion title="Signer rejected: missing Code Signing extended key usage">
    The template used for a code signing signer doesn't produce a code-signing certificate. Point the signer at a template that carries the Code Signing extended key usage.
  </Accordion>
