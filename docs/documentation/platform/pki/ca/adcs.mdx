      ...
      <Tab title="API">
        <Steps>
          ...
          <Step title="Install the Certificate">
            ```bash Request
            curl --location --request POST 'https://app.infisical.com/api/v1/cert-manager/ca/internal/<ca-id>/install-certificate-native-adcs' \
              --header 'Authorization: Bearer <access-token>' \
              --header 'Content-Type: application/json' \
              --data-raw '{
                  "maxPathLength": -1
...
- Revoking a certificate requires the connection account to have Manage CA (Certificate Manager) rights on the CA, which is a stronger permission than the Enroll right used for issuance.
