...
<Note>
  An Internal CA is managed by Infisical, which holds its signing key and uses it to issue
  certificates on request. If instead you want to hand a team a CA certificate of their own to run
  their own chain with their own key, that is a certificate, not a CA — see
  [issuing CA certificates from AWS Private CA](/documentation/platform/pki/ca/aws-pca#issuing-ca-certificates).
</Note>
...
<Steps>
  ...
  <Step title="Creating an intermediate CA">
    To create an intermediate CA, make an API request to the [Create CA](/api-reference/endpoints/certificate-authorities/create) API endpoint, specifying the `type` as `intermediate`.
...
By default, Infisical generates and manages the signing key for an Internal CA. You can instead keep the CA's signing key inside your own Hardware Security Module. When **Key Source** is set to **HSM**, Infisical generates the keypair on the HSM through an [HSM Connector](/documentation/platform/pki/settings/hsm-connectors) and performs every signing operation the CA does on the HSM. That covers the self-signed root certificate, intermediate CSRs, the certificates the CA issues, and its CRLs.
...
<Note>
  Set up an [HSM Connector](/documentation/platform/pki/settings/hsm-connectors) before creating an HSM-backed CA. Creating one requires the **Attach** permission on HSM Connectors in addition to permission to create Certificate Authorities.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Does Infisical support chaining an Intermediate CA to an external CA?">
    Yes. You can either manually obtain a CSR and import the signed certificate, or use an integrated external CA provider like [Venafi TLS Protect Cloud](/documentation/platform/pki/ca/venafi) or [Microsoft ADCS](/documentation/platform/pki/ca/adcs) to automate the signing process.
  </Accordion>
  <Accordion title="Can the CA signing key be kept in my own HSM?">
    Yes. Set **Key Source** to **HSM** when creating the CA and pick an [HSM Connector](/documentation/platform/pki/settings/hsm-connectors). The keypair is generated on your HSM and every signing operation the CA performs runs there. Both root and intermediate CAs support this. Supported algorithms are `RSA 2048`, `RSA 4096`, `ECDSA P-256`, and `ECDSA P-384`. See [HSM-backed Internal CAs](#hsm-backed-internal-cas).
  </Accordion>
</AccordionGroup>
