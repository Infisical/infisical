...
Certificates from AWS Private CA can be issued with `CA:TRUE`, the same as with an [Internal CA](/documentation/platform/pki/ca/private-ca). Set **Basic Constraints** to allow CA certificates on the [Certificate Policy](/documentation/platform/pki/settings/policies), then tick **Issue as Certificate Authority** when requesting one.

AWS Private CA applies its own rules to CA certificates:

- **Path length is required**, and must be between `0` and `3`. There is no unlimited option.
- **Key usages are set by AWS** to digital signature, certificate signing and CRL signing, overriding the profile.
- The CA must be general-purpose, and its own path length must leave room beneath it.
...
- Check the path length is between `0` and `3`, the policy's **Basic Constraints** allow CA certificates, and the CA is general-purpose. See [Issuing CA Certificates](#issuing-ca-certificates).
