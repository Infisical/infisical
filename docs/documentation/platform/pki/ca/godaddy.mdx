...
- An available **DV SSL** product in your GoDaddy account. The API consumes a product you already own; it does not purchase one (see the FAQ below).
- A certificate policy that allows an **RSA** key with **DNS-name** SANs (see below). The built-in **TLS Server** preset works as long as you select RSA, since GoDaddy rejects ECDSA.
