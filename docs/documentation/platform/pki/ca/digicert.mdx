...
DigiCert CertCentral supports the ACME protocol and requires **External Account Binding (EAB)** for all ACME registrations. You'll need to obtain the ACME Directory URL, a Key Identifier (KID), and an HMAC Key from your DigiCert CertCentral account before registering the ACME CA in Infisical.

## Guide to connecting Infisical to DigiCert

<Steps>
  <Step title="Retrieve ACME credentials from DigiCert CertCentral">
    Log in to your DigiCert CertCentral account and navigate to **Automation > ACME Directory URLs** in the left sidebar. Select **Add ACME Directory URL** at the top of the page.
...
<CardGroup cols={2}>
  ...
  <Card title="Import existing certificates" icon="file-input" href="/documentation/platform/pki/applications/certificates#importing-certificates">
    Bring certificates you already hold in CertCentral under Infisical management.
  </Card>
</CardGroup>
