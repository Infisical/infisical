...
When Infisical requests a certificate from an ACME-compatible CA, it creates a TXT record at `_acme-challenge.{your-domain}` in your configured DNS provider (e.g. Route53, Cloudflare, Azure DNS, DNS Made Easy, PowerDNS, etc.); this TXT record contains the challenge token issued by the ACME-compatible CA to validate domain control for the requested certificate.
...
<Steps>
    <Step title="Create an App Connection to your DNS provider">
        ...
        <Tabs>
            ...
            <Tab title="PowerDNS">
                1. In **Certificate Manager**, go to **Settings → App Connections** and create a new PowerDNS connection.

                2. Enable the HTTP API on your PowerDNS Authoritative Server and set an `api-key`. If the server is not reachable from the internet, attach a Gateway to the connection.

                For detailed instructions on setting up a PowerDNS connection, see the [PowerDNS Connection](/integrations/app-connections/powerdns) documentation.
            </Tab>
        </Tabs>
    </Step>
    <Step title="Register an ACME-compatible CA">
        <Tabs>
          <Tab title="Infisical UI">
              ...
              - Zones / Hosted Zone IDs: Select one or more zones for the domains you'll be requesting certificates for. When multiple zones are configured, Infisical automatically uses the zone matching each certificate domain during DNS validation.
            ...
            curl 'https://app.infisical.com/api/v1/cert-manager/ca/acme' \
              ...
              --data-raw '{
                ...
                "configuration": {
                  ...
                  "dnsProviderConfig": {
                    "provider": "route53",
                    "hostedZoneIds": ["Z040441124N1GOOMCQYX1"]
                  },
...
<AccordionGroup>
  <Accordion title="What DNS validation methods are supported?">
    Currently, Infisical supports DNS-01 validation through AWS Route53, Cloudflare, Azure DNS, DNS Made Easy, and PowerDNS. The DNS-01 challenge method is preferred for ACME integrations because it:
