...
If you have a CNAME record pointing `_acme-challenge.example.com` to another domain (e.g., `_acme-challenge.ssl.example.com`), Infisical will automatically follow the CNAME chain and create the TXT record at the final target. The ACME CA also follows CNAMEs during validation, so it finds the TXT record at the delegated location.
...
2. **Register the ACME CA** in Infisical with the **hosted zone ID** of the zone where the CNAME target lives.
...
<AccordionGroup>
  ...
  <Accordion title="Does Infisical support DNS CNAME delegation for ACME challenges?">
    Yes. Infisical automatically follows CNAME records when creating DNS-01 challenge TXT records. This allows you to delegate
    `_acme-challenge` to a different DNS zone or provider. See the [DNS CNAME Delegation](#dns-cname-delegation) section above for setup instructions.
  </Accordion>
</AccordionGroup>
