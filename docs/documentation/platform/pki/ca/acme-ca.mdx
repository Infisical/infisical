...
When Infisical requests a certificate from an ACME-compatible CA, it creates a TXT record at `_acme-challenge.{your-domain}` in your configured DNS provider (e.g. Route53, Cloudflare, Azure DNS, DNS Made Easy, UltraDNS, etc.); this TXT record contains the challenge token issued by the ACME-compatible CA to validate domain control for the requested certificate.
...
<Steps>
    <Step title="Create an App Connection to your DNS provider">
        ...
        <Tabs>
            ...
            <Tab title="UltraDNS">
                1. In **Certificate Manager**, go to **Settings → App Connections** and create a new UltraDNS connection.

                2. Ensure the UltraDNS user can read zones and manage resource records in the zone you want to use for validation. Only primary zones can be used, since secondary zones cannot accept record changes.

                <Note>
                  For UltraDNS the zone is identified by its fully qualified name, such as `example.com.`, rather than
                  by an ID. The Infisical UI offers this as a picker, so this only matters if you are creating the CA
                  through the API or Terraform.
                </Note>

                For detailed instructions on setting up an UltraDNS connection, see the [UltraDNS Connection](/integrations/app-connections/ultradns) documentation.
            </Tab>
        </Tabs>
    </Step>
...
<AccordionGroup>
  <Accordion title="What DNS validation methods are supported?">
    Currently, Infisical supports DNS-01 validation through AWS Route53, Cloudflare, Azure DNS, DNS Made Easy, and UltraDNS. The DNS-01 challenge method is preferred for ACME integrations because it:
