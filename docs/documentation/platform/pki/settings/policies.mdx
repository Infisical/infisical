...
A DER value is matched on the text inside it, or on its base64 when it holds no text. Pin one to exact bytes by listing that base64, such as `MAMCAQU=` for OCSP must-staple, or accept any with `*`.
