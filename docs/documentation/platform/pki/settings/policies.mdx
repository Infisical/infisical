...
<Note>
  ...
  - Enter the full sequence in distinguished name order, separated by commas. `corp,example,com`
    matches `CN=host,DC=corp,DC=example,DC=com`.
  - A request matches a sequence only when its components line up position by position, so the same
    labels in another order are rejected. Requests are read in distinguished name order whichever way
    the client encoded the name, so a CSR from Windows, Active Directory or Intune matches the same
    rule as one from OpenSSL.
