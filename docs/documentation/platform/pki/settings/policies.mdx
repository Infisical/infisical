...
<Note>
  [AWS Private CA](/documentation/platform/pki/ca/aws-pca#issuing-ca-certificates) requires a path length between `0` and `3` on every CA certificate, even when this is left unlimited.
</Note>
