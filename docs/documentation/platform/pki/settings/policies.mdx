...
Control which custom X.509 extensions certificates may carry, with one rule per object identifier (OID):

| Rule | Meaning |
|------|---------|
| Allow | The extension is permitted, and its value must match the pattern |
| Require | The extension must be present, and its value must match the pattern |
| Deny | The extension is permitted, but not with a value matching the pattern |

Each rule takes one value pattern, matched against the value you enter rather than its encoded form. Use `*` for any value, or a wildcard such as `S-1-5-21-1004336348-1177238915-682003330-*` to accept only identifiers from your own Active Directory domain. A rule can also pin criticality, except where an extension's specification already fixes it.

If you configure Custom Extensions, only the OIDs you list are permitted, each subject to its rule. Any other OID in a request is rejected, including one carried in a signing request, so a value pattern is what limits what an enrolling client can assert. An empty configuration forbids all custom extensions, and leaving the section unconfigured allows any OID with any value.
