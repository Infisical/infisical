...
| Smart Card Logon | Active Directory logon, Windows Hello and other PKINIT authentication |
...
Control which custom X.509 extensions certificates may carry. Each rule is one value pattern for one object identifier (OID), classified the same way subject alternative name rules are:

| Rule | Meaning |
|------|---------|
| Allowed | The value may match this pattern |
| Required | The extension must be present, and its value must match one of the required patterns |
| Denied | The value must not match this pattern, even when an allowed pattern also matches it |

Add several rules for the same OID to accept more than one value. An Active Directory estate with two domains, for example, takes two Allowed rules on the security identifier extension, one wildcard per domain.

Patterns are matched against the value you enter rather than its encoded form, and matching is case sensitive. Use `*` for any value, or a wildcard such as `S-1-5-21-1004336348-1177238915-682003330-*` to accept only identifiers from one domain. A rule can also pin criticality, except where an extension's specification already fixes it.

If you configure Custom Extensions, only the OIDs you list are permitted. Any other OID in a request is rejected, including one carried in a signing request, so the patterns are what limit what an enrolling client can assert. An empty configuration forbids all custom extensions, and leaving the section unconfigured allows any OID with any value.

Patterns are text, so a DER value, such as OCSP must-staple, matches nothing but `*`. Allow those OIDs with a `*` rule. A rule that denies values refuses a DER value outright, because its patterns can't be checked against one.

Infisical manages some extensions itself, so you can't set any OID under `2.5.29.` or authority information access here.

Custom extensions need a certificate authority that can emit them: an internal CA, Microsoft ADCS, or AWS Private CA. A [certificate profile](/documentation/platform/pki/settings/profiles) built on any other certificate authority can't declare them. AWS Private CA also caps a certificate at three custom extensions.
