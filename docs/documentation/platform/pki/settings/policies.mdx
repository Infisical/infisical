...
<Note>
  If the profile's issuing CA is [AWS Private CA](/documentation/platform/pki/ca/aws-pca#issuing-ca-certificates),
  a CA certificate request must carry a path length between `0` and `3` even when this policy leaves
  the maximum unlimited. AWS Private CA has no template for an unconstrained path length.
</Note>
