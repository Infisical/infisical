...
Two pieces of configuration are involved, and they're separate:
...
Your API key is a credential, so it goes only in Infisical as the PIN. It doesn't belong in `pkcs11.conf`.

## Set up the connection

<Steps>
  <Step title="Get your Fortanix application API key">
    In the Fortanix DSM dashboard, go to **Apps**, open the application you use for PKCS#11 access, and find the **API Key** section. Select **View API Key Details** to copy the key, or **Regenerate** to create a new one.
  ...
  <Step title="Create the HSM Connector in Infisical">
    In **Certificate Manager > Settings > HSM Connectors**, select **Add HSM Connector** and fill in:
