...
<Steps>
  ...
  <Step title="Install the driver and point it at your DSM">
    Install the Fortanix PKCS#11 library on the machine that will run the Gateway. Fortanix publishes it on its [PKCS#11 library page](https://support.fortanix.com/docs/fortanix-dsm-clients-pkcs11-library) as a `.deb` package, an `.rpm` package, or a standalone `.so` for Linux, and as an MSI installer for Windows. A `.deb` apt repository is also available (`deb https://download.fortanix.com/linux/apt focal main`). On Linux the driver installs to `/opt/fortanix/pkcs11/fortanix_pkcs11.so`. On Windows the default is `C:\Program Files\Fortanix\KmsClient\FortanixKmsPkcs11.dll`.
    ...
    Then list the driver's slots to find the **slot label** you will need for the Connector:

    ```bash
    pkcs11-tool --module /opt/fortanix/pkcs11/fortanix_pkcs11.so --list-token-slots
    ```

    Copy the `token label` value from the output.
  </Step>
  ...
  <Step title="Create the HSM Connector in Infisical">
    ...
    | **Slot label** | The `token label` you copied in Step 2 (`Fortanix Token` on Fortanix DSM). |
