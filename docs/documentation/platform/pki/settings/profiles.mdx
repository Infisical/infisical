...
![Certificate profile list](/images/platform/pki/certificate/cert-profile.png)

![Certificate profile modal](/images/platform/pki/certificate/cert-profile-modal.png)
...
| **Issuing CA** | The CA that signs certificates (if Issuer Type is CA) |
...
The **Certificate Details** tab lets you pre-configure sensible defaults for certificate fields:

![Certificate profile defaults](/images/platform/pki/certificate/cert-profile-modal-defaults.png)

When a requester doesn't specify a value, the profile default is applied automatically. This simplifies certificate requests — teams can issue certificates without making decisions about every field.
...
Teams don't interact with profiles directly. Instead:

<Steps>
  <Step title="Product admin creates a profile">
    Combines a CA + policy + defaults into a reusable template.
  </Step>
  <Step title="Product admin creates an Application">
    Creates an Application, attaches the profile, and assigns team members.
  </Step>
  <Step title="Team configures enrollment">
    Assigned team members set up API, ACME, EST, or SCEP enrollment on their Application.
  </Step>
  <Step title="Certificates are issued">
    Requests are validated against the profile's policy and signed by the profile's CA.
  </Step>
</Steps>

This separation means one profile can serve many Applications. When you need to update certificate rules, update the profile or policy — all Applications using it get the change automatically.
