...
- The **absolute path to the PKCS#11 driver** (`.so` on Linux, `.dll` on Windows) provided by your HSM vendor.
...
1. **A Gateway** running on a host that can talk to your HSM, started with `--pkcs11-module=<path to vendor .so/.dll>`. See [Gateway Deployment](/documentation/platform/gateways/gateway-deployment).
...
<Steps>
  <Step title="Install your HSM's PKCS#11 driver on the Gateway host">
    Install the PKCS#11 library shipped by your HSM vendor on the machine where the Gateway will run. Note the absolute path to the `.so` or `.dll`. You will pass it to the Gateway in the next step.
