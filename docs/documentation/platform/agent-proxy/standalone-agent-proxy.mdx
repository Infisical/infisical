...
- The Agent Proxy grants nothing on its own. For each agent it asks Infisical which proxied services that agent's identity holds the **Proxy** permission on, within the agent's exact project, environment, and folder scope. Credentials are applied only for those services; everything else doesn't exist as far as that agent is concerned.
- The agent's identity can't read secret values. Only the Agent Proxy's own identity can, and it lives on a separate machine. Even if an agent is tricked into leaking its token, that token grants no ability to fetch a secret; it can only route traffic through the same services the agent could already reach.
...
<Note>
  The proxy address is the one setting that can't come from `.infisical.json`. That file is usually committed to your repository, so a wrong or malicious value would silently route every agent's traffic through the wrong host. Pass it with `--proxy` or `INFISICAL_AGENT_PROXY_ADDRESS` instead.
</Note>
...
Starting the proxy in a shell is fine while you're trying it out, but it dies with the terminal. Pick one of these instead. Whichever you choose, provide the identity's credentials, publish port `17322` to your agent machines only, and set `INFISICAL_DOMAIN` for EU Cloud or self-hosted.
...
2 vCPU and 4 GB of RAM is a reasonable starting point for a few dozen agents. The proxy keeps its state in memory, so RAM is rarely the driver. CPU tracks the volume of traffic you route through it rather than the number of agents, and a single instance accepts up to 512 concurrent connections. The first limit most deployments reach is the API load from each agent's polling, which a longer poll interval reduces (see [Caching and polling](#caching-and-polling)). As guidance rather than a benchmarked limit, expect one instance to serve roughly 30 to 50 agents; to grow past that, add instances behind a load balancer instead of moving to a larger host (see [High availability](#high-availability)).
...
- **Credentials where they're used.** Provision the Agent Proxy identity's client credentials only on its host, and each agent identity's only on its agent machine.
...
- Only HTTP and HTTPS are brokered, so SSH can't be: use HTTPS with a brokered token instead. A tool that ignores the proxy variables isn't brokered either; nothing forces its traffic through the proxy.
...
| --- | --- | --- |
| **Agent** | `Proxy` on **Proxied Services**, scoped to the environments and paths where its services live | This alone lets it route traffic and have credentials applied. It doesn't need to read any secret. |
...
<AccordionGroup>
  <Accordion title="A 401 from the upstream API">
    The credential wasn't applied and the request went out as the agent sent it; the proxy doesn't block the call. Check the agent identity holds `Proxy` on the service, and that the proxy identity has `Read Value` on the secret the service references. The [activity log](/documentation/platform/agent-proxy/activity-logs) shows which decision each request got.
  </Accordion>
