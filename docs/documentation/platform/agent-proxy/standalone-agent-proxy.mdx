...
The Agent Proxy is light on hardware: 2 vCPU and 4 GB of RAM is a reasonable starting point for a few dozen agents. It keeps all of its state in memory and writes nothing to disk in standalone mode, and its internal caches are bounded, so memory use stays modest as agents connect.

- **CPU** scales with the volume of traffic you route through it, not with the number of agents, because every request is decrypted and re-encrypted. Size CPU against expected throughput.
- **Connections**: a single instance accepts up to 512 concurrent connections. Beyond that, new connections wait.
- **API load** is usually the first limit a deployment reaches, ahead of hardware. Polling grows with the number of active agents, and a longer `--poll-interval` reduces it. See [Caching and polling](#caching-and-polling).

As a rough guide, one instance comfortably serves roughly 30 to 50 agents on default settings, and more with a longer poll interval. Treat these figures as guidance rather than benchmarked limits. To grow past one instance, add instances behind a load balancer rather than moving to a larger host: see [High availability](#high-availability). If you set `--log-file`, the proxy appends a line per request and does not rotate the file, so rotate it externally.
