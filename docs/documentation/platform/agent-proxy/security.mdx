...
The two setups rest on different boundaries, so each has its own section. A [standalone proxy](#standalone-proxy) separates the agent from the credentials by host, with separate machine identities either side. A [local proxy](#local-proxy) shares one machine, so an OS sandbox does that job instead, and none of the identity setup applies.
...
- Authorization is re-checked, not granted once. The Agent Proxy re-validates each active agent's permissions on every poll and fails closed. See [Caching and polling](/documentation/platform/agent-proxy/standalone-agent-proxy#caching-and-polling) for the interval and exactly what happens when a grant is revoked.
