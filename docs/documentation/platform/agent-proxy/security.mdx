...
- **The gate is your own access.** `run` runs as you: `Read` on proxied services to find them, `Read Value` on the secrets to broker them, and `Lease` to mint a dynamic one. There is no `Proxy` check and no agent identity to grant one to.
