...
**Dedicated proxy.** You run the proxy yourself with `agent-proxy start`, on a host of its own, then launch each agent with `agent-proxy connect` so its traffic routes through it. A different machine from your credentials.
  ...
  <Tab title="Dedicated proxy">
    ...
    <Steps>
      ...
      <Step title="Start the proxy on its own host">
        ...
        <Note>
          Fine for trying it out, but this dies with your terminal. [Running it durably](/documentation/platform/agent-proxy/dedicated-agent-proxy#running-it-durably) covers systemd, Docker, and PaaS.
        </Note>
      </Step>
    ...
    Next: [Dedicated Agent Proxy](/documentation/platform/agent-proxy/dedicated-agent-proxy) is what to settle before real agents depend on this: process supervision, network placement, credential refresh, and high availability.
