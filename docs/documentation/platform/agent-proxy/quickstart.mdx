...
<Steps>
  ...
  <Step title="Create two machine identities">
    ...
    <Note>
      The built-in roles grant broad access across all environments and paths for a quick start. For production, consider [custom roles](/documentation/platform/access-controls/role-based-access-controls) scoped to specific environments and paths.
    </Note>
  </Step>
