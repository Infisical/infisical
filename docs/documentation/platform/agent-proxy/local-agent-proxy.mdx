...
For agents running anywhere else it is the other way round: a [dedicated proxy](/documentation/platform/agent-proxy/dedicated-agent-proxy) buys a stronger boundary, but there is real infrastructure to stand up first.
...
<Warning>
  **`run` cannot be pointed at a proxy running elsewhere.** It always starts its own, so `--proxy` is rejected with an error rather than silently ignored. To route an agent through a proxy on another host, see [Dedicated Agent Proxy](/documentation/platform/agent-proxy/dedicated-agent-proxy).
</Warning>
...
```text
  ...
  |-- agent-proxy.log   activity log, unless you set --log-file
...
`run` involves no agent identity and no `Proxy` permission check at all. Every API call is made with your own token, so **the gate is `Read Value`**: if you can read the secret, `run` can broker it.
...
The agent owns your terminal, so per-request activity does not go there. It is always written to a file: by default one inside the per-run temporary directory, which is removed on exit and whose path is not printed. Pass `--log-file` to put it somewhere you can read:
...
Every request records a decision: `brokered` (a service matched and a credential was applied), `passthrough` (no service matched, forwarded untouched), `blocked` (rejected by `--unmatched-host=block`), or `error`. Records carry the same fields as in [Activity Logs](/documentation/platform/agent-proxy/activity-logs), written in the human-readable console format. Startup messages print to your terminal: the sandbox status, the keychain prompt, and any path you re-opened. Anything the proxy reports while running goes to the log file instead, including a credential it had to skip and a dropped authorization, so pass `--log-file` when you are diagnosing why something was not brokered.

`passthrough` is the one decision left out by default, and it is usually the bulk of an agent's traffic, so a session's log stays small. `--log-level debug` adds it, which is useful when you are working out why a host is not being brokered and noisy the rest of the time.

<Note>
  A path you pass to `--log-file` is appended to on every run and never rotated for you, so rotate it with `logrotate` (`copytruncate`) if you keep one around. The default file needs no attention: it lives in the per-run temporary directory and goes away with the run.
</Note>
