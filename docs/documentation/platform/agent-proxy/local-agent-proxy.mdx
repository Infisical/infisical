...
`run` involves no agent identity and no `Proxy` permission check at all, and requests carry no proxy-authentication header. Every API call is made with your own token: **`Read` on proxied services** to find them, **`Read Value` on the secrets** they reference to broker them, and **`Lease`** to mint a dynamic one. A default project member already has all three.
