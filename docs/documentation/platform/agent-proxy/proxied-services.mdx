...
<Note>
  Host patterns are for routing only. Proxied services are also scoped to their folder: an agent connected with `--env=dev --path=/billing-agent` only ever matches services defined in that folder, so the same hostname can be configured differently in different folders, environments, or projects.
</Note>
