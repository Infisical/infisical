...
<ParamField path="Enabled" type="boolean" optional default="true">
  Toggle a service off without deleting it. A disabled service is skipped entirely: its placeholder environment variables are not set, no credentials are applied, and traffic to its hosts falls back to the [unmatched host policy](/documentation/platform/agent-proxy/standalone-agent-proxy#unmatched-hosts).
</ParamField>
