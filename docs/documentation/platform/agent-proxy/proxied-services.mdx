...
<Note>
  Not every provider fits HTTP brokering. AWS IAM leases return access keys that require SigV4 request signing (not just a header), which the proxy does not perform, and SSH leases return key material rather than an HTTP credential — these are not offered in the picker for injection.
</Note>
