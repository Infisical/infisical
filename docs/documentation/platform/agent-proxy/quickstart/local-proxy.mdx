...
<Info>
  For more depth, the [Local Agent Proxy](/documentation/platform/agent-proxy/local-agent-proxy) reference covers what the sandbox allows and denies, how certificates work, and every flag `run` takes. To serve agents that run unattended in your own infrastructure, follow the [Standalone Proxy](/documentation/platform/agent-proxy/quickstart/standalone-proxy) quickstart instead.
</Info>
