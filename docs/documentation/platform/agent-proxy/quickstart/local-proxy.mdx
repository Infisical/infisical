...
A local proxy is one command. `infisical secrets agent-proxy run` starts a proxy next to your agent on your own computer, hands the agent placeholder credentials, swaps in the real ones as its requests leave, and shuts the proxy down when the agent exits. Nothing to deploy, and no machine identities to create: the proxy brokers as your logged-in user.
...
<CardGroup cols={2}>
  <Card title="Local Proxy reference" icon="laptop-code" href="/documentation/platform/agent-proxy/local-agent-proxy">
    What the sandbox allows and denies, how certificates work, and every flag `run` takes.
  </Card>
  <Card title="Agents in your own infrastructure" icon="server" href="/documentation/platform/agent-proxy/quickstart/standalone-proxy">
    Deploy a proxy on a host of its own and point long-running agents at it.
  </Card>
</CardGroup>
