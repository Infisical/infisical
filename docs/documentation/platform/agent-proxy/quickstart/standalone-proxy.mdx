...
<Info>
  For more depth, the [Standalone Agent Proxy](/documentation/platform/agent-proxy/standalone-agent-proxy) reference covers deployment, network placement, high availability, and how each agent is authorized on every request. To broker credentials to an agent on your own computer, follow the [Local Proxy](/documentation/platform/agent-proxy/quickstart/local-proxy) quickstart instead.
</Info>
