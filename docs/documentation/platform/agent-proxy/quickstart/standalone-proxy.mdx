...
A standalone proxy is a long-lived process on a machine of its own. `infisical secrets agent-proxy start` runs it, and `infisical secrets agent-proxy connect` launches each agent so its traffic routes through it. This is the setup for agents that run unattended, and for one proxy shared across a team.
...
<CardGroup cols={2}>
  <Card title="Standalone Proxy reference" icon="server" href="/documentation/platform/agent-proxy/standalone-agent-proxy">
    Deployment, network placement, high availability, and how agents are authorized on every request.
  </Card>
  <Card title="An agent in your terminal" icon="laptop-code" href="/documentation/platform/agent-proxy/quickstart/local-proxy">
    Broker credentials to Claude Code or Codex on your own computer, in one command.
  </Card>
</CardGroup>
