...
Then, in your Infisical project, create a folder for the agent (for example `/coding-agent` in the `dev` environment) and add a secret named `GITHUB_PAT` with that token as its value. It's a regular Infisical secret.
...
<Steps>
  ...
  <Step title="Review and create">
    Double-check the host pattern and the substitution rule, then select **Create Proxied Service**.
...
<CardGroup cols={2}>
  <Card title="Local Proxy" icon="laptop-code" href="/documentation/platform/agent-proxy/quickstart/local-proxy">
    For agents that live as long as your terminal session: Claude Code, Codex, or a script you're iterating on.
  </Card>
