...
| `agentId` / `agentName` | Who made the request: the agent's machine identity behind a dedicated proxy, or your own account (or `token`) on a local proxy |
...
Records always go to a file, never your terminal, since the agent owns it. By default that file lives in the per-run temporary directory and goes away with the run; `--log-file <path>` keeps it somewhere you can `tail -f`:

```bash
infisical secrets agent-proxy run -e dev --log-file=/tmp/broker.log -- claude
```

The file is written in the human-readable format shown above. A path you pass to `--log-file` is appended to on every run and never rotated for you, so use `logrotate` (`copytruncate`) if you keep one around.
