...
`canceled` means the agent hung up before the request finished, which happens when you interrupt a prompt or the agent exits mid-request. Nothing failed and there was nobody left to answer, so these stay at debug and off your terminal. A request that really failed, an unreachable upstream or permissions that couldn't be resolved, is still an `error`.

## Where the records go

### Local proxy

Nothing is written unless you pass `--log-file`, since the agent owns the terminal and a file you didn't ask for is one you can't find. With it, records go to that path and you can `tail -f` them:
