...
- Each service token is of the form `st.<id>.<secret>`. Send it verbatim in the `Authorization` header as `Bearer st.<id>.<secret>` when calling the Infisical API.
...
<AccordionGroup>
  ...
  <Accordion title="Can you provide examples for using glob patterns?">
    **Important:** The `/` character acts as a path separator. A single `*` doesn't match across `/` boundaries — use `**` to match across multiple path segments.
