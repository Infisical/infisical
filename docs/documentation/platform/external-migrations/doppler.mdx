...
<Steps>
  ...
  <Step title="Create a Doppler App Connection in Infisical">
    ...
    - **Integrations > App Connections** in your organization — the connection is visible across every project.
...
When you're inside a **Secrets Manager** project and have **one** environment selected (not "all environments"):
...
4. If more than one Doppler App Connection is available to you (project- or organization-scoped), pick the **Doppler Connection** in the modal. With a single connection available, it's selected for you.
...
6. Select **Import Secrets**. Infisical imports the selected Doppler config's secrets into the **current** Infisical environment and folder path.
