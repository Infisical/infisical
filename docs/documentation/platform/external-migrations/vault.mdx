...
5. (Optional) Select **"Preserve folder structure"** to recreate each Vault path as a folder in Infisical. Leave it unchecked to copy every key from every selected path into the folder you are currently viewing.
...
6. Check the **Preview** to confirm where every secret will land, then click **"Import Secrets"**

The secrets are imported into the environment and folder path you are currently viewing. You can select up to 25 Vault paths per import.
...
<Tabs>
  <Tab title="Flattened">
    Every key lands directly in the folder you are viewing. This import runs from
