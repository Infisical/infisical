...
<Steps>
  ...
  <Step title="Add a Vault migration configuration">
    ...
    Under **In-Platform Migration Tooling**, click **Add configuration**. In the provider chooser, select **HashiCorp Vault** (listed by the same name as in **App Connections**).

    ![Choose provider for in-platform migration](/images/platform/external-migrations/in-platform-migration-select-provider.png)
