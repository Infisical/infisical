...
<Warning>
  Importing from Vault requires the `Connect` action on `App Connections` and
  the `Create` action on the relevant subjects. For example, the `Create` action
  on the `Secrets` subject at the target environment and folder path for KV
  imports, or the equivalent permission for Machine Identities, Dynamic Secrets,
  and Roles.
</Warning>
...
<Steps>
  <Step title="Create a Vault policy">
    ...
    <Note>
      If you're running an open-source (OSS) version of Vault, you still need to define a rule for the namespace path. OSS Vault doesn't support custom namespaces, but it still has a single default namespace, `root`.
    </Note>
  ...
  <Step title="Create a HashiCorp Vault App Connection in Infisical">
    ...
    - **Integrations > App Connections** — the connection is visible across every project in the organization.
...
<Tip>
  **Migration Strategy:** We recommend migrating one application at a time.
  Start with a non-critical application to familiarize yourself with the
  process, then proceed to more important services.
</Tip>

### Migrating KV secrets

KV secrets are imported into a specific **environment** (e.g., Development, Staging, Production) within a **project**. If you're coming from Vault, think of a project as similar to a KV secrets engine dedicated to one application.

<Info>
  **First time?** You'll need to [create a
  project](/documentation/platform/project) first. A project in Infisical
  typically represents a single application or microservice.
</Info>

<Warning>
  If your Vault token is restricted and can't list a mount from its root,
  Infisical discovers the available secret paths from the token's resultant ACL
  (`sys/internal/ui/resultant-acl`). Any path that contains a `+` wildcard
  (Vault's single-segment wildcard, e.g. `secret/+/db`) can't be expanded and is
  **skipped**, i.e., those secrets won't be imported. To migrate them, grant the
  token access to the explicit KV paths instead of relying on `+`.
</Warning>

**To import secrets from Vault:**

1. Navigate to your project and select a specific environment (e.g., Development, Production)
2. In the secrets view, select the dropdown icon (caret) next to the **"+ Add Secret"** button
...
4. Pick the **App Connection**, then the **Namespace**, then the **Secrets Engine** that holds the paths you want, and finally one or more entries under **Vault Secret Path**. Keys are copied using their names from Vault.

5. (Optional) Select **Preserve folder structure** to recreate each Vault path as a folder in Infisical. Leave it unchecked to copy every key from every selected path into the folder you're currently viewing.

   <Note>
     Importing without preserving the structure aborts if the same key name
     exists in more than one selected path, and no secrets are saved. Preserving
     the structure resolves the conflict, because each Vault path gets its own
     folder.
   </Note>

6. Check the **Preview** to confirm where every secret will land, then select **"Import Secrets"**

The secrets are imported into the environment and folder path you're currently viewing. You can select up to 25 Vault paths per import.

#### Preserving your Vault folder structure

With **"Preserve folder structure"** enabled, each Vault path becomes its own [folder](/documentation/platform/folder), nested under your current path. Otherwise, all selected secrets are imported into the current folder.

The preview reflects the choice, so you can compare both before importing:

<Tabs>
  <Tab title="Flattened">
    Every key lands directly in the folder you're viewing. This import runs from
    the project root, so all six secrets collapse into `/` and the Vault path each
    one came from is shown only for reference.

    ![Vault import preview without preserving the folder structure](/images/platform/external-migrations/vault-in-platform/import-vault-secrets-flattened.png)

  </Tab>
  <Tab title="Structure preserved">
    Each Vault path becomes a folder. This import runs from `/newww`, so
    `team-a/service-1`, `team-a/service-2`, and `team-b` are recreated beneath it,
    each holding the secrets from its own Vault path.

    ![Vault import preview with the folder structure preserved](/images/platform/external-migrations/vault-in-platform/import-vault-secrets-preserve-structure.png)
...
3. Select **"Add Authentication Method"** and choose **Kubernetes Auth**
4. In the configuration modal, select **"Load from Vault"**
...
<Note>
  Sensitive values like service account JWTs can't be retrieved from Vault and
...
2. Select **"+ Add Secret"** dropdown and choose **"Dynamic Secret"**
3. Select **Kubernetes** as the provider
4. Select **"Load from Vault"** at the top of the form
...
<Note>
  Sensitive values like cluster tokens can't be retrieved from Vault and must be
  manually provided in the form after loading the configuration.
</Note>

<Tip>
  The same **Load from Vault** flow is also available on **Cassandra**,
  **LDAP**, and **SQL Database** dynamic secrets. Pick the App Connection, Vault
  namespace, mount, and role to pre-fill the form.
</Tip>
...
2. In the policy configuration, select **"Add from HashiCorp Vault"**
...
<Tip>
  **How policy translation works:** Vault path patterns are analyzed to identify
  KV secret engines and environments. Vault capabilities (`read`, `list`,
  `create`, etc.) are mapped to Infisical permissions. Wildcards in paths are
  converted to glob patterns. Secret paths are preserved for granular access
  control.
</Tip>
...
If you want to move an entire Vault organization into Infisical in a single shot (rather than importing resources piecemeal from inside each project) use the **Bulk Data Import** flow. It doesn't require an App Connection and is intended for initial migrations only. KV Secret Engine is the only supported resource type for bulk import; other resource types (Kubernetes auth, dynamic secrets, policies) must use the per-resource flows described above.

<Warning>Bulk Data Import is restricted to **organization admins**.</Warning>

**To run a bulk import:**

1. Navigate to **Integrations** and select the tab **External Migrations**
2. Select **Import** and choose **HCP Vault**
...
<CardGroup cols={2}>
  ...
  <Card
    title="Set Up Kubernetes Integration"
    icon="dharmachakra"
    href="/integrations/platforms/kubernetes/overview"
  >
