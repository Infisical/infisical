...
<Warning>
  If your Vault token is restricted and can't list a mount from its root, Infisical discovers the available secret paths from the token's resultant ACL (`sys/internal/ui/resultant-acl`). Any path that contains a `+` wildcard (Vault's single-segment wildcard, e.g. `secret/+/db`) can't be expanded and is **skipped**, i.e., those secrets won't be imported. To migrate them, grant the token access to the explicit KV paths instead of relying on `+`.
</Warning>
...
<Note>
  Sensitive values like cluster tokens can't be retrieved from Vault and must
