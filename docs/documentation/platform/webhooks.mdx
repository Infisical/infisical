...
- **Access Request Modified** (`secrets.access-request.modified`): triggered when an access request is created, edited, reviewed, or revoked. This event matches on environment and on overlapping secret paths, so a webhook receives a request whenever the requested path could include the path the webhook watches. An access request's path is a pattern rather than a single folder, which is why the matching rule differs from the other events.
...
The change request and access request payloads below carry no secret values, no diff of the proposed change, and no free text a user typed, such as a commit message, a bypass reason, or a review note. Each also has a top-level `timestamp` field, in epoch milliseconds, set to when Infisical sent the webhook.
...
`requestedBy` is `null` when the person who created the access request has since deleted their account.
