...
<Note>
  If a policy has a **Max Time Period** configured, the requested duration can't exceed this limit.
</Note>
...
<Note>
  Only users who are designated approvers for the access policy can revoke approved requests. Once revoked, the request moves to the **Revoked** status and can't be re-approved.
</Note>
