...
Removes a suffix from the end of a string. If the string does not end with the given suffix, it is returned unchanged.

**Examples:**

Given `identity.auth.kubernetes.namespace` = `myapp-prod`:

```handlebars
{{ trimSuffix identity.auth.kubernetes.namespace '-prod' }}  ->  myapp
```

Given `identity.auth.kubernetes.namespace` = `myapp` (nothing matches the suffix):
