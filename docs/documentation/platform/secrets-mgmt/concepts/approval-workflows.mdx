...
- **Broad access creates exposure.** Most developers shouldn't have direct access to production secrets, yet they're often the ones who need to add or change them. Organizations rely on informal processes — Slack messages, manual handoffs — to gate these changes, which slows development and is error-prone.
...
When bypass is enabled, you can restrict it to specific users or groups. If no restriction is set, anyone can bypass. A bypass can only be performed by the person who created the change request — bypassers can't bypass requests submitted by others.
...
<CardGroup cols={2}>
  <Card title="Set Up Approval Workflows" icon="git-pull-request" href="/documentation/platform/pr-workflows">
    Create and use change policies step by step.
  </Card>
  <Card title="Access Requests" icon="hand" href="/documentation/platform/access-controls/access-requests">
    Grant request-based access to environments and paths.
  </Card>
