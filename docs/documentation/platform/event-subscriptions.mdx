...
Event Subscriptions are currently available via the [Events API](/api-reference/endpoints/events/project-events). Support for SDKs, Kubernetes Operator, and other integrations is coming soon.
...
<Tip>
  For complete API specifications and additional examples, see the [API Reference](/api-reference/endpoints/events/project-events).
</Tip>
