...
<Note>
  ...
  On Infisical Cloud it's available on the Pro and Enterprise plans, If you're self-hosting Infisical, contact sales@infisical.com to acquire a license.
</Note>
...
From a Secrets Manager project, open the **Insights** tab in the project sidebar. The page is divided into four sections:
...
The three cards along the top of the page give you a quick read on what needs attention. Each card opens a popover with a table you can drill into; selecting a row jumps directly to the matching secret in the **Overview** page with the appropriate filters applied.
...
Select **View Rotations** (or **View Failed Rotations**) to see the full list. Each row shows the rotation name, environment, secret path, and a relative-time status (`scheduled`, `in 3 days`, `retries in 1h`, or `failed`). Select a row to open that rotation's secret in the Overview page filtered by rotation.
...
Select **View Upcoming Reminders** (or **View Overdue Reminders**) to see the list. Each row shows the secret key, environment, path, and how soon it's due (or how long it's been overdue). Select a row to jump to that secret in the Overview page.
...
The card shows either **All secrets up to date** when nothing is stale, or **{N} need(s) review** when there are stale secrets to look at. The popover table is paginated 10 rows at a time and shows each secret's key, environment, path, and last-modified time. Select a row to open it in Overview.
...
<Warning>
  Audit reports can contain sensitive metadata — secret keys, paths, access patterns, and recipient emails. They're delivered by email to the addresses you specify; only send reports to recipients you trust, and treat the CSV as confidential.
</Warning>
...
**Event details.** Select a pill to view its details, then use the **View in Overview** button to jump to the corresponding secret in the project Overview with filters applied.
