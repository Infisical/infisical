...
A service represents one external API, such as GitHub's REST API, Slack's Web API, or Anthropic's messages endpoint. Each service holds:
...
- **Rules**: The HTTP methods and paths the service allows (every method and path by default)
- **Custom headers**: Extra headers the proxy adds to each request
- **Substitutions**: Placeholders that the proxy replaces with real values
...
You grant a bundle to a user, group, or machine identity. Anyone holding that grant can create sessions that reference it.
...
Even if an attacker stole the session token, they could only reach the hosts, methods, and paths that the access bundle's services allow, and only until the session expires or you revoke it.
...
A proxy runs where your agent's traffic leaves your network. It intercepts every outbound request, authorizes the request against the session's access bundle, attaches the matching service's credential as a header, and forwards the request to the upstream host. If the request breaks the matching service's [method or path rules](/documentation/platform/agent-vault/services#methods-and-paths), the proxy refuses it instead.
...
3. The proxy looks inside the session's access bundle and finds a [service](#services) whose host pattern covers `api.github.com`. If the service limits methods or paths, the proxy refuses a request that doesn't match them. The proxy reads the real GitHub token from that service.
