...
- **Rules.** The HTTP methods and paths the service allows, if you want to narrow it further
...
3. The proxy looks inside the session's access bundle and finds a [service](#services) whose host pattern covers `api.github.com`. If that service allows only certain methods or paths, the request is checked against them first. The proxy reads the real GitHub token from that service.
