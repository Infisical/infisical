...
<Steps>
  ...
  <Step>
    You create a session scoped to that bundle for a limited time. Infisical returns a session token that identifies the session.

    ```mermaid
    flowchart LR
      session["Session"] -->|"scoped to"| bundle
      subgraph bundle["Access bundle"]
        s1["Service"]
        s2["Service"]
        s3["Service"]
  ...
  <Step>
    The agent runs with the session token. It uses the token to authenticate to the Agent Vault proxy on every outbound request.

    ```mermaid
    flowchart LR
      agent["AI agent"] -->|"runs with"| session["Session"]
      session -->|"scoped to"| bundle
      subgraph bundle["Access bundle"]
        s1["Service"]
        s2["Service"]
        s3["Service"]
  ...
  <Step>
    Every outbound request goes through the proxy before leaving the network. The proxy authorizes the session.

    ```mermaid
    flowchart LR
      agent["AI agent"] -->|"routes through"| proxy["Proxy"]
      proxy -->|"authorizes"| session["Session"]
  ...
  <Step>
    The proxy attaches the real credential from the matching service and forwards the request to the upstream host.
