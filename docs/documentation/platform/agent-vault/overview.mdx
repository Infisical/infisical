...
<Info>
  Agent Vault is in preview and subject to change. Not recommended for
  production use.
</Info>
...
You describe what an agent may use in an access bundle: one service per API the agent may reach, each with the credential the proxy attaches. You grant the access bundle to whoever runs agents, and they create a session with it to launch the agent. Requests to those services get the right credential on the way out, and everything else is untouched. The agent needs no code changes.
...
<CardGroup cols={2}>
  <Card title="Access bundles" icon="box" href="/documentation/platform/agent-vault/access-bundles">
    Services, credential types, host patterns, and who can run agents with an access bundle.
  </Card>
