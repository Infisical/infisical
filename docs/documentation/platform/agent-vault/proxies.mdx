...
Go to **Proxies**, select **Create Proxy**, name it, and select **Create**. The dialog shows a one-time enrollment token, valid for an hour, and the command to run where the proxy will live:

```bash
infisical av proxy --enrollment-token <enrollment-token> --domain <your-instance-url>
```

`--domain` points the proxy at your Infisical instance, and the dialog fills in yours. It defaults to `https://app.infisical.com`, so EU Cloud and self-hosted instances have to pass it.
