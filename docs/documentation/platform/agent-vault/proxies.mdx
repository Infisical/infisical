...
A proxy is the process that attaches credentials. It runs where agent traffic leaves your network: on a server many agents share, or next to a single agent. You create it in the dashboard, enroll it once from the CLI, and change its settings from the dashboard afterwards.

<Info>
  This page is for the people who run proxies. Members see the Proxies page too, with each proxy's name, health, and certificate fingerprint.
</Info>

## Enrolling

Go to **Proxies**, select **Create Proxy**, name it, and select **Create Proxy**. The dialog shows a one-time enrollment token, valid for an hour, and the command to run where the proxy will live:
...
The same dialog has Docker and systemd snippets. The Docker one mounts a volume on the state directory so the container can be re-created.
...
- **Deny**: the request is rejected. The agent reaches the hosts in its bundles, the bypass list, and nothing else.

Use **Deny** when the proxy sits somewhere it could reach more than you intend, such as a network with internal services on it. Under **Deny**, a pass-through connection is how a bundle makes a host reachable without giving it a credential.
...
If the proxy can't reach Infisical, it keeps serving what it last knew for five poll intervals, then stops attaching credentials until it can check again. The Proxies page marks a proxy **Unhealthy** after three missed check-ins.
...
An agent you start without the CLI, such as a container that sets `HTTPS_PROXY` itself, needs the certificate mounted and those variables set. Fetch it from `http://<proxy-address>/_agent-vault/ca`.
