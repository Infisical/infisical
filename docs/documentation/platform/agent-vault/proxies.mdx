...
Ports are part of the match. A [service's host pattern](/documentation/platform/agent-vault/services#host-patterns) that names no port covers port 443 only, so a service on `example.com` doesn't cover `example.com:8443`, and under this policy a request to the second is refused. Name the port in the pattern to cover it.

The proxy also returns a 403 if a request breaks the matching service's [method or path rules](/documentation/platform/agent-vault/services#methods-and-paths), under either traffic policy.
...
Exceptions are hosts that agents can reach, even when you've enabled **Only allow requests to reach hosts specified in access bundles** on your proxy. Use exceptions for hosts every agent needs, such as a package registry or a documentation site.

To add exceptions, select the policy and enter the hosts in **Exceptions** as a comma-separated list. The field doesn't appear under **Allow requests to reach any host**, because that policy already lets every host through.

An exception host behaves differently from a host that a service covers:
...
<Tip>
To let only one access bundle reach a host, add a [pass-through service](/documentation/platform/agent-vault/services#pass-through) to that bundle instead of an exception.
</Tip>
