...
An organization can have up to 10 proxies. To add another after that, delete one you no longer use.
...
The proxy returns a 403 the same way for a request that breaks a service's own [method or path rules](/documentation/platform/agent-vault/services#methods-and-paths), whichever traffic policy it runs.
...
An exception written without a port matches that host on every port, unlike a [service pattern](/documentation/platform/agent-vault/services#default-port), which means port 443 when it names none. An exception never carries a credential, so the wider match attaches nothing extra.
