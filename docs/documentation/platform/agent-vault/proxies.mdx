...
The proxy returns a 403 the same way for a request that breaks a service's own [method or path rules](/documentation/platform/agent-vault/access-bundles#methods-and-paths), whichever traffic policy it runs.
