...
The search box on each tab covers every member, not only the current page. You can search users by name, username, or email address, and groups and machine identities by name.
...
Open an access bundle and select **Manage Access** to grant it to a user, machine identity, or group. The grantee has to be an Agent Vault member already. To find a grantee in a long list, use the search box in **Manage Access**.
