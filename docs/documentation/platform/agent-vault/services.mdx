...
With a substitution, the agent sends its request with a placeholder credential, such as a fake API token. Then, the proxy replaces the placeholder with the real value before forwarding the request. For example, an agent configured with `__GITHUB_PAT__` as its GitHub token sends:
