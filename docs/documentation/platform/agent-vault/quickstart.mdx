...
Every Agent Vault setup starts with the same things in Infisical: an access bundle holding a connection for each API, a proxy enrolled where your agent's traffic leaves the network, and a session for the agent to run with. This quickstart walks you through creating all three and launching an agent.
...
<Frame>
  <img src="/images/platform/agent-vault/access-bundle-create.png" alt="The Create Access Bundle dialog with coding-agent entered as the name" />
</Frame>

Select **Add Connection** and work through the steps:

<Frame>
  <img src="/images/platform/agent-vault/connection-template-picker.png" alt="The Choose a template panel with GitHub among the available services" />
</Frame>
...
Go to **Proxies**, select **Create Proxy**, name it `egress`, and select **Create**. The dialog shows a one-time enrollment token and the command to run.
...
<Frame>
  <img src="/images/platform/agent-vault/session-create.png" alt="The Create Session panel with coding-agent picked and a seven day expiry" />
</Frame>

The token appears once, with the command to run. Copy it now.

<Frame>
  <img src="/images/platform/agent-vault/session-created.png" alt="The Session Created dialog showing the infisical av run command with the session token" />
</Frame>
...
<Note>
  Getting a `401`? The credential wasn't applied. Open the `coding-agent` access bundle and confirm a connection covers `api.github.com`; if one does, the stored token is wrong. A `407` or `403` from the proxy itself means the session token was missing, revoked, or expired.
</Note>

<Note>
  Working on your own machine and signed in with `infisical login`? Skip the Sessions page: `infisical av run --access-bundle coding-agent --proxy <proxy-host>:17323 -- claude` creates a session for you and revokes it when the agent exits. For an agent that runs unattended, run it as a machine identity instead — see [Sessions](/documentation/platform/agent-vault/sessions#creating-a-session).
</Note>
