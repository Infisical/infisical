...
<Note>
  Not on the template list? Pick **Custom** and set the host, header name, and prefix yourself. See [Access bundles and services](/documentation/platform/agent-vault/access-bundles).
</Note>
...
<Note>
  Getting a `401`? The credential wasn't applied. Open the `coding-agent` access bundle and confirm a service covers `api.github.com`; if one does, the stored token is wrong. A `407` or `403` from the proxy itself means the session token was missing, revoked, or expired.
</Note>
