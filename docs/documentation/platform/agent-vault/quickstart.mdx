...
<Note>
  Getting a `401`? The credential wasn't applied. Open the `coding-agent` bundle and confirm a connection covers `api.github.com`; if one does, the token stored on it is wrong. A `407` or `403` from the proxy itself means the session token was missing, revoked, or expired.
</Note>
