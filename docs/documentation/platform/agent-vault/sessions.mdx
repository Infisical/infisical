...
<Tabs>
  <Tab title="Dashboard">
    ...
    <Frame>
      <img src="/images/platform/agent-vault/session-create.png" alt="The Create Session panel with an access bundle picked and a seven day expiry" />
    </Frame>
  ...
  <Tab title="CLI">
    `infisical av run` creates a session for you when you name the access bundle:
...
<Frame>
  <img src="/images/platform/agent-vault/sessions-list.png" alt="The Sessions page listing sessions with their access bundle, expiry, and status" />
</Frame>
...
Creating and revoking a session each write an audit event, as does every change to an access bundle, connection, or proxy. Open **Audit Logs** in Agent Vault, or scope an [audit log stream](/documentation/platform/audit-log-streams/audit-log-streams) to the Agent Vault product.
