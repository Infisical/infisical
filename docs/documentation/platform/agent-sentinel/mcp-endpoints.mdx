...
| IP Address | IPv4 addresses | `[REDACTED_IP]` |
...
<AccordionGroup>
  ...
  <Accordion title="What PII types are supported?">
    Infisical currently supports detection and redaction of: email addresses, phone numbers (US format), Social Security Numbers, credit card numbers, and IPv4 addresses.
  </Accordion>
  <Accordion title="Can I select different PII types for requests and responses?">
    Not currently. The selected PII types apply to both request and response filtering. If you need different filtering rules, consider creating separate endpoints.
  </Accordion>
