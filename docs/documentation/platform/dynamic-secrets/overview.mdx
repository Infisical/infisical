...
<Info>
  Dynamic secrets are available under Infisical's [Advanced](https://infisical.com/pricing) plan.
  
  If you're self-hosting Infisical, contact sales@infisical.com to purchase an Advanced license.
</Info>
...
**Dynamic secrets are unique to every identity using them**. Such secrets are generated only at the moment they're retrieved, eliminating the possibility of theft or reuse by another identity. Thanks to Infisical's integrated revocation capabilities, dynamic secrets can be promptly invalidated post-use, significantly reducing their lifespan.
...
- **Reduced Secret Lifetime**: The limited validity period of dynamic secrets means that they're less valuable targets for attackers. This inherently reduces the time window during which a secret can be exploited.
...
<Accordion title="Why is my SQL dynamic secret failing when I generate a lease?">
  This usually happens when the SQL statements defined for creating or revoking the secret aren't compatible with your database provider.
