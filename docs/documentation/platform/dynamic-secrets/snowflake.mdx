...
<Steps>
  <Step title="Open user settings">
   Select the tab **Governance & Security** tab and click on **Users & roles**
    ![Snowflake User
  ...
  On the top right click on **Create user**
    ![Snowflake User
Dashboard](/images/platform/dynamic-secrets/snowflake/dynamic-secret-create-user-button.png)
  ...
  <Step title="Generate a Programmatic Access Token">
    Select the tab **Governance & Security** and click on **User & roles**. Select the User created on the last step. 

    ![Select User](/images/platform/dynamic-secrets/snowflake/select-user-for-pat.png)

    Open the **Programmatic access tokens** tab and click **Generate new token**. Give the token a descriptive name and configure its expiration. The role restrictions in this case needs to be **USERADMIN**. 
