...
<Steps>
  ...
  <Step title="Generate dynamic secrets">
    Once configured, select `Generate` on the dynamic secret row (or `New Lease` in the lease list) to provision a lease.
...
Select any dynamic secret on the dashboard to see active leases, their expiration times, and revoke them early if needed.

![Lease Data](/images/platform/dynamic-secrets/lease-data.png)

## Renew leases

Select `Renew` on an active lease to extend its TTL.

![Renew Lease](/images/platform/dynamic-secrets/dynamic-secret-lease-renew.png)

<Warning>
  Renewals can't exceed the Max TTL set when the dynamic secret was configured.
</Warning>
