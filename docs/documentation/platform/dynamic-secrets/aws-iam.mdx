        ...
        <Tabs>
          <Tab title="IAM User">
            ...
            <ParamField path="AWS Policy ARNs" type="string">
              The AWS IAM managed policies that should be attached to the created users. Multiple values can be provided by separating them with commas. AWS allows at most 10 managed policies attached to a user.
            </ParamField>

            <ParamField path="AWS IAM Policy Document" type="string">
              The AWS IAM inline policy (JSON) that should be attached to the created users. Maximum 2,048 non-whitespace characters (AWS excludes whitespace from this quota).
            </ParamField>
          ...
          <Tab title="Temporary Credentials">
            ...
            <Info>
              The generated credentials will:
              - Inherit the permissions of the assumed role, optionally restricted by the session policies below
              - Include an AWS Session Token
              - Be valid for the duration specified in Default TTL
            </Info>

            <ParamField path="AWS Session Policy ARNs" type="string" optional>
              Comma-separated managed policy ARNs applied as session policies. The generated credentials are limited to the intersection of the assumed role's permissions and these policies. AWS allows up to 10 ARNs.
            </ParamField>
            <ParamField path="AWS Session Policy Document" type="string" optional>
              An inline session policy (JSON) that further restricts the generated credentials. Maximum 2,048 characters of plaintext. AWS additionally enforces a packed-size limit covering the inline policy, managed policy ARNs, and session tags combined.
            </ParamField>

            <Info>
              Session policies can only restrict the assumed role's permissions; they can never grant permissions beyond it.
            </Info>
            ...
            ![Dynamic Secret Setup Modal Temporary Credentials](../../../images/platform/dynamic-secrets/aws-iam-temporary-credentials.png)
          </Tab>
        </Tabs>
      </Step>
        ...
        <Step title="Provide the inputs for dynamic secret parameters">
            ...
            <Tabs>
              ...
              <Tab title="Temporary Credentials">
                ...
                <Note>
                  Session policies (AWS Session Policy ARNs / AWS Session Policy Document) are not supported with the IRSA method: STS GetSessionToken does not accept them. These fields are only available with the Assume Role method.
                </Note>
              </Tab>
            </Tabs>
        </Step>
       ...
       <ParamField path="Method" type="string" required>
        ...
        <Tabs>
          ...
          <Tab title="Temporary Credentials">
            ...
            <Note>
              Session policies (AWS Session Policy ARNs / AWS Session Policy Document) are not supported with the Access Key method: STS GetSessionToken does not accept them. These fields are only available with the Assume Role method.
            </Note>
          </Tab>
        </Tabs>
...
To extend the life of the generated dynamic secret lease past its initial time to live, simply select the **Renew** button as illustrated below.
![Provision Lease](/images/platform/dynamic-secrets/dynamic-secret-lease-renew.png)

<Warning>
  Lease renewals can't exceed the maximum TTL set when configuring the dynamic
  secret
</Warning>
