...
<Steps>
  ...
  <Step title="Provide the inputs for dynamic secret parameters">
    ...
    <ParamField path="Default TTL" type="string" required>
    	Default time-to-live for a generated certificate (e.g. `1h`, `8h`, up to `7d`)
    </ParamField>

    <ParamField path="Max TTL" type="string" required>
    	Maximum time-to-live for a generated certificate (e.g. `24h`, up to `7d`)
    </ParamField>
    ...
    <ParamField path="CA Key Algorithm" type="string" required>
    	Algorithm for the internal CA key pair that signs lease certificates. Supported options: **ED25519** (default, recommended), **RSA 2048**, **RSA 4096**, **ECDSA P-256**, **ECDSA P-384**. Changing this after creation generates a new CA, so hosts that trust the previous CA must be set up again.
    </ParamField>
...
<Note>
  SSH certificates are time-bound and expire automatically. Revoking a lease in
  Infisical removes the record, but the certificate remains valid until its TTL
  expires.
</Note>

## Edit dynamic secret

To update the configuration of an existing SSH dynamic secret (principals, key algorithm, CA key algorithm, TTL, or to access the CA setup options), select the dynamic secret item and then **Edit**.
...
<Note>
  SSH certificate leases can't be renewed because the certificate validity is
  baked in at signing time. To get a new certificate, create a new lease.
</Note>
