...
- **Condition-based matching**: Define which resources and accounts the policy applies to using [glob patterns](https://www.malikbrowne.com/blog/a-beginners-guide-glob-patterns/) (e.g. `prod-*`, `{prod,staging}-*`, `*-admin`).
...
- **Bypass Approvals**: Optionally allow designated users to self-approve their own request in break-glass situations.
...
<Steps>
  <Step title="Configuration">
    ...
    - **Bypass Approvals** (optional): Allow specific users or groups to self-approve their own request in break-glass situations. See *Bypass Approvals* below.
...
- When enabled, designated users (bypassers) can self-approve their own pending request with a written reason, skipping the configured approver workflow.
- When disabled, all requests must go through the full approval process.
- You can specify which users or groups are allowed to bypass.

The bypass is recorded as a `pam-access-policy-bypassed` audit event and notifies all approvers via in-app notification and email.

<Warning>
  If bypass approvals is enabled and no specific users or groups are selected, anyone in the project can bypass this policy.
</Warning>
...
<Steps>
  ...
  <Step title="Approve">
    ...
    If the policy has Bypass Approvals enabled and you are a configured bypasser on your own request, you can also tick **Approve immediately without approver review (bypass policy)**, provide a reason, and approve in the same step.
  </Step>
</Steps>
...
In addition to approval policies, project roles can scope PAM-account access using CASL conditions (Project > Access Control > Roles > edit role > PAM Accounts).
