...
<Tabs>
  <Tab title="Windows (local)">
    - **Self-rotation**: the account changes its own password. This requires the account to be able to sign in over WinRM and to hold the rights to change its own credential. Many local accounts can't, so delegated rotation is the common choice.
