...
<Warning>
  Only **SQL Server authentication** logins can be rotated. Windows-authenticated accounts aren't supported, because their passwords are managed by the domain or the operating system rather than by SQL Server. A rotation on a Windows-authenticated account is rejected before any change is attempted.
</Warning>
