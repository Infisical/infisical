...
2. **Authentication**: Credentials are stored securely in Infisical and used by the Gateway to authenticate with MsSQL on behalf of the user. SQL Server Authentication (username/password), Windows Authentication (NTLM), and Windows Authentication (Kerberos) are supported.
