...
3. **Credential Validation via WinRM**: When creating or updating accounts, Infisical validates credentials by connecting to the Windows server's WinRM service (port 5985). This ensures credentials are correct before they are stored.
...
3. **WinRM Enabled** - Windows Remote Management must be enabled for credential validation (port 5985)
4. **Windows Credentials** - A username and password for a Windows account on the server

<Warning>
  **WinRM Required**: Infisical validates account credentials using WinRM (Windows Remote Management) on port 5985. Ensure WinRM is enabled on your Windows server by running the following command in an elevated PowerShell prompt:

  ```powershell
  winrm quickconfig
  ```

  By default, only administrators can authenticate via WinRM. Ensure the account you're configuring has WinRM access.
</Warning>

<Warning>
  **Gateway Required**: Windows Server access requires an Infisical Gateway to be deployed and registered with your Infisical instance. The Gateway must have network connectivity to your Windows server on both the RDP port (default: 3389) and WinRM port (5985).
</Warning>
...
<Steps>
  ...
  <Step title="Fill in Account Details">
    ...
    <Note>
      When creating the account, Infisical will validate the credentials by connecting to the server's WinRM service. Ensure the account has WinRM access (administrators have this by default).
    </Note>
  </Step>
</Steps>
