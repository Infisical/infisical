...
You can connect to a Windows Server account using either the browser-based RDP client or the Infisical CLI.

- **Web Access**: Connect directly from your browser with a full graphical desktop session. See [Windows RDP Web Access](/documentation/platform/pam/product-reference/web-access/windows-rdp) for details.
- **CLI Access**: Run `infisical pam rdp access` to start a local RDP proxy that launches your native RDP client (mstsc, Microsoft Remote Desktop, or FreeRDP). Credentials are injected automatically.
