...
<AccordionGroup>
  ...
  <Accordion title="Do I need to configure SSL certificates for Gateway auth?">
    No. When using Gateway auth, the Gateway automatically uses the in-cluster CA certificate mounted at `/var/run/secrets/kubernetes.io/serviceaccount/ca.crt`. The SSL settings on the resource (certificate, verification toggle) are only used for the Service Account Token method.
  </Accordion>
