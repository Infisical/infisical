...
<Steps>
  ...
  <Step title="Create the Resource in Infisical">
    ...
    5. Enter the **Host** - the hostname or IP address of your MongoDB server (e.g., `mongo.example.com` or `192.168.1.100`). For MongoDB Atlas or SRV-based deployments, enter the SRV hostname (e.g., `cluster0.abc123.mongodb.net`) and leave the port empty.
    6. Enter the **Database Name** - the database to connect to (default: `admin`)
    7. Optionally enter the **Port** - the MongoDB port (default: `27017`). Leave this empty for SRV-based hosts — Infisical will automatically resolve the SRV record to determine the host and port.
    ...
    <Note>
      **SRV Hosts**: When connecting to MongoDB Atlas or other SRV-based deployments, leave the port field empty. Infisical will resolve the `_mongodb._tcp` SRV record to discover the actual host and port automatically.
    </Note>
