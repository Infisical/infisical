...
An **account** represents a single database or server you want to manage. You provide the connection details (host, port, database) and credentials (username, password), and Infisical stores them securely. Whoever connects, a person or a machine, goes through Infisical without ever seeing the underlying credentials.

Infisical supports databases (PostgreSQL, MySQL, MS SQL, MongoDB, Redis), servers (SSH, Windows), Kubernetes clusters, and cloud accounts (AWS IAM, GCP, Azure).
...
**Memberships** determine who can access what by assigning a user, group, or [machine identity](/documentation/platform/identities/machine-identities) a role on a folder or account.
...
Machine identities hold these same roles, which is how an [AI agent](/documentation/platform/pam/ai-agents/overview) reaches an account when it runs unattended. **Connector** is the usual choice.
