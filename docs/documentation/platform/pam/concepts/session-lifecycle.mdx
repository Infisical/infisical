...
An [AI agent](/documentation/platform/pam/ai-agents/overview) goes through the same stages, with the same checks and the same recording.

## Requesting access

When you select **Launch Session** on an account from the **Accounts** page (or run the CLI command), PAM checks:
...
3. **Approval, if the account is gated** — A gated account also needs an active [access request](/documentation/platform/pam/access-requests/overview) grant

If everything checks out, the session is approved and you can connect.

An [AI agent](/documentation/platform/pam/ai-agents/overview) can't be prompted, so it supplies its reason up front, and it can't use an account that requires MFA yet.
...
For **Web Access**, this all happens in the browser. For **CLI Access**, SSH accounts connect you straight to a shell, while every other type starts a local proxy you point your preferred client at on `localhost`.
