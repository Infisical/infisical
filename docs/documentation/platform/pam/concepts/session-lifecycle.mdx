...
When you click **Access** on an account (or run the CLI command), PAM checks:

1. **Do you have permission?** — You need a membership (on the folder or account) with the Connector or Admin role
2. **Template requirements** — If the template requires a reason, you'll be prompted to enter one. If it requires MFA, you'll need to verify your identity.
...
- **Recording** — If enabled in the template, every query (database) or command (SSH) is captured
- **Session timer** — The session has a maximum duration set by the template. When time runs out, you're disconnected.
- **Logs upload** — Session activity uploads to Infisical every 10 seconds, so admins can monitor in near real-time
...
After the session ends, the full recording is available for review.
...
- **Session recording** — captures everything that happened during the session (if recording was enabled)
