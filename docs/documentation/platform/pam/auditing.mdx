...
| **[Access requests](/documentation/platform/pam/access-requests/overview)** | Request filed, reviewed, revoked, approval policy bypassed |

Each event includes:
- **Timestamp** — when it happened
- **Actor** — who did it: a user (name, email, IP address) or a [machine identity](/documentation/platform/identities/machine-identities), named as it's named in your organization
- **Action** — what was done
- **Target** — what was affected
- **Details** — relevant context (e.g., what changed)

An identity has no email address, so its events show **Machine Identity** where a person's email would be. Everything an [AI agent](/documentation/platform/pam/ai-agents/overview) does is logged the same way a person's activity is.
...
**Activity reports** — Filter by actor to see everything a specific user or machine identity did.

**Break-glass review**: Filter for **PAM Access Policy Bypassed** to see every grant that skipped an approver, with the reason given. Each entry names the account, the folder, and how many approvers were skipped.

## Next steps

<CardGroup cols={2}>
  <Card title="Sessions" icon="monitor" href="/documentation/platform/pam/sessions/overview">
