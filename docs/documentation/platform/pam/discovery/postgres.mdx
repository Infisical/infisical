...
PostgreSQL discovery scans a set of PostgreSQL instances, enumerates the roles that can log in, stages them for review, and lets you import them into PAM as [PostgreSQL accounts](/documentation/platform/pam/accounts/postgresql). You point a source at the instances to scan and the [PostgreSQL accounts](/documentation/platform/pam/accounts/postgresql) to scan with.
...
2. Each host is probed on the ports of your credential accounts, so unreachable hosts are skipped before any login is attempted.
...
4. It reads the cluster's roles from `pg_roles`.
...
<Note>
The scanning account needs no special grant. `pg_roles` is readable by any role, so a low-privilege login is enough to enumerate an instance.
</Note>

<Warning>
Only password authentication can drive a scan. An AWS IAM account's token is minted for one host, port, and user, which a scan across a range of hosts cannot do, so IAM accounts are skipped as credential accounts.
</Warning>
...
Discovery surfaces only roles that can be logged into, meaning those with the `LOGIN` attribute. Group roles used purely to hold privileges cannot open a session, so they are not accounts and are dropped. This also excludes the built-in `pg_` roles, none of which can log in.
