...
<Warning>
Only password authentication can drive a scan. An AWS IAM account's token is minted for one host, port, and user, so it cannot be reused across the instances a source scans. IAM accounts are skipped as credential accounts.
</Warning>
