...
| Discovered type | What it is | When it's found |
|-----------------|------------|------------------|
...
<Steps>
  <Step title="Start adding a source">
    Go to **Privileged Access Manager → Discovery** and select **Add Source**, then choose **Active Directory**.
  </Step>
  ...
  <Step title="Save">
    Select **Create**. The source appears in the Discovery table.
  </Step>
</Steps>
...
<Note>
If a scan can't reach WinRM on a particular server, that server is skipped and the failure is reported on the run. The rest of the scan still completes.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Windows AD Accounts" icon="windows" iconType="brands" href="/documentation/platform/pam/accounts/windows-ad">
    Learn about the account type used to authenticate scans.
  </Card>
  <Card title="Account Dependencies" icon="workflow" href="/documentation/platform/pam/dependencies/overview">
