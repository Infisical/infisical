...
<Steps>
  ...
  <Step title="Configure policies">
    | Option | What it does |
    |--------|--------------|
    | **Require Approval** | Users must request and receive approval before launching a session |
    | **Require Approval for Credentials** | Users must request and receive approval before [viewing the account's credentials](/documentation/platform/pam/accounts/viewing-credentials) |
    ...
    <Note>
      The two approval options are separate gates. Turning on **Require Approval** does not gate the credential, and an approved session request does not reveal it. Gate both if a session and the raw password should each need sign-off.
    </Note>
