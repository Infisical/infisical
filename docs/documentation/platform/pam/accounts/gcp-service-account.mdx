  ...
  <Step title="Choose an authentication method">
    ...
    <Tabs>
      <Tab title="Impersonation">
        There are no credentials to enter — but you need to authorize Infisical's service account to impersonate your target service account. This is a one-time grant per target service account.

        **1. Find Infisical's service account**
        ...
        **2. Grant it the Token Creator role**

        In the Google Cloud Console, go to **IAM & Admin → Service Accounts**, select your **target** service account, open the **PERMISSIONS** tab, click **Grant Access**, add Infisical's service account (from step 1) as a principal, and assign it the **Service Account Token Creator** (`roles/iam.serviceAccountTokenCreator`) role.
