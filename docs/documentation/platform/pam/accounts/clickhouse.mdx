...
The account's **Database** sets what a session opens by default, not a boundary it stays inside: a session can query any database the user holds a grant on, and Web Access lists all of them. Scope the grants rather than the account's database field.
...
The Data Explorer shows the first 1,000 rows a statement produces and tells you when there are more. ClickHouse itself imposes no such limit; use the CLI with your own client to work with a full result set.
