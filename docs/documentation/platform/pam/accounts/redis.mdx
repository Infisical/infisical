...
<Tabs>
  <Tab title="Web Access">
    ...
    - Long replies are shortened. The first 256KB is shown, followed by a truncation notice. A reply over 8MB closes the session, so use the CLI for data that size.
    - Commands that stream results or change how the connection behaves (`SUBSCRIBE`, `PSUBSCRIBE`, `MONITOR`, `SELECT`, `CLIENT`, `HELLO`) are not available. Use the CLI for those.
