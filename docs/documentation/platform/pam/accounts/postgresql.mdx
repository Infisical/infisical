...
<Steps>
  ...
  <Step title="Enter credentials">
    ...
    | **Role ARN** | IAM role Infisical assumes to generate a token (AWS IAM only) |
  </Step>
  <Step title="Save">
    Click **Create**.
  </Step>
...
<Steps>
  ...
  <Step title="Create the role with a trust policy">
    Create the IAM role you'll set as **Role ARN**, with the trust policy from [AWS IAM accounts](/documentation/platform/pam/accounts/aws-iam#prerequisites). It uses your Infisical Organization ID as the External ID.
  </Step>
...
<Note>
  IAM authentication requires an encrypted connection, so **SSL Enabled** is turned on and cannot be disabled. To verify the server certificate, turn on **Reject Unauthorized** and paste your region's RDS root certificate into **SSL Certificate**, for example `https://truststore.pki.rds.amazonaws.com/us-east-1/us-east-1-bundle.pem`.
</Note>
