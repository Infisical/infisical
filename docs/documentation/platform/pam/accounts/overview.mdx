...
<Steps>
  ...
  <Step title="Click Add Account">
    Click **Add Account** and select the account type (PostgreSQL or SSH).
  </Step>
  <Step title="Select a template">
    Choose which template this account should use. The template determines the session rules (duration, MFA, recording).
  </Step>
  ...
  <Step title="Save">
    Click **Create**. Infisical validates the connection before saving.
  </Step>
</Steps>

If validation fails, check that the credentials are correct and that the account is reachable from your Gateway.

## Connecting to an Account

Once an account exists, users with access can connect in two ways:

### Web Access

Click **Access** on the account and select **Connect in Browser**. This opens:
- **PostgreSQL** — a Data Explorer for browsing tables and a SQL Terminal for running queries
...
1. Open the account and go to the **Memberships** tab
2. Click **Add Member**
