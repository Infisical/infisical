...
An account can gate two things independently, and each is requested separately:

- **Session access**, gated by the template's **Require Approval** policy.
- **Credential access**, gated by **Require Approval for Credentials**. See [Viewing credentials](/documentation/platform/pam/accounts/viewing-credentials).

Everything below applies to both. An approved session request never reveals the credential, and an approved credential request never launches a session, so a member who needs both files both.
...
- **Membership** grants use access, the right to launch a session and connect to the account. This comes from the [Admin, Operator, or Connector role](/documentation/platform/pam/concepts/access-control) on the [folder](/documentation/platform/pam/folders/overview) or account, and it is the record of who can ever get in. Credential access needs the Operator or Admin role specifically.
