...
<Steps>
  ...
  <Step title="Creating a Client Secret">
    ...
    With that, select **Add Client Secret**.
    ...
    - TTL (default is `0`): The time-to-live for the **Client Secret**. By default, the TTL will be set to 0 which implies that the **Client Secret** will never expire; a value of `0` implies an infinite lifetime. When you set a TTL, configure a [credential expiry alert](/documentation/platform/identities/machine-identities#credential-expiry-alerts) so your team is notified before the **Client Secret** expires.
    ...
    <Info>
      When **Max Number of Uses** is unlimited (`0`), the displayed usage count is approximate. Client Secrets with a finite limit are always counted exactly.
    </Info>
...
- As long as the token is renewed before its period expires, it remains valid, so you don't need to re-issue static credentials.
...
2. For improved security, configure the Client Secret with a low number of uses (e.g., `1`) or a short TTL. This ensures that after the initial login, the Client Secret can't be reused, and any disruption in token renewal will require manual intervention.
...
<AccordionGroup>
  ...
  <Accordion title="How do I reset a lockout?">
      You can reset (remove) all lockouts for an identity auth method by opening the auth method and selecting **Reset All Lockouts**.
