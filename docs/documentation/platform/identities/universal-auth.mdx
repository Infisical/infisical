...
<Steps>
  ...
  <Step title="Creating a Client Secret">
    ...
    - TTL (default is `0`): The time-to-live for the **Client Secret**. By default, the TTL will be set to 0 which implies that the **Client Secret** will never expire; a value of `0` implies an infinite lifetime. When you set a TTL, configure a [credential expiry alert](/documentation/platform/identities/machine-identities#credential-expiration) so your team is notified before the **Client Secret** expires.
