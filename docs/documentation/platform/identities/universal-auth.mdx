...
<Steps>
  ...
  <Step title="Creating a Client Secret">
    ...
    <Info>
      When **Max Number of Uses** is set to `0` (unlimited), the usage count and last-used timestamp displayed for a **Client Secret** are approximate. To avoid database contention under high login volume, usage updates for unlimited Client Secrets are batched over a short interval rather than written on every login, so the count may lag slightly behind the true number of uses. Client Secrets configured with a finite **Max Number of Uses** are always counted exactly, since the limit is enforced.
    </Info>
