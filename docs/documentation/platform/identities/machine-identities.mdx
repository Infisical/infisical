...
| Event | When it fires |
|-------|---------------|
| **Client Secret Expiration** | A set number of days before a Universal Auth client secret expires, checked once a day |
| **Auth Method or Credential Change** | Right after an auth method is added, updated, or removed, or a Universal Auth client secret or Token Auth token is created, updated, or revoked |

### Client secret expiration

A machine identity whose credential expires unnoticed takes your workload down with it. This alert watches the Universal Auth client secrets attached to the identity and notifies your team a configurable number of days before they expire, so the credential gets rotated before anything breaks.
...
| **Event** | **Client Secret Expiration** or **Auth Method or Credential Change**. The event can't be changed after the alert is created; remove the alert and create a new one instead. |
| **Condition** | For **Client Secret Expiration** only: how many days before expiration to notify, as a whole number from `1` to `90` |
| **Daily Reminder** | For **Client Secret Expiration** only: if enabled, repeats the alert every day from the threshold until the credential expires |
...
Webhook deliveries are [CloudEvents](https://cloudevents.io/) 1.0 JSON documents. The `type` names the event: `com.infisical.identity.authentication.expiration` for a client secret expiration and `com.infisical.identity.authentication.auth-method-changed` for an auth method change. Every credential due for this run, or the change that fired, is listed in `data.items`:
