...
A Token Auth token expires when its max TTL runs out, counted from the moment the token was created. Renewing the token extends the current session but can't push it past that point. A token with a max TTL of `0` can be renewed indefinitely, so it has no expiry to alert on and isn't monitored. Revoked tokens are skipped.
    ...
    "items": [
      {
        ...
        "title": "ci-deploy-bot",
        "fields": [
          { "label": "Credential Name", "value": "github-actions" },
