...
<Note>
  ...
  In the rare event that this cache invalidation cannot be written (for example, a transient Redis disruption during the revoke), the database record still reflects the revocation, but a previously cached "allow" result for that exact token may continue to be honored until it expires on its own. This window is bounded to roughly 12 minutes, after which the decision is re-evaluated against the database and the revoked token is rejected.
