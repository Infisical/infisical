...
A Token Auth token is watched until its access token TTL runs out, counted from the moment the token was created. That's when the token stops working unless the workload renews it. Infisical doesn't track renewals, so a token that is renewed still triggers this alert on that first expiry. Revoked tokens are skipped.
...
Select **Alerts** on the machine identity to see its alerts. The number next to **Alerts** counts the enabled ones. Use the toggle on a row to turn that alert on or off, or open the menu at the end of the row and select **Edit Details** or **Remove Alert**. Removing an alert asks you to type its name to confirm.
