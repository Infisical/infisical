...
<Step title="Creating an identity">
    ...
    **Common fields for both configurations**:
    ...
    Note that `*` does not match across `/` boundaries — use `**` to match across path segments.

    | Pattern | Matches | Does not match |
    |---------|---------|----------------|
    | `user-*` | `user-admin`, `user-readonly` | `user-team/lead` |
    | `{admin,service}-**` | `admin-api`, `service-db/primary` | `user-api` |
    | `system:serviceaccount:*:my-sa` | `system:serviceaccount:default:my-sa` | `system:serviceaccount:ns/sub:my-sa` |
    </Info>
    </Step>
