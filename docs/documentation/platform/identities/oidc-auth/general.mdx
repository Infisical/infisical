...
<Steps>
    ...
    To create an identity, go to **Access Control** > **Machine Identities** in your organization and select **Create**.

    ![identities organization](/images/platform/identities/identities-org.png)

    When creating an identity, you specify an organization level [role](/documentation/platform/access-controls/role-based-access-controls) for it to assume; you can configure roles in **Access Control** > **Roles**.
    ...
    | **Access Token TTL** | The lifetime for an access token in seconds. This value will be referenced at renewal time. Default is `2592000`, equivalent to 30 days. |
    | **Access Token Max TTL** | The maximum lifetime for an access token in seconds. This value will be referenced at renewal time. Default is `2592000`, equivalent to 30 days. |
    | **Access Token Max Number of Uses** | The maximum number of times that an access token can be used; a value of `0` implies infinite number of uses. Default is `0`. |
    ...
    <Info>
        ...
        Note that `*` acts as `**` in OIDC auth and matches any characters **including** `:` and `/`. Use specific patterns to avoid overly broad matching.

        | Pattern | Matches | Does not match |
        |---------|---------|----------------|
        | `repo:my-org/my-repo:*` | `repo:my-org/my-repo:ref:refs/heads/main`, `repo:my-org/my-repo:environment:prod` | `repo:other-org/my-repo:ref:refs/heads/main` |
        | `repo:my-org/{app,api}:*` | `repo:my-org/app:ref:refs/heads/main`, `repo:my-org/api:environment:prod` | `repo:my-org/web:ref:refs/heads/main` |
        | `repo:my-org/my-repo:ref:refs/heads/*` | `repo:my-org/my-repo:ref:refs/heads/main`, `repo:my-org/my-repo:ref:refs/heads/dev` | `repo:my-org/my-repo:environment:prod` |

        The subjects above are for illustration; see [GitHub OIDC Auth](/documentation/platform/identities/oidc-auth/github) for GitHub's current subject format.
        </Info>
    ...
    Next, select **Assign Existing**, choose the identity you want to add, and set the project role it should assume. The project role assigned will determine what project level resources this identity can have access to.
