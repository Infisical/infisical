...
<Steps>
    ...
    <Info>The `subject`, `audiences`, and `claims` fields support [glob pattern](https://www.malikbrowne.com/blog/a-beginners-guide-glob-patterns/) matching (e.g., `spiffe://example.org/ns/*`, `{dev,staging}-*`); however, we highly recommend using hardcoded SPIFFE IDs whenever possible for better security.</Info>

    Note that `*` does not match across `:` or `/` boundaries — use `**` when you need to match across separators.

    | Pattern | Matches | Does not match |
    |---------|---------|----------------|
    | `repo:my-org/*:ref:refs/heads/main` | `repo:my-org/any-repo:ref:refs/heads/main` | `repo:my-org/repo:ref:refs/heads/dev` |
