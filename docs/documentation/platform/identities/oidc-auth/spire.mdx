...
At a high-level, Infisical authenticates a SPIRE workload by verifying the JWT-SVID and checking that it meets specific requirements (e.g. it's issued by a trusted SPIRE server) at the `/api/v1/auth/oidc-auth/login` endpoint. If successful,
...
<Steps>
    ...
    <Info>The `subject`, `audiences`, and `claims` fields support glob pattern; however, we highly recommend using hardcoded SPIFFE IDs whenever possible for better security. See [General OIDC Auth](/documentation/platform/identities/oidc-auth/general#guide) for glob pattern matching behavior and examples. </Info>
    </Step>
