...
<Steps>
    ...
    <Info>The `subject`, `audiences`, and `claims` fields support glob pattern; however, we highly recommend using hardcoded SPIFFE IDs whenever possible for better security. See [General OIDC Auth](/documentation/platform/identities/oidc-auth/general#guide) for glob pattern behavior and examples. </Info>
    </Step>
