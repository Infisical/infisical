...
<Steps>
    ...
    <Info>
    The `subject`, `audiences`, and `claims` fields support glob pattern matching; however, we highly recommend using hardcoded values whenever possible. See [General OIDC Auth](/documentation/platform/identities/oidc-auth/general#guide) for glob pattern behavior and examples.
    </Info>
    </Step>
