...
<Steps>
    ...
    <Info>The `subject`, `audiences`, and `claims` fields support [glob pattern](https://www.malikbrowne.com/blog/a-beginners-guide-glob-patterns/) matching (e.g., `user-*`, `{admin,service}-**`); however, we highly recommend using hardcoded values whenever possible.</Info>
    </Step>
