...
Since the identity has been configured with [Universal Auth](https://infisical.com/docs/documentation/platform/identities/universal-auth) by default, you should reconfigure it to use OCI Auth instead. To do this, select the three dot menu next to **Universal Auth**, then select **Remove Auth Method**.
