...
<Steps>
  <Step title="Creating an identity">
    ...
    - Access Token TTL (default is `2592000` equivalent to 30 days): The lifetime for an access token in seconds. This value will be referenced at renewal time. A [credential expiry alert](/documentation/platform/identities/machine-identities#credential-expiration) notifies your team before a token reaches it.
  ...
  <Step title="Creating a Token">
    ...
    to an API Key used to authenticate with the Infisical API. With that, select **Add Token**.
