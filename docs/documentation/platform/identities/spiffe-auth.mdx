...
<Note>
  SPIFFE Auth currently supports JWT-SVID verification only. X.509-SVID (certificate-based) authentication isn't supported at this time. If you require X.509-SVID support, please open a feature request.
</Note>
...
<Steps>
  <Step title="Creating an identity">
    ...
    Since the identity has been configured with [Universal Auth](https://infisical.com/docs/documentation/platform/identities/universal-auth) by default, you should reconfigure it to use SPIFFE Auth instead. To do this, select the three dot menu next to **Universal Auth**, then select **Remove Auth Method**.
    ...
    **Trust Bundle Profile:**
    ...
    <Tabs>
      <Tab title="Static">
        Best for air-gapped environments or environments where the trust bundle doesn't change frequently. You paste your SPIRE server's trust bundle directly.

        - **CA Bundle JWKS (required):** The output of `spire-server bundle show -format spiffe`, which holds the public keys Infisical uses to verify JWT-SVID signatures

        Paste the bundle as SPIRE prints it. Infisical uses only the JWT-SVID signing keys in the bundle, so you don't need to remove the X.509-SVID keys or the `spiffe_sequence` field.
      </Tab>
      <Tab title="HTTPS Web Bundle">
        Best for dynamic environments where keys rotate regularly. Infisical automatically fetches and caches the trust bundle from a SPIRE bundle endpoint.
        ...
        - **Root CA Certificate (optional):** A PEM-encoded root CA certificate for verifying the bundle endpoint's TLS connection. When not provided, the system's default root CAs are used.
    ...
    **Common fields for both configurations:**
    ...
    - **Allowed SPIFFE IDs (required):** A comma-separated list of SPIFFE ID patterns that are permitted to authenticate. Supports [glob patterns](https://www.malikbrowne.com/blog/a-beginners-guide-glob-patterns/). Note that `*` matches within a single path segment (does not cross `/`), while `**` matches across multiple segments.

      | Pattern | Matches | Does not match |
      |---------|---------|----------------|
      | `spiffe://example.org/ns/*/sa/my-service` | `spiffe://example.org/ns/production/sa/my-service` | `spiffe://example.org/ns/a/b/sa/my-service` |
      | `spiffe://example.org/ns/production/**` | `spiffe://example.org/ns/production/sa/web`, `spiffe://example.org/ns/production/sa/api` | `spiffe://example.org/ns/staging/sa/web` |
      | `spiffe://example.org/ns/{dev,staging}/**` | `spiffe://example.org/ns/dev/sa/web`, `spiffe://example.org/ns/staging/sa/api` | `spiffe://example.org/ns/production/sa/web` |
