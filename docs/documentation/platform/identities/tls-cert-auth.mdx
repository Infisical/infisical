...
3. Infisical checks the user's properties against set criteria such as Allowed Common Names and Allowed Subject Alternative Names.
...
<Note>
  ...
  When you're authenticating with TLS Certificate Auth, you must use the port `8443` instead of the default `443`. Example: `https://app.infisical.com:8443/api/v1/auth/tls-cert-auth/login`
</Note>
...
To create an identity, go to **Access Control** > [**Machine Identities**](https://app.infisical.com/organization/access-management?selectedTab=identities) in your organization and select **Create**.

![identities organization](/images/platform/identities/identities-org.png)

When creating an identity, you specify an organization-level [role](/documentation/platform/access-controls/role-based-access-controls) for it to assume; you can configure roles in **Access Control** > [Roles](https://app.infisical.com/organization/access-management?selectedTab=roles).
...
Since the identity has been configured with [Universal Auth](https://infisical.com/docs/documentation/platform/identities/universal-auth) by default, you should reconfigure it to use TLS Certificate Auth instead. To do this, select the three dot menu next to **Universal Auth**, then select **Remove Auth Method**.
...
- **CA Certificate:** A PEM encoded CA Certificate used to validate incoming TLS request client certificate. Any name constraints it asserts are honored, so a client certificate whose name falls outside the namespace the CA may certify is rejected. Both the CA Certificate and the client certificate must permit client authentication: a certificate that lists an Extended Key Usage without `clientAuth` (or `anyExtendedKeyUsage`) is rejected, since a CA's key usages also limit what the certificates beneath it may be used for. A certificate that lists no Extended Key Usage is unrestricted and is accepted.
- **Verify Client Certificate Chain (default is disabled):** By default, the CA Certificate must be the **direct issuer** of the client's leaf certificate. Enable this to instead treat the CA Certificate as a **trust anchor** and accept any client whose presented chain (leaf followed by intermediates) builds a valid path up to it. This is helpful when the issuing intermediate rotates beneath a long-lived root, such as a **SPIRE X.509-SVID**: you can pin the stable root here once and let the workload present its current intermediate alongside its leaf, rather than updating the CA Certificate on every rotation. Either way the trust anchor is the only certificate trusted, the client certificate must still be an end-entity certificate, and the Common Name and Subject Alternative Name checks below still apply to the leaf. Restrictions that the CAs on the path place on their subordinates are honored: a client certificate whose name falls outside an issuing CA's name constraints is rejected, as is a chain with more intermediates than a CA's path length permits, or one that passes through a CA whose Extended Key Usage doesn't permit client authentication.
- **Allowed Common Names:** A comma separated list of client certificate common names allowed.
- **Allowed Subject Alternative Names:** A comma separated list of client certificate subject alternative names (SANs) allowed. This is useful when the client certificate carries its identity in a SAN rather than the Subject Common Name, such as a SPIFFE X.509-SVID whose identity is a URI SAN (`spiffe://trust-domain/path`) with an empty Subject. Matching is type-aware, so an entry only matches a SAN of the same type:
  - **Type-prefixed entries** are matched against the corresponding SAN type, e.g. `URI:spiffe://example.org/svc`, `IP:10.0.0.1`, `EMAIL:svc@example.com`.
  - **Bare entries** (no prefix) are treated as **DNS** names, e.g. `svc.example.com`. DNS and email matching is case-insensitive; URI and IP values are matched exactly.

  When both Allowed Common Names and Allowed Subject Alternative Names are set, the certificate must satisfy both checks. Leave empty to skip SAN validation.
...
<Note>
  If your CA restricts the namespace it may certify through the X.509 **name constraints** extension,
  those restrictions are enforced on every login, in both single-hop and chain mode. A URI name
  constraint restricts the **host** only and must be a fully qualified domain name, such as
  `example.org` or `.example.org`. Writing it the way the identity itself looks, such as
  `spiffe://example.org`, or writing one that carries a path, isn't a hostname, so no client
  certificate can satisfy it and every login is denied. Infisical rejects such a CA Certificate when you save it.

  A client certificate that carries several names of a constrained type has to keep **every** one of
  them inside the permitted namespace. Pairing an in-scope name with an out-of-scope one doesn't
  make the out-of-scope name acceptable; the login is denied.

  Name constraints can't scope individual workloads, because a URI constraint applies to the host
  and ignores the path. Use **Allowed Subject Alternative Names** for that.
</Note>
...
To do this, open the project you want to add the identity to, go to **Access Control** > **Machine Identities**, and select **Add Machine Identity to Project**.
