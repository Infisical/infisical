...
<Note>
  ...
  `spiffe://example.org`, or writing one that carries a path, is not a hostname, so no client
  ...
  them inside the permitted namespace. Pairing an in-scope name with an out-of-scope one does not
  make the out-of-scope name acceptable; the login is denied.

  Name constraints cannot scope individual workloads, because a URI constraint applies to the host
