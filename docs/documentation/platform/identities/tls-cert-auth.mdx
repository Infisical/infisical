...
- **Allowed Subject Alternative Names:** A comma separated list of client certificate subject alternative names (SANs) allowed. This is useful for SPIFFE X.509-SVID setups that do not use JWT-SVID: provide the SPIRE trust bundle as the CA Certificate and authorize on the SPIFFE ID carried in the certificate's URI SAN (e.g. `spiffe://example.org/svc`), where the Subject Common Name is typically empty. When both Allowed Common Names and Allowed Subject Alternative Names are set, the certificate must satisfy both checks. Leave empty to skip SAN validation.
