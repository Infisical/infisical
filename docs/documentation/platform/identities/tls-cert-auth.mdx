...
- **Allowed Subject Alternative Names:** A comma separated list of client certificate subject alternative names (SANs) allowed. Supports DNS, URI, and IP SANs. Type-prefixed entries (e.g. `URI:...`, `IP:...`) are matched against the corresponding SAN type; a bare entry is treated as a DNS name. This is useful when the client certificate carries its identity in a SAN rather than the Subject Common Name. When both Allowed Common Names and Allowed Subject Alternative Names are set, the certificate must satisfy both checks. Leave empty to skip SAN validation.
