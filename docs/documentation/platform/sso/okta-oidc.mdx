...
<Steps>
   <Step title="Create an OIDC application in Okta">
      ...
      <Note>
         Infisical takes a user's first name from the `given_name` claim and their last name from `family_name`. Configure `family_name` only if Infisical should store a separate last name. If `given_name` is missing, Infisical falls back to the `name` claim. See [General OIDC](/documentation/platform/sso/general-oidc/overview) for the exact fallback rules.
      </Note>

   </Step>
   <Step title="Retrieve Identity Provider (IdP) Information from Okta">
      After creating the application, you'll be taken to the application's settings page. From the **General** tab, copy the **Client ID** and **Client Secret** values.
   ...
   <Step title="Assign users in Okta to the application">
      Back in Okta, navigate to the **Assignments** tab of your Infisical application and select **Assign**. You can assign access to the application on a user-by-user basis using the **Assign to People** option, or in bulk using the **Assign to Groups** option.
...
<Tip>
    If you're only using one organization on your Infisical instance, you can configure a default organization in the [Server Admin Console](../admin-panel/server-admin#default-organization) to expedite OIDC login.
</Tip>
