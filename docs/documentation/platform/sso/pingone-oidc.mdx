...
<Steps>
   <Step title="Setup application in PingOne">
      ...
      <Note>
         Infisical takes a user's name from the `given_name` and `family_name` claims. If you map `name` instead, Infisical falls back to it and stores the whole name as the user's first name. See [General OIDC](/documentation/platform/sso/general-oidc/overview) for how that fallback resolves.
      </Note>
