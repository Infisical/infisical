...
<Steps>
   <Step title="Setup application in PingOne">
      ...
      <Note>
         Infisical takes a user's name from the `given_name` and `family_name` claims. If you map `name` in place of `given_name`, Infisical falls back to it and removes `family_name` from the end of it, so the surname isn't stored twice. See [General OIDC](/documentation/platform/sso/general-oidc/overview) for the exact fallback rules.
      </Note>
