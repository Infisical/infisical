...
<Steps>
   <Step title="Setup application in Auth0">
      ...
      <Note>
         Infisical takes a user's name from the `given_name` and `family_name` claims, so make sure both are set on your Auth0 users. If `given_name` is missing, Infisical falls back to the `name` claim. See [General OIDC](/documentation/platform/sso/general-oidc/overview) for how that fallback resolves.
      </Note>
