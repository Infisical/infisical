...
<Steps>
   <Step title="Prepare the SAML SSO configuration in Infisical">
      In Infisical, head to the **Single Sign-On (SSO)** page and select the **General** tab. Select **Connect** for **SAML** under the Connect to an Identity Provider section. Select **Azure / Entra**, then select **Connect** again.
   ...
   <Step title="Create a SAML application in Azure">
      In the Azure Portal, navigate to **Microsoft Entra ID** and select **Enterprise applications**. On this screen, select **+ New application**.
...
<Tip>
    If you're only using one organization on your Infisical instance, you can configure a default organization in the [Server Admin Console](../admin-panel/server-admin#default-organization) to expedite SAML login.
</Tip>
...
<Note>
   If you'd like to require Multi-factor Authentication for your team members to access Infisical check out our [Entra ID MFA](../mfa#entra-id-mfa) guide.
</Note>
