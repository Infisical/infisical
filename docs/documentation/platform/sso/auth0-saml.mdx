...
<Steps>
   <Step title="Prepare the SAML SSO configuration in Infisical">
       In Infisical, head to the **Single Sign-On (SSO)** page and select the **General** tab. Select **Connect** for **SAML** under the Connect to an Identity Provider section. Select **Auth0**, then select **Connect** again.
   ...
   <Step title="Create a SAML application in Auth0">
    ...
    <Warning>
        Make sure the `firstName` claim is mapped to a valid field of your Auth0 users. If your users don't have a `"given_name"` field available, you can replace it with `"name"` or another field that exists in your user profile on the left side of the mapping. Because `"name"` holds a user's full name, remove the `family_name` mapping when you do, otherwise the surname shows up twice in Infisical.
    </Warning>
    </Step>
