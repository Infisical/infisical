...
- The identity provider (Okta, Google, Entra ID, etc.) should support OIDC.
- Users in the IdP should have a configured `email` and either `given_name` or `name`. Configure `family_name` as well for Infisical to store a separate last name.
...
<Steps>
   <Step title="Setup Identity Provider">
      ...
      <Note>
         Infisical takes a user's first name from the `given_name` claim and their last name from `family_name`. If your IdP doesn't send `given_name`, Infisical falls back to the `name` claim and removes `family_name` from the end of it. When `family_name` isn't at the end of `name`, Infisical keeps the whole `name` as the first name.
      </Note>
...
<Tip>
    If you're only using one organization on your Infisical instance, you can configure a default organization in the [Server Admin Console](/documentation/platform/admin-panel/server-admin#default-organization) to expedite OIDC login.
</Tip>
