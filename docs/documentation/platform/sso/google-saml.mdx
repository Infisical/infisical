...
<Steps>
   <Step title="Prepare the SAML SSO configuration in Infisical">
        In Infisical, head to the **Single Sign-On (SSO)** page and select the **General** tab. Select **Connect** for **SAML** under the Connect to an Identity Provider section. Select **Google**, then select **Connect** again.
...
<Warning>
Group membership changes in the SAML provider only sync with Infisical when a user logs in via SAML. For example, if you remove a user from a group in the SAML provider, this change won't be reflected in Infisical until their next SAML login. To ensure this behavior, Infisical recommends enabling Enforce SAML SSO.
</Warning>
