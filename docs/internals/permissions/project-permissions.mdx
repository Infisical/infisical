...
<Note>
  Secret values within approval requests are controlled by the user's `secrets.readValue` permission for each secret's environment/path, unless the user is a designated reviewer for the request. The `read` permission does not override this behavior.
</Note>
