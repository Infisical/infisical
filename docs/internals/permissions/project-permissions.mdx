...
<Note>
  Listing and viewing all secret approval requests in the project is gated by the shared permission **Approval Requests** (`approval-requests`) with action **Read**. Without it, users only see requests where they are committer or approver. Secret values within approval requests are controlled by the user's `secrets.readValue` permission for each secret's environment/path, unless the user is a designated reviewer for the request.
</Note>
