...
| `edit-auth` | Modify authentication methods for a machine identity | `identityId` |
...
| `describeSecret` | View secret details such as key, path, metadata, tags, access insights and more. If you're using the API, you can pass `viewSecretValue: false` to the API call to retrieve secrets without their values. | `environment`, `secretPath`, `secretName`, `secretTags` |
...
#### Subject: `project-folder-grant`

Supports conditions and permission inversion.

| Action | Description | Condition keys |
| -------------- | ---------------------------------------- | ----- |
| `read-grant` | View access grants on secret paths | `environment`, `secretPath` |
| `create-grant` | Create access grants on secret paths | `environment`, `secretPath` |
| `revoke-grant` | Revoke access grants on secret paths | `environment`, `secretPath` |
...
#### Subject: `approval-request-grants`

| Action   | Description                            |
| -------- | -------------------------------------- |
| `read`   | View grants tied to approval requests  |
| `revoke` | Revoke grants tied to approval requests |
...
| Action | Description | Condition keys |
| ------------------------------ | ---------------------------------------------- | ----- |
| `read` | View honey tokens and their status | `environment`, `secretPath` |
| `read-credentials` | View the decoy credentials of a honey token | `environment`, `secretPath` |
| `create` | Create honey tokens | `environment`, `secretPath` |
| `edit` | Modify existing honey tokens | `environment`, `secretPath` |
| `reset` | Reset a triggered honey token back to active | `environment`, `secretPath` |
| `revoke` | Revoke a honey token | `environment`, `secretPath` |
...
#### Subject: `secret-validation-rules`

| Action   | Description                                   |
| -------- | --------------------------------------------- |
| `read`   | View secret validation rules                  |
| `create` | Add new secret validation rules               |
| `edit`   | Modify existing secret validation rules       |
| `delete` | Remove secret validation rules                |
...
| Action | Description                                                                          |
| ------ | ------------------------------------------------------------------------------------ |
| `read` | View project insights dashboard, including secret access volume, access locations, auth method distribution, and rotation/reminder calendar |
...
| `generate-mac`       | Use keys to generate message authentication codes (MACs)                    |
| `verify-mac`         | Use keys to verify message authentication codes (MACs)                      |
| `rotate`             | Rotate key material for encryption keys                                     |
...
| Action | Description |
| ------ | ----------- |
| `read` | View the PAM insights dashboard, including resource and account totals, session activity, top actors, resource breakdown, and upcoming rotations |
...
<Note>
When using `$glob`, note that `*` doesn't match across `/` boundaries. Use `**` to match across multiple path segments.

A trailing `/*` also doesn't match the folder itself: `/app/*` matches secrets in `/app/config` but not secrets directly in `/app`. To grant access to a folder and everything under it, use a trailing `/**`.
...
A member can hold a direct role, a group-inherited role, and additional privileges at the same time, so what they can actually do isn't always obvious. The **Permission Audit** view resolves every source into one effective answer per subject and action.

Open **Access Control**, select the **Users** tab, select the member, then select **Permission Audit** in the page header.

<Frame caption="Permission Audit for a project member, grouped by resource.">
  <img src="/images/internals/permissions/permission-audit-sheet.png" alt="Permission Audit sheet showing effective access per resource" />
</Frame>
...
Use the search box to filter by resource, action, or source, and the **All** dropdown to show only allowed, conditional, or forbidden actions. Select the **Export CSV** icon next to the search box to download the full list.
