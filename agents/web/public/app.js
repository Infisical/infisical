...
// Product images - synced with agents/src/shared/mock-data.ts inventory
const PRODUCT_IMAGES = {
  'MacBook Pro 16"':
    "https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/mbp16-spacegray-select-202301?wid=200&hei=200&fmt=jpeg&qlt=90",
  "iPhone 15 Pro":
    "https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/iphone-15-pro-finish-select-202309-6-1inch-naturaltitanium?wid=200&hei=200&fmt=jpeg&qlt=90",
  "AirPods Pro":
    "https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/MQD83?wid=200&hei=200&fmt=jpeg&qlt=90",
  "Sony WH-1000XM5 Headphones":
    "https://m.media-amazon.com/images/I/61vJtKbAssL._AC_SL1500_.jpg",
  default:
    'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect fill="%23f0f0f0" width="100" height="100"/><text x="50" y="55" text-anchor="middle" fill="%23999" font-size="40">📦</text></svg>',
};

// Mock orders data - synced with agents/src/shared/mock-data.ts
const ORDERS = [
  {
    orderId: "ORD-8891",
...
function renderOrders() {
  ordersList.innerHTML = ORDERS.map(
    (order) => `
...
function openSupportModal(orderId) {
  ...
  document.getElementById("modalOrderId").textContent =
    `Order ${currentOrder.orderId} • ${formatDate(currentOrder.orderDate)}`;
  ...
  if (currentOrder.hasIssue && currentOrder.issueType) {
    const radio = document.querySelector(
      `input[name="issueType"][value="${currentOrder.issueType}"]`,
    );
    if (radio) {
      radio.checked = true;
      document.getElementById("issueDetails").value =
        `I ordered ${currentOrder.expectedItem} but received ${currentOrder.shippedItem} instead.`;
      updateSubmitButton();
    }
  }
  ...
  if (currentOrder.hasBillingIssue) {
    const radio = document.querySelector(
      `input[name="issueType"][value="billing"]`,
    );
    if (radio) {
      radio.checked = true;
      document.getElementById("issueDetails").value =
        "I was charged twice for this order. Please investigate and refund the duplicate charge.";
      updateSubmitButton();
    }
  }
