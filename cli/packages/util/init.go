...
func GetOrganizationsNameList(organizationResponse api.GetOrganizationsResponse) []string {
	...
	if len(organizations) == 0 {
		message := fmt.Sprintf("You don't have any organization created in Infisical. You must first create a organization at %s", INFISICAL_DEFAULT_US_URL)
...
func GetWorkspacesInOrganization(workspaceResponse api.GetWorkSpacesResponse, orgId string) ([]models.Workspace, []string) {
	...
	if len(filteredWorkspaces) == 0 {
		message := fmt.Sprintf("You don't have any projects created in Infisical organization. You must first create a project at %s", INFISICAL_DEFAULT_US_URL)
		PrintErrorMessageAndExit(message)
	}
